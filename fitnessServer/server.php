...
if ($action == "version") {
    global $fs_version;
    echo "$fs_version";
} elseif ($action == "get_client_sequence_number") {
    fs_getClientSequenceNumber();
} elseif ($action == "get_server_sequence_number") {
    fs_getServerSequenceNumber();
} elseif ($action == "report_death") {
    fs_reportDeath();
} elseif ($action == "get_score") {
    fs_getScore();
} elseif ($action == "get_client_score") {
    fs_getClientScore();
} elseif ($action == "get_client_score_details") {
...
function fs_secondsToAgeSummary($inSeconds)
{
    if ($inSeconds < 120) {
        ...
        return "$inSeconds seconds";
    } elseif ($inSeconds < 3600 * 2) {
        $min = number_format($inSeconds / 60, 0);
        return "$min minutes";
    } elseif ($inSeconds < 24 * 3600 * 2) {
        $hours = number_format($inSeconds / 3600, 0);
        return "$hours hours";
    } elseif ($inSeconds < 24 * 3600 * 60) {
        $days = number_format($inSeconds / (3600 * 24), 0);
        return "$days days";
    } elseif ($inSeconds < 24 * 3600 * 365 * 2) {
