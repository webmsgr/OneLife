...
static int biomeDBGet(int inX, int inY, int *outSecondPlaceBiome = NULL,
...
static int eveDBGet(const char *inEmail, int *outX, int *outY, int *outRadius) {
...
static int computeXYCacheHash(int inKeyA, int inKeyB) {
  ...
  int hashKey =
      (inKeyA * CACHE_PRIME_A + inKeyB * CACHE_PRIME_B) % BIOME_CACHE_SIZE;
  if (hashKey < 0) {
...
// returns -2 on miss
static int biomeGetCached(int inX, int inY, int *outSecondPlaceIndex,
...
static int computeMapBiomeIndex(int inX, int inY,
  ...
  double randVal =
      (getXYFractal(inX, inY, 0.55, 0.83332 + 0.08333 * numBiomes));
  ...
  */
  ...
  if (pickedBiome >= regularBiomeLimit && numSpecialBiomes > 0) {
    ...
    for (int i = regularBiomeLimit; i < numBiomes; i++) {
      ...
      double randVal =
          getXYFractal(inX, inY, 0.55, 2.4999 + 0.2499 * numSpecialBiomes);
...
static int getMapBiomeIndex(int inX, int inY, int *outSecondPlaceIndex = NULL,
                            double *outSecondPlaceGap = NULL) {
  ...
  if (anyBiomesInDB && inX >= minBiomeXLoc && inX <= maxBiomeXLoc &&
  ...
  int pickedBiome =
      computeMapBiomeIndex(inX, inY, &secondPlace, &secondPlaceGap);
...
static int getBaseMap(int inX, int inY, char *outGridPlacement = NULL) {
  ...
  if (inX > xLimit || inX < -xLimit || inY > yLimit || inY < -yLimit) {
  ...
  for (int g = 0; g < gridPlacements.size(); g++) {
    ...
    */
    ...
    if ((inX + gp->phaseX + lrint(gridWiggleX * gp->wiggleScaleX)) %
      ...
      int pickedBiome =
          getMapBiomeIndex(inX, inY, &secondPlace, &secondPlaceGap);
  ...
  if (getXYRandom(inX, inY) < density) {
    ...
    if (getXYRandom(inX, inY) > .5 + secondPlaceReduction * secondPlaceGap) {
    ...
    for (int i = 0; i < numObjects; i++) {
      ...
      double randVal =
          getXYFractal(inX, inY, 0.3, 0.15 + 0.016666 * numObjects);
    ...
    float oldSpecialChance =
        naturalMapChances[pickedBiome].getElementDirect(specialObjectIndex);
...
void outputMapImage() {
  ...
  for (int i = 0; i < allNaturalMapIDs.size(); i++) {
    Color *c =
        Color::makeColorFromHSV((float)i / allNaturalMapIDs.size(), 1, 1);
  ...
  */
  ...
  for (int y = 0; y < h; y++) {
    ...
    for (int x = 0; x < w; x++) {
      ...
      */
      ...
      biomeIm.setColor(y * w + x, biomeColors.getElementDirect(biomeInd));
      (*(biomeCounts.getElement(biomeInd)))++;
      totalBiomeCount++;
    }
  }

  const char *biomeNames[] = {"Grass ", "Swamp ", "Yellow", "Gray  ",
                              "Snow  ", "Desert", "Jungle"};
  ...
  for (int i = 0; i < allNaturalMapIDs.size(); i++) {
    ...
    printf("%d\t%-30s  (actual=%f, expected=%f\n",
           objCounts.getElementDirect(i), obj->description,
...
static void dbPut(int inX, int inY, int inSlot, int inValue, int inSubCont = 0);
  ...
  if (placeFile != NULL) {
    for (int i = 0; i < NUM_RECENT_PLACEMENTS; i++) {
      fprintf(placeFile, "%d,%d %d\n", recentPlacements[i].pos.x,
              recentPlacements[i].pos.y, recentPlacements[i].depth);
    }
...
void printObjectSamples() {
  ...
  printf("Counted %d objects in %d/%d samples, expect %d total\n", count,
         numSamples, rangeSize, (int)(count / sampleFraction));
}
...
static int computeDBCacheHash(int inKeyA, int inKeyB, int inKeyC, int inKeyD) {
  ...
  int hashKey = (inKeyA * CACHE_PRIME_A + inKeyB * CACHE_PRIME_B +
                 inKeyC * CACHE_PRIME_C + inKeyD * CACHE_PRIME_D) %
                DB_CACHE_SIZE;
  if (hashKey < 0) {
...
static int computeBLCacheHash(int inKeyA, int inKeyB) {
  ...
  int hashKey =
      (inKeyA * CACHE_PRIME_A + inKeyB * CACHE_PRIME_B) % DB_CACHE_SIZE;
  if (hashKey < 0) {
    hashKey += DB_CACHE_SIZE;
  }
  return hashKey;
...
static int dbGetCached(int inX, int inY, int inSlot, int inSubCont) {
  ...
  if (r.x == inX && r.y == inY && r.slot == inSlot && r.subCont == inSubCont &&
      r.value != -2) {
    return r.value;
  } else {
    return -2;
...
static char blockingGetCached(int inX, int inY) {
  BlockingCacheRecord r = blockingCache[computeBLCacheHash(inX, inY)];
...
int DB_open_timeShrunk(DB *db, const char *path, int mode,
                       unsigned long hash_table_size, unsigned long key_size,
  ...
  int error =
      DB_open(&oldDB, path, mode, hash_table_size, key_size, value_size);
  if (error) {
  ...
  AppLog::infoF("Shrinking hash table in %s from %d down to %d", path,
                DB_getCurrentSize(&oldDB), newSize);
...
int cleanMap() {
  ...
  while (DB_Iterator_next(&dbi, key, value) > 0) {
    ...
    if (s == 0) {
      ...
      if (id > 0) {
        ...
        if (o == NULL || getIsCategory(id) || o->description[0] == '@' ||
            o->isOwned) {
  ...
  AppLog::infoF("...%d contained objects present, and %d needed to be cleared.",
...
static char loadIntoMapFromFile(FILE *inFile, int inOffsetX = 0,
                                int inOffsetY = 0, double inTimeLimitSec = 0) {
  ...
  while (inTimeLimitSec == 0 ||
    ...
    int numRead = fscanf(inFile, "%d %d %d %d %999s", &(r.x), &(r.y),
                         &(r.biome), &(r.floor), stringBuff);
    ...
    for (int c = 0; c < r.contained.size(); c++) {
      ...
      setContained(r.x, r.y, r.subContained.getElement(c)->size(), subContArray,
                   c + 1);
...
static inline void changeContained(int inX, int inY, int inSlot, int inSubCont,
                                   int inID) {
...
static char isAdjacent(GridPos inPos, int inX, int inY) {
  if (inX <= inPos.x + 1 && inX >= inPos.x - 1 && inY <= inPos.y + 1 &&
      inY >= inPos.y - 1) {
    return true;
  }
  return false;
    ...
    for (int p = 0; p < speechPipesIn[i].size(); p++) {
      ...
      if (isAdjacent(inPos, inX, inY)) {
        ...
        if (id > 0) {
          ...
          if (oIn->speechPipeIn && oIn->speechPipeIndex == i) {
            stillHere = true;
          }
        }
        if (!stillHere) {
...
SimpleVector<GridPos> *getSpeechPipesOut(int inIndex) {
  ...
  for (int p = 0; p < speechPipesOut[inIndex].size(); p++) {
    ...
    if (id > 0) {
      ...
      if (oOut->speechPipeOut && oOut->speechPipeIndex == inIndex) {
        stillHere = true;
      }
    }
    if (!stillHere) {
...
static void setupMapChangeLogFile() {
  ...
  if (logFolder.isDirectory()) {
    ...
    if (mapChangeLogFile == NULL) {
      ...
      // file does not exist
      char *newFileName =
          autoSprintf("%.ftime_mapLog.txt", Time::getCurrentTime());
...
void reseedMap(char inForceFresh) {
  ...
  if (!set) {
    ...
    char *secretA =
        SettingsManager::getStringSetting("statsServerSharedSecret", "secret");
    int secretALen = strlen(secretA);

    unsigned int seedBaseA = crc32((unsigned char *)secretA, secretALen);

    const char *nonce = "8TX8sr7weEK8UIqrE0xV"
    ...
    unsigned int seedBaseB = crc32((unsigned char *)secretB, secretALen);

    delete[] secretA;
    delete[] secretB;

    unsigned int modTimeSeedA =
        (unsigned int)fmod(Time::getCurrentTime() + seedBaseA, 4294967295U);
    ...
    unsigned int modTimeSeedB =
        (unsigned int)fmod(Time::getCurrentTime() + seedBaseB, 4294967295U);
    ...
    for (int i = 0; i < numObjects; i++) {
      ...
      if (p > 0) {
        ...
        char *randPlacementLoc = strstr(o->description, "randPlacement");
        ...
        if (randPlacementLoc != NULL) {
          ...
          for (int p = 0; p < count; p++) {
            ...
            while (!placed) {
              int pickX =
                  placementRandSource.getRandomBoundedInt(-safeR, safeR);
              int pickY =
                  placementRandSource.getRandomBoundedInt(-safeR, safeR);
  ...
  if (!set && mapChangeLogFile != NULL) {
    ...
    char *newFileName =
        autoSprintf("%.ftime_mapSeed.txt", Time::getCurrentTime());
...
char initMap() {
  ...
  if (placeFile != NULL) {
    for (int i = 0; i < NUM_RECENT_PLACEMENTS; i++) {
      fscanf(placeFile, "%d,%d %d", &(recentPlacements[i].pos.x),
             &(recentPlacements[i].pos.y), &(recentPlacements[i].depth));
    }
  ...
  if (!skipLookTimeCleanup) {
    ...
    int error = DB_open(&lookTimeDB_old, lookTimeDBName,
                        KISSDB_OPEN_MODE_RWCREAT, 80000,
    ...
    if (lookTimeDBExists && staleSec > 0) {
      ...
      AppLog::infoF("Shrinking hash table for lookTimes from "
                    "%d down to %d",
                    DB_getCurrentSize(&lookTimeDB_old), newSize);

      error = DB_open(&lookTimeDB_temp, lookTimeDBName_temp,
                      KISSDB_OPEN_MODE_RWCREAT, newSize,
                      8, // two 32-bit ints, xy
                      8  // one 64-bit double, representing an ETA time
                      // in whatever binary format and byte order
                      // "double" on the server platform uses
  ...
  int error =
      DB_open(&lookTimeDB, lookTimeDBName, KISSDB_OPEN_MODE_RWCREAT, 80000,
              8, // two 32-bit ints, xy
              8  // one 64-bit double, representing an ETA time
              // in whatever binary format and byte order
              // "double" on the server platform uses
  ...
  // in mapTime.db
  error = DB_open_timeShrunk(&db, "map.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
  ...
  error =
      DB_open_timeShrunk(&timeDB, "mapTime.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                         16, // four 32-bit ints, xysb
                         // s is the slot number
                         // s=0 for base object
                         // s=1 decay ETA seconds (wall clock time)
  ...
  error =
      DB_open_timeShrunk(&biomeDB, "biome.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                         8, // two 32-bit ints, xy
                         12 // three ints,
  ...
  printf("Min (x,y) of biome in db = (%d,%d), "
         ...
         minBiomeXLoc, minBiomeYLoc, maxBiomeXLoc, maxBiomeYLoc);

  error =
      DB_open_timeShrunk(&floorDB, "floor.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                         8, // two 32-bit ints, xy
  ...
  error = DB_open_timeShrunk(&floorTimeDB, "floorTime.db",
                             KISSDB_OPEN_MODE_RWCREAT, 80000,
                             8, // two 32-bit ints, xy
                             8  // one 64-bit double, representing an ETA time
                             // in whatever binary format and byte order
  ...
  error = DB_open(&graveDB, "grave.db", KISSDB_OPEN_MODE_RWCREAT, 80000,
                  8, // two 32-bit ints, xy
  ...
  error = DB_open(&eveDB, "eve.db", KISSDB_OPEN_MODE_RWCREAT,
  ...
  error = DB_open(&metaDB, "meta.db", KISSDB_OPEN_MODE_RWCREAT,
  ...
  AppLog::infoF("MetadataDB:  Found %d records with max MetadataID of %d",
  ...
  for (int i = 0; i < numObjects; i++) {
    ...
    if (p > 0) {
      ...
      if (gridPlacementLoc != NULL) {
        ...
        int numRead = sscanf(gridPlacementLoc, "gridPlacement%d,%d,p%d,p%d",
        ...
        MapGridPlacement gp = {
            id, spacingX, spacingY, phaseX, phaseY,
            // phaseRandSource.getRandomBoundedInt( 0,
            //                                     spacingX - 1 ),
            // phaseRandSource.getRandomBoundedInt( 0,
            //                                     spacingY - 1 ),
            wiggleScaleX, wiggleScaleY, permittedBiomes};
...
static void rememberDummy(FILE *inFile, int inX, int inY,
                          ObjectRecord *inDummyO, int inSlot = -1,
                          int inB = 0) {
...
static void dbPut(int inX, int inY, int inSlot, int inValue, int inSubCont) {
...
static char isDecayTransAlwaysLiveTracked(TransRecord *inTrans) {
  if (inTrans != NULL && inTrans->move >= 4 && inTrans->move <= 7) {
...
static void trackETA(int inX, int inY, int inSlot, timeSec_t inETA,
                     int inSubCont = 0, TransRecord *inApplicableTrans = NULL) {
  ...
  if (timeLeft < maxSecondsForActiveDecayTracking) {
    ...
    char exists;
    timeSec_t existingETA = liveDecayRecordPresentHashTable.lookup(
        inX, inY, inSlot, inSubCont, &exists);

    if (!exists || existingETA != inETA) {

      liveDecayQueue.insert(r, inETA);

      liveDecayRecordPresentHashTable.insert(inX, inY, inSlot, inSubCont,
                                             inETA);

      char exists;

      liveDecayRecordLastLookTimeHashTable.lookup(inX, inY, inSlot, inSubCont,
                                                  &exists);

      if (!exists) {
        // don't overwrite old one
        liveDecayRecordLastLookTimeHashTable.insert(inX, inY, inSlot, inSubCont,
                                                    MAP_TIMESEC);
        if (inSlot > 0 || inSubCont > 0) {

          ContRecord *oldCount =
              liveDecayRecordLastLookTimeMaxContainedHashTable.lookupPointer(
                  inX, inY, 0, 0);
          ...
          if (oldCount != NULL) {
            ...
            liveDecayRecordLastLookTimeMaxContainedHashTable.insert(inX, inY, 0,
                                                                    0, r);
          }
        }
      }
...
int *getContained(int inX, int inY, int *outNumContained, int inSubCont) {
  ...
  for (int i = 0; i < *outNumContained; i++) {
    timeSec_t *oldLookTime = liveDecayRecordLastLookTimeHashTable.lookupPointer(
        inX, inY, i + 1, inSubCont);
    if (oldLookTime != NULL) {
      // look at it now
...
static int getContainerDecaySlot(int inX, int inY, int inSlot,
                                 int inSubCont = 0, int inNumContained = -1) {
  if (inNumContained == -1) {
...
timeSec_t *getContainedEtaDecay(int inX, int inY, int *outNumContained,
  ...
  *outNumContained = num;
  ...
  for (int i = 0; i < num; i++) {
    ...
    containedEta[i] =
        dbTimeGet(inX, inY, getContainerDecaySlot(inX, inY, i, inSubCont, num),
                  inSubCont);
  }
  return containedEta;
}
...
int checkDecayObject(int inX, int inY, int inID) {
  ...
  if (mapETA != 0) {
    ...
    if ((int)mapETA <= MAP_TIMESEC) {
      ...
      if (t->move != 0) {
        ...
        if (dir.x == 0 && dir.y == 0) {
          ...
          dir = rotate(dir,
                       2 * M_PI * randSource.getRandomBoundedInt(0, 7) / 8.0);
            ...
            while (!stopCheckingDir && tryDist > 0) {
              ...
              for (int i = 0; i < tryDist + tryRadius; i++) {
                ...
                if (i >= tryDist && oID == 0) {
                  ...
                  if (stayInBiome && curBiome != getMapBiome(testX, testY)) {
        ...
        if (newX != inX || newY != inY) {
          ...
          if (destTrans == NULL || destTrans->newActor == 0) {
            ...
            if (destTrans != NULL && destTrans->newTarget != newID &&
          ...
          if (destTrans != NULL) {
            ...
            if (leftDecayT != NULL) {
              ...
              int tweakedSeconds = randSource.getRandomBoundedInt(
                  lrint(leftDecayT->autoDecaySeconds * 0.9),
          ...
          double moveDist =
              sqrt((newX - inX) * (newX - inX) + (newY - inY) * (newY - inY));
      ...
      if (mapETA != 0 && (newX != inX || newY != inY)) {
        ...
        timeSec_t lastLookTime = liveDecayRecordLastLookTimeHashTable.lookup(
            inX, inY, 0, 0, &foundInOldSpot);

        if (foundInOldSpot) {

          char foundInNewSpot;
          liveDecayRecordLastLookTimeHashTable.lookup(newX, newY, 0, 0,
                                                      &foundInNewSpot);
          ...
          if (!foundInNewSpot) {
            ...
            liveDecayRecordLastLookTimeHashTable.insert(newX, newY, 0, 0,
                                                        lastLookTime);
          }
        }
      }
    ...
    int decayTime = randSource.getRandomBoundedInt(t->autoDecaySeconds / 2,
                                                   t->autoDecaySeconds);
    if (decayTime < 1) {
      decayTime = 1;
    }
      ...
      if ((int)mapETA <= MAP_TIMESEC) {
        ...
        if (newID != 0) {
          ...
          if (newDecayT != NULL) {
            ...
            mapETA = MAP_TIMESEC + tweakedSeconds / getMapContainerTimeStretch(
                                                        inX, inY, inSubCont);
          } else {
            // no further decay
            mapETA = 0;
...
static int getPossibleBarrier(int inX, int inY) {
  if (barrierOn)
    if (inX == barrierRadius || inX == -barrierRadius || inY == barrierRadius ||
        inY == -barrierRadius) {
      ...
      if (inX <= barrierRadius && inX >= -barrierRadius &&
          inY <= barrierRadius && inY >= -barrierRadius) {
  ...
  for (int y = inYStart; y <= inYEnd; y++) {
    for (int x = inXStart; x <= inXEnd; x++) {
      ...
      if (!lookTimeTracking.checkExists(x, y, currentTime)) {
        ...
        if (floorEtaDecay != 0 &&
            floorEtaDecay < currentTime + maxSecondsForActiveDecayTracking) {
        ...
        if (etaDecay != 0 &&
            etaDecay < currentTime + maxSecondsForActiveDecayTracking) {
      ...
      timeSec_t *oldLookTime =
          liveDecayRecordLastLookTimeHashTable.lookupPointer(x, y, 0, 0);
      ...
      ContRecord *contRec =
          liveDecayRecordLastLookTimeMaxContainedHashTable.lookupPointer(x, y,
                                                                         0, 0);
      ...
      if (contRec != NULL) {
        ...
        for (int c = 1; c <= contRec->maxSlots; c++) {
          ...
          oldLookTime =
              liveDecayRecordLastLookTimeHashTable.lookupPointer(x, y, c, 0);
          if (oldLookTime != NULL) {
            // look at it now
            *oldLookTime = currentTime;
          }
          ...
          for (int s = 1; s <= contRec->maxSubSlots; s++) {
            ...
            oldLookTime =
                liveDecayRecordLastLookTimeHashTable.lookupPointer(x, y, c, s);
            if (oldLookTime != NULL) {
              // look at it now
              *oldLookTime = currentTime;
            }
...
int getMapBiome(int inX, int inY) { return biomes[getMapBiomeIndex(inX, inY)]; }

// returns properly formatted chunk message for chunk centered
// around x,y
unsigned char *getChunkMessage(int inStartX, int inStartY, int inWidth,
                               int inHeight, GridPos inRelativeToPos,
  ...
  for (int y = inStartY; y < endY; y++) {
    ...
    for (int x = inStartX; x < endX; x++) {
      ...
      if (lastCheckedBiome == -1 || lastCheckedBiomeX != x ||
          lastCheckedBiomeY != y) {
  ...
  for (int i = 0; i < chunkCells; i++) {
    ...
    char *cell =
        autoSprintf("%d:%d:%d", chunkBiomes[i], hideIDForClient(chunkFloors[i]),
    ...
    if (containedStacks[i] != NULL) {
      for (int c = 0; c < containedStackSizes[i]; c++) {
        char *containedString =
            autoSprintf(",%d", hideIDForClient(containedStacks[i][c]));
        ...
        if (subContainedStacks[i][c] != NULL) {
          ...
          for (int s = 0; s < subContainedStackSizes[i][c]; s++) {
            ...
            char *subContainedString = autoSprintf(
                ":%d", hideIDForClient(subContainedStacks[i][c][s]));

            chunkDataBuffer.appendArray((unsigned char *)subContainedString,
  ...
  unsigned char *compressedChunkData =
      zipCompress(chunkData, chunkDataBuffer.size(), &compressedSize);

  char *header =
      autoSprintf("MC\n%d %d %d %d\n%d %d\n#", inWidth, inHeight,
                  inStartX - inRelativeToPos.x, inStartY - inRelativeToPos.y,
...
static char tooClose(GridPos inA, GridPos inB, int inMinComponentDistance) {
  ...
  if (xDist < inMinComponentDistance && yDist < inMinComponentDistance) {
    return true;
  }
  return false;
}
...
static int neighborWallAgree(int inX, int inY, ObjectRecord *inSetO,
  ...
  if (inSetO->horizontalVersionID != -1 && inSetO->verticalVersionID != -1 &&
      inSetO->cornerVersionID != -1) {

    // this object can react to its neighbors

    if (inSetO->id != inSetO->verticalVersionID && (nSet[2] || nSet[3]) &&
        !(nSet[0] || nSet[1])) {
      // should be vert

      returnID = inSetO->verticalVersionID;
    } else if (inSetO->id != inSetO->horizontalVersionID &&
               !(nSet[2] || nSet[3]) && (nSet[0] || nSet[1])) {
      // should be horizontal

      returnID = inSetO->horizontalVersionID;
    } else if (inSetO->id != inSetO->cornerVersionID && (nSet[2] || nSet[3]) &&
               (nSet[0] || nSet[1])) {
      // should be corner
  ...
  if (inRecurse) {
    ...
    for (int n = 0; n < 4; n++) {
      if (nSet[n]) {
        ...
        if (nO->horizontalVersionID != -1 && nO->verticalVersionID != -1 &&
            nO->cornerVersionID != -1) {

          neighborWallAgree(inX + nX[n], inY + nY[n], nO, false);
        }
      }
    }
  }
...
static void runTapoutOperation(int inX, int inY, int inRadiusX, int inRadiusY,
                               int inSpacingX, int inSpacingY, int inTriggerID,
                               char inIsPost = false) {
        ...
        if (x >= inX && newTarget > 0) {
          ...
          if (flipTrans != NULL) {
            ...
            if (x == inX && newTarget > 0) {
              ...
              flipTrans = getPTrans(newTarget, newTarget);
...
void setMapObjectRaw(int inX, int inY, int inID) {
  ...
  if (o->speechPipeIn) {
    ...
    GlobalTriggerState *s =
        globalTriggerStates.getElement(o->globalTriggerIndex);
    ...
    if (r != NULL) {
      ...
      runTapoutOperation(inX, inY, r->limitX, r->limitY, r->gridSpacingX,
                         r->gridSpacingY, inID);
      ...
      if (r->buildCountLimit != -1 && r->buildCount >= r->buildCountLimit) {
        ...
        runTapoutOperation(inX, inY, r->postBuildLimitX, r->postBuildLimitY,
                           r->gridSpacingX, r->gridSpacingY, inID, true);
      }
    }
  ...
  if (mapChangeLogFile != NULL) {
    ...
    if (o != NULL && o->isUseDummy) {
      fprintf(mapChangeLogFile, "%.2f %d %d %s%du%d %d\n", timeDelta, inX, inY,
              extraFlag, o->useDummyParent, o->thisUseDummyIndex, respPlayer);
    } else if (o != NULL && o->isVariableDummy) {
      fprintf(mapChangeLogFile, "%.2f %d %d %s%dv%d %d\n", timeDelta, inX, inY,
              extraFlag, o->variableDummyParent, o->thisVariableDummyIndex,
              respPlayer);
    } else {
      fprintf(mapChangeLogFile, "%.2f %d %d %s%d %d\n", timeDelta, inX, inY,
              extraFlag, inID, respPlayer);
    }
  }
}
...
timeSec_t getSlotEtaDecay(int inX, int inY, int inSlot, int inSubCont) {
  // 0 if not found
  return dbTimeGet(inX, inY, getContainerDecaySlot(inX, inY, inSlot, inSubCont),
                   inSubCont);
}

void addContained(int inX, int inY, int inContainedID, timeSec_t inEtaDecay,
                  int inSubCont) {
  int oldNum;
  ...
  if (inEtaDecay != 0) {
    ...
    inEtaDecay =
        curTime + etaOffset / getMapContainerTimeStretch(inX, inY, inSubCont);
  }

  int *oldContained = getContained(inX, inY, &oldNum, inSubCont);

  timeSec_t *oldContainedETA =
  ...
  if (oldNum != 0) {
    memcpy(newContainedETA, oldContainedETA, oldNum * sizeof(timeSec_t));
  }
...
void setContainedEtaDecay(int inX, int inY, int inNumContained,
  ...
  for (int i = 0; i < inNumContained; i++) {
    dbTimePut(inX, inY,
              getContainerDecaySlot(inX, inY, i, inSubCont, inNumContained),
...
int removeContained(int inX, int inY, int inSlot, timeSec_t *outEtaDecay,
  ...
  timeSec_t resultEta = dbTimeGet(
      inX, inY, getContainerDecaySlot(inX, inY, inSlot, inSubCont, num),
      inSubCont);
    ...
    for (int i = inNumNewSlots; i < oldNum; i++) {
      ...
      if (foundEmpty) {
        ...
        if (subCont) {
          ...
          for (int s = 0; s < numSub; s++) {
            addContained(emptyX, emptyY, getContained(inX, inY, s, i + 1),
                         getSlotEtaDecay(inX, inY, s, i + 1));
          }
        }
      }
    }
...
MapChangeRecord getMapChangeRecord(ChangePosition inPos) {
  ...
  char *header = autoSprintf("%%d %%d %d ",
                             hideIDForClient(getMapFloor(inPos.x, inPos.y)));
  ...
  char *idString =
      autoSprintf("%d", hideIDForClient(getMapObjectNoLook(inPos.x, inPos.y)));
  ...
  for (int i = 0; i < numContained; i++) {
    ...
    if (subCont) {
      ...
      int numSubContained;
      int *subContained =
          getContainedNoLook(inPos.x, inPos.y, &numSubContained, i + 1);
...
char *getMapChangeLineString(MapChangeRecord *inRecord, int inRelativeToX,
                             int inRelativeToY) {
  ...
  if (inRecord->oldCoordsUsed) {
    lineString =
        autoSprintf(inRecord->formatString, inRecord->absoluteX - inRelativeToX,
...
static void cleanMaxContainedHashTable(int inX, int inY) {
  ...
  ContRecord *oldCount =
      liveDecayRecordLastLookTimeMaxContainedHashTable.lookupPointer(inX, inY,
                                                                     0, 0);
  ...
  if (oldCount != NULL) {
    ...
    for (int c = 1; c <= oldCount->maxSlots; c++) {
      ...
      for (int s = 0; s <= oldCount->maxSubSlots; s++) {
        timeSec_t *val =
            liveDecayRecordLastLookTimeHashTable.lookupPointer(inX, inY, c, s);
...
void stepMap(SimpleVector<MapChangeRecord> *inMapChanges,
  ...
  while (liveDecayQueue.size() > 0 &&
    ...
    char storedFound;
    timeSec_t storedETA = liveDecayRecordPresentHashTable.lookup(
        r.x, r.y, r.slot, r.subCont, &storedFound);
    ...
    if (storedFound && storedETA == r.etaTimeSeconds) {
      ...
      if (storedFound) {
        ...
        if (MAP_TIMESEC - lastLookTime > maxSecondsNoLookDecayTracking &&
          ...
          liveDecayRecordLastLookTimeHashTable.remove(r.x, r.y, r.slot,
                                                      r.subCont);
...
void restretchMapContainedDecays(int inX, int inY, int inOldContainerID,
...
void getEvePosition(const char *inEmail, int inID, int *outX, int *outY,
                    ...
                    char inAllowRespawn, char inIncrementPosition) {
  ...
  if (inAllowRespawn && result == 1 && pR > 0) {
    ...
    if (false)
      if (eveLocationUsage >= maxEveLocationUsage &&
        ...
        for (int p = 0; p < recentlyUsedPrimaryEvePositions.size(); p++) {
          ...
          if (curTime -
                  recentlyUsedPrimaryEvePositionTimes.getElementDirect(p) >
              recentlyUsedPrimaryEvePositionTimeout) {
            // timed out
            reusePos = true;
          } else if (!doesEveLineExist(recentlyUsedPrimaryEvePositionPlayerIDs
                                           .getElementDirect(p))) {
            // eve line extinct
            reusePos = true;
          }
          ...
          for (int y = -r; y <= r; y++) {
            for (int x = -r; x <= r; x++) {
              ...
              for (int p = 0; p < recentlyUsedPrimaryEvePositions.size(); p++) {
                ...
                GridPos pos =
                    recentlyUsedPrimaryEvePositions.getElementDirect(p);
              ...
              if (mapID == evePrimaryLocObjectID) {
                ...
                found = true;
                foundP = tryP;
              } else if (eveSecondaryLocObjectIDs.getElementIndex(mapID) !=
                         -1) {
    ...
    if (eveLocationUsage < maxEveLocationUsage) {
      ...
      if (eveStartSpiralPosSet && longTermCullEnabled) {
        ...
        int longTermCullingSeconds = SettingsManager::getIntSetting(
        ...
        timeSec_t lastLookTime =
            dbLookTimeGet(eveStartSpiralPos.x, eveStartSpiralPos.y);
  ...
  while (!found) {
    ...
    while (!found && tryCount < 100) {
      ...
      doublePair p = {randSource.getRandomBoundedDouble(-currentEveRadius,
                                                        +currentEveRadius),
...
char loadTutorialStep(TutorialLoadProgress *inTutorialLoad,
  ...
  char moreLeft = loadIntoMapFromFile(inTutorialLoad->file, inTutorialLoad->x,
                                      inTutorialLoad->y, inTimeLimitSec);
...
GridPos getNextCloseLandingPos(GridPos inCurPos, doublePair inDir,
                               char *outFound) {
...
GridPos getNextFlightLandingPos(int inCurrentX, int inCurrentY,
                                doublePair inDir, int inRadiusLimit) {
  int closestIndex = -1;
  GridPos closestPos;
  double closestDist = DBL_MAX;
  ...
  if (abs(inCurrentX) <= inRadiusLimit && abs(inCurrentY) <= inRadiusLimit) {
    useLimit = true;
  }
  ...
  for (int i = 0; i < flightLandingPos.size(); i++) {
    ...
    if (useLimit &&
        (abs(thisPos.x) > inRadiusLimit || abs(thisPos.x) > inRadiusLimit)) {
  ...
  if (inRadiusLimit > 0 &&
      (abs(eveX) >= inRadiusLimit || abs(eveY) >= inRadiusLimit)) {
...
void stepMapLongTermCulling(int inNumCurrentPlayers) {
  ...
  if (curTime - lastSettingsLoadTime > settingsLoadInterval) {
    ...
    numTilesExaminedPerCullStep =
        SettingsManager::getIntSetting("numTilesExaminedPerCullStep", 10);
    longTermCullingSeconds =
        SettingsManager::getIntSetting("longTermNoLookCullSeconds", 3600 * 12);
    minActivePlayersForLongTermCulling = SettingsManager::getIntSetting(
