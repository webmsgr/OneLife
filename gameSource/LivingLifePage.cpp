...
static const char *bugEmail = "jason"
                              "rohrer"
                              "@"
                              "fastmail.fm";
...
static int getHomeDir(doublePair inCurrentPlayerPos,
                      double *outTileDistance = NULL, char *outTooClose = NULL,
                      char *outTemp = NULL,
...
char *getRelationName(SimpleVector<int> *ourLin, SimpleVector<int> *theirLin,
                      int ourID, int theirID, int ourDisplayID,
                      int theirDisplayID, double ourAge, double theirAge,
  ...
  if (!found) {
    ...
    for (int i = 0; i < ourLin->size(); i++) {
      for (int j = 0; j < theirLin->size(); j++) {
        ...
        if (ourLin->getElementDirect(i) == theirLin->getElementDirect(j)) {
          ourMatchIndex = i;
          theirMatchIndex = j;
          break;
        }
    ...
    if (ourMatchIndex == -1) {
      ...
      if (ourEveID != -1 && theirEveID != -1 && ourEveID == theirEveID) {
  ...
  if (numGreats <= 4) {
    ...
    char *greatCount = autoSprintf("%dX %s", numGreats, translate("great"));
...
char *getRelationName(LiveObject *inOurObject, LiveObject *inTheirObject) {
  ...
  return getRelationName(
      ourLin, theirLin, ourID, theirID, inOurObject->displayID,
      inTheirObject->displayID, inOurObject->age, inTheirObject->age,
      inOurObject->lineageEveID, inTheirObject->lineageEveID);
}
    ...
    if (inObj->tempAgeOverrideSet) {
      ...
      if (curTime - inObj->tempAgeOverrideSetTime < 5) {
        ...
        return inObj->tempAgeOverride +
               inObj->ageRate * (curTime - inObj->tempAgeOverrideSetTime);
      } else {
        // temp override over
...
doublePair getVectorFromCamera(int inMapX, int inMapY) {
  doublePair vector = {inMapX - lastScreenViewCenter.x / CELL_D,
...
char *getNextServerMessageRaw() {
  ...
  if (pendingCMData) {
    if (serverSocketBuffer.size() >= pendingCMCompressedSize) {
      ...
      unsigned char *decompressedMessage = zipDecompress(
          compressedData, pendingCMCompressedSize, pendingCMDecompressedSize);
  ...
  if (getMessageType(message) == MAP_CHUNK) {
    ...
    sscanf(message, "MC\n%d %d %d %d\n%d %d\n", &sizeX, &sizeY, &x, &y,
           &binarySize, &pendingCompressedChunkSize);
  ...
  if (!waitForFrameMessages) {
    ...
    if (!serverFrameReady) {
      ...
      while (message != NULL) {
        ...
        } else if (t == MAP_CHUNK || t == PONG || t == FLIGHT_DEST ||
                   t == PHOTO_SIGNATURE) {
...
static double measurePathLength(LiveObject *inObject, int inPathLength) {
...
void updateMoveSpeed(LiveObject *inObject) {
  ...
  if (inObject->currentPathStep < inObject->pathLength - 1) {
    GridPos lastDir = sub(inObject->pathToDest[inObject->currentPathStep + 1],
                          inObject->pathToDest[inObject->currentPathStep]);

    for (int p = inObject->currentPathStep + 1; p < inObject->pathLength - 1;
         p++) {
...
static void fixSingleStepPath(LiveObject *inObject) {
  ...
  if (!equal(inObject->currentPos, nextWorld)) {
    ...
    vectorAway = normalize(sub(inObject->currentPos, nextWorld));
  } else {
    vectorAway.x = 1;
    vectorAway.y = 0;
  }
...
char LivingLifePage::isBadBiome(int inMapI) {
  ...
  if (mMapFloors[inMapI] == 0 && mBadBiomeIndices.getElementIndex(b) != -1) {
    return true;
  }
  return false;
}
...
void LivingLifePage::computePathToDest(LiveObject *inObject) {
  ...
  if (startInd != -1) {
    ...
    if (startPointBad || isBadBiome(startInd - 1) || isBadBiome(startInd + 1) ||
        isBadBiome(startInd - mMapD) || isBadBiome(startInd + mMapD)) {
  ...
  if (inObject->holdingID > 0 && getObject(inObject->holdingID)->rideable) {
      ...
      if (mapY >= 0 && mapY < mMapD && mapX >= 0 && mapX < mMapD) {
        ...
        if (mMap[mapI] > 0) {
          ...
          if (o->wide) {
            ...
            for (int dx = -o->leftBlockingRadius; dx <= o->rightBlockingRadius;
                 dx++) {
  ...
  if (inObject->useWaypoint) {
    ...
    pathFound = pathFind(pathFindingD, pathFindingD, blockedMap, start,
                         waypoint, end, &(inObject->pathLength),
                         &(inObject->pathToDest), &closestFound);
    ...
    pathFound = pathFind(pathFindingD, pathFindingD, blockedMap, start, end,
                         &(inObject->pathLength), &(inObject->pathToDest),
                         &closestFound);
  }
  ...
  if (pathFound && inObject->pathToDest != NULL) {
    ...
    inObject->currentMoveDirection = normalize(sub(bPos, aPos));
  } else {
    ...
    if (!pathFound) {
      ...
      inObject->closestDestIfPathFailedX = closestFound.x - pathOffsetX;

      inObject->closestDestIfPathFailedY = closestFound.y - pathOffsetY;
    } else {
...
static void addNewAnimDirect(LiveObject *inObject, AnimType inNewAnim) {
  ...
  } else if (inObject->curAnim == moving && inObject->lastAnim != moving &&
...
static void addNewHeldAnimDirect(LiveObject *inObject, AnimType inNewAnim) {
  ...
  if (inObject->lastHeldAnim == moving) {
    inObject->heldFrozenRotFrameCount = inObject->lastHeldAnimationFrameCount;
    ...
    if (inObject->lastAnimFade != 0 && inObject->curAnim != ground &&
      ...
      if (inObject->futureAnimStack->size() == 0 ||
        ...
        while (!foundNonGround && inObject->futureAnimStack->size() > 0) {
          ...
          int prevAnim = inObject->futureAnimStack->getElementDirect(
    ...
    if (inObject->lastHeldAnimFade != 0) {
      ...
      if (inObject->futureHeldAnimStack->size() == 0 ||
          inObject->futureHeldAnimStack->getElementDirect(
              inObject->futureHeldAnimStack->size() - 1) != newHeldAnim) {
...
void LivingLifePage::clearMap() {
  for (int i = 0; i < mMapD * mMapD; i++) {
    ...
    mMapAnimationLastFrameCount[i] = randSource.getRandomBoundedInt(0, 10000);

    mMapAnimationFrozenRotFrameCount[i] = 0;
    mMapAnimationFrozenRotFrameCountUsed[i] = false;

    mMapFloorAnimationFrameCount[i] = randSource.getRandomBoundedInt(0, 10000);
...
LivingLifePage::LivingLifePage()
    : mServerSocket(-1), mForceRunTutorial(false), mTutorialNumber(0),
      mGlobalMessageShowing(false), mGlobalMessageStartTime(0),
      mFirstServerMessagesReceived(0), mMapGlobalOffsetSet(false), mMapD(MAP_D),
      mMapOffsetX(0), mMapOffsetY(0), mEKeyEnabled(false), mEKeyDown(false),
      ...
      mHomeSlip2Sprite(loadSprite("homeSlip2.tga", false)), mLastMouseOverID(0),
      mCurMouseOverID(0), mChalkBlotSprite(loadWhiteSprite("chalkBlot.tga")),
      mPathMarkSprite(loadWhiteSprite("pathMark.tga")),
      mSayField(handwritingFont, 0, 1000, 10, true, NULL,
                "ABCDEFGHIJKLMNOPQRSTUVWXYZ.-,'?!/ "),
      mDeathReason(NULL), mShowHighlights(true), mUsingSteam(false),
      mZKeyDown(false), mXKeyDown(false),
  ...
  const char *badgeSettingsNames[3] = {"badgeObjects", "badgeObjectsHalfX",
                                       "badgeObjectsFullX"};
  for (int i = 0; i < 3; i++) {
  ...
  mEKeyEnabled = SettingsManager::getIntSetting("eKeyForRightClick", 0);
  ...
  if (teaserVideo) {
    ...
    mTeaserArrowVeryShortSprite = loadWhiteSprite("teaserArrowVeryShort.tga");
...
void LivingLifePage::runTutorial() { mForceRunTutorial = true; }

void LivingLifePage::clearLiveObjects() {
  for (int i = 0; i < gameObjects.size(); i++) {

    LiveObject *nextObject = gameObjects.getElement(i);
...
LivingLifePage::~LivingLifePage() {
  printf("Total received = %d bytes (+%d in headers), "
         ...
         numServerBytesRead, overheadServerBytesRead, numServerBytesSent,
         overheadServerBytesSent);
...
char LivingLifePage::isMapBeingPulled() { return mapPullMode; }
...
void LivingLifePage::adjustAllFrameCounts(double inOldFrameRateFactor,
  ...
  for (int i = 0; i < numMapCells; i++) {
    ...
    mMapAnimationFrozenRotFrameCount[i] = lrint(timeVal / inNewFrameRateFactor);

    timeVal = inOldFrameRateFactor * mMapAnimationLastFrameCount[i];
    mMapAnimationLastFrameCount[i] = lrint(timeVal / inNewFrameRateFactor);

    timeVal = inOldFrameRateFactor * mMapFloorAnimationFrameCount[i];
    mMapFloorAnimationFrameCount[i] = lrint(timeVal / inNewFrameRateFactor);
  }
...
LiveObject *LivingLifePage::getOurLiveObject() { return getLiveObject(ourID); }
...
// forces uppercase
void LivingLifePage::drawChalkBackgroundString(
    doublePair inPos, const char *inString, double inFade, double inMaxWidth,
    LiveObject *inSpeaker, int inForceMinChalkBlots,
    FloatColor *inForceBlotColor, FloatColor *inForceTextColor) {
  ...
  for (int i = 0; i < lines->size(); i++) {
    ...
    doublePair firstBlot = {inPos.x, firstLineY - i * lineSpacing};
  ...
  for (int i = 0; i < lines->size(); i++) {
    ...
    doublePair lineStart = {inPos.x, firstLineY - i * lineSpacing};
  ...
  if (speechColorImage != NULL && speechMaskImage != NULL) {
    ...
    memcpy(alpha, subMask->getChannel(0), w * h * sizeof(double));
    ...
    Image *trimmed = blend.getSubImage(minX, minY, maxX - minX, maxY - minY);

    File screenShots(NULL, "screenShots");

    char *fileName = autoSprintf("speechBlend%04d.tga", savingSpeechNumber);
    savingSpeechNumber++;
...
static void addOffScreenSound(int inSourcePlayerID, double inPosX,
                              double inPosY, char *inDescription,
                              double inFadeSec = 4) {
...
void LivingLifePage::drawOffScreenSounds() {
  ...
  for (int i = 0; i < offScreenSounds.size(); i++) {
    ...
    if (fabs(s->pos.x - lastScreenViewCenter.x) > xRadius ||
      ...
      drawChalkBackgroundString(drawPos, stringToDraw, s->fade, 100, NULL, -1,
                                bgColor, textColor);
    }
  }
}

void LivingLifePage::handleAnimSound(int inSourcePlayerID, int inObjectID,
                                     double inAge, AnimType inType,
  ...
  if (anim != NULL) {
    ...
    for (int s = 0; s < anim->numSounds; s++) {
      ...
      if (hz != 0) {
        ...
        int oldPeriods = lrint(floor((oldTimeVal - startOffsetSec) / period));

        int newPeriods = lrint(floor((newTimeVal - startOffsetSec) / period));
        ...
        if (newPeriods > oldPeriods) {
          ...
          if (inSourcePlayerID != ourID &&
            ...
            addOffScreenSound(inSourcePlayerID, inPosX * CELL_D,
                              inPosY * CELL_D, des, 0.5);
          }
        }
      }
    }
...
void LivingLifePage::drawMapCell(int inMapI, int inScreenX, int inScreenY,
                                 char inHighlightOnly, char inNoTimeEffects) {
  ...
  if (oID > 0) {
    ...
    if (mMapDropOffsets[inMapI].x != 0 || mMapDropOffsets[inMapI].y != 0) {

      doublePair nullOffset = {0, 0};

      doublePair delta = sub(nullOffset, mMapDropOffsets[inMapI]);
      ...
      if (length(delta) < step) {
        ...
        mMapDropOffsets[inMapI] =
            add(mMapDropOffsets[inMapI], mult(normalize(delta), step));
      }
      ...
      if (mMapDropOffsets[inMapI].x == 0 && mMapDropOffsets[inMapI].y == 0) {
        ...
        if (mMapDropSounds[inMapI].numSubSounds > 0) {
          ...
          playSound(mMapDropSounds[inMapI],
                    getVectorFromCamera((double)inScreenX / CELL_D,
      ...
      if (fabs(rotDelta) < rotStep) {
        ...
        mMapDropRot[inMapI] = mMapDropRot[inMapI] + rotSign * rotStep;
      }
    ...
    if (mMapMoveSpeeds[inMapI] > 0 &&
        (mMapMoveOffsets[inMapI].x != 0 || mMapMoveOffsets[inMapI].y != 0)) {
    ...
    double timeVal = frameRateFactor * mMapAnimationFrameCount[inMapI] / 60.0;

    double frozenRotTimeVal =
        frameRateFactor * mMapAnimationFrozenRotFrameCount[inMapI] / 60.0;
    ...
    if (mMapLastAnimFade[inMapI] != 0) {
      ...
      timeVal = frameRateFactor * mMapAnimationLastFrameCount[inMapI] / 60.0;
    }
    ...
    if (obj->noFlip ||
        (obj->permanent && (obj->blocksWalking || obj->drawBehindPlayer ||
    ...
    if (!mapPullMode && !inHighlightOnly && !inNoTimeEffects) {
      handleAnimSound(-1, oID, 0, mMapCurAnimType[inMapI], oldFrameCount,
                      mMapAnimationFrameCount[inMapI], pos.x / CELL_D,
                      pos.y / CELL_D);
    }
    ...
    for (int i = startPass; i < numPasses; i++) {
      ...
      if (mMapContainedStacks[inMapI].size() > 0) {
        ...
        drawObjectAnim(
            oID, curType, timeVal, animFade, fadeTargetType, targetTimeVal,
            frozenRotTimeVal, &(mMapAnimationFrozenRotFrameCountUsed[inMapI]),
            endAnimType, endAnimType, passPos, rot, false, flip, -1, false,
            false, false, getEmptyClothingSet(), NULL,
            mMapContainedStacks[inMapI].size(), stackArray, subStackArray);
        delete[] stackArray;
        delete[] subStackArray;
      } else {
        drawObjectAnim(oID, 2, curType, timeVal, animFade, fadeTargetType,
                       targetTimeVal, frozenRotTimeVal,
                       &(mMapAnimationFrozenRotFrameCountUsed[inMapI]),
                       endAnimType, endAnimType, passPos, rot, false, flip, -1,
                       false, false, false, getEmptyClothingSet(), NULL);
      }
...
GridPos LivingLifePage::getMapPos(int inWorldX, int inWorldY) {
  GridPos p = {inWorldX - mMapOffsetX + mMapD / 2,
...
int LivingLifePage::getMapIndex(int inWorldX, int inWorldY) {
  ...
  if (mapTarget.y >= 0 && mapTarget.y < mMapD && mapTarget.x >= 0 &&
      mapTarget.x < mMapD) {
...
ObjectAnimPack
LivingLifePage::drawLiveObject(LiveObject *inObj,
  ...
  if (inObj->heldByDropOffset.x != 0 || inObj->heldByDropOffset.y != 0) {

    doublePair nullOffset = {0, 0};

    doublePair delta = sub(nullOffset, inObj->heldByDropOffset);
    ...
    if (length(delta) < step) {
      ...
      ObjectRecord *displayObj = getObject(inObj->displayID);
      ...
      if (displayObj->usingSound.numSubSounds > 0) {
        ...
        playSound(
            displayObj->usingSound,
            getVectorFromCamera(inObj->currentPos.x, inObj->currentPos.y));
      }
    } else {
      inObj->heldByDropOffset =
          add(inObj->heldByDropOffset, mult(normalize(delta), step));
    }
  ...
  if (inObj->curAnim != eating && inObj->lastAnim != eating &&
    ...
    double offset =
        halfWiggleMax -
        halfWiggleMax * cos(2 * M_PI * inObj->pendingActionAnimationProgress);
  ...
  double timeVal = frameRateFactor * inObj->animationFrameCount / 60.0;

  double targetTimeVal = timeVal;

  double frozenRotTimeVal = frameRateFactor * inObj->frozenRotFrameCount / 60.0;
  ...
  if (inObj->lastAnimFade > 0) {
    ...
    timeVal = frameRateFactor * inObj->lastAnimationFrameCount / 60.0;
  }
  ...
  if (hideClosestArm == -2 ||
      ...
      (inObj->lastHoldingID > 0 && getObject(inObj->lastHoldingID)->rideable)) {
  ...
  if (inObj->hasBadge && inObj->clothing.tunic != NULL) {
    ...
    if (inObj->leadershipLevel < mLeadershipBadges[badgeXIndex].size()) {
      badge = mLeadershipBadges[badgeXIndex].getElementDirect(
          inObj->leadershipLevel);
    } else if (mLeadershipBadges[badgeXIndex].size() > 0) {
      badge = mLeadershipBadges[badgeXIndex].getElementDirect(
          mLeadershipBadges[badgeXIndex].size() - 1);
    }
  ...
  if (inObj->holdingID > 0 && heldObject->rideable) {
    // don't draw now,
    ...
    if (inObj->ridingOffset.x != 0 || inObj->ridingOffset.y != 0) {
      ...
      if (length(delta) < step) {
        ...
        inObj->ridingOffset =
            add(inObj->ridingOffset, mult(normalize(delta), step));
      }
    ...
    setAnimationBadge(badge, (badge == mFullXObjectID));
    if (badge != -1) {
    ...
    holdingPos = drawObjectAnim(
        inObj->displayID, 2, curType, timeVal, animFade, fadeTargetType,
        targetTimeVal, frozenRotTimeVal, &(inObj->frozenRotFrameCountUsed),
        frozenArmType, frozenArmFadeTargetType, personPos, rot, false,
        inObj->holdingFlip, age,
        // don't actually hide body parts until
        // held object is done sliding into place
        hideClosestArm, hideAllLimbs,
        inObj->heldPosOverride && !inObj->heldPosOverrideAlmostOver,
        inObj->clothing, inObj->clothingContained);
  ...
  if (inObj->holdingID != 0) {
    ...
    computeHeldDrawPos(holdingPos, pos, heldObject, inObj->holdingFlip,
                       &holdPos, &holdRot);
    ...
    if (inObj->currentSpeed == 0 && inObj->heldPosOverride &&
      ...
      if (length(delta) < step) {
        ...
        inObj->heldObjectPos =
            add(inObj->heldObjectPos, mult(normalize(delta), step));
      ...
      if (fabs(rotDelta) < rotStep) {
        ...
        inObj->heldObjectRot = inObj->heldObjectRot + rotDir * rotStep;
    ...
    double heldTimeVal =
        frameRateFactor * inObj->heldAnimationFrameCount / 60.0;

    double targetHeldTimeVal = heldTimeVal;

    double frozenRotHeldTimeVal =
        frameRateFactor * inObj->heldFrozenRotFrameCount / 60.0;

    char heldFlip = inObj->holdingFlip;

    if (heldObject != NULL && heldObject->noFlip) {
      heldFlip = false;
    }
    ...
    if (!alreadyDrawnPerson) {
      ...
      targetRidingOffset =
          sub(targetRidingOffset,
              getAgeBodyOffset(
                  age, personObj->spritePos[getBodyIndex(personObj, age)]));

      // step toward target to smooth
      doublePair delta = sub(targetRidingOffset, inObj->ridingOffset);
      ...
      if (heldObject->anySpritesBehindPlayer) {
        ...
        if (inObj->numContained == 0) {
          drawObjectAnim(
              inObj->holdingID, curHeldType, heldTimeVal, heldAnimFade,
              fadeTargetHeldType, targetHeldTimeVal, frozenRotHeldTimeVal,
              &(inObj->heldFrozenRotFrameCountUsed), endAnimType, endAnimType,
              heldObjectDrawPos, holdRot, false, heldFlip, -1, false, false,
              false, getEmptyClothingSet(), NULL, 0, NULL, NULL);
        } else {
          drawObjectAnim(
              inObj->holdingID, curHeldType, heldTimeVal, heldAnimFade,
              fadeTargetHeldType, targetHeldTimeVal, frozenRotHeldTimeVal,
              &(inObj->heldFrozenRotFrameCountUsed), endAnimType, endAnimType,
              heldObjectDrawPos, holdRot, false, heldFlip, -1, false, false,
              false, getEmptyClothingSet(), NULL, inObj->numContained,
              inObj->containedIDs, inObj->subContainedIDs);
        }
      ...
      holdingPos = drawObjectAnim(
          inObj->displayID, 2, curType, timeVal, animFade, fadeTargetType,
          targetTimeVal, frozenRotTimeVal, &(inObj->frozenRotFrameCountUsed),
          frozenArmType, frozenArmFadeTargetType, personPos, 0, false,
          inObj->holdingFlip, age,
          // don't actually hide body parts until
          // held object is done sliding into place
          hideClosestArm, hideAllLimbs,
    ...
    if (inObj->lastHeldAnimFade > 0) {
      ...
      heldTimeVal = frameRateFactor * inObj->lastHeldAnimationFrameCount / 60.0;
    }
    ...
    if (inObj->holdingID < 0) {
      ...
      if (babyO != NULL) {
        ...
        if (babyO->holdingID > 0) {
          ObjectRecord *babyHoldingObj = getObject(babyO->holdingID);

          getArmHoldingParameters(babyHoldingObj, &hideClosestArmBaby,
                                  &hideAllLimbsBaby);
        }
        ...
        if (babyO->babyWiggle) {
          ...
          if (babyO->babyWiggleProgress > 1) {
            ...
            babyHeldPos.x +=
                wiggleDir * 8 *
                (cos(babyO->babyWiggleProgress * 2 * M_PI + M_PI) * 0.5 + 0.5);
          }
        }

        returnPack = drawObjectAnimPacked(
            babyO->displayID, curHeldType, heldTimeVal, heldAnimFade,
            fadeTargetHeldType, targetHeldTimeVal, frozenRotHeldTimeVal,
            &(inObj->heldFrozenRotFrameCountUsed), endAnimType, endAnimType,
            babyHeldPos,
            // never apply held rot to baby
            0, false, heldFlip, computeCurrentAge(babyO), hideClosestArmBaby,
            hideAllLimbsBaby, false, babyO->clothing, babyO->clothingContained,
            0, NULL, NULL);
      ...
      returnPack = drawObjectAnimPacked(
          ...
          fadeTargetHeldType, targetHeldTimeVal, frozenRotHeldTimeVal,
          &(inObj->heldFrozenRotFrameCountUsed), endAnimType, endAnimType,
          heldObjectDrawPos, holdRot, false, heldFlip, -1, false, false, false,
          getEmptyClothingSet(), NULL, 0, NULL, NULL);
    } else {
      returnPack = drawObjectAnimPacked(
          inObj->holdingID, curHeldType, heldTimeVal, heldAnimFade,
          fadeTargetHeldType, targetHeldTimeVal, frozenRotHeldTimeVal,
          &(inObj->heldFrozenRotFrameCountUsed), endAnimType, endAnimType,
          heldObjectDrawPos, holdRot, false, heldFlip, -1, false, false, false,
          getEmptyClothingSet(), NULL, inObj->numContained, inObj->containedIDs,
...
void LivingLifePage::drawHungerMaxFillLine(doublePair inAteWordsPos,
                                           ...
                                           char inSkipBar, char inSkipDashes) {
  ...
  while (dashPos.x > barPos.x + 9) {
    ...
    drawSprite(inDashSprites[numDashes % NUM_HUNGER_DASHES], drawPos);
    dashPos.x -= 15;
  ...
  drawSprite(inDashSprites[numDashes % NUM_HUNGER_DASHES], dashPos);
}

static void drawLine(SpriteHandle inSegmentSprite, doublePair inStart,
                     doublePair inEnd, FloatColor inColor) {
  ...
  doublePair spriteVerts[4] = {{inStart.x - perpDir.x, inStart.y - perpDir.y},
                               {inEnd.x - perpDir.x, inEnd.y - perpDir.y},
                               {inEnd.x + perpDir.x, inEnd.y + perpDir.y},
                               {inStart.x + perpDir.x, inStart.y + perpDir.y}};

  FloatColor spriteColors[4] = {inColor, inColor, inColor, inColor};

  drawSprite(inSegmentSprite, spriteVerts, spriteColors);
}

static double getBoundedRandom(int inX, int inY, double inUpper,
                               double inLower) {
...
static void drawFixedShadowStringWhite(const char *inString, doublePair inPos) {
  setDrawColor(1, 1, 1, 1);
...
static void drawGraph(SimpleVector<double> *inHistory, doublePair inPos,
  ...
  drawRect(inPos.x - 2, inPos.y - 2, inPos.x + historyGraphLength + 2,
  ...
  for (int i = 0; i < inHistory->size(); i++) {
    ...
    drawRect(inPos.x + i, inPos.y, inPos.x + i + 1,
...
static const char *numberTenKeys[8] = {"twenty", "thirty",  "forty",  "fifty",
                                       "sixty",  "seventy", "eighty", "ninety"};

// for numbers < 999
char *getSmallNumberString(int inNumber, const char *inUnits = "",
  ...
  if (tensPlusOnes < NUM_NUMBER_KEYS) {
    ...
    if (tensPlusOnes > 0) {
      tensString = stringDuplicate(translate(numberKeys[tensPlusOnes]));
    } else {
    ...
    tensString =
        autoSprintf("%s%s", translate(numberTenKeys[tens - 2]), hyphen);
  }
  if (hundreds > 0) {
    ...
    if (hundreds < NUM_NUMBER_KEYS) {
      rawHundredString = stringDuplicate(translate(numberKeys[hundreds]));
    } else {
      // recurse
      rawHundredString = getSmallNumberString(hundreds);
    }

    hundredsString = autoSprintf("%s %s%s%s", rawHundredString,
                                 translate("hundred"), andString, andSpace);
  ...
  char *final = autoSprintf("%s%s%s%s%s%s", hundredsString, tensString,
                            onesString, unitsSpace, inUnits, inPadding);
...
char *getSpokenNumber(unsigned int inNumber, int inSigFigs = 2) {
  ...
  if (billions > 0) {
    ...
    billionsString =
        getSmallNumberString(billions, translate("billion"), padding);
  } else {
  ...
  if (millions > 0) {
    ...
    millionsString =
        getSmallNumberString(millions, translate("million"), padding);
  } else {
  ...
  if (thousands > 0) {
    ...
    thousandsString =
        getSmallNumberString(thousands, translate("thousand"), padding);
...
void LivingLifePage::drawHomeSlip(doublePair inSlipPos, int inIndex) {
  ...
  if (ourLiveObject != NULL) {
    ...
    int arrowIndex = getHomeDir(ourLiveObject->currentPos, &homeDist, &tooClose,
                                &temporary, inIndex);

    if (arrowIndex == -1 || !mHomeArrowStates[inIndex][arrowIndex].solid) {
      // solid change
    ...
    if (homeDist > 1000) {
      ...
      if (thousands < 1000) {
        if (thousands < 10) {
          ...
          distString = autoSprintf("%.0fK", thousands);
        }
      } else {
      ...
      for (int i = 0; i < mPreviousHomeDistStrings[inIndex].size(); i++) {
        char *oldString = mPreviousHomeDistStrings[inIndex].getElementDirect(i);
      ...
      if (!alreadyOld) {
        ...
        // fade old ones
        for (int i = 0; i < mPreviousHomeDistFades[inIndex].size() - 1; i++) {
          float fade = mPreviousHomeDistFades[inIndex].getElementDirect(i);
          ...
          *(mPreviousHomeDistFades[inIndex].getElement(i)) = fade;

          if (fade <= 0) {
            mPreviousHomeDistFades[inIndex].deleteElement(i);
            mPreviousHomeDistStrings[inIndex].deallocateStringElement(i);
            i--;
          }
        }
      } else {
    ...
    if (numPrevious > 1 || (numPrevious == 1 && drawTopAsErased)) {
      ...
      for (int i = 0; i < limit; i++) {
        float fade = mPreviousHomeDistFades[inIndex].getElementDirect(i);
        char *string = mPreviousHomeDistStrings[inIndex].getElementDirect(i);

        setDrawColor(0, 0, 0, fade * pencilErasedFontExtraFade);
        pencilErasedFont->drawString(string, distPos, alignCenter);
      }
    }
  }
}
...
void LivingLifePage::draw(doublePair inViewCenter, double inViewSize) {
  ...
  if (stillWaitingBirth) {
    ...
    if (usingCustomServer) {
      ...
      char *message =
          autoSprintf(translate("customServerMesssage"), upperIP, serverPort);
      delete[] upperIP;
    ...
    if (!serverSocketConnected) {
      ...
      char *message = autoSprintf(translate("waitingBirthFriends"), sizeString);
    ...
    if (false && mStartedLoadingFirstObjectSet) {
      ...
      drawRect(pos.x - 100, pos.y - 120, pos.x + 100, pos.y - 100);

      // inner black
      setDrawColor(0, 0, 0, 1);

      drawRect(pos.x - 98, pos.y - 118, pos.x + 98, pos.y - 102);
    ...
    for (int y = yEndFloor; y >= yStartFloor; y--) {
      ...
      for (int x = xStartFloor; x <= xEndFloor; x++) {
        ...
        if (s == NULL) {
          ...
          // random draw color
          setDrawColor(getXYRandom(b, b), getXYRandom(b, b + 100),
            ...
            if (setY == 0 && setX == 0) {
              ...
              for (int nY = y + 1; nY >= y - s->numTilesHigh; nY--) {
                ...
                if (nY >= 0 && nY < mMapD) {
                  ...
                  for (int nX = x - 1; nX <= x + s->numTilesWide; nX++) {
              ...
              if (allSameBiome) {
                ...
                doublePair sheetPos = mult(add(pos, lastCornerPos), 0.5);
                ...
                for (int sY = y; sY > y - s->numTilesHigh; sY--) {
                  ...
                  if (sY >= 0 && sY < mMapD) {
                    ...
                    for (int sX = x; sX < x + s->numTilesWide; sX++) {
          ...
          if (pass == 0)
            if (!inBounds || !mMapCellDrawnFlags[mapI]) {
              ...
              if (leftB == b && aboveB == b && diagB == b) {
                ...
                if (!(floorAt && floorR && floorB && floorBR && floorL &&
                      floorA && floorAL && floorAR && floorBL)) {
            ...
            if (yMod == 0) {
              ...
              if (mCulvertStoneSpriteIDs.size() > 0) {
                ...
                for (int s = 0; s < 2; s++) {
                  ...
                  int stoneIndex = stonePicker.getRandomBoundedInt(
                      0, mCulvertStoneSpriteIDs.size() - 1);

                  int stoneSpriteID =
                      mCulvertStoneSpriteIDs.getElementDirect(stoneIndex);
                  ...
                  stoneJigglePos.y +=
                      getXYFractal(stoneJigglePos.x, stoneJigglePos.y,
  ...
  if (showFPS)
    startCountingSpritePixelsDrawn();
  ...
  for (int y = yEnd; y >= yStart; y--) {
    ...
    for (int x = xStart; x <= xEnd; x++) {
      ...
      for (int p = 0; p < 3; p++) {
        ...
        if (p == 1) {
          drawRect(pos.x - hugR, pos.y + hugR, pos.x, pos.y - hugR);
        } else if (p == 2) {

          drawRect(pos.x, pos.y + hugR, pos.x + hugR, pos.y - hugR);
        }
        ...
        if (!mapPullMode) {
          handleAnimSound(-1, oID, 0, ground, oldFrameCount,
                          mMapFloorAnimationFrameCount[mapI],
                          (double)screenX / CELL_D, (double)screenY / CELL_D);
        }

        double timeVal =
            frameRateFactor * mMapFloorAnimationFrameCount[mapI] / 60.0;
        ...
        char used;
        drawObjectAnim(oID, 2, ground, timeVal, 0, ground, timeVal, timeVal,
                       &used, ground, ground, pos, 0, false, false, -1, false,
                       false, false, getEmptyClothingSet(), NULL);
  ...
  if (showFPS)
    runningPixelCount += endCountingSpritePixelsDrawn();
  ...
  if (ourLiveObject != NULL &&
      (mCurrentHintTargetObject[0] > 0 || mCurrentHintTargetObject[1] > 0)) {
    ...
    if (heldID > 0 && mLastHintSortedList.size() > mCurrentHintIndex) {

      TransRecord *t = mLastHintSortedList.getElementDirect(mCurrentHintIndex);
      ...
      if (t->actor > 0 && getObjectParent(t->actor) == heldParent) {
        holdingActor = true;
      }
      if (t->target > 0 && getObjectParent(t->target) == heldParent) {
        holdingTarget = true;
      }

      if (t->newActor > 0 && t->newActor == heldID && !holdingActor) {
        hit = true;
      } else if (t->newTarget > 0 && t->newTarget == heldID && !holdingTarget) {
        hit = true;
      }
    }
    ...
    startWatchForClosestObjectDraw(actualWatchTargets,
                                   mult(ourLiveObject->currentPos, CELL_D));
  }
  ...
  if (mShowHighlights && mCurMouseOverCellFade > 0 &&
    ...
    int screenX = CELL_D * (mCurMouseOverCell.x + mMapOffsetX - mMapD / 2);
  ...
  if (mShowHighlights)
    for (int i = 0; i < mPrevMouseOverCells.size(); i++) {
      ...
      if (prev.x < 0 || prev.x >= mMapD || prev.y < 0 || prev.y >= mMapD) {
      ...
      int screenX = CELL_D * (prev.x + mMapOffsetX - mMapD / 2);
  ...
  if (ourLiveObject != NULL) {
    ...
    if (ourLiveObject->currentPos.x != ourLiveObject->xd ||
      ...
      if (ourLiveObject->pathToDest != NULL &&
          ourLiveObject->shouldDrawPathMarks && mShowHighlights) {
        // highlight path
        ...
        doublePair playerPos = mult(ourLiveObject->currentPos, CELL_D);
        ...
        for (int p = 1; p < ourLiveObject->pathLength; p++) {
          ...
          while (distance(curPos, nextPos) > closeDist) {
            ...
            if (dot(dir, curDir) >= 0) {
              curDir = normalize(add(curDir, mult(dir, turnFactor)));
            ...
            curPos = add(curPos, mult(curDir, 6));

            setDrawColor(0, 0, 0, ourLiveObject->pathMarkFade * endFade);
      ...
      if (o->currentPos.x != o->xd || o->currentPos.y != o->yd) {
        ...
        if (o->pathToDest != NULL) {
          ...
          for (int p = 0; p < o->pathLength; p++) {
            ...
            mainFont->drawString("P", pos, alignCenter);
          }
          ...
          mainFont->drawString("W", pos, alignCenter);
  ...
  for (int y = 0; y < mMapD; y++) {
    for (int x = 0; x < mMapD; x++) {
      ...
      if (mMap[mapI] > 0 && mMapMoveSpeeds[mapI] > 0) {
  ...
  for (int y = yEnd; y >= yStart; y--) {
    ...
    for (int d = 0; d < 2; d++)
      for (int x = xStart; x <= xEnd; x++) {
        ...
        for (int i = 0; i < gameObjects.size(); i++) {
          ...
          if (d == 0 &&
              (o->heldByDropOffset.x != 0 || o->heldByDropOffset.y != 0)) {
            // recently dropped baby, skip
            continue;
          } else if (d == 1 && o->heldByDropOffset.x == 0 &&
    ...
    for (int i = 0; i < mMapExtraMovingObjects.size(); i++) {
      ...
      ExtraMapObject *extraO = mMapExtraMovingObjects.getElement(i);
      ...
      if (movingCellY == worldY && movingX >= worldXStart &&
          movingX <= worldXEnd) {
    ...
    for (int q = 0; q < numQueued; q++) {
      ...
      if (drawRec.person) {
        ...
        ObjectAnimPack heldPack = drawLiveObject(o, &speakers, &speakersPos);
        ...
        if (heldPack.inObjectID != -1) {
          ...
          if (o->holdingID < 0) {
            ...
            if (babyO != NULL && babyO->dying && babyO->holdingID > 0) {
              // baby still holding something while dying,
              // likely a wound
              // add to pack to draw it on top of baby
              heldPack.additionalHeldID = babyO->holdingID;
            }
          }
          ...
          if (!o->heldPosOverride) {
            ...
            if (heldPack.inObjectID > 0 &&
                ...
                getObject(heldPack.inObjectID)->anySpritesBehindPlayer) {
              skippingSome = true;
            }
            if (skippingSome) {
              prepareToSkipSprites(getObject(heldPack.inObjectID), false);
            }

            if (heldPack.inObjectID > 0) {
              ObjectRecord *heldO = getObject(heldPack.inObjectID);

              if (heldO->toolSetIndex != -1 && !o->heldLearned) {
                // unleared tool

                if (heldO->heldInHand) {
                  // rotate 180

                  doublePair newCenterOffset = getObjectCenterOffset(heldO);

                  if (heldPack.inFlipH) {
                    newCenterOffset.x *= -1;
                  }
                  newCenterOffset =
                      rotate(newCenterOffset, -heldPack.inRot * 2 * M_PI);

                  heldPack.inPos =
                      add(heldPack.inPos, mult(newCenterOffset, 2));
                  ...
                  newHeldOffset =
                      rotate(newHeldOffset, -heldPack.inRot * 2 * M_PI);

                  heldPack.inPos = sub(heldPack.inPos, mult(newHeldOffset, 2));
            ...
            if (skippingSome) {
              restoreSkipDrawing(getObject(heldPack.inObjectID));
            }
          } else {
        ...
        ExtraMapObject *mO =
            mMapExtraMovingObjects.getElement(drawRec.extraMovingIndex);
    ...
    for (int x = xStart; x <= xEnd; x++) {
      ...
      if (mMap[mapI] > 0) {
        ...
        if (!o->drawBehindPlayer && !o->wallLayer && o->permanent &&
    ...
    for (int x = xStart; x <= xEnd; x++) {
      ...
      if (mMap[mapI] > 0) {
        ...
        if (!o->drawBehindPlayer && o->wallLayer && o->permanent &&
            !o->frontWall && mMapMoveSpeeds[mapI] == 0) {
  ...
  if (!takingPhoto)
    if (mCurMouseOverID > 0 && !mCurMouseOverSelf && mCurMouseOverBehind) {
      ...
      int screenX = CELL_D * (mCurMouseOverSpot.x + mMapOffsetX - mMapD / 2);
        ...
        if (mCurrentHintTargetPointerFade[i] == 0) {
          ...
          if (i == 0) {
            mCurrentHintTargetPointerBounce[i] =
                mCurrentHintTargetPointerBounce[1] + M_PI / 2;
          } else {
            mCurrentHintTargetPointerBounce[i] =
                mCurrentHintTargetPointerBounce[0] + M_PI / 2;
          }
        }
  ...
  if (!takingPhoto)
    for (int i = 0; i < speakers.size(); i++) {
      ...
      doublePair headPos = displayObj->spritePos[getHeadIndex(displayObj, age)];

      doublePair bodyPos = displayObj->spritePos[getBodyIndex(displayObj, age)];

      doublePair frontFootPos =
          displayObj->spritePos[getFrontFootIndex(displayObj, age)];

      headPos =
          add(headPos, getAgeHeadOffset(age, headPos, bodyPos, frontFootPos));
      headPos = add(headPos, getAgeBodyOffset(age, bodyPos));
      ...
      double fullWidth = handwritingFont->measureString(o->currentSpeech);
      ...
      drawChalkBackgroundString(speechPos, o->currentSpeech, o->speechFade,
                                widthLimit, o);
    }
  ...
  if (!takingPhoto)
    for (int i = 0; i < locationSpeech.size(); i++) {
      ...
      double fullWidth = handwritingFont->measureString(ls->speech);
      ...
      drawChalkBackgroundString(speechPos, ls->speech, ls->fade, widthLimit);
    }
  ...
  */
  ...
  if (false)
    for (int i = 0; i < trail.size(); i++) {
      ...
      mainFont->drawString(".", *p, alignCenter);
    }
  ...
  if (mapPullMode) {
    ...
    if (!mapPullCurrentSaved && isLiveObjectSetFullyLoaded(&progress)) {

      int screenWidth, screenHeight;
      getScreenDimensions(&screenWidth, &screenHeight);

      Image *screen = getScreenRegionRaw(0, 0, screenWidth, screenHeight);
      ...
      if (totalImStartX >= 0 && totalImStartX < totalW && totalImStartY >= 0 &&
          totalImStartY < totalH) {

        mapPullTotalImage->setSubImage(totalImStartX, totalImStartY,
          ...
          for (int x = xStart; x <= xEnd; x++) {
            ...
            if (worldX >= xLimit) {
              ...
              if (mMap[mapI] > 0) {
                ...
                numbered = (o->mapChance == 0 && worldX > 10 && !o->isUseDummy);
              } else if (worldX == -20 && worldY == -10 &&
                         (gameObjects.size() == 1 || gameObjects.size() == 2)) {
                baseDes = "PLAYER ONE";
              ...
              if (baseDes != NULL) {
                ...
                double randOrder = getXYRandom(worldX / 6, worldY + 300);

                int orderOffsets[3] = {0, 2, 4};

                // even distribution among 0,1,2
                int orderIndex = lrint((randOrder * 2.98) - 0.49);
                ...
                if (worldY != 0 || (worldX > 0 && worldX < 7)) {
                  ...
                  if ((worldX - 11) % 2 == 0) {
                    ...
                    if ((worldX - 11) % 6 == 2) {
                      ...
                      if (labelPos.y - lastScreenViewCenter.y > 224 + 86) {
                        labelPos.y = lastScreenViewCenter.y + 224 + 86;
                      }
                    }
                  } else {
                    labelPos.y -= 202;
                ...
                if (fabs(lastScreenViewCenter.x - screenX) < 400) {

                  if (fabs(lastScreenViewCenter.x - screenX) < 300) {
                    fade = 1;
                  } else {
                    fade =
                        (400 - fabs(lastScreenViewCenter.x - screenX)) / 100.0;
                  }
                }

                if (fade > 0) {
                  if (fabs(lastScreenViewCenter.y - screenY) > 200) {
                    fade = 0;
                  } else if (fabs(lastScreenViewCenter.y - screenY) > 100) {
                    fade =
                        (200 - fabs(lastScreenViewCenter.y - screenY)) / 100.0;
                  }
                }
                ...
                if (pass == 0) {
                  ...
                  double deltaY = arrowTipY - (labelPos.y + arrowStart);
                  ...
                  doublePair spriteVerts[4] = {
                      {labelPos.x - w, labelPos.y + arrowStart},
                      {labelPos.x + w, labelPos.y + arrowStart},
                      {(double)arrowTipX + w, (double)arrowTipY},
                      {(double)arrowTipX - w, (double)arrowTipY}};

                  FloatColor spriteColors[4] = {{1, 1, 1, (float)fade},
                                                {1, 1, 1, (float)fade},
                                                {1, 1, 1, (float)fade * .25f},
                                                {1, 1, 1, (float)fade * .25f}};

                  drawSprite(arrowSprite, spriteVerts, spriteColors);
                } else {
                  ...
                  SimpleVector<char *> *words = tokenizeString(des);
                  ...
                  char *newDes = join(wordArray, finalWords.size(), " ");
                  ...
                  if (worldY == 1 && numbered) {
                    finalDes = autoSprintf("%d. %s", worldX - 10, newDes);
                  } else {
                  ...
                  double w = mainFontReview->measureString(finalDes);
                  ...
                  if (blackBorder) {
                    ...
                    setDrawColor(1, 1, 1, 1);
                    drawRect(rectPos,
                             w / 2 + mainFontReview->getFontHeight() / 2, 24);

                    startDrawingThroughStencil(true);

                    setDrawColor(0, 0, 0, fade);
                    drawRect(rectPos,
                             w / 2 + mainFontReview->getFontHeight() / 2 + 2,
                             24 + 2);

                    stopStencil();
                  }

                  double rectW = w / 2 + mainFontReview->getFontHeight() / 2;
                  ...
                  if (whiteBorder) {
                    ...
                    FloatColor lineColor = {1, 1, 1, (float)fade};
                    ...
                    corners[0].x = rectPos.x - rectW -
                                   getBoundedRandom(worldX, 34, rOut, rIn);
                    corners[0].y = rectPos.y - rectH -
                                   getBoundedRandom(worldX, 87, rOut, rIn);

                    corners[1].x = rectPos.x + rectW +
                                   getBoundedRandom(worldX, 94, rOut, rIn);
                    corners[1].y = rectPos.y - rectH -
                                   getBoundedRandom(worldX, 103, rOut, rIn);

                    corners[2].x = rectPos.x + rectW +
                                   getBoundedRandom(worldX, 99, rOut, rIn);
                    corners[2].y = rectPos.y + rectH +
                                   getBoundedRandom(worldX, 113, rOut, rIn);

                    corners[3].x = rectPos.x - rectW -
                                   getBoundedRandom(worldX, 123, rOut, rIn);
                    corners[3].y = rectPos.y + rectH +
                                   getBoundedRandom(worldX, 135, rOut, rIn);

                    for (int i = 0; i < 4; i++) {

                      drawLine(mLineSegmentSprite, corners[i],
                               corners[(i + 1) % 4], lineColor);
                    }
                  }

                  setDrawColor(0, 0, 0, fade);
                  mainFontReview->drawString(finalDes, labelPos, alignCenter);
  ...
  if (takingPhoto) {
    ...
    if (photoSequenceNumber == -1) {
      ...
      char *message = autoSprintf("PHOTO %d %d %d#", takingPhotoGlobalPos.x,
                                  takingPhotoGlobalPos.y, photoSequenceNumber);
      ...
      takePhoto(pos, takingPhotoFlip ? -1 : 1, photoSequenceNumber, photoSig,
                ourID, ourName, &subjectIDs, &subjectNames);
  ...
  if (showFPS) {
    ...
    if (frameBatchMeasureStartTime == -1) {
      ...
      if (framesInBatch == 30) {
        double delta = game_getCurrentTime() - frameBatchMeasureStartTime;
        ...
        addToGraph(&uniqueSpriteHistoryGraph, uniqueSpriteCountToDraw);
    ...
    if (fpsToDraw != -1) {
      ...
      char *fpsString = autoSprintf("%5.1f %s", fpsToDraw, translate("fps"));
      ...
      for (int i = MEASURE_TIME_NUM_CATEGORIES - 1; i >= 0; i--) {
        ...
        char *timeString =
            autoSprintf("%4.1f %s %s", timeMeasureToDraw[i] * 1000,
                        timeMeasureNames[i], translate("ms/f"));
        pos.y -= 20;
      ...
      pos.y += 60;
      char *spriteString =
          autoSprintf("%6.0f %s", spriteCountToDraw, translate("spritesDrawn"));
      ...
      char *unqString = autoSprintf("%6.0f %s", uniqueSpriteCountToDraw,
      ...
      char *pixString =
          autoSprintf("%9s %s", pixBuffer, translate("pixelsDrawn"));
  ...
  if (showNet) {
    ...
    if (messagesInPerSec != -1) {
      char *netStringA = autoSprintf(translate("netStringA"), messagesOutPerSec,
                                     messagesInPerSec);
      char *netStringB =
          autoSprintf(translate("netStringB"), bytesOutPerSec, bytesInPerSec);
  ...
  if (showPing) {
    if (pongDeltaTime != -1 && pingDisplayStartTime == -1) {
    ...
    if (pongDeltaTime == -1) {
      pingString = autoSprintf("%s...", translate("ping"));
    } else {
      pingString = autoSprintf("%s %d %s", translate("ping"),
                               lrint(pongDeltaTime * 1000), translate("ms"));
    }
  ...
  if (!equal(mNotePaperPosOffset, mNotePaperHideOffset)) {
    ...
    handwritingFont->drawString(translate("enterHint"), drawPos, alignRight);
  }
  ...
  if (mSayField.isFocused()) {
    ...
    for (int i = 0; i < mLastKnownNoteLines.size(); i++) {
      ...
      pencilFont->getCharPos(&charPos, oldString, drawPosTemp, alignLeft);
    ...
    for (int i = 0; i < lines->size(); i++) {
      ...
      pencilFont->drawString(line, drawPos, alignLeft);

      SimpleVector<doublePair> charPos;

      pencilFont->getCharPos(&charPos, line, drawPos, alignLeft);
    ...
    for (int i = 0; i < mLastKnownNoteLines.size(); i++) {
      ...
      pencilFont->getCharPos(&charPos, oldString, drawPos, alignLeft);
  ...
  for (int i = 0; i < NUM_HINT_SHEETS; i++) {
    if (!equal(mHintPosOffset[i], mHintHideOffset[i]) &&
      ...
      if (mNumTotalHints[i] > 1) {
        ...
        pageNum = autoSprintf("(%d %s %d)", mHintMessageIndex[i] + 1,
                              translate("ofHint"), mNumTotalHints[i]);

        double extraA = handwritingFont->measureString(pageNum);

        double extraB = handwritingFont->measureString(translate("tabHint"));

        double extraC = handwritingFont->measureString(translate(shiftHintKey));
      ...
      for (int l = 0; l < numLines; l++) {
        ...
        if (l == 1) {
          ...
          handwritingFont->drawString("+", drawPos, alignRight);
        }
        ...
        handwritingFont->drawString(lines[l], lineStart, alignLeft);
      ...
      if (pageNum != NULL) {
        ...
        handwritingFont->drawString(translate(shiftHintKey), lineStart,
                                    alignCenter);

        lineStart.y -= lineSpacing;

        handwritingFont->drawString(translate("tabHint"), lineStart,
                                    alignCenter);
      }
    }
  }
  ...
  for (int i = 0; i < mErasedNoteChars.size(); i++) {
    ...
    pencilErasedFont->drawCharacterSprite(
        mErasedNoteChars.getElementDirect(i),
        add(paperPos, mErasedNoteCharOffsets.getElementDirect(i)));
  }
  ...
  for (int i = 0; i < 3; i++) {
    if (!equal(mHungerSlipPosOffset[i], mHungerSlipHideOffsets[i])) {
      ...
      if (mHungerSlipWiggleAmp[i] > 0) {
        ...
        double slipHarmonic = (0.5 * (1 - cos(mHungerSlipWiggleTime[i]))) *
        ...
        if (i == 2) {
          ...
          if (mStarvingSlipLastPos[0] != 0 && mStarvingSlipLastPos[1] != 0) {
            double lastDir = mStarvingSlipLastPos[1] - mStarvingSlipLastPos[0];

            if (lastDir > 0) {

              double newDir = slipHarmonic - mStarvingSlipLastPos[1];

              if (newDir < 0) {
                // peak
                if (mPulseHungerSound && mHungerSound != NULL) {
                  // make sure it can be heard, even
                  // if paused
                  setSoundLoudness(1.0);
                  playSoundSprite(mHungerSound, getSoundEffectsLoudness(),
                                  // middle
                                  0.5);
                }
              }
  ...
  for (int i = 0; i < NUM_YUM_SLIPS; i++) {
    ...
    if (!equal(mYumSlipPosOffset[i], mYumSlipHideOffset[i])) {
      doublePair slipPos = add(mYumSlipPosOffset[i], lastScreenViewCenter);
      setDrawColor(1, 1, 1, 1);
  ...
  if (ourLiveObject != NULL && ourLiveObject->dying && !ourLiveObject->sick) {
    toggleMultiplicativeBlend(true);
  ...
  if (ourLiveObject != NULL) {
    ...
    if (ourLiveObject->excessCursePoints > 0) {
      ...
      char *pointString = autoSprintf("%d", ourLiveObject->excessCursePoints);
    ...
    for (int i = 0; i < ourLiveObject->foodCapacity; i++) {
      ...
      pos.x += i * 30;
      drawSprite(mHungerBoxSprites[i % NUM_HUNGER_BOX_SPRITES], pos);

      if (i < ourLiveObject->foodStore) {
        drawSprite(mHungerBoxFillSprites[i % NUM_HUNGER_BOX_SPRITES], pos);
      } else if (i < ourLiveObject->maxFoodStore) {
        drawSprite(mHungerBoxFillErasedSprites[i % NUM_HUNGER_BOX_SPRITES],
                   pos);
      }
    }
    for (int i = ourLiveObject->foodCapacity;
      ...
      pos.x += i * 30;
      drawSprite(mHungerBoxErasedSprites[i % NUM_HUNGER_BOX_SPRITES], pos);
    ...
    for (int i = 0; i < mOldDesStrings.size(); i++) {
      doublePair pos = {lastScreenViewCenter.x, lastScreenViewCenter.y - 313};
      float fade = mOldDesFades.getElementDirect(i);

      setDrawColor(0, 0, 0, fade * pencilErasedFontExtraFade);
      pencilErasedFont->drawString(mOldDesStrings.getElementDirect(i), pos,
                                   alignCenter);
    }
    ...
    for (int i = 0; i < mOldYumBonus.size(); i++) {
      float fade = mOldYumBonusFades.getElementDirect(i);

      setDrawColor(0, 0, 0, fade * pencilErasedFontExtraFade);
      char *yumString = autoSprintf("+%d", mOldYumBonus.getElementDirect(i));
      pencilErasedFont->drawString(yumString, yumPos, alignLeft);
      delete[] yumString;
    }

    doublePair atePos = {lastScreenViewCenter.x, lastScreenViewCenter.y - 347};

    int shortestFill = 100;

    for (int i = 0; i < mOldLastAteStrings.size(); i++) {
      float fade = mOldLastAteFades.getElementDirect(i);

      setDrawColor(0, 0, 0, fade * pencilErasedFontExtraFade);

      pencilErasedFont->drawString(mOldLastAteStrings.getElementDirect(i),
                                   atePos, alignLeft);
      ...
      drawHungerMaxFillLine(atePos, fillMax, mHungerBarErasedSprites,
                            mHungerDashErasedSprites, false,
    ...
    if (shortestFill < 100) {
      ...
      drawHungerMaxFillLine(atePos, shortestFill, mHungerBarErasedSprites,
                            mHungerDashErasedSprites, true,
    ...
    if (mCurrentLastAteString != NULL) {
      ...
      pencilFont->drawString(mCurrentLastAteString, atePos, alignLeft);

      toggleMultiplicativeBlend(true);
      setDrawColor(1, 1, 1, 1);

      drawHungerMaxFillLine(atePos, mCurrentLastAteFillMax, mHungerBarSprites,
                            mHungerDashSprites, false, false);

      toggleMultiplicativeBlend(false);
    }

    doublePair tipPos = {lastScreenViewCenter.x, lastScreenViewCenter.y - 313};

    char overTempMeter = false;

    doublePair mousePos = {lastMouseX, lastMouseY};

    if (mousePos.y < tipPos.y + 13 && mousePos.x > tipPos.x + 480 &&
    ...
    if (mCurMouseOverBiome != -1 &&
        mBadBiomeIndices.getElementIndex(mCurMouseOverBiome) != -1) {
      badBiome = true;
      mLastMouseOverID = 0;
      mCurMouseOverID = 0;
    }

    if ((overTempMeter && ourLiveObject->foodDrainTime > 0) || badBiome ||
      ...
      if (overTempMeter && ourLiveObject->foodDrainTime > 0) {
        ...
        // changes
        if (mCurrentDes == NULL ||
            strstr(mCurrentDes, translate("foodTimeString")) == NULL) {
          ...
          if (ourLiveObject->indoorBonusTime > 0) {
            indoorBonusString =
                autoSprintf(translate("indoorBonusFormatString"),
          ...
          des = autoSprintf(translate("foodTimeFormatString"),
                            translate("foodTimeString"), mainSeconds,
                            indoorBonusString);
        ...
        if (ourLiveObject->holdingID > 0 &&
          ...
          if (strstr(getObject(ourLiveObject->holdingID)->description,
                     "+drink") != NULL) {
            key = "drink";
          }

          des = autoSprintf("%s %s", translate(key),
                            getObject(ourLiveObject->holdingID)->description);
          desToDelete = des;
        } else if ((ourLiveObject->dying || isSick(ourLiveObject)) &&
                   ourLiveObject->holdingID > 0) {
          des = autoSprintf("%s %s", translate("youWith"),
                            getObject(ourLiveObject->holdingID)->description);
          desToDelete = des;
        } else {
          ...
          if (ourLiveObject->leadershipNameTag != NULL ||
            ...
            char *workingName;
            if (ourLiveObject->name != NULL) {
              workingName = autoSprintf(" %s", ourLiveObject->name);
            } else {
            ...
            des = autoSprintf("%s - %s%s", des, leaderString, workingName);
            desToDelete = des;
        ...
        if (otherObj != NULL && otherObj->name != NULL) {
          des = autoSprintf("%s - %s", otherObj->name, des);
        ...
        if (otherObj != NULL && otherObj->leadershipNameTag != NULL) {
          if (otherObj->name == NULL) {
            des = autoSprintf("%s - %s", otherObj->leadershipNameTag, des);
          } else {
            des = autoSprintf("%s %s", otherObj->leadershipNameTag, des);
          }
        ...
        if (otherObj != NULL && (otherObj->dying || isSick(otherObj)) &&
            otherObj->holdingID > 0) {
          des = autoSprintf("%s - %s %s", des, translate("with"),
                            getObject(otherObj->holdingID)->description);
          if (desToDelete != NULL) {
            delete[] desToDelete;
          }
        ...
        int bInd = mBadBiomeIndices.getElementIndex(mCurMouseOverBiome);
        ...
        if (strstr(des, "origGrave") != NULL) {
          ...
          for (int g = 0; g < mGraveInfo.size(); g++) {
            ...
            if (gI->worldPos.x == mCurMouseOverWorld.x &&
              ...
              int years = lrint((game_getCurrentTime() - gI->creationTime) *
              ...
              if (years >= NUM_NUMBER_KEYS) {
                if (years > 1000000) {
                  ...
                  yearsString = autoSprintf("%d,%d,%d", mil, thou, extra);
                } else if (years > 1000) {
                  yearsString =
                      autoSprintf("%d,%d", years / 1000, years % 1000);
                } else {
                  yearsString = autoSprintf("%d", years);
                }
              } else {
                yearsString = stringDuplicate(translate(numberKeys[years]));
              }
              ...
              if (years == 0) {
                ...
                deathPhrase = autoSprintf(" - %s %s %s", translate("died"),
                                          yearsString, yearWord);
              }

              delete[] yearsString;

              des = autoSprintf("%s %s %s%s", desNoComment, translate("of"),
                                gI->relationName, deathPhrase);
              delete[] desNoComment;
              delete[] deathPhrase;
          ...
          if (!found) {
            ...
            if (!alreadySent) {
              char *graveMessage = autoSprintf(
                  "GRAVE %d %d#", mCurMouseOverWorld.x, mCurMouseOverWorld.y);
          ...
          for (int g = 0; g < mOwnerInfo.size(); g++) {
            ...
            if (gI->worldPos.x == mCurMouseOverWorld.x &&
              ...
              const char *personName = translate("unknownPerson");
              ...
              for (int p = 0; p < gI->ownerList->size(); p++) {
                ...
                if (pO != NULL) {
                  double thisDist =
                      distance(pO->currentPos, ourLiveObject->currentPos);
                  if (thisDist < minDist) {
                    minDist = thisDist;
                    if (pO->name != NULL) {
                      personName = pO->name;
                    } else {
                      personName = translate("namelessPerson");
                    }
                  }
                }
              }
              ...
              des = autoSprintf("%s %s %s", desNoComment, translate("ownedBy"),
                                personName);
              delete[] desNoComment;
          ...
          if (!found) {
            ...
            if (!alreadySent) {
              char *ownerMessage = autoSprintf(
                  "OWNER %d %d#", mCurMouseOverWorld.x, mCurMouseOverWorld.y);
            ...
            *(mOldDesFades.getElement(i)) = fade;
            if (fade <= 0) {
              ...
              i--;
                              ...
                              mOldDesStrings.getElementDirect(i)) == 0) {
      ...
      if (mCurrentDes != NULL) {
        ...
        for (int i = 0; i < mOldDesStrings.size(); i++) {
          if (strcmp(mCurrentDes, mOldDesStrings.getElementDirect(i)) == 0) {
            // already in stack, move to top
            mOldDesStrings.deallocateStringElement(i);
            mOldDesFades.deleteElement(i);
            i--;
...
static char getTransHintable(TransRecord *inTrans) {
  ...
  if (inTrans->actor >= 0 && (inTrans->target > 0 || inTrans->target == -1) &&
...
static int getTransMostImportantResult(TransRecord *inTrans) {
  ...
  if (target != newTarget && newTarget > 0 && actor != newActor &&
      newActor > 0) {
    ...
    if (actor == 0 && newActor > 0) {
      ...
      if (actor > 0 && getObjectDepth(newActor) > getObjectDepth(actor)) {
        result = newActor;
      } else if (target > 0 &&
                 getObjectDepth(newTarget) > getObjectDepth(target)) {
        result = newTarget;
      }
      // else neither actor or target becomes deeper
      // which result is deeper?
      else if (getObjectDepth(newActor) > getObjectDepth(newTarget)) {
        result = newActor;
      } else {
        result = newTarget;
      }
    }
  } else if (target != newTarget && newTarget > 0) {

    result = newTarget;
  } else if (actor != newActor && newActor > 0) {
...
int LivingLifePage::getNumHints(int inObjectID) {
  ...
  if (mLastHintFilterString == NULL && mHintFilterString == NULL) {
    sameFilter = true;
  } else if (mLastHintFilterString != NULL && mHintFilterString != NULL &&
  ...
  if (false)
    if (mLastHintFilterString != NULL && filteredTrans.size() > 0) {
      ...
      SimpleVector<int> hitIDs = findObjectsMatchingWords(
          mLastHintFilterString, inObjectID, 200, &numFilterHits);
      ...
      if (numHits > 0) {
          ...
          for (int s = 0; s < 10; s++) {
            ...
            for (int i = 0; i < oldLastStep.size(); i++) {
              ...
              TransRecord **prodTrans =
                  searchProduces(oldStepID, 0, 200, &numResults, &numRemain);
              ...
              if (prodTrans != NULL) {
                ...
                for (int t = 0; t < numResults; t++) {
                  ...
                  if (actor > 0) {
                    ...
                    if (transDepth == UNREACHABLE) {
                      // is this a category?
                      CategoryRecord *cat = getCategory(actor);

                      if (cat != NULL && cat->objectIDSet.size() > 0 &&
                          !cat->isPattern) {
                        continue;
                      }
                    }
                  ...
                  if (target > 0) {
                    ...
                    if (targetDepth == UNREACHABLE) {
                      // must be category
                      // is this a category?
                      CategoryRecord *cat = getCategory(target);
                    ...
                    if (targetDepth > transDepth || transDepth == UNREACHABLE) {
                ...
                if (shallowestTransIndex != -1 &&
                  ...
                  int actor = prodTrans[shallowestTransIndex]->actor;
                  int target = prodTrans[shallowestTransIndex]->target;

                  if (actor > 0 && precursorIDs.getElementIndex(actor) == -1) {
                  ...
                  if (target > 0 &&
                      precursorIDs.getElementIndex(target) == -1) {
        ...
        for (int i = 0; i < filteredTrans.size(); i++) {
          ...
          for (int h = 0; h < numHits; h++) {
            ...
            if (t->actor != id && t->target != id && (resultOfTrans == id)) {
              matchesFilter = true;
              break;
            }
          }
  ...
  if (mLastHintFilterString != NULL) {
    ...
    SimpleVector<int> hitIDs =
        findObjectsMatchingWords(mLastHintFilterString,
    ...
    if (hitIDs.size() > 0 && hitIDs.size() < 10) {
      ...
      while (frontier.size() > 0) {
        ...
        for (int i = 0; i < frontier.size(); i++) {
          printf(" [ %s ]  ",
                 getObject(frontier.getElementDirect(i))->description);
        }
        printf("\n");
        ...
        for (int i = 0; i < frontier.size(); i++) {
          ...
          int numRemain = 0;
          TransRecord **prodTrans =
              searchProduces(oID, 0, 200, &numResults, &numRemain);
          ...
          if (prodTrans != NULL) {
            ...
            for (int r = 0; r < numResults; r++) {
              ...
              if (actor >= -1 && (actor <= 0 || actorD < oD) && target != 0 &&
                  targetD < oD &&
                  filteredTrans.getElementIndex(prodTrans[r]) == -1) {

                if (prodTrans[r]->lastUseTarget || prodTrans[r]->lastUseActor) {
                  // skip last use transitions
                  continue;
                }

                // skip dummy versions of transitions, too
                if (actor > 0 && (getObject(actor)->isUseDummy ||
                                  getObject(actor)->isVariableDummy)) {
                  continue;
                }
                if (target > 0 && (getObject(target)->isUseDummy ||
            ...
            if (minDepthTrans != NULL) {
              ...
              if (actor > 0) {
                if (alreadySeenObjects.getElementIndex(actor) == -1) {
              ...
              if (target > 0 &&
                  alreadySeenObjects.getElementIndex(target) == -1) {
  ...
  if (false && numTrans == 0 && unfilteredTrans.size() > 0) {
  ...
  if (mLastHintFilterString != NULL) {
    ...
    if (numRelevant == 0 || numFilterHits == 0) {
      ...
      mPendingFilterString = autoSprintf("%s %s %s", translate("making"),
                                         mLastHintFilterString, reasonString);
      delete[] reasonString;
    } else {
      mPendingFilterString =
          autoSprintf("%s %s", translate("making"), mLastHintFilterString);
    }
    ...
    if (mLastHintFilterString == NULL)
      if (tr->actor > 0 && tr->actor != inObjectID) {
        ...
        for (int s = 0; s < otherActorStrings.size(); s++) {
          ...
          if (strcmp(trimmedDesc, otherActorStrings.getElementDirect(s)) == 0) {
    ...
    if (mLastHintFilterString == NULL)
      if (tr->target > 0 && tr->target != inObjectID) {
        ...
        for (int s = 0; s < otherTargetStrings.size(); s++) {
          ...
          if (strcmp(trimmedDesc, otherTargetStrings.getElementDirect(s)) ==
              0) {
...
char *LivingLifePage::getHintMessage(int inObjectID, int inIndex,
  ...
  if (found != NULL) {
    ...
    if (target > 0) {
      targetString = stringToUpperCase(getObject(target)->description);
        ...
        if (result == 0 && actor == newActor && actorObj->numUses > 1) {
          ...
          if (lastUseTrans != NULL) {
            if (lastUseTrans->newActor > 0) {
              ...
              result = findMainObjectID(lastUseTrans->newTarget);
              eventually = true;
            }
          }
        }
    ...
    if (target > 0 && target != inDoNotPointAtThis) {
      mCurrentHintTargetObject[1] = target;
    }
    if (actor > 0 && actor != inDoNotPointAtThis) {
    ...
    if (actor > 0 && target > 0 && actor == target) {
      ...
      mCurrentHintTargetObject[0] = actor;
      mCurrentHintTargetObject[1] = 0;
    } else if (actor == 0 && target > 0 && target != inDoNotPointAtThis) {
      // bare hand action
    ...
    if (result > 0) {
      resultString = stringToUpperCase(getObject(result)->description);
    } else {
    ...
    if (eventually) {
      ...
      resultString = autoSprintf("%s %s", old, translate("eventuallyHint"));
      delete[] old;
    }
...
static char shouldCreationSoundPlay(int inOldID, int inNewID) {
  ...
  if (inOldID > 0 && inNewID > 0) {
    ...
    if (obj->isUseDummy && oldObj->isUseDummy &&
        obj->useDummyParent == oldObj->useDummyParent) {
      sameParent = true;
    } else if (obj->numUses > 1 && oldObj->isUseDummy &&
               oldObj->useDummyParent == inNewID) {
      sameParent = true;
    } else if (oldObj->numUses > 1 && obj->isUseDummy &&
               obj->useDummyParent == inOldID) {
      sameParent = true;
    }
  }

  if (!sameParent && (!obj->creationSoundInitialOnly || inOldID <= 0 ||
...
static char checkIfHeldContChanged(LiveObject *inOld, LiveObject *inNew) {
  ...
  if (inOld->numContained != inNew->numContained) {
    ...
    for (int c = 0; c < inOld->numContained; c++) {
      if (inOld->containedIDs[c] != inNew->containedIDs[c]) {
        return true;
      }
      ...
      for (int s = 0; s < inOld->subContainedIDs[c].size(); s++) {
        if (inOld->subContainedIDs[c].getElementDirect(s) !=
            inNew->subContainedIDs[c].getElementDirect(s)) {
          return true;
        }
      }
    }
...
void LivingLifePage::sendBugReport(int inBugNumber) {
  ...
  if (lastMessageSentToServer != NULL) {
    char *temp = bugString;
    bugString =
        autoSprintf("%s   Just sent: [%s]", temp, lastMessageSentToServer);
    delete[] temp;
  }
  if (nextActionMessageToSend != NULL) {
    char *temp = bugString;
    bugString = autoSprintf("%s   Waiting to send: [%s]", temp,
...
void LivingLifePage::endExtraObjectMove(int inExtraIndex) {
  ...
  GridPos worldPos = mMapExtraMovingObjectsDestWorldPos.getElementDirect(i);
  ...
  if (mapI != -1) {
    // put it in dest
    putInMap(mapI, o);
    mMap[mapI] = mMapExtraMovingObjectsDestObjectIDs.getElementDirect(i);
  }
...
static const char *badgeColors[NUM_BADGE_COLORS] = {
...
void LivingLifePage::step() {
  ...
  for (int i = 0; i < numCells; i++) {
    ...
    if (mMapMoveSpeeds[i] > 0 &&
        (mMapMoveOffsets[i].x != 0 || mMapMoveOffsets[i].y != 0)) {
      ...
      if (length(delta) < step) {
        ...
        mMapMoveOffsets[i] =
            add(mMapMoveOffsets[i], mult(normalize(delta), step));
      }
    }
  }
  ...
  if (mCurMouseOverCell.x != -1) {
    ...
    if (mCurMouseOverID == 0 && !mCurMouseOverPerson &&
        // AND we're not moving AND we're holding something
        ourObject != NULL && !ourObject->inMotion && ourObject->holdingID > 0) {
      // fade cell in
      mCurMouseOverCellFade += mCurMouseOverCellFadeRate * frameRateFactor;
  ...
  if (!equal(mNotePaperPosOffset, mNotePaperPosTargetOffset)) {
    doublePair delta = sub(mNotePaperPosTargetOffset, mNotePaperPosOffset);
    ...
    if (d <= 1) {
      ...
      mNotePaperPosOffset = add(mNotePaperPosOffset, mult(dir, speed));
    }
  ...
  if (ourObject != NULL) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (!apocalypseInProgress && homeArrow != -1 && !tooClose) {
        ...
        if (temporary || (mapHintEverDrawn[j] && longDistance)) {
          if (j == 0) {
            mHomeSlipPosTargetOffset[j].y += 20;
          } else {
            mHomeSlipPosTargetOffset[j].y += 20;
    ...
    if (cm != -1) {
      ourObject->clothingHighlightFades[cm] += 0.2 * frameRateFactor;
    ...
    for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
      if (c != cm) {
        ourObject->clothingHighlightFades[c] -= 0.1 * frameRateFactor;
  ...
  for (int i = 0; i < NUM_YUM_SLIPS; i++) {
    ...
    if (!equal(mYumSlipPosOffset[i], mYumSlipPosTargetOffset[i])) {
      doublePair delta = sub(mYumSlipPosTargetOffset[i], mYumSlipPosOffset[i]);

      double d = distance(mYumSlipPosTargetOffset[i], mYumSlipPosOffset[i]);
      ...
      if (d <= 1) {
        ...
        mYumSlipPosOffset[i] = add(mYumSlipPosOffset[i], mult(dir, speed));
      }
    }
  }
  ...
  for (int j = 0; j < 2; j++)
    if (!equal(mHomeSlipPosOffset[j], mHomeSlipPosTargetOffset[j])) {
      ...
      double d = distance(mHomeSlipPosTargetOffset[j], mHomeSlipPosOffset[j]);
      ...
      if (d <= 1) {
        ...
        mHomeSlipPosOffset[j] = add(mHomeSlipPosOffset[j], mult(dir, speed));
      }
    }
  ...
  if (ourObject != NULL && mNextHintObjectID != 0 &&
      ...
      if (mLastHintSortedList.size() > mCurrentHintIndex) {
        ...
        if (currHintTrans->actor != matchID &&
          ...
          for (int i = 0; i < mLastHintSortedList.size(); i++) {
            ...
            if (t->actor == matchID || t->target == matchID) {
    ...
    if ((isHintFilterStringInvalid() &&
         mCurrentHintObjectID != mNextHintObjectID) ||
        mCurrentHintIndex != mNextHintIndex || mForceHintRefresh ||
        justHitTab) {
      ...
      if (mLiveHintSheetIndex != -1) {
        ...
        newLiveSheetIndex = (mLiveHintSheetIndex + 1) % (NUM_HINT_SHEETS - 1);
      }
      ...
      mNumTotalHints[i] = getNumHints(mCurrentHintObjectID);
      ...
      mHintMessage[i] =
          getHintMessage(mCurrentHintObjectID, mHintMessageIndex[i],
                         // don't set pointer
    ...
    if (!isHintFilterStringInvalid() && mCurrentHintIndex >= 0) {
      // special case
  ...
  if (mPendingFilterString != NULL &&
    ...
    if (equal(mHintPosOffset[lastSheet], mHintHideOffset[lastSheet])) {
      ...
      mHintMessage[lastSheet] = stringDuplicate(mPendingFilterString);

      double len = handwritingFont->measureString(mHintMessage[lastSheet]);

      mHintExtraOffset[lastSheet].x = -len;
    }

  } else if (mPendingFilterString == NULL && mHintMessage[lastSheet] != NULL) {
    mHintTargetOffset[lastSheet] = mHintHideOffset[lastSheet];
  ...
  for (int i = 0; i < NUM_HINT_SHEETS; i++) {
    ...
    if (!equal(mHintPosOffset[i], mHintTargetOffset[i])) {
      doublePair delta = sub(mHintTargetOffset[i], mHintPosOffset[i]);
      ...
      if (d <= 1) {
        ...
        mHintPosOffset[i] = add(mHintPosOffset[i], mult(dir, speed));
      }
    }
  }
  ...
  if ((mTutorialNumber > 0 || mGlobalMessageShowing) && ourObject != NULL) {
          ...
          if (mapID > 0) {
            ...
            if (tutLoc != NULL) {
              ...
              if (tutPage != -1) {
                ...
                if (strstr(mapO->description, "done") != NULL) {
                  closestIsFinal = true;
                } else {
    ...
    if (closeDist > 4 && mLiveTutorialTriggerNumber != -1) {
      // don't set a new one unless we get close enough to it
      // OR we haven't even set the first one yet.
      closestNumber = -1;
    }

    if (closestNumber > -1 && closestNumber != mLiveTutorialTriggerNumber) {
      ...
      if (mUsingSteam && mLiveTutorialTriggerNumber == 8) {
        transString =
            autoSprintf("tutorial_%d_steam", mLiveTutorialTriggerNumber);
      } else {
        transString = autoSprintf("tutorial_%d", mLiveTutorialTriggerNumber);
      }

      mTutorialMessage[mLiveTutorialSheetIndex] = translate(transString);
      ...
      double longestLine =
          getLongestLine((char *)(mTutorialMessage[mLiveTutorialSheetIndex]));
  ...
  if ((mTutorialNumber > 0 || mGlobalMessageShowing) &&
    ...
    for (int i = 0; i < NUM_HINT_SHEETS; i++) {
      ...
      if (!equal(mTutorialPosOffset[i], mTutorialTargetOffset[i])) {
        doublePair delta = sub(mTutorialTargetOffset[i], mTutorialPosOffset[i]);

        double d = distance(mTutorialTargetOffset[i], mTutorialPosOffset[i]);
        ...
        if (d <= 1) {
          ...
          mTutorialPosOffset[i] = add(mTutorialPosOffset[i], mult(dir, speed));
        }

        if (equal(mTutorialTargetOffset[i], mTutorialHideOffset[i])) {
          // fully hidden
        } else if (equal(mTutorialPosOffset[i], mTutorialTargetOffset[i])) {
          ...
          if (mTutorialSound != NULL) {
            playSoundSprite(mTutorialSound, 0.18 * getSoundEffectsLoudness(),
                            stereoPos);
          }
        }
      }
  ...
  for (int i = 0; i < 3; i++) {
    if (!equal(mHungerSlipPosOffset[i], mHungerSlipPosTargetOffset[i])) {

      doublePair delta =
          sub(mHungerSlipPosTargetOffset[i], mHungerSlipPosOffset[i]);

      double d =
          distance(mHungerSlipPosTargetOffset[i], mHungerSlipPosOffset[i]);
      ...
      if (d <= 1) {
        ...
        mHungerSlipPosOffset[i] =
            add(mHungerSlipPosOffset[i], mult(dir, speed));
      }

      if (equal(mHungerSlipPosTargetOffset[i], mHungerSlipHideOffsets[i])) {
        // fully hidden
        // reset wiggle time
        mHungerSlipWiggleTime[i] = 0;
      }
  ...
  if (playerActionPending && ourObject != NULL &&
    ...
    delete[] pingMessage;
  } else if (playerActionPending && ourObject != NULL &&
             curTime - ourObject->pendingActionAnimationStartTime > 5 &&
             curTime - lastServerMessageReceiveTime > 10 && !waitingForPong) {
  ...
  while (message != NULL) {
    ...
    if (type == SHUTDOWN || type == FORCED_SHUTDOWN) {
      ...
      if (mTutorialNumber <= 0) {
        ...
        mTutorialMessage[mLiveTutorialSheetIndex] = stringDuplicate(spaces);
      ...
      sscanf(message,
             ...
             "%199s\n"
             "%d\n",
             &currentPlayers, &maxPlayers, challengeString, &mRequiredVersion);
      ...
      if (mRequiredVersion > versionNumber ||
        ...
        if (!usingCustomServer && mRequiredVersion < dataVersionNumber) {
      ...
      if (strlen(userEmail) <= 80) {
        outMessage =
            autoSprintf("LOGIN %s %-80s %s %s %d%s#", clientTag, tempEmail,
                        pwHash, keyHash, mTutorialNumber, twinExtra);
      } else {
        ...
        outMessage =
            autoSprintf("LOGIN %s %s %s %s %d%s#", clientTag, tempEmail, pwHash,
                        keyHash, mTutorialNumber, twinExtra);
      }
      ...
      if (numRead == 3) {
        ...
        if (ourLiveObject != NULL) {
          ...
          if (d > 32) {
            ...
            if (monObj != NULL && monObj->creationSound.numSubSounds > 0) {

              doublePair realVector =
                  getVectorFromCamera(lrint(posX), lrint(posY));
      ...
      if (numRead == 3) {
        ...
        if (gravePerson != NULL &&
            (gravePerson->relationName || gravePerson->name != NULL)) {
          ...
          if (gravePerson->name != NULL) {
            des = autoSprintf("%s - %s", gravePerson->name, des);
            desToDelete = des;
          }
          ...
          for (int i = 0; i < mGraveInfo.size(); i++) {
            ...
            if (otherG->worldPos.x == posX && otherG->worldPos.y == posY) {
      ...
      if (numRead == 4 || numRead == 5) {
        ...
        for (int i = mGraveInfo.size() - 1; i >= 0; i--) {
          ...
          if (g->worldPos.x == posX && g->worldPos.y == posY) {
        ...
        if (found && !swapDest) {
          ...
          for (int i = 1; i < mGraveInfo.size(); i++) {
            ...
            if (g->worldPos.x == posXNew && g->worldPos.y == posYNew) {
      ...
      int numRead = sscanf(message, "GO\n%d %d %d %d %lf %199s", &posX, &posY,
                           &playerID, &displayID, &age, nameBuffer);
      if (numRead == 6) {
        applyReceiveOffset(&posX, &posY);
        ...
        for (int i = 0; i < graveRequestPos.size(); i++) {
          if (equal(graveRequestPos.getElementDirect(i), thisPos)) {
        ...
        if (numLines > 1) {
          SimpleVector<char *> *tokens = tokenizeString(lines[1]);

          int numNormalTokens = tokens->size();

          if (tokens->size() > 6) {
            char *lastToken = tokens->getElementDirect(tokens->size() - 1);
        ...
        char *relationName = getRelationName(
            &(ourLiveObject->lineage), &otherLin, ourID, playerID,
            ourLiveObject->displayID, displayID, ourLiveObject->age, age,
            ourLiveObject->lineageEveID, eveID);
        ...
        if (strcmp(nameBuffer, "") != 0 && strcmp(nameBuffer, "~") != 0) {
          des = autoSprintf("%s - %s", nameBuffer, des);
          desToDelete = des;
        }
        ...
        g.creationTime = game_getCurrentTime() - age / ourLiveObject->ageRate;
      ...
      if (tokens->size() >= 3) {
        ...
        for (int i = 0; i < ownerRequestPos.size(); i++) {
          if (equal(ownerRequestPos.getElementDirect(i), thisPos)) {
        ...
        for (int i = 0; i < mOwnerInfo.size(); i++) {
          ...
          if (thisInfo->worldPos.x == x && thisInfo->worldPos.y == y) {
        ...
        for (int i = 1; i < tokens->size() - 1; i += 3) {
          ...
          if (l > 0 && c != -1) {
            ...
            if (lo != NULL) {
              ...
              lo->personalLeadershipColor = getFloatColor(badgeColors[c]);
            }
          }
        }
      }
      ...
      sscanf(message, "VS\n%d %d", &valleySpacing, &valleyOffset);
    } else if (type == FLIGHT_DEST) {
      int posX, posY, playerID;

      int numRead = sscanf(message, "FD\n%d %d %d", &playerID, &posX, &posY);
      if (numRead == 3) {
        applyReceiveOffset(&posX, &posY);
      ...
      memcpy(mMapAnimationLastFrameCount, newMapAnimationLastFrameCount,
             mMapD * mMapD * sizeof(double));

      memcpy(mMapAnimationFrozenRotFrameCount,
             newMapAnimationFrozenRotFameCount, mMapD * mMapD * sizeof(double));

      memcpy(mMapAnimationFrozenRotFrameCountUsed,
             newMapAnimationFrozenRotFameCountUsed,
             mMapD * mMapD * sizeof(char));

      memcpy(mMapFloorAnimationFrameCount, newMapFloorAnimationFrameCount,
      ...
      unsigned char *decompressedChunk =
          zipDecompress(compressedChunk, compressedSize, binarySize);
        ...
        if (tokens->size() == numCells) {
          ...
          for (int i = 0; i < tokens->size(); i++) {
            ...
            if (mapX >= 0 && mapX < mMapD && mapY >= 0 && mapY < mMapD) {
              ...
              sscanf(tokens->getElementDirect(i), "%d:%d:%d",
                     &(mMapBiomes[mapI]), &(mMapFloors[mapI]), &(mMap[mapI]));
                ...
                for (int c = 0; c < numContained; c++) {
                  ...
                  if (strstr(ints[c + 1], ":") != NULL) {
                    ...
                    SimpleVector<int> *subStack =
                        mMapSubContainedStacks[mapI].getElement(c);
            ...
            for (int mapX = 0; mapX < mMapD; mapX++) {
              ...
              if (id > 0) {
                ...
                if (getObject(id)->homeMarker) {
                  ...
                  double dist =
                      pow(worldY - mapCenterY, 2) + pow(worldX - mapCenterX, 2);
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int numRead =
            sscanf(lines[i], "%d %d %d %499s %d %d %d %f", &x, &y, &floorID,
                   idBuffer, &responsiblePlayerID, &oldX, &oldY, &speed);
        if (numRead == 5 || numRead == 8) {
          ...
          if (mapX >= 0 && mapX < mMapD && mapY >= 0 && mapY < mMapD) {
            ...
            int oldContainedCount = mMapContainedStacks[mapI].size();
            ...
            if (responsiblePlayerID != -1) {
              ...
              if (rObj != NULL && rObj->pendingReceivedMessages.size() > 0) {

                printf("Holding MX message caused by "
                       "%d until later, "
                       "%d other messages pending for them\n",
                       rID, rObj->pendingReceivedMessages.size());
            ...
            if (strstr(idBuffer, ",") != NULL) {
              int numInts;
              char **ints = split(idBuffer, ",", &numInts);
              ...
              for (int c = 0; c < numContained; c++) {
                ...
                mMapContainedStacks[mapI].push_back(atoi(ints[c + 1]));
            ...
            if (speed > 0) {
              ...
              if (oldMapI != -1) {
                ...
                TransRecord *moveTrans = getTrans(-1, sourceObjID);

                if (moveTrans != NULL && moveTrans->move > 0) {
              ...
              if (oldMapI != -1) {
                ...
                mMapAnimationFrozenRotFrameCount[mapI] =
                    mMapAnimationFrozenRotFrameCount[oldMapI];

                mMapCurAnimType[mapI] = mMapCurAnimType[oldMapI];
                mMapLastAnimType[mapI] = mMapLastAnimType[oldMapI];

                mMapLastAnimFade[mapI] = mMapLastAnimFade[oldMapI];
                ...
                if (mMapLastAnimFade[mapI] == 0 &&
                  ...
                  if (mMapAnimationFrozenRotFrameCountUsed[mapI]) {
                    mMapAnimationFrameCount[mapI] =
                        mMapAnimationFrozenRotFrameCount[oldMapI];
                  }
                }
                ...
                if (oldLocID > 0) {
                  TransRecord *decayTrans = getTrans(-1, oldLocID);

                  if (decayTrans != NULL) {

                    if (decayTrans->move == 6 || decayTrans->move == 7) {
                      moveEastOrWest = true;
                    }
                  }
                }
              ...
              if (old > 0 && sourceObjID != 0 &&
                ...
                mMapExtraMovingObjectsDestWorldPos.push_back(worldDestPos);

                mMapExtraMovingObjectsDestObjectIDs.push_back(newID);
            ...
            if (nextDecayTrans != NULL) {
              ...
              if (nextDecayTrans->move == 6 || nextDecayTrans->move == 7) {
            ...
            if (oldFloor != floorID && floorID > 0) {
              ...
              if (obj->creationSound.numSubSounds > 0) {
                ...
                playSound(obj->creationSound, getVectorFromCamera(x, y));
              }
            }
            ...
            if (responsiblePlayerID > 0) {
              responsiblePlayerObject = getGameObject(responsiblePlayerID);
            }

            if (old > 0 && newID > 0 && old != newID &&
                responsiblePlayerID == -ourID) {

              // check for photo triggered
              if (strstr(getObject(newID)->description, "+photo") != NULL) {
            ...
            if (old > 0 && old == newID &&
                mMapContainedStacks[mapI].size() > oldContainedCount &&
              ...
              SoundUsage contSound = getObject(mMap[mapI])->usingSound;

              if (contSound.numSubSounds > 0) {
                playSound(contSound, getVectorFromCamera(x, y));
                soundPlayed = true;
              }
              ...
              if (!soundPlayed) {
                ...
                SoundUsage s =
                    getObject(responsiblePlayerObject->displayID)->usingSound;
            ...
            if (responsiblePlayerID != -1 && (old != 0 || newID != 0)) {
              ...
              if (rID == ourID) {
                ...
                if (newID > 0 && getObject(newID)->homeMarker) {

                  addHomeLocation(x, y);
                  addedOrRemoved = true;
                } else if (old > 0 && getObject(old)->homeMarker) {
                ...
                if (addedOrRemoved) {
                  ...
                  for (int ry = y - 7; ry <= y + 7; ry++) {
                    for (int rx = x - 7; rx <= x + 7; rx++) {
                      ...
                      int mapRX = rx - mMapOffsetX + mMapD / 2;
                      int mapRY = ry - mMapOffsetY + mMapD / 2;

                      if (mapRX >= 0 && mapRX < mMapD && mapRY >= 0 &&
                          mapRY < mMapD) {

                        int mapRI = mapRY * mMapD + mapRX;

                        int cellID = mMap[mapRI];

                        if (cellID == 0 ||
                            (cellID > 0 && !getObject(cellID)->homeMarker)) {

                          removeHomeLocation(rx, ry);
                        }
                      }
                    }
                  }
            ...
            if (old != newID && newID != 0 && responsiblePlayerID <= -1) {
              ...
              if (old > 0) {
                ...
                if (responsiblePlayerID == -1) {
                  ...
                  if (obj->decaySound.numSubSounds > 0) {
                    ...
                    playSound(obj->decaySound, getVectorFromCamera(x, y));
                  }
              ...
              if (newObj->creationSound.numSubSounds > 0) {
                ...
                if (old == 0 && responsiblePlayerID < -1) {
                  ...
                  if (responsiblePlayerObject != NULL) {
                    ...
                    if (old > 0) {
                      TransRecord *p = getTransProducing(old, newID);
                ...
                if (shouldCreationSoundPlay(old, newID)) {
                  playSound(newObj->creationSound, getVectorFromCamera(x, y));
                }
              }
            }
            ...
            if (old != mMap[mapI] && mMap[mapI] != 0) {
              ...
              if (mMapMoveSpeeds[mapI] == 0) {
                ...
                if (old == 0) {
                  ...
                  // animation
                  AnimationRecord *newAnim = getAnimation(mMap[mapI], ground);

                  if (newAnim != NULL && newAnim->forceZeroStart) {
              ...
              if (responsiblePlayerID != -1 && responsiblePlayerID != 0 &&
              ...
              if (responsiblePlayerObject == NULL ||
                ...
                AnimationRecord *animR = getAnimation(mMap[mapI], ground);

                if (animR != NULL && animR->randomStartPhase) {
                  mMapAnimationFrameCount[mapI] =
                ...
                if (responsiblePlayerObject != NULL) {
                  ...
                  mMapTileFlips[mapI] = responsiblePlayerObject->holdingFlip;
                } else if (responsiblePlayerID < -1 && old == 0 &&
                           mMap[mapI] > 0 &&
                           !getObject(mMap[mapI])->permanent) {
                  // use-on-bare-ground
                  // with non-permanent result
                  // honor flip direction of player
                  mMapTileFlips[mapI] =
                      getLiveObject(-responsiblePlayerID)->holdingFlip;
                }
              } else {
                ...
                mMapAnimationFrozenRotFrameCount[mapI] =
                    responsiblePlayerObject->heldFrozenRotFrameCount;

                if (responsiblePlayerObject->lastHeldAnimFade == 0) {

                  mMapAnimationLastFrameCount[mapI] =
                      responsiblePlayerObject->heldAnimationFrameCount;

                  mMapLastAnimType[mapI] = responsiblePlayerObject->curHeldAnim;
                } else {
                  // dropped object was already
                  // in the middle of a fade
                  mMapCurAnimType[mapI] = responsiblePlayerObject->curHeldAnim;
                  ...
                  mMapAnimationLastFrameCount[mapI] =
                      responsiblePlayerObject->lastHeldAnimationFrameCount;

                  mMapLastAnimFade[mapI] =
                      responsiblePlayerObject->lastHeldAnimFade;
                }

                mMapDropOffsets[mapI].x =
                    responsiblePlayerObject->heldObjectPos.x - x;

                mMapDropOffsets[mapI].y =
                    responsiblePlayerObject->heldObjectPos.y - y;

                mMapDropRot[mapI] = responsiblePlayerObject->heldObjectRot;
                ...
                if (responsiblePlayerObject->holdingID > 0 && old == 0) {
                  ...
                  if (getObject(mMap[mapI])->permanent) {
                ...
                if (x > responsiblePlayerObject->xServer) {
                  responsiblePlayerObject->holdingFlip = false;
                } else if (x < responsiblePlayerObject->xServer) {
                  responsiblePlayerObject->holdingFlip = true;
                }
              }
            }
          }
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int heldOriginValid, heldOriginX, heldOriginY, heldTransitionSourceID;
        ...
        int numRead = sscanf(
            ...
            "%lf %lf %lf %499s %d %d %d "
            "%d %d",
            &(o.id), &(o.displayID), &facingOverride, &actionAttempt,
            &actionTargetX, &actionTargetY, holdingIDBuffer, &heldOriginValid,
            &heldOriginX, &heldOriginY, &heldTransitionSourceID, &(o.heat),
            &done_moving, &forced, &(o.xd), &(o.yd), &(o.age), &invAgeRate,
            &(o.lastSpeed), clothingBuffer, &justAte, &justAteID,
            &responsiblePlayerID, &heldYum, &heldLearned);
        ...
        if (numRead >= 23) {
          ...
          printf("PLAYER_UPDATE with heldOrVal=%d, "
                 ...
                 "holdingString=%s\n",
                 heldOriginValid, heldOriginX, heldOriginY, o.xd, o.yd,
                 heldTransitionSourceID, holdingIDBuffer);
          if (forced) {
          ...
          if (numClothes == NUM_CLOTHING_PIECES) {
            ...
            for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
              ...
              if (numParts > 0) {
                ...
                if (id != 0) {
                  setClothingByIndex(&(o.clothing), c, getObject(id));
                  ...
                  if (numParts > 1) {
                    for (int p = 1; p < numParts; p++) {
                      ...
                      if (cID != 0) {
                        ...
                        o.clothingContained[c].push_back(cID);
                      }
                    }
                  }
                }
          ...
          if (strstr(holdingIDBuffer, ",") != NULL) {
            ...
            o.subContainedIDs = new SimpleVector<int>[o.numContained];
            ...
            for (int c = 0; c < o.numContained; c++) {
              ...
              if (strstr(ints[c + 1], ":") != NULL) {
                ...
                int numSubCont = numSubInts - 1;
                for (int s = 0; s < numSubCont; s++) {
                  o.subContainedIDs[c].push_back(atoi(subInts[s + 1]));
                  delete[] subInts[s + 1];
                }
          ...
          if (existing != NULL && existing->id == ourID) {
            // got a PU for self
            ...
            if (existing->holdingID != o.holdingID) {
              ...
              if (r != NULL && r->temporary && r->temporaryExpireETA == 0) {
                r->temporaryExpireETA = game_getCurrentTime() + 60;
              }
            }
            ...
            if (heldYum) {
              ...
              if (o.holdingID > 0 && getObject(o.holdingID)->foodValue > 0) {
                // MEH
                slipIndexToShow = 3;
              }
            }
            ...
            if (existing->lastActionSendStartTime != 0) {
              ...
              existing->lastResponseTimeDelta =
                  game_getCurrentTime() - existing->lastActionSendStartTime;

              existing->lastActionSendStartTime = 0;
            }
          }

          if (existing != NULL && existing->destTruncated &&
              (existing->xd != o.xd || existing->yd != o.yd)) {
            // edge case:
              ...
              if (heldBaby != NULL) {
                ...
                if (heldBaby->heldByAdultID > 0 &&
                  ...
                  LiveObject *oldHolder =
                      getLiveObject(heldBaby->heldByAdultID);

                  if (oldHolder != NULL && oldHolder->outOfRange) {
                    heldBaby->heldByAdultID = -1;
                  }
                }

                if (heldBaby->heldByAdultPendingID > 0 &&
                    heldBaby->heldByAdultPendingID != existing->id) {

                  LiveObject *oldPendingHolder =
                      getLiveObject(heldBaby->heldByAdultPendingID);
                ...
                if (heldBaby->heldByAdultID == existing->id) {
                  ...
                  if (holdingAdult != NULL) {
                    ...
                    char *pendingMessage = autoSprintf("PU\n%s\n#", lines[i]);

                    holdingAdult->pendingReceivedMessages.push_back(
                        pendingMessage);

                    alreadyHeldAsPending = true;
                  }
                } else if (heldBaby->heldByAdultPendingID == -1) {
                  // mark held pending
                  heldBaby->heldByAdultPendingID = existing->id;
                } else if (heldBaby->heldByAdultPendingID != existing->id) {
          ...
          if (alreadyHeldAsPending) {
          } else if (existing != NULL && existing->id != ourID &&
                     existing->currentSpeed != 0 && !forced &&
                     (done_moving > 0 ||
            ...
            if (existing->pathToDest != NULL && existing->pathLength > 0) {

              GridPos pathEnd = existing->pathToDest[existing->pathLength - 1];

              if (o.xd == pathEnd.x && o.yd == pathEnd.y) {
                ...
                for (int p = existing->pathLength - 1; p >= 0; p--) {

                  GridPos thisStep = existing->pathToDest[p];

                  if (thisStep.x == o.xd && thisStep.y == o.yd) {
                    // found
            ...
            if (done_moving > 0 ||
              ...
              printf("Holding PU message for "
                     "%d until later, "
                     "%d other messages pending for them\n",
                     existing->id, existing->pendingReceivedMessages.size());

              existing->pendingReceivedMessages.push_back(
                  autoSprintf("PU\n%s\n#", lines[i]));
            }
          } else if (existing != NULL && existing->heldByAdultID != -1 &&
            ...
            LiveObject *holdingPlayer = getLiveObject(existing->heldByAdultID);

            printf("Holding PU message for baby %d held by %d "
                   "until later, "
                   "%d other messages pending for them\n",
                   existing->id, existing->heldByAdultID,
                   holdingPlayer->pendingReceivedMessages.size());

            holdingPlayer->pendingReceivedMessages.push_back(
                autoSprintf("PU\n%s\n#", lines[i]));
          } else if (existing != NULL && existing->heldByAdultPendingID != -1 &&
                     getLiveObject(existing->heldByAdultPendingID) != NULL &&
            ...
            printf("Holding PU message for baby %d with "
                   ...
                   "until later, "
                   "%d other messages pending for them\n",
                   existing->id, existing->heldByAdultPendingID,
                   holdingPlayer->pendingReceivedMessages.size());

            holdingPlayer->pendingReceivedMessages.push_back(
                autoSprintf("PU\n%s\n#", lines[i]));
          } else if (existing != NULL && responsiblePlayerID != -1 &&
            ...
            printf("Holding PU message for %d caused by %d "
                   "until later, "
                   "%d other messages pending for them\n",
                   existing->id, responsiblePlayerID,
            ...
            if (oldHeld > 0 && oldHeld == existing->holdingID) {

              heldContChanged = checkIfHeldContChanged(&o, existing);
            }
            ...
            if (o.id == ourID && existing->holdingID > 0 &&
              ...
              if (isHintFilterStringInvalid()) {
                mNextHintIndex = mHintBookmarks[mNextHintObjectID];
              }
            }

            ObjectRecord *newClothing =
                getClothingAdded(&(existing->clothing), &(o.clothing));

            if (newClothing == NULL) {
              // has something been removed instead?
              newClothing =
                  getClothingAdded(&(o.clothing), &(existing->clothing));
            }
            ...
            if (newClothing != NULL) {
              ...
              SoundUsage clothingSound = newClothing->usingSound;

              if (clothingSound.numSubSounds > 0) {
                playSound(clothingSound, getVectorFromCamera(o.xd, o.yd));

                clothingSoundPlayed = true;
              } else {
                // play generic pickup sound
                ObjectRecord *existingObj = getObject(existing->displayID);

                if (existingObj->usingSound.numSubSounds > 0) {

                  playSound(existingObj->usingSound,
                            getVectorFromCamera(o.xd, o.yd));
                  clothingSoundPlayed = true;
                }
              }
            }
            ...
            if (o.id != ourID) {
              ...
              if (actionAttempt && !justAte && nearEndOfMovement(existing)) {
              ...
              if (heldOriginValid || facingOverride != 0) {

                if ((heldOriginValid && heldOriginX > existing->xd) ||
                    facingOverride == 1) {

                  existing->holdingFlip = false;
                } else if ((heldOriginValid && heldOriginX < existing->xd) ||
                           facingOverride == -1) {

                  existing->holdingFlip = true;
                }
              }
            } else if (o.id == ourID && actionAttempt && !justAte &&
                       existing->killMode &&
              ...
              if (existing->pendingActionAnimationProgress == 0) {
                existing->pendingActionAnimationProgress =
                    0.025 * frameRateFactor;
              }
            ...
            if (justAte && o.id != ourID && existing->holdingID == oldHeld) {
              ...
              if (nearEndOfMovement(existing)) {
                addNewAnimPlayerOnly(existing, eating);
              }

              ObjectRecord *ateObj = getObject(justAteID);

              if (ateObj->eatingSound.numSubSounds > 0) {
                playSound(ateObj->eatingSound,
                          getVectorFromCamera(existing->currentPos.x,
            ...
            if (justAte && o.id == ourID) {
              ...
              if (strstr(ateObj->description, "remapStart") != NULL) {

                if (mRemapPeak == 0) {
                  // reseed
                  setRemapSeed(
                      remapRandSource.getRandomBoundedInt(0, 10000000));
                  mRemapDelay = 0;
                }
            ...
            if (existing->holdingID == 0) {
              ...
              if (oldHeld != 0) {
                if (o.id == ourID) {
                  ...
                  if (justAte) {
                    ...
                    if (oldHeld > 0) {
                      ObjectRecord *oldHeldObj = getObject(oldHeld);

                      if (oldHeldObj->eatingSound.numSubSounds > 0) {

                        playSound(oldHeldObj->eatingSound,
                                  getVectorFromCamera(existing->currentPos.x,
                                                      existing->currentPos.y));
                        otherSoundPlayed = true;
                      }
            ...
            } else if (oldHeld != existing->holdingID || heldContChanged) {
              // holding something new
              ...
              if (o.id == ourID) {
                ...
                if (justAte) {
                  ...
                  // don't interrupt walking
                  if (actionAttempt && nearEndOfMovement(existing)) {
                    addNewAnimPlayerOnly(existing, doing);
                  }
                }
                // don't interrupt walking
              ...
              if (heldOriginValid) {
                ...
                if (existingObj->usingSound.numSubSounds > 0) {
                  ...
                  playSound(existingObj->usingSound,
                            getVectorFromCamera(heldOriginX, heldOriginY));
                  otherSoundPlayed = true;
                }
                ...
                if (!existing->onScreen) {
                  ...
                  // had a moving offset
                  int mapHeldOriginX = heldOriginX - mMapOffsetX + mMapD / 2;
                  int mapHeldOriginY = heldOriginY - mMapOffsetY + mMapD / 2;

                  if (mapHeldOriginX >= 0 && mapHeldOriginX < mMapD &&
                      mapHeldOriginY >= 0 && mapHeldOriginY < mMapD) {

                    int mapHeldOriginI =
                        mapHeldOriginY * mMapD + mapHeldOriginX;

                    if (mMapMoveSpeeds[mapHeldOriginI] > 0 &&
                        (mMapMoveOffsets[mapHeldOriginI].x != 0 ||
                         mMapMoveOffsets[mapHeldOriginI].y != 0)) {

                      // only do this if tweak not already
                      // set.  Don't want to interrupt
                      // an existing tweak
                      if (existing->actionTargetTweakX == 0 &&
                          existing->actionTargetTweakY == 0) {

                        existing->actionTargetTweakX =
                            lrint(mMapMoveOffsets[mapHeldOriginI].x);

                        existing->actionTargetTweakY =
                            lrint(mMapMoveOffsets[mapHeldOriginI].y);
                      }

                      existing->heldObjectPos =
                          add(existing->heldObjectPos,
                              mMapMoveOffsets[mapHeldOriginI]);
                    }
                  }

                  existing->heldObjectRot = 0;

                  int mapX = heldOriginX - mMapOffsetX + mMapD / 2;
                  int mapY = heldOriginY - mMapOffsetY + mMapD / 2;
                  ...
                  if (mapX >= 0 && mapX < mMapD && mapY >= 0 && mapY < mMapD) {
                    ...
                    existing->heldFrozenRotFrameCount =
                        mMapAnimationFrozenRotFrameCount[mapI];
                    existing->heldFrozenRotFrameCountUsed = false;

                    if (mMapLastAnimFade[mapI] == 0) {
                      existing->lastHeldAnim = mMapCurAnimType[mapI];
                      ...
                      // an animation fade
                      existing->lastHeldAnim = mMapLastAnimType[mapI];
                      existing->lastHeldAnimationFrameCount =
                          mMapAnimationLastFrameCount[mapI];

                      existing->curHeldAnim = mMapCurAnimType[mapI];

                      existing->heldAnimationFrameCount =
                          mMapAnimationFrameCount[mapI];

                      existing->lastHeldAnimFade = mMapLastAnimFade[mapI];

                      existing->futureHeldAnimStack->push_back(held);
                    }
                  }
                }
              } else {
                ...
                if (existing->holdingID > 0) {
                  ...
                  ObjectRecord *heldObj = getObject(existing->holdingID);
                  ...
                  if (oldHeld > 0 && oldHeld != existing->holdingID &&
                    ...
                    // play decay sound
                    ObjectRecord *obj = getObject(oldHeld);
                    if (obj->decaySound.numSubSounds > 0) {
                  ...
                  } else if (oldHeld > 0 && heldTransitionSourceID > 0) {

                    TransRecord *t = getTrans(oldHeld, heldTransitionSourceID);

                    if (t == NULL && oldHeld == heldTransitionSourceID) {
                      // see if use-on-bare-ground
                      // transition exists
                      t = getTrans(oldHeld, -1);
                    }

                    if (t != NULL && t->target != t->newTarget &&
                        t->newTarget > 0) {
                      ...
                      if (getObject(t->newTarget)->creationSound.numSubSounds >
                          0) {
                        ...
                        if (shouldCreationSoundPlay(sourceID, t->newTarget)) {
                  ...
                  if (oldHeld != existing->holdingID &&
                      (!otherSoundPlayed || heldObj->creationSoundForce) &&
                      !clothingChanged && heldTransitionSourceID >= 0 &&
                      heldObj->creationSound.numSubSounds > 0) {

                    int testAncestor = oldHeld;

                    if (oldHeld <= 0 && heldTransitionSourceID > 0) {

                      testAncestor = heldTransitionSourceID;
                    }

                    if (heldTransitionSourceID > 0) {
                      TransRecord *groundTrans =
                          getTrans(oldHeld, heldTransitionSourceID);
                      if (groundTrans != NULL && groundTrans->newTarget > 0 &&
                          groundTrans->newActor == existing->holdingID) {
                        if (shouldCreationSoundPlay(groundTrans->target,
                    ...
                    if ((!groundSoundPlayed || heldObj->creationSoundForce) &&
                        testAncestor > 0) {
                      ...
                      if (testAncestor > 0 && existing->holdingID > 0) {

                        int holdID = existing->holdingID;

                        ObjectRecord *ancObj = getObject(testAncestor);

                        if (heldObj->isUseDummy && ancObj->isUseDummy &&
                            heldObj->useDummyParent == ancObj->useDummyParent) {
                          sameParent = true;
                        } else if (heldObj->numUses > 1 && ancObj->isUseDummy &&
                                   ancObj->useDummyParent == holdID) {
                          sameParent = true;
                        } else if (ancObj->numUses > 1 && heldObj->isUseDummy &&
                                   heldObj->useDummyParent == testAncestor) {
                          sameParent = true;
                        }
                      }

                      if (!sameParent &&
                          (!heldObj->creationSoundInitialOnly ||
                           (!isSpriteSubset(testAncestor,
                                            existing->holdingID)))) {

                        playSound(heldObj->creationSound,
                                  getVectorFromCamera(existing->currentPos.x,
                                                      existing->currentPos.y));
                        creationSoundPlayed = true;

                        if (existing->id != ourID &&
                            strstr(heldObj->description, "offScreenSound") !=
                                NULL) {

                          addOffScreenSound(existing->id,
                                            existing->currentPos.x * CELL_D,
                                            existing->currentPos.y * CELL_D,
                                            heldObj->description);
                        }
                      }
                    }
                  ...
                  if (oldHeld > 0 && existing->holdingID > 0) {
                    TransRecord *dR = getTrans(-1, oldHeld);

                    if (dR != NULL && dR->newActor == 0 &&
                        dR->newTarget == existing->holdingID) {

                      autoDecay = true;
                    }
                  }

                  if (oldHeld == 0 || heldContChanged ||
                      (heldTransitionSourceID == -1 && !autoDecay &&
                       !creationSoundPlayed && !clothingSoundPlayed)) {
                    // we're holding something new

                    existing->lastHeldAnim = held;
                    existing->lastHeldAnimationFrameCount = 0;
                    existing->curHeldAnim = held;
                    existing->heldAnimationFrameCount = 0;
                    existing->lastHeldAnimFade = 0;

                    if (!otherSoundPlayed && !creationSoundPlayed &&
                        !groundSoundPlayed && !clothingSoundPlayed) {
                      // play generic pickup sound
                      ...
                      if (!existing->dying && !isSick(existing) &&
                          existingObj->usingSound.numSubSounds > 0) {
            ...
            if (existing->holdingID >= 0 && oldHeld >= 0 &&
                oldHeld != existing->holdingID && !heldOriginValid &&
                heldTransitionSourceID > 0 && !creationSoundPlayed &&
                !clothingSoundPlayed && !groundSoundPlayed &&
                !otherSoundPlayed) {
              ...
              TransRecord *tr = getTrans(oldHeld, heldTransitionSourceID);

              if (tr != NULL && tr->newActor == existing->holdingID &&
                ...
                if (tr->newTarget > 0 && tr->target > 0 &&
                    shouldCreationSoundPlay(tr->target, tr->newTarget)) {
                ...
                if (!creationWillPlay) {
                  ...
                  if (targetObject->usingSound.numSubSounds > 0) {

                    playSound(targetObject->usingSound,
                              getVectorFromCamera(existing->currentPos.x,
                                                  existing->currentPos.y));
                  }
                }
            ...
            if (babyDropped) {
              ...
              if (existing->lastHeldByRawPosSet) {
                existing->heldByDropOffset =
                    sub(existing->lastHeldByRawPos, existing->currentPos);
              ...
              LiveObject *adultO = getGameObject(existing->heldByAdultID);

              if (adultO != NULL) {
                // fade from held animation
                existing->lastAnim = adultO->curHeldAnim;

                existing->animationFrameCount = adultO->heldAnimationFrameCount;
            ...
            if (existing->id == ourID) {
              ...
              if (forced) {
                ...
                char *forceMessage =
                    autoSprintf("FORCE %d %d#", existing->xd, existing->yd);
            ...
            if (existing->id != ourID) {
              ...
              if (done_moving == existing->lastMoveSequenceNumber) {
            ...
            for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
              ...
              int oldNumCont = existing->clothingContained[c].size();

              existing->clothingContained[c].deleteAll();

              int newNumClothingCont = o.clothingContained[c].size();
              int *newClothingCont = o.clothingContained[c].getElementArray();

              existing->clothingContained[c].appendArray(newClothingCont,
                                                         newNumClothingCont);
              delete[] newClothingCont;

              if (!clothingSoundPlayed && newNumClothingCont > oldNumCont) {
                ...
                SoundUsage contSound =
                    clothingByIndex(existing->clothing, c)->usingSound;

                if (contSound.numSubSounds > 0) {
                  playSound(contSound,
                            getVectorFromCamera(existing->xd, existing->yd));
                  soundPlayed = true;
                }
                ...
                if (!soundPlayed) {
                  ...
                  SoundUsage s = getObject(existing->displayID)->usingSound;
            ...
            o.futureAnimStack = new SimpleVector<AnimType>();
            o.futureHeldAnimStack = new SimpleVector<AnimType>();
            ...
            if (obj->creationSound.numSubSounds > 0) {
              ...
              playSound(obj->creationSound,
                        getVectorFromCamera(o.currentPos.x, o.currentPos.y));
            }
        ...
        } else if (o.id == ourID && strstr(lines[i], "X X") != NULL) {
          // we died
          ...
          if (reasonPos == NULL) {
            mDeathReason = stringDuplicate(translate("reasonUnknown"));
          } else {
            ...
            if (apocalypseInProgress) {
              mDeathReason = stringDuplicate(translate("reasonApocalypse"));
            } else if (strcmp(reasonString, "nursing_hunger") == 0) {
              mDeathReason = stringDuplicate(translate("reasonNursingHunger"));
            } else if (strcmp(reasonString, "hunger") == 0) {
              mDeathReason = stringDuplicate(translate("reasonHunger"));
            } else if (strcmp(reasonString, "SID") == 0) {
              mDeathReason = stringDuplicate(translate("reasonSID"));
            } else if (strcmp(reasonString, "age") == 0) {
              mDeathReason = stringDuplicate(translate("reasonOldAge"));
              ...
              if (weaponO == NULL) {
                mDeathReason =
                    stringDuplicate(translate("reasonKilledUnknown"));
              } else {

                char *stringUpper = stringToUpperCase(weaponO->description);

                stripDescriptionComment(stringUpper);

                mDeathReason =
                    autoSprintf("%s%s", translate("reasonKilled"), stringUpper);
              ...
              if (sicknessO == NULL) {
                mDeathReason =
                    stringDuplicate(translate("reasonSuccumbedUnknown"));
              } else {

                char *stringUpper = stringToUpperCase(sicknessO->description);

                stripDescriptionComment(stringUpper);

                mDeathReason = autoSprintf("%s%s", translate("reasonSuccumbed"),
                                           stringUpper);
          ...
          numRead = sscanf(lines[i], "%d %d", &(o.id), &(o.holdingID));
          ...
          for (int i = 0; i < gameObjects.size(); i++) {
            ...
            if (nextObject->id == o.id) {
              ...
              if (nextObject->heldByAdultID > 0) {
                ...
                // parent to drop them
                LiveObject *parent = getGameObject(nextObject->heldByAdultID);

                if (parent != NULL && parent->holdingID == -o.id) {
                  parent->holdingID = 0;
                }
              }
              ...
              for (int j = 0; j < gameObjects.size(); j++) {
                LiveObject *otherObject = gameObjects.getElement(j);

                dropPendingReceivedMessagesRegardingID(otherObject, o.id);
              }

              // play any pending messages that are
              // waiting for them to finish their move
              playPendingReceivedMessagesRegardingOthers(nextObject);
      ...
      for (int i = 0; i < unusedHolderID.size(); i++) {
        ...
        LiveObject *babyO = getGameObject(unusedHeldID.getElementDirect(i));
        ...
        if (babyO != NULL && existing != NULL) {
          ...
          existing->heldFrozenRotFrameCount = babyO->frozenRotFrameCount;

          existing->heldFrozenRotFrameCountUsed = false;

          if (babyO->lastAnimFade == 0) {

            existing->lastHeldAnim = babyO->curAnim;

            existing->heldAnimationFrameCount = babyO->animationFrameCount;
            ...
            existing->lastHeldAnim = babyO->lastAnim;
            existing->lastHeldAnimationFrameCount =
                babyO->lastAnimationFrameCount;

            existing->curHeldAnim = babyO->curAnim;

            existing->heldAnimationFrameCount = babyO->animationFrameCount;

            existing->lastHeldAnimFade = babyO->lastAnimFade;
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int numRead =
            sscanf(lines[i], "%d %d %d %lf %lf %d", &(o.id), &(startX),
                   &(startY), &(o.moveTotalTime), &etaSec, &truncated);

        SimpleVector<char *> *tokens = tokenizeString(lines[i]);
        ...
        if (tokens->size() < 8 || tokens->size() % 2 != 0) {
          ...
          for (int e = 1; e < o.pathLength; e++) {
            ...
            char *xToken = tokens->getElementDirect(6 + (e - 1) * 2);
            char *yToken = tokens->getElementDirect(6 + (e - 1) * 2 + 1);
          ...
          for (int j = 0; j < gameObjects.size(); j++) {
            if (gameObjects.getElement(j)->id == o.id) {
              ...
              if (existing->pendingReceivedMessages.size() > 0) {
                ...
                printf("Holding PM message for %d "
                       "until later, "
                       "%d other messages pending for them\n",
                       existing->id, existing->pendingReceivedMessages.size());
                ...
                existing->pendingReceivedMessages.push_back(
                    autoSprintf("PM\n%s\n#", lines[i]));
                existing->somePendingMessageIsMoreMovement = true;
              ...
              double timePassed = o.moveTotalTime - etaSec;

              double fractionPassed = timePassed / o.moveTotalTime;
              ...
              if (existing->currentSpeed != 0 && existing->pathToDest != NULL) {

                // an interrupted move
                oldPathLength = existing->pathLength;
                oldCurrentPathPos =
                    existing->pathToDest[existing->currentPathStep];

                oldPath.appendArray(existing->pathToDest, existing->pathLength);
              ...
              if (existing->id != ourID) {
                ...
                removeDoubleBacksFromPath(&(o.pathToDest), &(o.pathLength));
              }

              if (existing->id != ourID || truncated) {
                ...
                memcpy(existing->pathToDest, o.pathToDest,
                ...
                if (existing->id != ourID) {
                  ...
                  int b =
                      (int)floor(fractionPassed * (existing->pathLength - 1));
                  ...
                  if (distance(curr, existing->currentPos) > 5) {
              ...
              if (existing->id != ourID) {
                ...
                if (oldPathLength != 0) {
                  ...
                  for (int p = 0; p < existing->pathLength - 1; p++) {

                    if (equal(existing->pathToDest[p], oldCurrentPathPos)) {
                  ...
                  if (found) {
                    ...
                    existing->currentPathStep = foundStep;

                    doublePair foundWorld =
                        gridToDouble(existing->pathToDest[foundStep]);

                    // where we should move toward
                    doublePair nextWorld;

                    if (foundStep < existing->pathLength - 1) {

                      // point from here to our
                      // next step
                      nextWorld =
                          gridToDouble(existing->pathToDest[foundStep + 1]);
                    } else {
                    ...
                    existing->currentMoveDirection =
                        normalize(sub(nextWorld, existing->currentPos));
                  } else {
                    // other case
                    ...
                    printf("    CUR PATH:  ");
                    printPath(oldPath.getElementArray(), oldPathLength);
                    printf("    WE AT:  %d (%d,%d)  \n", oldCurrentPathIndex,
                           oldCurrentPathPos.x, oldCurrentPathPos.y);

                    int foundStartIndex = -1;

                    for (int i = 0; i < oldPathLength; i++) {
                      GridPos p = oldPath.getElementDirect(i);
                    ...
                    if (foundStartIndex != -1) {
                      ...
                      if (foundStartIndex > oldCurrentPathIndex) {
                        step = 1;
                      } else if (foundStartIndex < oldCurrentPathIndex) {
                        step = -1;
                      }
                      ...
                      for (int i = oldCurrentPathIndex; i != foundStartIndex;
                           i += step) {

                        newPath.push_back(oldPath.getElementDirect(i));
                      }

                      for (int i = 0; i < existing->pathLength; i++) {
                        // now add rest of new path
                        newPath.push_back(existing->pathToDest[i]);
                      }

                      printf("    OLD PATH:  ");
                      printPath(existing->pathToDest, existing->pathLength);

                      // now replace path
                      // with new, lead-appended path
                      existing->pathLength = newPath.size();

                      delete[] existing->pathToDest;

                      existing->pathToDest = newPath.getElementArray();
                      existing->currentPathStep = 0;
                      existing->numFramesOnCurrentStep = 0;

                      printf("    NEW PATH:  ");
                      printPath(existing->pathToDest, existing->pathLength);

                      removeDoubleBacksFromPath(&(existing->pathToDest),
                                                &(existing->pathLength));

                      printf("    NEW PATH (DB free):  ");
                      printPath(existing->pathToDest, existing->pathLength);
                      ...
                      doublePair nextWorld =
                          gridToDouble(existing->pathToDest[1]);
                ...
                if (!usingOldPathStep && !appendingLeadPath) {
                  ...
                  for (int testB = 0; testB < existing->pathLength - 1;
                       testB++) {

                    doublePair worldPos =
                        gridToDouble(existing->pathToDest[testB]);

                    double thisDist = distance(worldPos, existing->currentPos);
                    if (thisDist < minDist) {
                      b = testB;
                      minDist = thisDist;
                    }
                  ...
                  doublePair nWorld = gridToDouble(existing->pathToDest[n]);

                  // point toward next path pos
                  existing->currentMoveDirection =
                      normalize(sub(nWorld, existing->currentPos));
                }
                ...
                if (usingOldPathStep) {
                  ...
                  double oldFractionPassed =
                      measurePathLength(existing,
                                        existing->currentPathStep + 1) /
                      measurePathLength(existing, existing->pathLength);
                  ...
                  // should get smaller)
                  double fractionDiff = oldFractionPassed - fractionPassed;

                  double timeAdjust = existing->moveTotalTime * fractionDiff;
                ...
                if (!stillOnPath) {
                  ...
                  existing->currentPos.x = existing->xd;
                  existing->currentPos.y = existing->yd;
                }
              }
          ...
          for (int j = 0; j < gameObjects.size(); j++) {
            if (gameObjects.getElement(j)->id == id) {
              ...
              if (firstSpace != NULL) {
                ...
                if (existing->age < 1 && existing->heldByAdultID == -1) {
                  ...
                  existing->tempAgeOverrideSetTime = game_getCurrentTime();
                }

                if (curseFlag && mCurseSound != NULL) {
                  playSound(mCurseSound,
                ...
                if (existing->id == ourID) {
                  ...
                  char *starPos = strstr(existing->currentSpeech, " *map");

                  if (starPos != NULL) {

                    int mapX, mapY;

                    int numRead = sscanf(starPos, " *map %d %d", &mapX, &mapY);
                    if (numRead == 2) {
                    ...
                    doublePair dest = {(double)mapX, (double)mapY};
                    double d = distance(dest, existing->currentPos);

                    if (d >= 5) {
                      char *dString = getSpokenNumber(d);
                      char *newSpeech =
                          autoSprintf("%s - %s %s", existing->currentSpeech,
                                      dString, translate("metersAway"));
                      delete[] dString;
                      delete[] existing->currentSpeech;
                      existing->currentSpeech = newSpeech;
                    }
                  }
                }
              }
        ...
        if (numRead == 2) {
          ...
          if (firstSpace != NULL) {
            ...
            if (secondSpace != NULL) {
              ...
              // longer time for longer speech
              ls.fadeETATime =
                  game_getCurrentTime() + 3 + strlen(ls.speech) / 5;
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int numRead = sscanf(lines[i], "%d %d %d", &pid, &emotIndex, &ttlSec);
          ...
          for (int j = 0; j < gameObjects.size(); j++) {
            if (gameObjects.getElement(j)->id == pid) {
              ...
              if (ttlSec < 0) {
                ...
                if (newEmotPlaySound != NULL) {
                  if (existing->permanentEmots.getElementIndex(
                    ...
                    existing->permanentEmots.push_back(newEmotPlaySound);
                  }
                }
                if (ttlSec == -2) {
                ...
                if (numRead == 3 && ttlSec > 0) {
                  existing->emotClearETATime = game_getCurrentTime() + ttlSec;
                } else {
              ...
              if (newEmotPlaySound != NULL) {
                ...
                // if in range
                if (!existing->outOfRange)
                  for (int i = 0; i < getEmotionNumObjectSlots(); i++) {

                    int id = getEmotionObjectByIndex(newEmotPlaySound, i);
                    ...
                    if (id > 0) {
                      ...
                      if (obj->creationSound.numSubSounds > 0) {
                        ...
                        playSound(
                            obj->creationSound,
                            getVectorFromCamera(playerPos.x, playerPos.y));

                        if (existing->id != ourID &&
                            strstr(obj->description, "offScreenSound") !=
                                NULL) {

                          addOffScreenSound(existing->id, playerPos.x * CELL_D,
                                            playerPos.y * CELL_D,
                                            obj->description);
                        }
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int id;
        int numRead = sscanf(lines[i], "%d ", &(id));
          ...
          for (int j = 0; j < gameObjects.size(); j++) {
            if (gameObjects.getElement(j)->id == id) {
              ...
              if (firstSpace != NULL) {
                ...
                SimpleVector<char *> *tokens = tokenizeString(linStart);

                int numNormalTokens = tokens->size();

                if (tokens->size() > 0) {
                  char *lastToken =
                      tokens->getElementDirect(tokens->size() - 1);

                  if (strstr(lastToken, "eve=")) {
                    // eve tag at end
                    numNormalTokens--;

                    sscanf(lastToken, "eve=%d", &(existing->lineageEveID));

                    if (existing->lineageEveID > 0) {
                      // copy war status from someone
                      // else in this lineage
                      for (int i = 0; i < gameObjects.size(); i++) {
                        LiveObject *other = gameObjects.getElement(i);

                        if (other->id != existing->id &&
                            other->lineageEveID == existing->lineageEveID) {
                          existing->warPeaceStatus = other->warPeaceStatus;
                          break;
                        }
                      }
                    }
      ...
      for (int j = 0; j < gameObjects.size(); j++) {
        if (gameObjects.getElement(j)->id != ourID) {
          ...
          if (other->relationName == NULL) {
            ...
            */
            other->relationName = getRelationName(ourObject, other);
          }
        }
      }
    } else if (type == CURSED) {
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int id, level;
        int numRead = sscanf(lines[i], "%d %d", &id, &level);
      ...
      if (ourLiveObject != NULL) {
        ...
        sscanf(message, "CX\n%d", &(ourLiveObject->curseTokenCount));
      }
      ...
      if (ourLiveObject != NULL) {
        ...
        sscanf(message, "CS\n%d", &(ourLiveObject->excessCursePoints));
      }
    } else if (type == PONG) {
      sscanf(message, "PONG\n%d", &(lastPongReceived));
      ...
      for (int i = 1; i < numLines; i++) {
        ...
        int numRead = sscanf(lines[i], "%d %d", &(id), &sickFlag);
        ...
        if (numRead == 1) {
          for (int j = 0; j < gameObjects.size(); j++) {
            if (gameObjects.getElement(j)->id == id) {
              ...
              // wound state
              if (existing->holdingID > 0) {
                ObjectRecord *held = getObject(existing->holdingID);

                if (held->decaySound.numSubSounds > 0) {

                  playSound(held->decaySound,
                            getVectorFromCamera(existing->currentPos.x,
                                                existing->currentPos.y));
                }
              }
      ...
      if (ourLiveObject != NULL) {
        ...
        sscanf(message, "FX\n%d %d %d %d %lf %d %d %d", &(foodStore),
               &(foodCapacity), &(lastAteID), &(lastAteFillMax), &(lastSpeed),
               &responsiblePlayerID, &mYumBonus, &mYumMultiplier);
        ...
        if (responsiblePlayerID != -1 &&
            getLiveObject(responsiblePlayerID) != NULL &&
            getLiveObject(responsiblePlayerID)->pendingReceivedMessages.size() >
                0) {
          ...
          printf("Holding FX message caused by %d until later, "
                 "%d other messages pending for them\n",
                 responsiblePlayerID, rO->pendingReceivedMessages.size());
          rO->pendingReceivedMessages.push_back(stringDuplicate(message));
        } else {
          ...
          if (mCurrentLastAteString != NULL) {
            ...
            for (int i = 0; i < mOldLastAteStrings.size(); i++) {
              ...
              *(mOldLastAteFades.getElement(i)) = fade;
              ...
              float barFade = mOldLastAteBarFades.getElementDirect(i);
              ...
              *(mOldLastAteBarFades.getElement(i)) = barFade;
              ...
              else if (strcmp(mCurrentLastAteString,
                              mOldLastAteStrings.getElementDirect(i)) == 0) {
                // already in stack, move to top
                mOldLastAteStrings.deallocateStringElement(i);
                mOldLastAteFillMax.deleteElement(i);
                mOldLastAteFades.deleteElement(i);
          ...
          if (lastAteID != 0) {
            ...
            char *strUpper = stringToUpperCase(lastAteObj->description);

            stripDescriptionComment(strUpper);

            const char *key = "lastAte";

            if (strstr(lastAteObj->description, "+drink") != NULL) {
              key = "lastDrank";
            ...
            mCurrentLastAteString =
                autoSprintf("%s %s", translate(key), strUpper);
            delete[] strUpper;
            ...
            mCurrentLastAteString = stringDuplicate(translate("breastMilk"));
          ...
          if (ourLiveObject->foodCapacity > ourLiveObject->maxFoodCapacity) {

            ourLiveObject->maxFoodCapacity = ourLiveObject->foodCapacity;
          }
  ...
  if (mapPullMode && mapPullCurrentSaved && !mapPullCurrentSent) {
    char *message = autoSprintf("MAP %d %d#", sendX(mapPullCurrentX),
  ...
  if (!mapPullMode && mDoneLoadingFirstObjectSet && ourLiveObject != NULL) {
    ...
    if (vogMode) {
      // don't adjust camera
    } else if (cameraFollowsObject->currentPos.x != cameraFollowsObject->xd ||
      ...
      screenCenterPlayerOffsetX -=
          lrint(moveScale * cameraFollowsObject->currentMoveDirection.x);
      ...
      screenCenterPlayerOffsetY -=
          lrint(moveScale * cameraFollowsObject->currentMoveDirection.y);
    ...
    if (!vogMode) {
      ...
      screenTargetPos.x =
          CELL_D * targetObjectPos.x - screenCenterPlayerOffsetX;

      screenTargetPos.y =
          CELL_D * targetObjectPos.y - screenCenterPlayerOffsetY;
    }
    ...
    if (false && length(dir) > maxR) {
      ...
      if (length(moveStep) > 0) {
        lastScreenViewCenter = add(lastScreenViewCenter, moveStep);
        viewChange = true;
      }
    }
  ...
  if (!mapPullMode)
    for (int i = 0; i < gameObjects.size(); i++) {
      ...
      if (o->holdingID > 0 && getObject(o->holdingID)->rideable) {
      ...
      if (!o->outOfRange) {
        ...
        if (o->curAnim != moving || !holdingRideable) {
          ...
          handleAnimSound(o->id, o->displayID, computeCurrentAge(o), t,
                          oldFrameCount, o->animationFrameCount, pos.x, pos.y);
        }
        ...
        if (o->currentEmot != NULL) {
          ...
          for (int e = 0; e < numSlots; e++) {
            int oID = getEmotionObjectByIndex(o->currentEmot, e);

            if (oID != 0) {

              handleAnimSound(o->id, oID, 0, t, oldFrameCount,
                              o->animationFrameCount, pos.x, pos.y);
            }
          }
        }
      }
      ...
      if (o->lastAnimFade > 0) {
        ...
        if (o->lastAnimFade == 1) {
          ...
          if (isAnimFadeNeeded(o->displayID, o->lastAnim, o->curAnim)) {
        ...
        if (o->lastAnimFade < 0) {
          ...
          if (o->futureAnimStack->size() > 0) {
            ...
            addNewAnimDirect(o, o->futureAnimStack->getElementDirect(0));
      ...
      if (o->holdingID > 0) {
        handleAnimSound(o->id, o->holdingID, 0, o->curHeldAnim, oldFrameCount,
                        o->heldAnimationFrameCount, o->currentPos.x,
                        o->currentPos.y);
      }
      ...
      if (o->lastHeldAnimFade > 0) {
        ...
        if (o->lastHeldAnimFade == 1) {
          ...
          if (heldDisplayID > 0 &&
              isAnimFadeNeeded(heldDisplayID, o->lastHeldAnim,
                               o->curHeldAnim)) {
            // fade needed, do nothing
          } else {
            // fade not needed
      ...
      if (o->currentSpeed != 0 && o->pathToDest != NULL) {
        ...
        doublePair startPos = {(double)curStepDest.x, (double)curStepDest.y};

        doublePair endPos = {(double)nextStepDest.x, (double)nextStepDest.y};
        ...
        if (o->numFramesOnCurrentStep * o->currentSpeed * frameRateFactor > 2) {
        ...
        if (o->currentPathStep < o->pathLength - 2) {
          ...
          o->currentPos = add(o->currentPos,
                              mult(o->currentMoveDirection, o->currentSpeed));

          addNewAnim(o, moving);

          if (pathStepDistFactor * distance(o->currentPos, startPos) >
              distance(o->currentPos, endPos)) {
          ...
          if (o->id == ourID && mouseDown && shouldMoveCamera &&
            ...
            screenToWorld(lastScreenMouseX, lastScreenMouseY, &worldMouseX,
                          &worldMouseY);

            doublePair worldMouse = {worldMouseX, worldMouseY};

            doublePair worldCurrent = mult(o->currentPos, CELL_D);
            ...
            if (nextActionMessageToSend == NULL ||
                mouseDownFrames > minMouseDownFrames / frameRateFactor) {

              double absX = fabs(delta.x);
              double absY = fabs(delta.y);

              if (absX > CELL_D * 1 || absY > CELL_D * 1) {

                if (absX < CELL_D * 4 && absY < CELL_D * 4 &&
                    mouseDownFrames > minMouseDownFrames / frameRateFactor) {
                  ...
                  doublePair mouseVector = mult(
                      normalize(sub(worldMouse, worldCurrent)), CELL_D * 4);

                  doublePair fakeClick = add(worldCurrent, mouseVector);
                  ...
                  isAutoClick = false;
                     ...
                     nextActionMessageToSend == NULL &&
                     distance(endPos, o->currentPos) < o->currentSpeed) {
            ...
            if (mapIF != -1 && mapIP != -1) {
              ...
              if (floor > 0 && mMapFloors[mapIP] == floor &&
                ...
                if (isSameFloor(floor, finalStep, xDir, yDir)) {
                  ...
                  // with next click
                  while (len < 5 && isSameFloor(floor, nextStep, xDir, yDir)) {
                    nextStep.x += xDir;
                    nextStep.y += yDir;
                    len++;
                  }
                  ...
                  for (int i = 0; i < 8; i++) {
                    ...
                    if (p.x == lastDir.x * -1 && p.y == lastDir.y * -1) {
                      continue;
                    }

                    allDirs.push_back(p);
                    allDist.push_back(distance2(lastDir, p));
                  }
                  ...
                  while (allDirs.size() > 0) {
                    ...
                    for (int i = 0; i < allDirs.size(); i++) {
                      double d = allDist.getElementDirect(i);
                      if (d < minDist) {
                        minDist = d;
                        minInd = i;
                      }
                    }
                    sortedDirs.push_back(allDirs.getElementDirect(minInd));
                    sortedDist.push_back(allDist.getElementDirect(minInd));
                  ...
                  for (int i = 0; i < 6 && !foundBranch; i++) {
                    ...
                    GridPos d = sortedDirs.getElementDirect(i);
                    xDir = d.x;
                    yDir = d.y;
                  ...
                  if (foundBranch) {
                    ...
                    while (len < 5 &&
                           isSameFloor(floor, nextStep, xDir, yDir)) {
                      nextStep.x += xDir;
                      nextStep.y += yDir;
                      len++;
                    }
                ...
                if (!equal(nextStep, finalStep)) {
                  ...
                  pointerDown(nextStep.x * CELL_D, nextStep.y * CELL_D);

                  pointerUp(nextStep.x * CELL_D, nextStep.y * CELL_D);
          ...
          if (distance(endPos, o->currentPos) < o->currentSpeed) {
            ...
            if ((o->id != ourID && !o->somePendingMessageIsMoreMovement) ||
                (o->id == ourID && nextActionMessageToSend == NULL)) {
            ...
            printf("Reached dest (%.0f,%.0f) %f seconds early\n", endPos.x,
                   endPos.y, o->moveEtaTime - game_getCurrentTime());
          } else {
            ...
            if (1.5 * distance(o->currentPos, startPos) >
                distance(o->currentPos, endPos)) {
        ...
        if (game_getCurrentTime() - o->timeOfLastSpeedUpdate > .25) {
  ...
  if (nextActionMessageToSend != NULL && ourLiveObject != NULL &&
      ourLiveObject->currentSpeed == 0 &&
      (isGridAdjacent(ourLiveObject->xd, ourLiveObject->yd, playerActionTargetX,
                      playerActionTargetY) ||
    ...
    if (!playerActionPending) {
      ...
      ourLiveObject->pendingActionAnimationProgress = 0.025 * frameRateFactor;

      ourLiveObject->pendingActionAnimationStartTime = currentTime;
    ...
    if (!ourLiveObject->inMotion &&
      ...
      // bug just yet.
      ourLiveObject->pendingActionAnimationStartTime = game_getCurrentTime();
        ...
        if (false && ourLiveObject->holdingID > 0) {
          ...
          if (held->eatingSound.numSubSounds > 0) {
            playSound(held->eatingSound,
                      getVectorFromCamera(ourLiveObject->currentPos.x,
  ...
  if (mFirstServerMessagesReceived == 3) {
    ...
    if (mStartedLoadingFirstObjectSet && !mDoneLoadingFirstObjectSet) {
      ...
      if (mDoneLoadingFirstObjectSet &&
          game_getCurrentTime() - mStartedLoadingFirstObjectSetStartTime < 1) {
      ...
      if (mDoneLoadingFirstObjectSet) {
        ...
        restartMusic(computeCurrentAge(ourLiveObject), ourLiveObject->ageRate);
        setSoundLoudness(1.0);
        ...
        mapPullStartX = SettingsManager::getIntSetting("mapPullStartX", -10);
        mapPullStartY = SettingsManager::getIntSetting("mapPullStartY", -10);
        mapPullEndX = SettingsManager::getIntSetting("mapPullEndX", 10);
        mapPullEndY = SettingsManager::getIntSetting("mapPullEndY", 10);
        ...
        if (mapPullMode) {
          ...
          mapPullTotalImage = new Image(
              lrint((10 + mapPullEndX - mapPullStartX) * CELL_D * scale),
              lrint((6 + mapPullEndY - mapPullStartY) * CELL_D * scale), 3,
              false);
          numScreensWritten = 0;
        }
      }
      ...
      SimpleVector<int> *allPossibleDeathMarkerIDs = getAllPossibleDeathIDs();
      ...
      for (int i = 0; i < gameObjects.size(); i++) {
        ...
        if (!o->allSpritesLoaded) {
          ...
          for (int s = 0; s < displayObj->numSprites; s++) {
            ...
            if (markSpriteLive(displayObj->sprites[s])) {
              numLoaded++;
            } else if (getSpriteRecord(displayObj->sprites[s]) == NULL) {
        ...
        if (o->holdingID > 0) {
          ...
          for (int j = 0; j < o->numContained; j++) {
            addBaseObjectToLiveObjectSet(o->containedIDs[j]);
      ...
      for (int i = 0; i < numMapCells; i++) {
        ...
        if (mMap[i] > 0) {
          ...
          for (int j = 0; j < numCont; j++) {
            ...
            SimpleVector<int> *subVec = mMapSubContainedStacks[i].getElement(j);

            int numSub = subVec->size();
            for (int s = 0; s < numSub; s++) {
              addBaseObjectToLiveObjectSet(subVec->getElementDirect(s));
            }
          }
        }
      }
...
char LivingLifePage::isSameFloor(int inFloor, GridPos inFloorPos, int inDX,
                                 int inDY) {
  ...
  if (nextMapI != -1 && mMapFloors[nextMapI] == inFloor &&
...
void LivingLifePage::checkForPointerHit(PointerHitRecord *inRecord, float inX,
                                        float inY) {
  ...
  if (clickDestMapY >= 0 && clickDestMapY < mMapD && clickDestMapX >= 0 &&
      clickDestMapX < mMapD) {
    ...
    if (oID > 0 && getObjectHeight(oID) < CELL_D) {
      ObjectRecord *obj = getObject(oID);
      ...
      if (mMapMoveSpeeds[clickDestMapI] > 0) {
        thisObjClickExtraX -= mMapMoveOffsets[clickDestMapI].x * CELL_D;
        thisObjClickExtraY -= mMapMoveOffsets[clickDestMapI].y * CELL_D;
      }

      int sp, cl, sl;

      double dist = getClosestObjectPart(
          obj, NULL, &(mMapContainedStacks[clickDestMapI]), NULL, false, -1, -1,
          mMapTileFlips[clickDestMapI], thisObjClickExtraX, thisObjClickExtraY,
          &sp, &cl, &sl,
  ...
  for (int y = clickDestY - 3; y <= clickDestY + 3 && !p->hit; y++) {
    ...
    // (in front of people)
    for (int x = clickDestX + 3; x >= clickDestX - 3 && !p->hit; x--) {
      ...
      if (oID > 0 && !getObject(oID)->drawBehindPlayer) {
        ObjectRecord *obj = getObject(oID);
        ...
        if (mMapMoveSpeeds[mapI] > 0) {
          thisObjClickOffsetX -= mMapMoveOffsets[mapI].x * CELL_D;
          thisObjClickOffsetY -= mMapMoveOffsets[mapI].y * CELL_D;
        }

        int sp, cl, sl;

        double dist = getClosestObjectPart(
            obj, NULL, &(mMapContainedStacks[mapI]), NULL, false, -1, -1,
            mMapTileFlips[mapI], thisObjClickOffsetX, thisObjClickOffsetY, &sp,
            &cl, &sl,
            // ignore transparent parts
            // allow objects behind smoke to be picked up
            false);
        ...
        if (dist < minDistThatHits) {
          ...
          if (p->hitOurPlacement && getObjectHeight(oID) > .75 * CELL_D) {
        ...
        for (int x = clickDestX + 1; x >= clickDestX - 1 && !p->hit; x--) {
          ...
          for (int i = gameObjects.size() - 1; i >= 0 && !p->hit; i--) {
            ...
            if (d == 1 &&
              ...
              // recently dropped baby, skip
              continue;
            } else if (d == 0 && o->heldByDropOffset.x == 0 &&
                       o->heldByDropOffset.y == 0) {
              // not a recently-dropped baby, skip
              continue;
            }
            ...
            if (round(oX) == x && round(oY) == y) {
              ...
              if (computeCurrentAgeNoOverride(o) < noMoveAge) {
                ...
                // rotate
                doublePair c = {personClickOffsetX, personClickOffsetY};
              ...
              double dist = getClosestObjectPart(
                  obj, &(o->clothing), NULL, o->clothingContained, false,
                  computeCurrentAge(o), -1, o->holdingFlip, personClickOffsetX,
                  personClickOffsetY, &sp, &cl, &sl);
  ...
  if (p->hitOurPlacement && (p->hitOtherPerson || p->hitSelf)) {
    p->hitAnObject = false;
  } else {
    ...
    if (p->hit && p->hitAnObject && !p->hitOtherPerson && !p->hitSelf) {
      ...
      for (int y = p->closestCellY + 1;
        ...
        for (int x = p->closestCellX - 1;
          ...
          for (int i = gameObjects.size() - 1; i >= 0 && !p->hitOtherPerson;
               i--) {
            ...
            if (o->heldByDropOffset.x != 0 || o->heldByDropOffset.y != 0) {
              // recently dropped baby, skip
              continue;
            }
            ...
            if (oY == y && oX == x) {
              ...
              double dist = getClosestObjectPart(
                  obj, &(o->clothing), NULL, o->clothingContained, false,
                  computeCurrentAge(o), -1, o->holdingFlip, clickOffsetX,
                  clickOffsetY, &sp, &cl, &sl);
...
void LivingLifePage::pointerMove(float inX, float inY) {
  ...
  if (mCurMouseOverID > 0 && !mCurMouseOverSelf &&
      (mCurMouseOverID != destID || mCurMouseOverSpot.x != mapX ||
  ...
  if (destID == 0) {
    if (p.hitSelf) {
      ...
      if (p.hitClothingIndex != -1) {
        if (p.hitSlotIndex != -1) {
          mCurMouseOverID = ourLiveObject->clothingContained[p.hitClothingIndex]
                                .getElementDirect(p.hitSlotIndex);
        } else {
          ObjectRecord *c =
              clothingByIndex(ourLiveObject->clothing, p.hitClothingIndex);
          mCurMouseOverID = c->id;
        }

        ourLiveObject->currentMouseOverClothingIndex = p.hitClothingIndex;
      }
    }
    if (p.hitOtherPerson) {
  ...
  if (destID > 0) {
    ...
    if ((destID != mCurMouseOverID || mCurMouseOverSpot.x != mapX ||
         mCurMouseOverSpot.y != mapY)) {
    ...
    if (p.hitSlotIndex != -1) {
      mCurMouseOverID =
          mMapContainedStacks[mapY * mMapD + mapX].getElementDirect(
              p.hitSlotIndex);
    }
    ...
    for (int i = 0; i < mPrevMouseOverSpots.size(); i++) {
      ...
      if (equal(old, mCurMouseOverSpot)) {
        mCurMouseOverFade = mPrevMouseOverSpotFades.getElementDirect(i);
...
char LivingLifePage::getCellBlocksWalking(int inMapX, int inMapY) {
  ...
  if (inMapY >= 0 && inMapY < mMapD && inMapX >= 0 && inMapX < mMapD) {
...
void LivingLifePage::pointerDown(float inX, float inY) {
  ...
  // no one around
  if (!mouseAlreadyDown && modClick && ourLiveObject->holdingID > 0 &&
      getObject(ourLiveObject->holdingID)->deadlyDistance > 0 &&
      isShiftKeyDown() && !p.hitOtherPerson) {
  ...
  printf("clickDestX,Y = %d, %d,  mapX,Y = %d, %d, curX,Y = %d, %d\n",
         clickDestX, clickDestY, mapX, mapY, ourLiveObject->xd,
         ourLiveObject->yd);
  if (!killMode && mapY >= 0 && mapY < mMapD && mapX >= 0 && mapX < mMapD) {
  ...
  if (p.hitSelf) {
    ...
    if (!ourLiveObject->inMotion) {
      ...
      if (!modClick || p.hitClothingIndex == -1) {
        ...
        nextActionMessageToSend =
            autoSprintf("SELF %d %d %d#", sendX(clickDestX), sendY(clickDestY),
                        p.hitClothingIndex);
        printf("Use on self\n");
      } else {
        ...
        if (ourLiveObject->holdingID > 0) {
          nextActionMessageToSend =
              autoSprintf("DROP %d %d %d#", sendX(clickDestX),
                          sendY(clickDestY), p.hitClothingIndex);
          nextActionDropping = true;
          printf("Add to own clothing container\n");
        } else {
          nextActionMessageToSend = autoSprintf(
              "SREMV %d %d %d %d#", sendX(clickDestX), sendY(clickDestY),
              p.hitClothingIndex, p.hitSlotIndex);
  ...
  if (destID > 0 && p.hitAnObject) {
    ...
    if (ourLiveObject->holdingID > 0) {
      ...
      } else if (tr->newActor > 0 && ourLiveObject->holdingID != tr->newActor) {
    ...
    if (!getObject(destID)->blocksWalking) {
      ...
      for (int y = -5; y < 5; y++) {
        for (int x = -5; x < 5; x++) {
          ...
          if (mapPY >= 0 && mapPY < mMapD && mapPX >= 0 && mapPX < mMapD) {

            int oID = mMap[mapPY * mMapD + mapPX];

            if (oID == 0 || (oID > 0 && !getObject(oID)->blocksWalking)) {
  ...
  if (destID == 0 && p.hitOtherPerson && modClick &&
      ourLiveObject->holdingID > 0 &&
      getObject(ourLiveObject->holdingID)->deadlyDistance > 0 &&
      isShiftKeyDown()) {
    ...
    if (o->id != ourID && o->heldByAdultID == -1) {
      ...
      char *killMessage = autoSprintf("KILL %d %d %d#", sendX(clickDestX),
                                      sendY(clickDestY), p.hitOtherPersonID);
  ...
  if (destID != 0 && !modClick && ourLiveObject->holdingID > 0 &&
    ...
    double d = sqrt(
        (clickDestX - ourLiveObject->xd) * (clickDestX - ourLiveObject->xd) +
        (clickDestY - ourLiveObject->yd) * (clickDestY - ourLiveObject->yd));
    ...
    if (getObject(ourLiveObject->holdingID)->useDistance >= d) {
      ...
      nextActionMessageToSend =
          autoSprintf("USE %d %d#", sendX(clickDestX), sendY(clickDestY));
      ...
      printf("USE from a distance, src=(%d,%d), dest=(%d,%d)\n",
             ourLiveObject->xd, ourLiveObject->yd, clickDestX, clickDestY);
  ...
  if (ourLiveObject->holdingID >= 0 && p.hitOtherPerson &&
    ...
    if (targetPlayer->holdingID > 0) {
      ...
      TransRecord *r =
          getTrans(ourLiveObject->holdingID, targetPlayer->holdingID);
  ...
  if (p.hitOtherPerson && !modClick && destID == 0 &&
    ...
    for (int i = 0; i < gameObjects.size(); i++) {
      ...
      if (o->id != ourID && o->heldByAdultID == -1) {
        ...
        if (distance(targetPos, o->currentPos) < 1) {
          ...
          if (!ourLiveObject->inMotion &&
              (isGridAdjacent(clickDestX, clickDestY, ourLiveObject->xd,
                              ourLiveObject->yd) ||
            ...
            nextActionMessageToSend = autoSprintf(
                "UBABY %d %d %d %d#", sendX(clickDestX), sendY(clickDestY),
                p.hitClothingIndex, p.hitOtherPersonID);
  ...
  if (destID == 0 && p.hit && p.hitOtherPerson && !p.hitAnObject && !modClick &&
      ourLiveObject->holdingID == 0 &&
  ...
  if (!killMode && destID == 0 && !modClick && !tryingToPickUpBaby &&
      !useOnBabyLater &&
      !(clickDestX == ourLiveObject->xd && clickDestY == ourLiveObject->yd)) {
    ...
    mustMove = true;
              ...
              // up
              (ourLiveObject->holdingID != 0 ||
               (destObjInClickedTile > 0 && !destObjInClickedTilePermanent))) ||
             killMode || tryingToPickUpBaby || useOnBabyLater || destID != 0 ||
    ...
    if (isGridAdjacent(clickDestX, clickDestY, ourLiveObject->xd,
                       ourLiveObject->yd) ||
        (clickDestX == ourLiveObject->xd && clickDestY == ourLiveObject->yd)) {
    ...
    if (!canExecute) {
      ...
      if (sideAccess) {
        ...
        nLimit = 3;
      } else if (destID > 0 && ourLiveObject->holdingID == 0 &&
      ...
      if (!foundEmpty && !sideAccess && nStart > 0 &&
          (destID == 0 || !getObject(destID)->blocksWalking)) {
    ...
    if (canExecute && !killMode) {
      ...
      if (tryingToPickUpBaby) {
        ...
        action = "UBABY";
        delete[] extra;
        extra = autoSprintf(" %d %d", p.hitClothingIndex, p.hitOtherPersonID);
        send = true;
        ...
        if (ourLiveObject->holdingID > 0) {
          ...
          if (held->foodValue == 0 && destObjInClickedTile == 0) {
          ...
          if (!foundAlt && floorDestID > 0) {
            ...
            TransRecord *r = getTrans(ourLiveObject->holdingID, floorDestID);
            ...
            if (r != NULL) {
              ...
              action = "USE";
                 ...
                 (modClick || (getObject(destID)->permanent &&
      ...
      } else if (modClick && ourLiveObject->holdingID != 0 && destID != 0 &&
                 getNumContainerSlots(destID) > 0 &&
      ...
      if (strcmp(action, "USE") == 0 && destID > 0) {
      ...
      if (send) {
        ...
        nextActionMessageToSend = autoSprintf(
            "%s %d %d%s#", action, sendX(clickDestX), sendY(clickDestY), extra);
        ...
        if (mapY >= 0 && mapY < mMapD && mapX >= 0 && mapX < mMapD) {
          ...
          if (mMapMoveSpeeds[mapI] > 0) {
            ...
            trueClickDestX = lrint(clickDestX + mMapMoveOffsets[mapI].x);
            trueClickDestY = lrint(clickDestY + mMapMoveOffsets[mapI].y);
          }
        }
  ...
  if (mustMove) {
    ...
    if (ourLiveObject->pathToDest == NULL) {
      ...
      if (ourLiveObject->pathToDest == NULL && ourLiveObject->useWaypoint) {
      ...
      if (ourLiveObject->pathToDest == NULL) {
        ...
        doublePair dir = normalize(sub(dest, ourLiveObject->currentPos));
        ...
        ourLiveObject->currentMoveDirection =
            normalize(sub(gridToDouble(ourLiveObject->pathToDest[1]),
    ...
    // start is absolute
    char *startString =
        autoSprintf(" %d %d @%d", sendX(ourLiveObject->pathToDest[0].x),
    ...
    for (int i = 1; i < ourLiveObject->pathLength; i++) {
      ...
      char *stepString = autoSprintf(
          " %d %d",
          ourLiveObject->pathToDest[i].x - ourLiveObject->pathToDest[0].x,
          ourLiveObject->pathToDest[i].y - ourLiveObject->pathToDest[0].y);
    ...
    double floorSpeedMod =
        computePathSpeedMod(ourLiveObject, ourLiveObject->pathLength);

    ourLiveObject->moveTotalTime =
        measurePathLength(ourLiveObject, ourLiveObject->pathLength) /
        (ourLiveObject->lastSpeed * floorSpeedMod);

    ourLiveObject->moveEtaTime =
        game_getCurrentTime() + ourLiveObject->moveTotalTime;
...
void LivingLifePage::pointerUp(float inX, float inY) {
  ...
  if (mouseDown && getOurLiveObject()->inMotion &&
      mouseDownFrames > minMouseDownFrames / frameRateFactor) {
    ...
    if (o->pathToDest != NULL && o->currentPathStep < o->pathLength - 2) {
...
void LivingLifePage::keyDown(unsigned char inASCII) {
  ...
  */
  case 'V':
    if (!mSayField.isFocused() && serverSocketConnected &&
  ...
  case 'I':
    ...
    break;
  case 'N':
    if (!mSayField.isFocused() && serverSocketConnected && vogMode) {
  ...
  case 'M':
    ...
    break;
  case 'S':
    if (savingSpeechEnabled && !mSayField.isFocused()) {
  ...
  */
  ...
  case 13: // enter
    ...
    if (!TextField::isAnyFocused()) {
      ...
      SimpleVector<char *> *tokens = tokenizeString(typedText);
      ...
      if (strcmp(typedText, "") == 0) {
        ...
        if (strlen(typedText) > 0 && typedText[0] == '/') {
          ...
          if (strstr(typedText, filterCommand) == typedText) {
            ...
            if (emotIndex != -1) {
              char *message = autoSprintf("EMOT 0 0 %d#", emotIndex);

              sendToServerSocket(message);
              delete[] message;
            } else if (strstr(typedText, translate("dieCommand")) ==
                           typedText &&
              ...
              sendToServerSocket(message);
              delete[] message;
            } else if (strstr(typedText, translate("fpsCommand")) ==
                       typedText) {
              showFPS = !showFPS;
            ...
            } else if (strstr(typedText, translate("netCommand")) ==
                       typedText) {
              showNet = !showNet;
              ...
              bytesInCount = 0;
              bytesOutCount = 0;
            } else if (strstr(typedText, translate("pingCommand")) ==
                       typedText) {
            ...
            } else if (strstr(typedText, translate("disconnectCommand")) ==
                       typedText) {
              forceDisconnect = true;
            } else {
              // filter hints
              char *filterString = &(typedText[strlen(filterCommand)]);
              ...
              char *trimmedFilterString = trimWhitespace(filterString);

              int filterStringLen = strlen(trimmedFilterString);

              if (filterStringLen > 0) {
                // not blank
                mHintFilterString = stringDuplicate(trimmedFilterString);
              }
          ...
          char *message = autoSprintf("%s 0 0 %s#", sayCommand, typedText);
          sendToServerSocket(message);
          delete[] message;
        }

        for (int i = 0; i < mSentChatPhrases.size(); i++) {
          if (strcmp(typedText, mSentChatPhrases.getElementDirect(i)) == 0) {
...
void LivingLifePage::specialKeyDown(int inKeyCode) {
  ...
  } else if (inKeyCode == MG_KEY_UP || inKeyCode == MG_KEY_DOWN) {
    if (!mSayField.isFocused() && inKeyCode == MG_KEY_UP) {
      if (mSentChatPhrases.size() > 0) {
        mSayField.setText(
            mSentChatPhrases.getElementDirect(mSentChatPhrases.size() - 1));
      } else {
        mSayField.setText("");
      }
      mSayField.focus();
      ...
      if (strcmp(curText, "") != 0) {
        for (int i = mSentChatPhrases.size() - 1; i >= 0; i--) {
          if (strcmp(curText, mSentChatPhrases.getElementDirect(i)) == 0) {
            curBufferIndex = i;
            break;
          }
        }
      ...
      if (newIndex >= 0) {
        ...
        for (int i = 0; i < mCurrentNoteChars.size(); i++) {
          ...
          doublePair pos = mCurrentNoteCharOffsets.getElementDirect(i);

          for (int j = 0; j < mErasedNoteChars.size(); j++) {
            if (mErasedNoteChars.getElementDirect(j) == c &&
                equal(mErasedNoteCharOffsets.getElementDirect(j), pos)) {
        ...
        for (int i = 0; i < mCurrentNoteChars.size(); i++) {
          mErasedNoteChars.push_back(mCurrentNoteChars.getElementDirect(i));
        ...
        if (newIndex >= mSentChatPhrases.size()) {
          ...
          mSayField.setText(mSentChatPhrases.getElementDirect(newIndex));
        }
      }
    }
  }
...
void LivingLifePage::actionPerformed(GUIComponent *inTarget) {
  if (vogMode && inTarget == &mObjectPicker) {
    ...
    if (objectID != -1) {
      char *message = autoSprintf("VOGI %d %d %d#", lrint(vogPos.x),
...
void LivingLifePage::pushOldHintArrow(int inIndex) {
  int i = inIndex;
  if (mCurrentHintTargetPointerFade[i] > 0) {
    OldHintArrow h = {mLastHintTargetPos[i], mCurrentHintTargetPointerBounce[i],
...
static void prependLeadershipTag(LiveObject *inPlayer, const char *inPrefix) {
  ...
  if (o->leadershipNameTag != NULL) {
    ...
    newTag = autoSprintf("%s %s", inPrefix, o->leadershipNameTag);
...
void LivingLifePage::updateLeadership() {
  ...
  for (int i = 0; i < gameObjects.size(); i++) {
    ...
    for (int e = 0; e < o->exiledByIDs.size(); e++) {
      ...
      if (eID == ourID || ourLeadershipChain.getElementIndex(eID) != -1) {
  ...
  for (int i = 0; i < ourLeadershipChain.size(); i++) {
    ...
    LiveObject *l = getGameObject(ourLeadershipChain.getElementDirect(i));
  ...
  if (ourLiveObject->highestLeaderID != -1) {
    for (int i = 0; i < gameObjects.size(); i++) {
      ...
      if (o != ourLiveObject &&
          ...
          !o->isExiled && !o->followingUs && o->leadershipNameTag == NULL) {

        o->leadershipNameTag =
            autoSprintf("%s %s", translate("your"), translate("ally"));
      }
    }
  }
}
