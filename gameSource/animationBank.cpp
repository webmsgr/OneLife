...
static const char *animTypeNames[9] = {"ground",      "held",   "moving",
                                       "ground2",     "eating", "doing",
...
int initAnimationBankStart(char *outRebuildingCache) {
  ...
  if (drawMouthShapes) {
    ...
    if (mouthShapeFolder.exists() && mouthShapeFolder.isDirectory()) {
      ...
      if (numMouthShapes > 0) {
        ...
        if (mouthFrameFile.exists() && !mouthFrameFile.isDirectory()) {

          char *cont = mouthFrameFile.readFileContents();

          if (cont != NULL) {
            SimpleVector<char *> *tokens = tokenizeString(cont);
            ...
            for (int i = 0; i < numMouthShapeFrames; i++) {
              int index = -1;
              sscanf(tokens->getElementDirect(i), "%d", &index);

              if (index >= 0 && index < numMouthShapes) {
  ...
  cache = initFolderCache("animations", outRebuildingCache, shouldFileBeCached);
  ...
  if (shouldFileBeCached(txtFileName)) {
    ...
    if (animText != NULL) {
      ...
      if (numLines > 4) {
        ...
        r->objectID = 0;
        sscanf(lines[next], "id=%d", &(r->objectID));
        ...
        if (strchr(lines[next], ':') != NULL) {
          ...
          sscanf(lines[next], "type=%d:%d,randStartPhase=%d", &(typeRead),
                 &extraIndexRead, &randomStartPhaseRead);
        } else {
          sscanf(lines[next], "type=%d,randStartPhase=%d", &(typeRead),
        ...
        if (strstr(lines[next], "forceZeroStart=") != NULL) {
          ...
          sscanf(lines[next], "forceZeroStart=%d", &forceZeroStartRead);
        ...
        if (strstr(lines[next], "numSounds=") != NULL) {
          r->numSounds = 0;
          sscanf(lines[next], "numSounds=%d", &(r->numSounds));
          next++;
        }
        ...
        if (r->numSounds > 0) {
          ...
          for (int j = 0; j < r->numSounds && next < numLines; j++) {
            ...
            if (end != NULL) {
              ...
              int footstepValue = 0;
              sscanf(&(end[1]), "%lf %lf %lf %lf %d",
                     &(r->soundAnim[j].repeatPerSec),
                     &(r->soundAnim[j].repeatPhase),
                     &(r->soundAnim[j].ageStart), &(r->soundAnim[j].ageEnd),
                     &footstepValue);
        ...
        r->numSprites = 0;
        sscanf(lines[next], "numSprites=%d", &(r->numSprites));
        next++;

        r->numSlots = 0;
        sscanf(lines[next], "numSlots=%d", &(r->numSlots));
        next++;

        r->spriteAnim = new SpriteAnimationRecord[r->numSprites];

        r->slotAnim = new SpriteAnimationRecord[r->numSlots];

        for (int j = 0; j < r->numSprites && next < numLines; j++) {
          ...
          if (strstr(lines[next], "offset") != NULL) {
            sscanf(lines[next], "offset=(%lf,%lf)",
                   &(r->spriteAnim[j].offset.x), &(r->spriteAnim[j].offset.y));
            next++;
            if (next >= numLines) {
              break;
            }
          ...
          if (strstr(lines[next], "startPause") != NULL) {
            sscanf(lines[next], "startPause=%lf",
          ...
          sscanf(lines[next],
                 ...
                 "%lf %lf %lf %lf %lf "
                 "%lf %lf %lf %lf %lf",
                 &(r->spriteAnim[j].xOscPerSec), &(r->spriteAnim[j].xAmp),
                 &(r->spriteAnim[j].xPhase),

                 &(r->spriteAnim[j].yOscPerSec), &(r->spriteAnim[j].yAmp),
                 &(r->spriteAnim[j].yPhase),

                 &(r->spriteAnim[j].rotationCenterOffset.x),
                 &(r->spriteAnim[j].rotationCenterOffset.y),

                 &(r->spriteAnim[j].rotPerSec), &(r->spriteAnim[j].rotPhase),

                 &(r->spriteAnim[j].rockOscPerSec), &(r->spriteAnim[j].rockAmp),
                 &(r->spriteAnim[j].rockPhase),

                 &(r->spriteAnim[j].durationSec), &(r->spriteAnim[j].pauseSec),

                 &(r->spriteAnim[j].fadeOscPerSec),
                 &(r->spriteAnim[j].fadeHardness), &(r->spriteAnim[j].fadeMin),
                 &(r->spriteAnim[j].fadeMax), &(r->spriteAnim[j].fadePhase));

          next++;
        }

        for (int j = 0; j < r->numSlots && next < numLines; j++) {
          ...
          if (strstr(lines[next], "offset") != NULL) {
            sscanf(lines[next], "offset=(%lf,%lf)", &(r->slotAnim[j].offset.x),
          ...
          sscanf(lines[next],
                 ...
                 &(r->slotAnim[j].xOscPerSec), &(r->slotAnim[j].xAmp),
                 &(r->slotAnim[j].xPhase),

                 &(r->slotAnim[j].yOscPerSec), &(r->slotAnim[j].yAmp),
                 &(r->slotAnim[j].yPhase),

                 &(r->slotAnim[j].durationSec), &(r->slotAnim[j].pauseSec));
          next++;
        }
...
void setAnimLayerCutoff(int inCutoff) { animLayerCutoff = inCutoff; }
...
void setExtraIndex(int inIndex) { extraIndex = inIndex; }

void setExtraIndexB(int inIndex) { extraIndexB = inIndex; }
...
static File *getFile(int inID, AnimType inType) {
  ...
  if (animationsDir.exists() && animationsDir.isDirectory()) {
    ...
    if (inType > endAnimType) {
      fileName = autoSprintf("%d_%dx%d.txt", inID, extra, extraIndex);
    } else {
  ...
  if (inID < mapSize) {
    if (inType < endAnimType && inType != ground2) {
      ...
      for (int i = 0; i < numExtra; i++) {
        AnimationRecord *r = idExtraMap[inID].getElementDirect(i);
...
void addAnimation(AnimationRecord *inRecord, char inNoWriteToFile) {
  ...
  AnimationRecord *oldRecord = getAnimation(inRecord->objectID, inRecord->type);
  ...
  clearAnimation(inRecord->objectID, inRecord->type);
  ...
  if (!inNoWriteToFile) {
    ...
    if (animationFile != NULL) {
      ...
      if (inRecord->type < endAnimType) {
        lines.push_back(autoSprintf("type=%d,randStartPhase=%d", inRecord->type,
                                    inRecord->randomStartPhase));
      } else {
        lines.push_back(autoSprintf("type=%d:%d,randStartPhase=%d", extra,
                                    extraIndex, inRecord->randomStartPhase));
      }

      lines.push_back(
          autoSprintf("forceZeroStart=%d", inRecord->forceZeroStart));

      lines.push_back(autoSprintf("numSounds=%d", inRecord->numSounds));

      for (int j = 0; j < inRecord->numSounds; j++) {
        lines.push_back(autoSprintf(
            "soundParam=%s %lf %lf %lf %lf %d",
            printSoundUsage(inRecord->soundAnim[j].sound),
            inRecord->soundAnim[j].repeatPerSec,
            inRecord->soundAnim[j].repeatPhase, inRecord->soundAnim[j].ageStart,
            inRecord->soundAnim[j].ageEnd, inRecord->soundAnim[j].footstep));
      }

      lines.push_back(autoSprintf("numSprites=%d", inRecord->numSprites));

      lines.push_back(autoSprintf("numSlots=%d", inRecord->numSlots));

      for (int j = 0; j < inRecord->numSprites; j++) {
        lines.push_back(autoSprintf("offset=(%lf,%lf)",
                                    inRecord->spriteAnim[j].offset.x,
                                    inRecord->spriteAnim[j].offset.y));

        lines.push_back(autoSprintf("startPause=%lf",
        ...
        lines.push_back(autoSprintf(
            ...
            "%lf %lf %lf %lf %lf "
            "%lf %lf %lf %lf %lf",
            inRecord->spriteAnim[j].xOscPerSec, inRecord->spriteAnim[j].xAmp,
            inRecord->spriteAnim[j].xPhase,

            inRecord->spriteAnim[j].yOscPerSec, inRecord->spriteAnim[j].yAmp,
            inRecord->spriteAnim[j].yPhase,

            inRecord->spriteAnim[j].rotationCenterOffset.x,
            inRecord->spriteAnim[j].rotationCenterOffset.y,

            inRecord->spriteAnim[j].rotPerSec, inRecord->spriteAnim[j].rotPhase,

            inRecord->spriteAnim[j].rockOscPerSec,
            inRecord->spriteAnim[j].rockAmp, inRecord->spriteAnim[j].rockPhase,
            ...
            inRecord->spriteAnim[j].fadeMin, inRecord->spriteAnim[j].fadeMax,
      ...
      for (int j = 0; j < inRecord->numSlots; j++) {
        ...
        lines.push_back(
            autoSprintf("startPause=%lf", inRecord->slotAnim[j].startPauseSec));

        lines.push_back(autoSprintf(
            "animParam="
            "%lf %lf %lf %lf %lf %lf %lf %lf",
            inRecord->slotAnim[j].xOscPerSec, inRecord->slotAnim[j].xAmp,
            inRecord->slotAnim[j].xPhase,

            inRecord->slotAnim[j].yOscPerSec, inRecord->slotAnim[j].yAmp,
            inRecord->slotAnim[j].yPhase,

            inRecord->slotAnim[j].durationSec, inRecord->slotAnim[j].pauseSec));
      }
...
static double getOscOffset(double inFrameTime, double inOffset,
                           double inOscPerSec, double inAmp, double inPhase) {
  return inOffset +
...
char isAnimFadeNeeded(int inObjectID, AnimationRecord *inCurR,
                      AnimationRecord *inTargetR) {
  ...
  for (int i = 0; i < obj->numSprites; i++) {
    ...
    if (inCurR->spriteAnim[i].offset.x != 0)
      return true;
    if (inCurR->spriteAnim[i].offset.y != 0)
      return true;

    // if current is moving at all, must fade

    if (inCurR->spriteAnim[i].xOscPerSec > 0)
      return true;

    if (inCurR->spriteAnim[i].xAmp != 0 && inCurR->spriteAnim[i].xPhase != 0 &&
        inCurR->spriteAnim[i].xPhase != 0.5)
      return true;

    if (inCurR->spriteAnim[i].yOscPerSec > 0)
      return true;

    if (inCurR->spriteAnim[i].yAmp != 0 && inCurR->spriteAnim[i].yPhase != 0 &&
        inCurR->spriteAnim[i].yPhase != 0.5)
      return true;

    if (inCurR->spriteAnim[i].rockOscPerSec > 0)
      return true;

    if (inCurR->spriteAnim[i].rockAmp != 0 &&
        inCurR->spriteAnim[i].rockPhase != 0 &&
        inCurR->spriteAnim[i].rockPhase != 0.5)
      return true;

    if (inCurR->spriteAnim[i].rotPerSec != 0)
      return true;

    if (inCurR->spriteAnim[i].rotPhase != 0)
      return true;

    if (inCurR->spriteAnim[i].fadeOscPerSec > 0)
      return true;
  ...
  for (int i = 0; i < obj->numSlots; i++) {
    // offset at all, must fade
    if (inCurR->slotAnim[i].offset.x != 0)
      return true;
    if (inCurR->slotAnim[i].offset.y != 0)
      return true;

    // if current is moving at all, must fade

    if (inCurR->slotAnim[i].xOscPerSec > 0)
      return true;

    if (inCurR->slotAnim[i].xAmp != 0 && inCurR->slotAnim[i].xPhase != 0 &&
        inCurR->slotAnim[i].xPhase != 0.5)
      return true;

    if (inCurR->slotAnim[i].yOscPerSec > 0)
      return true;

    if (inCurR->slotAnim[i].yAmp != 0 && inCurR->slotAnim[i].yPhase != 0 &&
        inCurR->slotAnim[i].yPhase != 0.5)
      return true;
    ...
    if (inTargetR->slotAnim[i].xAmp != 0 &&
        ...
        inTargetR->slotAnim[i].xPhase != 0.5)
      return true;

    if (inTargetR->slotAnim[i].yAmp != 0 &&
        inTargetR->slotAnim[i].yPhase != 0 &&
        inTargetR->slotAnim[i].yPhase != 0.5)
      return true;
  }
...
char isAnimEmpty(int inObjectID, AnimType inType) {
  ...
  for (int i = 0; i < obj->numSprites; i++) {
    ...
    if (r->spriteAnim[i].offset.x != 0)
      return false;
    if (r->spriteAnim[i].offset.y != 0)
      return false;

    if (r->spriteAnim[i].xOscPerSec > 0)
      return false;

    if (r->spriteAnim[i].xAmp != 0 && r->spriteAnim[i].xPhase != 0 &&
        r->spriteAnim[i].xPhase != 0.5)
      return false;

    if (r->spriteAnim[i].yOscPerSec > 0)
      return false;

    if (r->spriteAnim[i].yAmp != 0 && r->spriteAnim[i].yPhase != 0 &&
        r->spriteAnim[i].yPhase != 0.5)
      return false;

    if (r->spriteAnim[i].rockOscPerSec > 0)
      return false;

    if (r->spriteAnim[i].rockAmp != 0 && r->spriteAnim[i].rockPhase != 0 &&
        r->spriteAnim[i].rockPhase != 0.5)
      return false;

    if (r->spriteAnim[i].rotPerSec != 0)
      return false;

    if (r->spriteAnim[i].rotPhase != 0)
      return false;
  }

  for (int i = 0; i < obj->numSlots; i++) {

    if (r->slotAnim[i].offset.x != 0)
      return false;
    if (r->slotAnim[i].offset.y != 0)
      return false;

    if (r->slotAnim[i].xOscPerSec > 0)
      return false;

    if (r->slotAnim[i].xAmp != 0 && r->slotAnim[i].xPhase != 0 &&
        r->slotAnim[i].xPhase != 0.5)
      return false;

    if (r->slotAnim[i].yOscPerSec > 0)
      return false;

    if (r->slotAnim[i].yAmp != 0 && r->slotAnim[i].yPhase != 0 &&
        r->slotAnim[i].yPhase != 0.5)
      return false;
  }
...
static char logicalXOR(char inA, char inB) { return !inA != !inB; }
...
void hidePersonShadows(char inHide) { shouldHidePersonShadows = inHide; }

ObjectAnimPack drawObjectAnimPacked(
    int inObjectID, AnimType inType, double inFrameTime, double inAnimFade,
    AnimType inFadeTargetType, double inFadeTargetFrameTime,
    double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed,
    // set endAnimType for no frozen arms
    AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType,
    doublePair inPos, double inRot, char inWorn, char inFlipH, double inAge,
    int inHideClosestArm, char inHideAllLimbs, char inHeldNotInPlaceYet,
    ClothingSet inClothing, SimpleVector<int> *inClothingContained,
    int inNumContained, int *inContainedIDs,
    SimpleVector<int> *inSubContained) {

  ObjectAnimPack outPack = {inObjectID,
                            inType,
                            inFrameTime,
                            inAnimFade,
                            inFadeTargetType,
...
void drawObjectAnim(ObjectAnimPack inPack) {
  ...
  if (inPack.inContainedIDs == NULL) {
    p = drawObjectAnim(
        inPack.inObjectID, 2, inPack.inType, inPack.inFrameTime,
        inPack.inAnimFade, inPack.inFadeTargetType,
        inPack.inFadeTargetFrameTime, inPack.inFrozenRotFrameTime,
        inPack.outFrozenRotFrameTimeUsed, inPack.inFrozenArmType,
        inPack.inFrozenArmFadeTargetType, inPack.inPos, inPack.inRot,
        inPack.inWorn, inPack.inFlipH, inPack.inAge, inPack.inHideClosestArm,
        inPack.inHideAllLimbs, inPack.inHeldNotInPlaceYet, inPack.inClothing,
        inPack.inClothingContained);
  } else {
    drawObjectAnim(inPack.inObjectID, inPack.inType, inPack.inFrameTime,
                   inPack.inAnimFade, inPack.inFadeTargetType,
                   inPack.inFadeTargetFrameTime, inPack.inFrozenRotFrameTime,
                   inPack.outFrozenRotFrameTimeUsed, inPack.inFrozenArmType,
                   inPack.inFrozenArmFadeTargetType, inPack.inPos, inPack.inRot,
                   inPack.inWorn, inPack.inFlipH, inPack.inAge,
                   inPack.inHideClosestArm, inPack.inHideAllLimbs,
                   inPack.inHeldNotInPlaceYet, inPack.inClothing,
                   inPack.inClothingContained, inPack.inNumContained,
                   inPack.inContainedIDs, inPack.inSubContained);
  }
  ...
  if (inPack.additionalHeldID > 0 && p.valid) {
    ...
    computeHeldDrawPos(p, inPack.inPos, heldObject, inPack.inFlipH, &holdPos,
                       &holdRot);

    drawObjectAnim(inPack.additionalHeldID, 2, inPack.inType,
                   inPack.inFrameTime, inPack.inAnimFade,
                   inPack.inFadeTargetType, inPack.inFadeTargetFrameTime,
                   inPack.inFrozenRotFrameTime,
                   inPack.outFrozenRotFrameTimeUsed, endAnimType, endAnimType,
                   holdPos, holdRot, false, inPack.inFlipH, -1, 0, false, false,
                   emptyClothing, NULL);
  }
}

HoldingPos drawObjectAnim(
    int inObjectID, int inDrawBehindSlots, AnimType inType, double inFrameTime,
    double inAnimFade, AnimType inFadeTargetType, double inFadeTargetFrameTime,
    double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed,
    AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType,
    doublePair inPos, double inRot, char inWorn, char inFlipH, double inAge,
    int inHideClosestArm, char inHideAllLimbs, char inHeldNotInPlaceYet,
    ClothingSet inClothing, SimpleVector<int> *inClothingContained,
    double *outSlotRots, doublePair *outSlotOffsets) {
  ...
  if (r == NULL) {
    ...
    animLayerCutoff = -1;
    return drawObject(getObject(inObjectID), inDrawBehindSlots, inPos, inRot,
                      inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
    ...
    AnimationRecord *rArmFade =
        getAnimation(inObjectID, inFrozenArmFadeTargetType);
    ...
    return drawObjectAnim(
        inObjectID, inDrawBehindSlots, r, inFrameTime, inAnimFade, rB,
        inFadeTargetFrameTime, inFrozenRotFrameTime, outFrozenRotFrameTimeUsed,
        rF, rArm, rArmFade, inPos, inRot, inWorn, inFlipH, inAge,
        inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet, inClothing,
        inClothingContained, outSlotRots, outSlotOffsets);
  }
}

void drawObjectAnimHighlighted(
    float inHighlightFade, int inObjectID, AnimType inType, double inFrameTime,
    double inAnimFade, AnimType inFadeTargetType, double inFadeTargetFrameTime,
    double inFrozenRotFrameTime, char *outFrozenRotFrameTimeUsed,
    AnimType inFrozenArmType, AnimType inFrozenArmFadeTargetType,
    doublePair inPos, double inRot, char inWorn, char inFlipH, double inAge,
  ...
  drawObjectAnim(
      inObjectID, inType, inFrameTime, inAnimFade, inFadeTargetType,
      inFadeTargetFrameTime, inFrozenRotFrameTime, outFrozenRotFrameTimeUsed,
      inFrozenArmType, inFrozenArmFadeTargetType, inPos, inRot, inWorn, inFlipH,
      inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet, inClothing,
      inClothingContained, inNumContained, inContainedIDs, inSubContained);
  ...
  if (inHighlightFade > 0) {
    ...
    for (int i = startPass; i < numPasses; i++) {
      ...
      drawObjectAnim(inObjectID, inType, inFrameTime, inAnimFade,
                     inFadeTargetType, inFadeTargetFrameTime,
                     inFrozenRotFrameTime, outFrozenRotFrameTimeUsed,
                     inFrozenArmType, inFrozenArmFadeTargetType, inPos, inRot,
                     inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
                     inHeldNotInPlaceYet, inClothing, inClothingContained,
                     inNumContained, inContainedIDs, inSubContained);
...
void unfixWatchedObjectDrawPos() { useFixedWatchedDrawPos = false; }
...
HoldingPos drawObjectAnim(
    int inObjectID, int inDrawBehindSlots, AnimationRecord *inAnim,
    double inFrameTime, double inAnimFade, AnimationRecord *inFadeTargetAnim,
    double inFadeTargetFrameTime, double inFrozenRotFrameTime,
    char *outFrozenRotFrameTimeUsed, AnimationRecord *inFrozenRotAnim,
    AnimationRecord *inFrozenArmAnim,
    AnimationRecord *inFrozenArmFadeTargetAnim, doublePair inPos, double inRot,
    char inWorn, char inFlipH, double inAge, int inHideClosestArm,
    char inHideAllLimbs, char inHeldNotInPlaceYet, ClothingSet inClothing,
    SimpleVector<int> *inClothingContained, double *outSlotRots,
  ...
  *outFrozenRotFrameTimeUsed = false;
  ...
  if (obj->numSprites > MAX_WORKING_SPRITES) {
    ...
    drawObject(obj, inDrawBehindSlots, inPos, inRot, inWorn, inFlipH, inAge,
  ...
  for (int i = 0; i < obj->numSprites; i++) {
    ...
    spriteFrameTime =
        processFrameTimeWithPauses(spriteAnim, i, true, spriteFrameTime);
    if (inAnimFade < 1) {
      targetSpriteFrameTime = processFrameTimeWithPauses(
          spriteFadeTargetAnim, i, true, targetSpriteFrameTime);
    }
    ...
    if (obj->person && i == headIndex) {
      spritePos = add(spritePos,
                      getAgeHeadOffset(inAge, headPos, bodyPos, frontFootPos));
    ...
    if (i < spriteAnim->numSprites) {
      ...
      double sinVal = getOscOffset(spriteFrameTime, 0,
                                   spriteAnim->spriteAnim[i].fadeOscPerSec, 1.0,
      ...
      if (hardness == 1) {
        ...
        if (absSinVal != 0) {
          hardVersion =
              (sinVal / absSinVal) * pow(absSinVal, 1.0 / (hardness * 10 + 1));
        } else {
          hardVersion = 0;
        }
      }

      double fade = (spriteAnim->spriteAnim[i].fadeMax -
      ...
      spritePos.x +=
          inAnimFade * getOscOffset(spriteFrameTime,
      ...
      double rock =
          inAnimFade * getOscOffset(spriteFrameTime, 0,
      ...
      doublePair rotCenterOffset =
          mult(spriteAnim->spriteAnim[i].rotationCenterOffset, inAnimFade);
      ...
      if (inAnimFade < 1 && i < spriteFadeTargetAnim->numSprites) {
        ...
        double sinValB =
            getOscOffset(targetSpriteFrameTime, 0,
                         spriteFadeTargetAnim->spriteAnim[i].fadeOscPerSec, 1.0,
        ...
        // https://thatsmaths.com/2015/12/31/
        //         squaring-the-circular-functions/
        double hardnessB = spriteFadeTargetAnim->spriteAnim[i].fadeHardness;
        ...
        if (hardnessB == 1) {
          ...
          if (absSinValB != 0) {
            hardVersionB = (sinValB / absSinValB) *
                           pow(absSinValB, 1.0 / (hardnessB * 10 + 1));
          } else {
            hardVersionB = 0;
          }
        }

        double fadeB = (spriteFadeTargetAnim->spriteAnim[i].fadeMax -
        ...
        spritePos.x +=
            targetWeight *
            getOscOffset(targetSpriteFrameTime,
        ...
        rotCenterOffset =
            add(rotCenterOffset,
                mult(spriteFadeTargetAnim->spriteAnim[i].rotationCenterOffset,

                     targetWeight));
      }

      double totalRotOffset =
          spriteAnim->spriteAnim[i].rotPerSec * spriteFrameTime +
          spriteAnim->spriteAnim[i].rotPhase;

      // use frozen rot instead if either current or
      // target satisfies
      if (inAnim->type != moving && spriteAnim->spriteAnim[i].rotPerSec == 0 &&
        ...
        // use frozen instead
        totalRotOffset =
            inFrozenRotAnim->spriteAnim[i].rotPerSec * inFrozenRotFrameTime +
            inFrozenRotAnim->spriteAnim[i].rotPhase;

        *outFrozenRotFrameTimeUsed = *outFrozenRotFrameTimeUsed || true;
      ...
      double relativeRotOffset = totalRotOffset - floor(totalRotOffset);
      ...
      if (inAnimFade < 1 && i < spriteFadeTargetAnim->numSprites) {
        ...
        double aveCenteredOffset =
            inAnimFade * centeredOffset + targetWeight * centeredTargetOffset;
      ...
      if (rotCenterOffset.x != 0 || rotCenterOffset.y != 0) {

        // move spritePos as if it were applying rot based
        // on this offset
        // the rot itself will be applied later

        doublePair newCenter = rotate(rotCenterOffset, -2 * M_PI * rot);
  ...
  for (int i = 0; i < limit; i++) {
    ...
    if (obj->person && !isSpriteVisibleAtAge(obj, i, inAge)) {
      // skip drawing this aging layer entirely
      continue;
    }

    if (obj->person && shouldHidePersonShadows &&
        strstr(getSpriteRecord(obj->sprites[i])->tag, "Shadow") != NULL) {
      continue;
    }
    ...
    while (nextParent != -1) {
      ...
      if (workingDeltaRot[nextParent] != 0) {
        ...
        spritePos =
            add(spritePos, rotate(workingDeltaSpritePos[nextParent], -angle));

        // add in positional change based on arm's length rotation
        // around parent

        doublePair childOffset = sub(spritePos, obj->spritePos[nextParent]);

        doublePair newChildOffset = rotate(childOffset, angle);

        spritePos = add(spritePos, sub(newChildOffset, childOffset));
      } else {
        spritePos = add(spritePos, workingDeltaSpritePos[nextParent]);
      }
    ...
    if (!inHeldNotInPlaceYet && inHideClosestArm == 1 &&
        frontArmIndices.getElementIndex(i) != -1) {
      skipSprite = true;
    } else if (!inHeldNotInPlaceYet && inHideClosestArm == -1 &&
               backArmIndices.getElementIndex(i) != -1) {
      skipSprite = true;
    ...
    if (obj->clothing != 'n' && obj->spriteInvisibleWhenWorn[i] != 0) {

      if (inWorn && obj->spriteInvisibleWhenWorn[i] == 1) {

        // skip invisible layer in worn clothing
        skipSprite = true;
      } else if (!inWorn && obj->spriteInvisibleWhenWorn[i] == 2) {
    ...
    if (inDrawBehindSlots != 2) {
      if (inDrawBehindSlots == 0 && !obj->spriteBehindSlots[i]) {
        skipSprite = true;
      } else if (inDrawBehindSlots == 1 && obj->spriteBehindSlots[i]) {
        skipSprite = true;
      }
    }

    if (!skipSprite && i == backFootIndex && inClothing.backShoe != NULL) {
    ...
    if (i == bodyIndex) {
      ...
      if (drawWithBadge != -1 && (inClothing.tunic != NULL || bareBadge)) {
      ...
      for (int e = 0; e < drawWithEmots.size(); e++)
        if (drawWithEmots.getElementDirect(e)->bodyEmot != 0 && obj->person) {

          char used;
          drawObjectAnim(
              drawWithEmots.getElementDirect(e)->bodyEmot, clothingAnimType,
              inFrameTime, inAnimFade, clothingFadeTargetAnimType,
              inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
              endAnimType, add(animBodyPos, inPos), animBodyRotDelta, true,
              inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
        }
      ...
      if (inClothing.bottom != NULL) {
        ...
        char used;
        drawObjectAnimHighlighted(
            clothingHighlightFades[4], inClothing.bottom->id, clothingAnimType,
            inFrameTime, inAnimFade, clothingFadeTargetAnimType,
            inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
            endAnimType, bottomPos, bottomRot, true, inFlipH, -1, 0, false,
            false, emptyClothing, NULL, numCont, cont, NULL);
      ...
      if (inClothing.tunic != NULL) {
        ...
        char used;
        drawObjectAnimHighlighted(
            clothingHighlightFades[1], inClothing.tunic->id, clothingAnimType,
            inFrameTime, inAnimFade, clothingFadeTargetAnimType,
            inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
            endAnimType, tunicPos, tunicRot, true, inFlipH, -1, 0, false, false,
            emptyClothing, NULL, numCont, cont, NULL);
      ...
      if (inClothing.backpack != NULL) {
        ...
        char used;
        drawObjectAnimHighlighted(
            clothingHighlightFades[5], inClothing.backpack->id,
            clothingAnimType, inFrameTime, inAnimFade,
            clothingFadeTargetAnimType, inFadeTargetFrameTime,
            inFrozenRotFrameTime, &used, endAnimType, endAnimType, backpackPos,
            backpackRot, true, inFlipH, -1, 0, false, false, emptyClothing,
            NULL, numCont, cont, NULL);
      ...
      if (drawWithBadge != -1 && (inClothing.tunic != NULL || bareBadge)) {
        ...
        char used;
        drawObjectAnimHighlighted(
            clothingHighlightFades[1], drawWithBadge, clothingAnimType,
            inFrameTime, inAnimFade, clothingFadeTargetAnimType,
            inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
            endAnimType, badgePos, badgeRot, true, inFlipH, -1, 0, false, false,
            emptyClothing, NULL, 0, NULL, NULL);
        spriteColorOverrideOn = false;
      }
    }

    if (!skipSprite && i == frontFootIndex && inClothing.frontShoe != NULL) {
    ...
    if (!skipSprite) {
      ...
      char multiplicative = getUsesMultiplicativeBlending(obj->sprites[i]);

      if (multiplicative) {
        toggleMultiplicativeBlend(true);

        if (workingSpriteFade[i] < 1 || getTotalGlobalFade() < 1) {
      ...
      if (drawMouthShapes && spriteID == mouthAnchorID &&
        ...
        drawSprite(mouthShapeFrameList[mouthShapeFrame], pos, 1.0, rot,
      ...
      if (i == backHandIndex && inHideClosestArm == 0 && !inHideAllLimbs) {
      ...
      for (int e = 0; e < drawWithEmots.size(); e++)
        if (drawWithEmots.getElementDirect(e)->eyeEmot != 0) {
          ...
          drawObjectAnim(drawWithEmots.getElementDirect(e)->eyeEmot,
                         clothingAnimType, inFrameTime, inAnimFade,
                         clothingFadeTargetAnimType, inFadeTargetFrameTime,
                         inFrozenRotFrameTime, &used, endAnimType, endAnimType,
                         cPos, animHeadRotDelta, true, inFlipH, -1, 0, false,
                         false, emptyClothing, NULL, 0, NULL, NULL);
        }
    ...
    if (((eyesIndex != -1 && i == eyesIndex) ||
      ...
      for (int e = 0; e < drawWithEmots.size(); e++)
        if (drawWithEmots.getElementDirect(e)->faceEmot != 0) {
          ...
          char used;
          drawObjectAnim(
              drawWithEmots.getElementDirect(e)->faceEmot, clothingAnimType,
              inFrameTime, inAnimFade, clothingFadeTargetAnimType,
              inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
              endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
              inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
        }
    ...
    if (i == headIndex && mouthIndex != -1)
      for (int e = 0; e < drawWithEmots.size(); e++)
        if (drawWithEmots.getElementDirect(e)->mouthEmot != 0) {
          ...
          char used;
          drawObjectAnim(
              drawWithEmots.getElementDirect(e)->mouthEmot, clothingAnimType,
              inFrameTime, inAnimFade, clothingFadeTargetAnimType,
              inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
              endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
              inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
    ...
    if (i == headIndex && obj->person)
      for (int e = 0; e < drawWithEmots.size(); e++)
        if (drawWithEmots.getElementDirect(e)->otherEmot != 0) {
          ...
          char used;
          drawObjectAnim(
              drawWithEmots.getElementDirect(e)->otherEmot, clothingAnimType,
              inFrameTime, inAnimFade, clothingFadeTargetAnimType,
              inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
              endAnimType, add(animHeadPos, inPos), animHeadRotDelta, true,
              inFlipH, -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
    ...
    if (!skipSprite && inClothing.backShoe != NULL && i == backFootIndex) {
      ...
      char used;
      drawObjectAnimHighlighted(
          clothingHighlightFades[3], inClothing.backShoe->id, clothingAnimType,
          inFrameTime, inAnimFade, clothingFadeTargetAnimType,
          inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
          endAnimType, backShoePos, backShoeRot, true, inFlipH, -1, 0, false,
          false, emptyClothing, NULL, numCont, cont, NULL);

      if (cont != NULL) {
        delete[] cont;
      }
    } else if (!skipSprite && inClothing.frontShoe != NULL &&
               i == frontFootIndex) {
      int numCont = 0;
      int *cont = NULL;
      if (inClothingContained != NULL) {
      ...
      char used;
      drawObjectAnimHighlighted(
          clothingHighlightFades[2], inClothing.frontShoe->id, clothingAnimType,
          inFrameTime, inAnimFade, clothingFadeTargetAnimType,
          inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
          endAnimType, frontShoePos, frontShoeRot, true, inFlipH, -1, 0, false,
          false, emptyClothing, NULL, numCont, cont, NULL);
  ...
  if (inClothing.hat != NULL) {
    ...
    char used;
    drawObjectAnimHighlighted(
        clothingHighlightFades[0], inClothing.hat->id, clothingAnimType,
        inFrameTime, inAnimFade, clothingFadeTargetAnimType,
        inFadeTargetFrameTime, inFrozenRotFrameTime, &used, endAnimType,
        endAnimType, cPos, animHeadRotDelta, true, inFlipH, -1, 0, false, false,
        emptyClothing, NULL, numCont, cont, NULL);
    ...
    for (int e = 0; e < drawWithEmots.size(); e++)
      if (drawWithEmots.getElementDirect(e)->headEmot != 0) {
        ...
        drawObjectAnim(drawWithEmots.getElementDirect(e)->headEmot,
                       ...
                       clothingFadeTargetAnimType, inFadeTargetFrameTime,
                       inFrozenRotFrameTime, &used, endAnimType, endAnimType,
                       add(animHeadPos, inPos), animHeadRotDelta, true, inFlipH,
                       -1, 0, false, false, emptyClothing, NULL, 0, NULL, NULL);
      }
    ...
    for (int i = 0; i < obj->numSlots; i++) {
      ...
      while (nextParent != -1) {
        ...
        if (workingDeltaRot[nextParent] != 0) {
          ...
          slotPos =
              add(slotPos, rotate(workingDeltaSpritePos[nextParent], -angle));

          // add in positional change based on arm's length rotation
          // around parent

          doublePair childOffset = sub(slotPos, obj->spritePos[nextParent]);

          doublePair newChildOffset = rotate(childOffset, angle);

          slotPos = add(slotPos, sub(newChildOffset, childOffset));
        } else {
          slotPos = add(slotPos, workingDeltaSpritePos[nextParent]);
        }
...
void drawObjectAnim(int inObjectID, AnimType inType, double inFrameTime,
                    double inAnimFade, AnimType inFadeTargetType,
                    double inFadeTargetFrameTime, double inFrozenRotFrameTime,
                    char *outFrozenRotFrameTimeUsed, AnimType inFrozenArmType,
                    AnimType inFrozenArmFadeTargetType, doublePair inPos,
                    double inRot, char inWorn, char inFlipH, double inAge,
                    int inHideClosestArm, char inHideAllLimbs,
                    char inHeldNotInPlaceYet, ClothingSet inClothing,
                    SimpleVector<int> *inClothingContained, int inNumContained,
                    int *inContainedIDs, SimpleVector<int> *inSubContained) {
  ...
  if (r == NULL) {
    ...
    drawObject(getObject(inObjectID), inPos, inRot, inWorn, inFlipH, inAge,
               inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet,
               inClothing, inNumContained, inContainedIDs, inSubContained);
  } else {
    if (inFadeTargetType == ground2) {
      inFadeTargetType = ground;
    }
    ...
    drawObjectAnim(inObjectID, r, inFrameTime, inAnimFade, rB,
                   inFadeTargetFrameTime, inFrozenRotFrameTime,
                   outFrozenRotFrameTimeUsed, rF, rArm, rArmFade, inPos, inRot,
                   inWorn, inFlipH, inAge, inHideClosestArm, inHideAllLimbs,
                   inHeldNotInPlaceYet, inClothing, inClothingContained,
                   inNumContained, inContainedIDs, inSubContained);
  }
...
void drawObjectAnim(int inObjectID, AnimationRecord *inAnim, double inFrameTime,
                    double inAnimFade, AnimationRecord *inFadeTargetAnim,
                    double inFadeTargetFrameTime, double inFrozenRotFrameTime,
                    char *outFrozenRotFrameTimeUsed,
                    AnimationRecord *inFrozenRotAnim,
                    AnimationRecord *inFrozenArmAnim,
                    AnimationRecord *inFrozenArmFadeTargetAnim,
                    doublePair inPos, double inRot, char inWorn, char inFlipH,
                    double inAge, int inHideClosestArm, char inHideAllLimbs,
                    char inHeldNotInPlaceYet, ClothingSet inClothing,
                    SimpleVector<int> *inClothingContained, int inNumContained,
                    int *inContainedIDs, SimpleVector<int> *inSubContained) {
  ...
  drawObjectAnim(inObjectID, 0, inAnim, inFrameTime, inAnimFade,
                 inFadeTargetAnim, inFadeTargetFrameTime, inFrozenRotFrameTime,
                 outFrozenRotFrameTimeUsed, inFrozenRotAnim, inFrozenArmAnim,
                 inFrozenArmFadeTargetAnim, inPos, inRot, inWorn, inFlipH,
                 inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet,
                 inClothing, inClothingContained, slotRots, slotOffsets);
  ...
  for (int i = 0; i < obj->numSlots; i++) {
    if (i < inNumContained) {
      ...
      slotFrameTime =
          processFrameTimeWithPauses(inAnim, i, false, slotFrameTime);

      if (inAnimFade < 1) {
        targetSlotFrameTime = processFrameTimeWithPauses(
            inFadeTargetAnim, i, false, targetSlotFrameTime);
      }
      ...
      if (slotRots != NULL) {
        ...
        centerOffset = rotate(centerOffset, -slotRots[i] * 2 * M_PI);
      }
      ...
      if (i < inAnim->numSlots) {
        ...
        if (inAnimFade < 1) {
          ...
          posWiggle.x += targetWeight *
                         getOscOffset(targetSlotFrameTime,
      ...
      if (inSubContained != NULL && inSubContained[i].size() > 0) {
        ...
        AnimationRecord *subAnim = getAnimation(contained->id, inAnim->type);
        AnimationRecord *subFadeTargetAnim =
            getAnimation(contained->id, inFadeTargetAnim->type);

        // behind sub-contained
        drawObject(contained, 0, pos, rot, false, inFlipH, inAge, 0, false,
                   false, emptyClothing);
        ...
        for (int s = 0; s < contained->numSlots; s++) {
          if (s < inSubContained[i].size()) {
            ...
            ObjectRecord *subContained =
                getObject(inSubContained[i].getElementDirect(s));

            doublePair subCenterOffset = getObjectCenterOffset(subContained);

            double subRot = rot;

            if (contained->slotVert[s]) {
              double rotOffset = 0.25 + subContained->vertContainRotationOffset;

              if (inFlipH) {
                subCenterOffset =
                    rotate(subCenterOffset, -rotOffset * 2 * M_PI);
                subRot -= rotOffset;
              } else {
                subCenterOffset =
                    rotate(subCenterOffset, -rotOffset * 2 * M_PI);
            ...
            if (subAnim != NULL && s < subAnim->numSlots) {
              ...
              subPos.x +=
                  inAnimFade * getOscOffset(slotFrameTime,
            ...
            drawObject(subContained, 2, subPos, subRot, false, inFlipH, inAge,
                       0, false, false, emptyClothing);
          }
        }

        // in front of sub-contained
        drawObject(contained, 1, pos, rot, false, inFlipH, inAge, 0, false,
                   false, emptyClothing);

      } else {
        // no sub-contained
        // draw contained all at once
        drawObject(contained, 2, pos, rot, false, inFlipH, inAge, 0, false,
                   false, emptyClothing);
      }
    }
  }
  ...
  drawObjectAnim(inObjectID, 1, inAnim, inFrameTime, inAnimFade,
                 inFadeTargetAnim, inFadeTargetFrameTime, inFrozenRotFrameTime,
                 outFrozenRotFrameTimeUsed, inFrozenRotAnim, inFrozenArmAnim,
                 inFrozenArmFadeTargetAnim, inPos, inRot, inWorn, inFlipH,
                 inAge, inHideClosestArm, inHideAllLimbs, inHeldNotInPlaceYet,
                 inClothing, inClothingContained);
}
...
AnimationRecord *copyRecord(AnimationRecord *inRecord,
  ...
  for (int i = 0; i < newRecord->numSounds; i++) {
    newRecord->soundAnim[i] =
        copyRecord(inRecord->soundAnim[i], inCountLiveSoundUses);
  }
