...
char doesOverrideGameImageSize() { return true; }
...
char shouldNativeScreenResolutionBeUsed() { return true; }

char isNonIntegerScalingAllowed() { return true; }

const char *getWindowTitle() { return "OneLife"; }

const char *getAppName() { return "OneLife"; }

int getAppVersion() { return versionNumber; }
...
const char *getFontTGAFileName() { return "font_32_64.tga"; }

char isDemoMode() { return false; }

const char *getDemoCodeSharedSecret() { return "fundamental_right"; }
...
char *getCustomRecordedGameData() {
  ...
  int musicOffSetting = SettingsManager::getIntSetting("musicOff", 0);
  ...
  char *result = autoSprintf(customDataFormatWriteString, versionNumber,
                             mouseSpeedSetting, musicOffSetting,
                             musicLoudnessSetting, webRetrySecondsSetting);
...
char *getHashSalt() { return stringDuplicate(SETTINGS_HASH_SALT); }
...
void freeDrawString() { delete mainFont; }

void initFrameDrawer(int inWidth, int inHeight, int inTargetFrameRate,
                     const char *inCustomRecordedGameData, char inPlayingBack) {
  ...
  handwritingFont = new Font("font_handwriting_32_32.tga", 3, 6, false, 16);

  handwritingFont->setMinimumPositionPrecision(1);

  pencilFont = new Font("font_pencil_32_32.tga", 3, 6, false, 16);

  pencilFont->setMinimumPositionPrecision(1);

  pencilErasedFont = new Font("font_pencil_erased_32_32.tga", 3, 6, false, 16);
  ...
  int numRead = sscanf(inCustomRecordedGameData, customDataFormatReadString,
                       &readVersionNumber, &mouseSpeedSetting, &musicOffSetting,
                       &musicLoudnessSetting, &webRetrySecondsSetting);
  if (numRead != 6) {
    // no recorded game?
  } else {
  ...
  if (reflectorURL == NULL) {
    reflectorURL =
        stringDuplicate("http://localhost/jcr13/oneLifeReflector/server.php");
  }
  ...
  getServerAddressPage =
      new ServerActionPage(reflectorURL, "reflect", 4, resultNamesA, false);
  ...
  char *reviewURL = SettingsManager::getStringSetting("reviewServerURL", "");
    ...
    while (tokens->size() > 0) {
      ...
      if (tokens->size() > 0) {
        ...
        if (nextToken[0] == 28) {
          ...
          nextLongerString = autoSprintf("%s %s ", currentLineString,
                                         *(tokens->getElement(0)));
        }
      }
    ...
    if (currentGamePage == loadingPage) {
      ...
      case 1: {
        ...
        if (progress == 1.0) {
          ...
          if (rebuilding) {
            loadingPage->setCurrentPhase(translate("animationsRebuild"));
          } else {
            loadingPage->setCurrentPhase(translate("animations"));
          }
          loadingPage->setCurrentProgress(0);
      ...
      case 2: {
        ...
        if (progress == 1.0) {
          ...
          if (rebuilding) {
            loadingPage->setCurrentPhase(translate("objectsRebuild"));
          } else {
      ...
      case 3: {
        ...
        if (progress == 1.0) {
          ...
          if (rebuilding) {
            loadingPage->setCurrentPhase(translate("categoriesRebuild"));
          } else {
      ...
      case 4: {
        ...
        if (progress == 1.0) {
          ...
          if (rebuilding) {
            loadingPage->setCurrentPhase(translate("transitionsRebuild"));
          } else {
      ...
      if (existingAccountPage->checkSignal("quit")) {
        ...
        currentGamePage->base_makeActive(true);
      } else if (existingAccountPage->checkSignal("done") || mapPullMode ||
                 autoLogIn) {
      ...
      if (getServerAddressPage->isResponseReady()) {
        ...
        if (strstr(serverIP, "NONE_FOUND") != NULL) {
          ...
          int requiredVersion =
              getServerAddressPage->getResponseInt("requiredVersionNumber");
          ...
          if (versionNumber < requiredVersion) {
            ...
            if (SettingsManager::getIntSetting("useSteamUpdate", 0)) {
              ...
              finalMessagePage->setMessageKey("upgradeMessageSteam");
              ...
              char updateStarted = startUpdate(autoUpdateURL, versionNumber);
      ...
      if (autoUpdatePage->checkSignal("failed")) {
        ...
        finalMessagePage->setMessageKey("updateWritePermissionMessage");
      ...
      if (livingLifePage->checkSignal("loginFailed")) {
        ...
        char *message = autoSprintf(translate("versionMismatch"), versionNumber,
                                    livingLifePage->getRequiredVersion());
        ...
        if (failedFileName != NULL) {
          ...
          char *detailMessage = autoSprintf(
              translate("loadingMapFailedSubMessage"), failedFileName);
...
char getUsesSound() { return !musicOff; }
