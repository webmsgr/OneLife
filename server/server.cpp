...
void sendPeaceWarMessage(const char *inPeaceOrWar, char inWar,
...
static void addPeaceTreaty(int inLineageAEveID, int inLineageBEveID) {
  ...
  if (p != NULL) {
    ...
    if (p->dirAToB && p->dirBToA && !peaceBefore) {
      // new peace!
      ...
      sendPeaceWarMessage("PEACE", false, p->lineageAEveID, p->lineageBEveID);
      sendWarReportToAll();
    }
  } else {
    // else doesn't exist, create new unidirectional
    PeaceTreaty p = {inLineageAEveID, inLineageBEveID, true,
                     false,           false,           false};
...
static void removePeaceTreaty(int inLineageAEveID, int inLineageBEveID) {
  ...
  if (p != NULL) {
    if (p->dirAToB && p->dirBToA) {
      ...
      if (p->dirAToBBroken && p->dirBToABroken) {
        ...
        // new war!
        sendPeaceWarMessage("WAR", true, p->lineageAEveID, p->lineageBEveID);
        messageSent = true;
      }
    } else {
  ...
  if (remove || p == NULL) {
    ...
    if (!found) {
      ...
      if (!messageSent) {
        sendPeaceWarMessage("WAR", true, inLineageAEveID, inLineageBEveID);
        messageSent = true;
      }
    }
  }
...
typedef struct LiveObject {
  ...
  SimpleVector<timeSec_t> clothingContainedEtaDecays[NUM_CLOTHING_PIECES];
...
char *getOwnershipString(int inX, int inY) {
  ...
  for (int j = 0; j < players.size(); j++) {
    LiveObject *otherPlayer = players.getElement(j);
    if (!otherPlayer->error && isOwned(otherPlayer, inX, inY)) {
      char *playerIDString = autoSprintf(" %d", otherPlayer->id);
      messageWorking.appendElementString(playerIDString);
...
FullMapContained getFullMapContained(int inX, int inY) {
  ...
  r.containedEtaDecays = getContainedEtaDecay(inX, inY, &(r.numContained));
  ...
  for (int c = 0; c < r.numContained; c++) {
    if (r.containedIDs[c] < 0) {
      ...
      int numSub;
      int *subContainedIDs = getContained(inX, inY, &numSub, c + 1);
      ...
      timeSec_t *subContainedEtaDecays =
          getContainedEtaDecay(inX, inY, &numSub, c + 1);

      if (subContainedEtaDecays != NULL) {

        r.subContainedEtaDecays[c].appendArray(subContainedEtaDecays, numSub);
...
void transferHeldContainedToMap(LiveObject *inPlayer, int inX, int inY) {
  if (inPlayer->numContained != 0) {
    ...
    float stretch = getObject(inPlayer->holdingID)->slotTimeStretch;

    for (int c = 0; c < inPlayer->numContained; c++) {

      // undo decay stretch before adding
      // (stretch applied by adding)
      if (stretch != 1.0 && inPlayer->containedEtaDecays[c] != 0) {

        timeSec_t offset = inPlayer->containedEtaDecays[c] - curTime;
      ...
      addContained(inX, inY, inPlayer->containedIDs[c],
      ...
      if (numSub > 0) {
        ...
        int *subIDs = inPlayer->subContainedIDs[c].getElementArray();
        timeSec_t *subDecays =
        ...
        for (int s = 0; s < numSub; s++) {
          ...
          // undo decay stretch before adding
          // (stretch applied by adding)
          if (subStretch != 1.0 && subDecays[s] != 0) {
          ...
          addContained(inX, inY, subIDs[s], subDecays[s], c + 1);
        }
        delete[] subIDs;
        delete[] subDecays;
      }
...
static double measurePathLength(int inXS, int inYS, GridPos *inPathPos,
                                int inPathLength) {
...
char readSocketFull(Socket *inSock, SimpleVector<char> *inBuffer) {
  ...
  if (numRead == -1) {
    ...
    if (!inSock->isSocketInFDRange()) {
      ...
      // save a bug report
      int allow = SettingsManager::getIntSetting("allowBugReports", 0);

      if (allow) {
        char *bugName = autoSprintf("bug_socket_%f", Time::getCurrentTime());
...
char *getNextClientMessage(SimpleVector<char> *inBuffer) {
  ...
  if (index > 1 && inBuffer->getElementDirect(0) == 'K' &&
...
ClientMessage parseMessage(LiveObject *inPlayer, char *inMessage) {
  ...
  if (numRead >= 2 && strcmp(nameBuffer, "BUG") == 0) {
    m.type = BUG;
    m.bug = m.x;
  ...
  if (numRead != 3) {
    ...
    if (numRead == 2 && strcmp(nameBuffer, "TRIGGER") == 0) {
      m.type = TRIGGER;
      m.trigger = m.x;
    } else {
      m.type = UNKNOWN;
  ...
  if (strcmp(nameBuffer, "MOVE") == 0) {
    ...
    SimpleVector<char *> *tokens = tokenizeStringInPlace(inMessage);

    // require an even number of extra coords beyond offset
    if (tokens->size() < offset + 2 || (tokens->size() - offset) % 2 != 0) {
    ...
    numRead =
        sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.id));
    ...
    numRead =
        sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.i));
    ...
    // id param optional
    numRead = sscanf(inMessage, "%99s %d %d %d %d", nameBuffer, &(m.x), &(m.y),
                     &(m.i), &(m.id));
    ...
    m.type = DROP;
    numRead =
        sscanf(inMessage, "%99s %d %d %d", nameBuffer, &(m.x), &(m.y), &(m.c));
...
double computePartialMovePathStepPrecise(LiveObject *inPlayer) {
  ...
  double fractionDone = (Time::getCurrentTime() - inPlayer->moveStartTime) /
...
doublePair computePartialMoveSpotPrecise(LiveObject *inPlayer) {
  ...
  if (c == -1) {
    doublePair result = {(double)inPlayer->xs, (double)inPlayer->ys};
    return result;
  }
...
GridPos getPlayerPos(LiveObject *inPlayer) {
  if (inPlayer->xs == inPlayer->xd && inPlayer->ys == inPlayer->yd) {
...
static void restockPostWindowFamilies() {
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (!o->error && !o->isTutorial && o->curseStatus.curseLevel == 0 &&
        familyLineageEveIDsAfterEveWindow.getElementIndex(o->lineageEveID) ==
            -1) {
      ...
      if (barrierOn) {
        ...
        if (abs(pos.x) >= barrierRadius || abs(pos.y) >= barrierRadius) {
          // player outside barrier
          continue;
        }
      }

      familyLineageEveIDsAfterEveWindow.push_back(o->lineageEveID);
...
static void logFamilyCounts() {
  if (postWindowFamilyLogFile != NULL) {
    ...
    for (int i = 0; i < familyLineageEveIDsAfterEveWindow.size(); i++) {
      int lineageEveID = familyLineageEveIDsAfterEveWindow.getElementDirect(i);
...
static int getNextBabyFamilyLineageEveIDRoundRobin() {
  ...
  char wrapOnce = false;
  while (familyCountsAfterEveWindow.getElementDirect(nextBabyFamilyIndex) ==
         0) {
    nextBabyFamilyIndex++;
  ...
  return familyLineageEveIDsAfterEveWindow.getElementDirect(
      nextBabyFamilyIndex);
}
...
double computeFoodDecrementTimeSeconds(LiveObject *inPlayer) {
  ...
  if (inPlayer->isIndoors && inPlayer->indoorBonusFraction > 0 &&
      computeAge(inPlayer) > defaultActionAge) {

    // non-babies get a bonus for being indoors
    inPlayer->indoorBonusTime =
        indoorFoodDecrementSecondsBonus * inPlayer->indoorBonusFraction;
...
double getAgeRate() { return 1.0 / secondsPerYear; }
...
static void setDeathReason(LiveObject *inPlayer, const char *inTag,
  ...
  if (strcmp(inTag, "killed") == 0 || strcmp(inTag, "succumbed") == 0) {

    inPlayer->deathReason = autoSprintf(" reason_%s_%d", inTag, inOptionalID);
  } else {
    // ignore ID
...
int getSecondsPlayed(LiveObject *inPlayer) {
  double deltaSeconds = Time::getCurrentTime() - inPlayer->trueStartTimeSeconds;
...
static int getFirstFertileAge() { return 14; }
...
int computeFoodCapacity(LiveObject *inPlayer) {
  ...
  if (ageInYears < 44) {
    ...
    if (lostBars > 0 && inPlayer->fitnessScore > 0) {
      ...
      double maxLostBars = 16 - 16 * pow(inPlayer->fitnessScore / 60.0, 2);
...
char *slurSpeech(int inSpeakerID, char *inTranslatedPhrase,
                 double inDrunkenness) {
  ...
  JenkinsRandomSource slurRand(inSpeakerID + words->size() + inDrunkenness);
  ...
  for (int i = 0; i < words->size(); i++) {
    if (slurRand.getRandomBoundedDouble(0, 1) < wordSwapChance) {
      ...
      if (maxDist > 0) {
        ...
        *(words->getElement(i)) = words->getElementDirect(i + jump);
...
double computeMoveSpeed(LiveObject *inPlayer) {
  ...
  if (age < fullSpeedAge) {
    ...
    double speedFactor =
        babySpeedFactor + (1.0 - babySpeedFactor) * age / fullSpeedAge;
  ...
  */
  ...
  if (inPlayer->killPosseSize > 0) {
    ...
    if (inPlayer->killPosseSize <= 4) {
      posseSpeedMult = posseSizeSpeedMultipliers[inPlayer->killPosseSize - 1];
    } else {
      // 4+ same value as 4
...
static float computeClothingR(LiveObject *inPlayer) {
  ...
  if (inPlayer->clothing.frontShoe != NULL) {
    clothingR += eachFootWeigth * inPlayer->clothing.frontShoe->rValue;
  }
  if (inPlayer->clothing.backShoe != NULL) {
    clothingR += eachFootWeigth * inPlayer->clothing.backShoe->rValue;
  }
...
static float computeClothingHeat(LiveObject *inPlayer) {
  ...
  for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
    ...
    if (cO != NULL) {
      ...
      for (int s = 0; s < inPlayer->clothingContained[c].size(); s++) {

        ObjectRecord *sO =
            getObject(inPlayer->clothingContained[c].getElementDirect(s));

        clothingHeat += sO->heatValue * cRFactor;
      }
    }
  }
  return clothingHeat;
...
static void recomputeHeatMap(LiveObject *inPlayer) {
        ...
        if (false && o->numSlots > 0) {
          ...
          if (cont != NULL) {
            ...
            for (int c = 0; c < numCont; c++) {
              ...
              heatOutputGrid[j] += cO->heatValue * oRFactor;

              if (hasSub) {
                double cRFactor = 1 - cO->rValue;

                int numSub;
                int *sub = getContained(mapX, mapY, &numSub, c + 1);
                if (sub != NULL) {
                  for (int s = 0; s < numSub; s++) {
                    ObjectRecord *sO = getObject(sub[s]);

                    heatOutputGrid[j] += sO->heatValue * cRFactor * oRFactor;
                  }
                  delete[] sub;
                }
              }
  ...
  int playerMapIndex = (HEAT_MAP_D / 2) * HEAT_MAP_D + (HEAT_MAP_D / 2);
  ...
  while (thisFrontier->size() > 0) {
    ...
    for (int f = 0; f < thisFrontier->size(); f++) {
      ...
      for (int n = 0; n < numNeighbors; n++) {
        ...
        if (nx >= 0 && nx < HEAT_MAP_D && ny >= 0 && ny < HEAT_MAP_D) {

          int nj = ny * HEAT_MAP_D + nx;

          if (!airSpaceGrid[nj] && rGrid[nj] <= rAir) {
  ...
  inPlayer->envHeat = radiantHeatWeight * radiantAirSpaceHeatVal +
                      containedHeatWeight * containedAirSpaceHeatVal +
                      biomeHeatWeight * biomeHeat + constHeatValue;
...
MoveRecord getMoveRecord(LiveObject *inPlayer, char inNewMovesOnly,
  ...
  // start is absolute
  char *startString =
      autoSprintf("%d %%d %%d %.3f %.3f %d", inPlayer->id,
                  inPlayer->moveTotalSeconds, etaSec, inPlayer->pathTruncated);
  ...
  for (int p = 0; p < inPlayer->pathLength; p++) {
    ...
    char *stepString =
        autoSprintf(" %d %d", inPlayer->pathToDest[p].x - inPlayer->xs,
...
SimpleVector<MoveRecord>
getMoveRecords(char inNewMovesOnly,
               SimpleVector<ChangePosition> *inChangeVector = NULL) {
  ...
  for (int i = 0; i < numPlayers; i++) {
    ...
    if ((o->xd != o->xs || o->yd != o->ys) && (o->newMove || !inNewMovesOnly)) {
...
char *getMovesMessageFromList(SimpleVector<MoveRecord> *inMoves,
  ...
  for (int i = 0; i < inMoves->size(); i++) {
    ...
    char *line = autoSprintf(r.formatString, r.absoluteX - inRelativeToPos.x,
...
char *getMovesMessage(char inNewMovesOnly, GridPos inRelativeToPos,
                      GridPos inLocalPos,
                      SimpleVector<ChangePosition> *inChangeVector = NULL) {

  SimpleVector<MoveRecord> v = getMoveRecords(inNewMovesOnly, inChangeVector);
  ...
  for (int i = 0; i < v.size(); i++) {
    ...
    double d = intDist(r.absoluteX, r.absoluteY, inLocalPos.x, inLocalPos.y);
...
static int getMaxChunkDimension() { return chunkDimensionX; }
...
static void setPlayerDisconnected(LiveObject *inPlayer, const char *inReason) {
  ...
  */
  ...
  AppLog::infoF("Player %d (%s) marked as disconnected (%s).", inPlayer->id,
                inPlayer->email, inReason);
...
static void sendGlobalMessage(char *inMessage,
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (!o->error && !o->isTutorial && o->connected) {
      int numSent =
          o->sock->send((unsigned char *)fullMessage, len, false, false);
...
void sendPeaceWarMessage(const char *inPeaceOrWar, char inWar,
  ...
  WarPeaceMessageRecord r = {inWar, inLineageAEveID, inLineageBEveID, curTime};
  ...
  for (int j = 0; j < players.size(); j++) {
    ...
    if (!o->error && o->lineageEveID == inLineageAEveID &&
        o->familyName != NULL) {
      nameA = o->familyName;
      break;
    }
  ...
  for (int j = 0; j < players.size(); j++) {
    ...
    if (!o->error && o->lineageEveID == inLineageBEveID &&
        o->familyName != NULL) {
      nameB = o->familyName;
      break;
    }
  ...
  char *message =
      autoSprintf("%s BETWEEN %s**AND %s FAMILIES", inPeaceOrWar, nameA, nameB);
...
void checkCustomGlobalMessage() {
  ...
  double spacing = SettingsManager::getDoubleSetting(
      "customGlobalMessageSecondsSpacing", 10.0);

  double lastTime =
      SettingsManager::getDoubleSetting("customGlobalMessageLastSendTime", 0.0);
  ...
  if (strcmp(message, "") != 0) {
    ...
    int nextLine =
        SettingsManager::getIntSetting("customGlobalMessageNextLine", 0);

    if (nextLine < numLines) {
      sendGlobalMessage(lines[nextLine]);

      nextLine++;
      SettingsManager::setSetting("customGlobalMessageNextLine", nextLine);

      SettingsManager::setDoubleSetting("customGlobalMessageLastSendTime",
                                        curTime);
    } else {
      // out of lines
...
int sendMapChunkMessage(LiveObject *inO, char inDestOverride = false,
                        int inDestOverrideX = 0, int inDestOverrideY = 0) {
  ...
  if (!inO->firstMapSent) {
    ...
    unsigned char *mapChunkMessage =
        getChunkMessage(fullStartX, fullStartY, chunkDimensionX,
                        chunkDimensionY, inO->birthPos, &messageLength);

    numSent += inO->sock->send(mapChunkMessage, messageLength, false, false);
    ...
    if (horBarW > 0 && horBarH > 0) {
      ...
      unsigned char *mapChunkMessage = getChunkMessage(
          horBarStartX, horBarStartY, horBarW, horBarH, inO->birthPos, &len);
      messageLength += len;

      numSent += inO->sock->send(mapChunkMessage, len, false, false);
    ...
    if (vertBarW > 0 && vertBarH > 0) {
      int len;
      unsigned char *mapChunkMessage =
          getChunkMessage(vertBarStartX, vertBarStartY, vertBarW, vertBarH,
                          inO->birthPos, &len);
      messageLength += len;
...
char *getHoldingString(LiveObject *inObject) {
  ...
  if (inObject->numContained > 0) {
    for (int i = 0; i < inObject->numContained; i++) {
      ...
      char *idString =
          autoSprintf(",%d", hideIDForClient(abs(inObject->containedIDs[i])));
...
char isMapSpotEmptyOfPlayers(int inX, int inY) {
  ...
  for (int i = 0; i < numLive; i++) {
    ...
    if ( // not about to be deleted
        ...
        // stationary
        nextPlayer->xs == nextPlayer->xd && nextPlayer->ys == nextPlayer->yd &&
        // in this spot
        inX == nextPlayer->xd && inY == nextPlayer->yd) {
      return false;
    }
  }
...
static void setFreshEtaDecayForHeld(LiveObject *inPlayer) {
  ...
  if (newDecayT != NULL) {
    inPlayer->holdingEtaDecay = Time::timeSec() + newDecayT->autoDecaySeconds;
  } else {
    // no further decay
    inPlayer->holdingEtaDecay = 0;
  }
...
static void truncateMove(LiveObject *otherPlayer, int blockedStep) {
  ...
  // update timing
  double dist =
      measurePathLength(otherPlayer->xs, otherPlayer->ys,
                        otherPlayer->pathToDest, otherPlayer->pathLength);

  double distAlreadyDone = measurePathLength(otherPlayer->xs, otherPlayer->ys,
                                             otherPlayer->pathToDest, c);

  double moveSpeed =
      computeMoveSpeed(otherPlayer) *
      getPathSpeedModifier(otherPlayer->pathToDest, otherPlayer->pathLength);

  otherPlayer->moveTotalSeconds = dist / moveSpeed;

  double secondsAlreadyDone = distAlreadyDone / moveSpeed;

  otherPlayer->moveStartTime = Time::getCurrentTime() - secondsAlreadyDone;
...
static void endAnyMove(LiveObject *nextPlayer) {
  ...
  if (nextPlayer->xd != nextPlayer->xs || nextPlayer->yd != nextPlayer->ys) {
    ...
    int truncationSpot = computePartialMovePathStep(nextPlayer);
  ...
  if (inNewObject->blocksWalking) {
    ...
    for (int j = 0; j < numLive; j++) {
      ...
      if (otherPlayer->xd != otherPlayer->xs ||
        ...
        GridPos cPos = computePartialMoveSpot(otherPlayer);
        ...
        if (distance(cPos, dropSpot) <= 2 * pathDeltaMax) {
          ...
          for (int p = c; p < otherPlayer->pathLength; p++) {

            if (equal(otherPlayer->pathToDest[p], dropSpot)) {
          ...
          if (blocked) {
            printf("  Blocked by drop\n");
          }

          if (blocked && blockedStep > 0) {
  ...
  for (int d = 1; d < maxR && !found; d++) {
    ...
    for (int yD = -d; yD <= d && !found; yD++) {
      ...
      for (int xD = -d; xD <= d && !found; xD++) {
        ...
        if (isMapSpotEmpty(x, y) && getMapBiome(x, y) == targetBiome &&
            getMapFloor(x, y) == targetFloor) {

          found = true;
          if (barrierOn) {
            if (abs(x) >= barrierRadius || abs(y) >= barrierRadius) {
              // outside barrier
              found = false;
            }
          }
...
GridPos findClosestEmptyMapSpot(int inX, int inY, int inMaxPointsToCheck,
  ...
  for (int i = 0; i < inMaxPointsToCheck; i++) {
    ...
    if (isMapSpotEmpty(p.x, p.y, false)) {
      ...
      if (barrierOn) {
        if (abs(p.x) >= barrierRadius || abs(p.y) >= barrierRadius) {
          // outside barrier
          found = false;
        }
      }
...
static void makePlayerSay(LiveObject *inPlayer, char *inToSay) {
  ...
  if (cursedName != NULL || isYouShortcut) {
    ...
    if (!SettingsManager::getIntSetting("allowCrossLineageCursing", 0)) {

      // cross-lineage cursing in English forbidden

      int namedPersonLineageEveID = getCurseReceiverLineageEveID(cursedName);
  ...
  if (!inPlayer->isTwin && cursedName == NULL &&
    ...
    for (int i = 0; i < players.size(); i++) {
      ...
      if (otherPlayer == inPlayer || otherPlayer->error ||
      ...
      char *translatedPhrase = mapLanguagePhrase(
          inToSay, inPlayer->lineageEveID, otherPlayer->lineageEveID,
          inPlayer->id, otherPlayer->id, speakerAge, computeAge(otherPlayer),
          inPlayer->parentID, otherPlayer->parentID,
  ...
  if (cursedName != NULL && strcmp(cursedName, "") != 0) {
  ...
  if (cursedName != NULL && strcmp(cursedName, "") != 0) {
    ...
    isCurse = cursePlayer(inPlayer->id, inPlayer->lineageEveID, inPlayer->email,
                          speakerPos, getMaxChunkDimension(), cursedName);
  ...
  if (isCurse) {
    if (!inPlayer->isTwin && inPlayer->curseStatus.curseLevel == 0 &&
  ...
  if (inPlayer->heldByOther) {
    LiveObject *holdingPlayer = getLiveObject(inPlayer->heldByOtherID);
...
static void forcePlayerToRead(LiveObject *inPlayer, int inObjectID) {

  char metaData[MAP_METADATA_LENGTH];
  char found = getMetadata(inObjectID, (unsigned char *)metaData);
  ...
  if (found) {
    ...
    if (computeAge(inPlayer) < 10 && strlen(metaData) > sayLimit) {
      // truncate with ...
      metaData[sayLimit] = '.';
      metaData[sayLimit + 1] = '.';
      metaData[sayLimit + 2] = '.';
...
static void holdingSomethingNew(LiveObject *inPlayer, int inOldHoldingID = 0) {
  if (inPlayer->holdingID > 0) {
    ...
    if (o->written && (oldO == NULL || !(oldO->written || oldO->writable))) {
...
static int isGraveSwapDest(int inTargetX, int inTargetY,
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (o->holdingID > 0 &&
        strstr(getObject(o->holdingID)->description, "origGrave") != NULL) {
...
void handleDrop(int inX, int inY, LiveObject *inDroppingPlayer,
  ...
  if (oldHoldingID > 0 && getObject(oldHoldingID)->permanent) {
    ...
    TransRecord *bareTrans = getPTrans(oldHoldingID, -1);

    if (bareTrans == NULL || bareTrans->newTarget == 0) {
      ...
      if (timeTrans != NULL && timeTrans->newTarget != 0) {
        ...
        inDroppingPlayer->holdingID = timeTrans->newTarget;
      ...
      if (getObject(oldHoldingID)->permanent) {
        ...
        bareTrans = getPTrans(oldHoldingID, -1);
      }
    }
    ...
    if (bareTrans != NULL && bareTrans->newTarget > 0) {
      ...
      inDroppingPlayer->holdingID = bareTrans->newTarget;
      holdingSomethingNew(inDroppingPlayer, oldHoldingID);

      setFreshEtaDecayForHeld(inDroppingPlayer);
    }
  } else if (oldHoldingID > 0 && !getObject(oldHoldingID)->permanent) {
  ...
  if ((inDroppingPlayer->holdingID < 0 && mapSpotBlocking) ||
    ...
    char found = findDropSpot(inX, inY, playerPos.x, playerPos.y, &spot);
        ...
        if (babyO != NULL) {
          ...
          if (isFertileAge(inDroppingPlayer)) {
            ...
            babyO->foodDecrementETASeconds =
                Time::getCurrentTime() + computeFoodDecrementTimeSeconds(babyO);
          }
  ...
  if (strstr(o->description, "origGrave") != NULL) {
    ...
    setGravePlayerID(targetX, targetY, inDroppingPlayer->heldGravePlayerID);

    int swapDest = isGraveSwapDest(targetX, targetY, inDroppingPlayer->id);

    // see if another player has target location in air

    GraveMoveInfo g = {{inDroppingPlayer->heldGraveOriginX,
...
void handleForcedBabyDrop(
  ...
  for (int j = 0; j < numLive; j++) {
    ...
    if (-adultO->holdingID == inBabyObject->id) {
      ...
      if (adultO->xd == adultO->xs && adultO->yd == adultO->ys) {

        dropPos.x = adultO->xd;
        dropPos.y = adultO->yd;
      } else {
        dropPos = computePartialMoveSpot(adultO);
      }

      handleDrop(dropPos.x, dropPos.y, adultO,
                 inPlayerIndicesToSendUpdatesAbout);
...
static void
swapHeldWithGround(LiveObject *inPlayer, int inTargetID, int inMapX, int inMapY,
                   SimpleVector<int> *inPlayerIndicesToSendUpdatesAbout) {

  if (inTargetID == inPlayer->holdingID && inPlayer->numContained == 0 &&
  ...
  if (inPlayer->holdingID > 0 &&
      strstr(getObject(inPlayer->holdingID)->description, "origGrave") !=
          NULL &&
      gravePlayerID > 0) {
...
static char *getUpdateLineFromRecord(UpdateRecord *inRecord,
                                     GridPos inRelativeToPos,
  ...
  if (inRecord->posUsed) {
    ...
    if (distance(updatePos, inObserverPos) > getMaxChunkDimension() * 2) {
      ...
      return autoSprintf(inRecord->formatString, 1977, 1977, 1977, 1977, 1977,
                         1977);
    }
    ...
    return autoSprintf(inRecord->formatString, 0, 0, 0, 0);
  }
}
...
static void updateYum(LiveObject *inPlayer, int inFoodEatenID,
  ...
  if (wasYummy || inPlayer->yummyFoodChain.size() == 0) {
  ...
  if (wasYummy) {
    ...
    inPlayer->yummyBonusStore += lrint(foodScaleFactor * currentBonus);
  }
}
...
static char canPlayerUseTool(LiveObject *inPlayer, int inToolID) {
  ...
  if (toolSet != -1 && inPlayer->learnedTools.getElementIndex(toolSet) == -1) {
...
static UpdateRecord getUpdateRecord(LiveObject *inPlayer, char inDelete,
  ...
  if (inPartial || inPlayer->xs != inPlayer->xd ||
  ...
  if (inDelete) {
    ...
    posString = autoSprintf("%d %d %%d %%d", doneMoving, inPlayer->posForced);
    r.absolutePosX = x;
    r.absolutePosY = y;
  }
  ...
  if (inPlayer->holdingID > 0 && isYummy(inPlayer, inPlayer->holdingID)) {
    heldYum = 1;
  }
  ...
  r.formatString = autoSprintf(
      ...
      inPlayer->id, inPlayer->displayID, inPlayer->facingOverride,
      inPlayer->actionAttempt,
      // inPlayer->actionTarget.x - inRelativeToPos.x,
      // inPlayer->actionTarget.y - inRelativeToPos.y,
      holdingString, inPlayer->heldOriginValid,
      // inPlayer->heldOriginX - inRelativeToPos.x,
      // inPlayer->heldOriginY - inRelativeToPos.y,
      hideIDForClient(inPlayer->heldTransitionSourceID), inPlayer->heat,
      posString, computeAge(inPlayer), 1.0 / getAgeRate(),
      computeMoveSpeed(inPlayer), clothingList, inPlayer->justAte,
      hideIDForClient(inPlayer->justAteID), inPlayer->responsiblePlayerID,
      heldYum, heldLearned, deathReason);
...
static char *getUpdateLine(LiveObject *inPlayer, GridPos inRelativeToPos,
                           GridPos inObserverPos, char inDelete,
                           char inPartial = false) {
...
static LiveObject *getHitPlayer(int inX, int inY, int inTargetID = -1,
                                char inCountMidPath = false, int inMaxAge = -1,
                                int inMinAge = -1, int *outHitIndex = NULL) {
  ...
  for (int j = 0; j < numLive; j++) {
    ...
    if (inMaxAge != -1 && computeAge(otherPlayer) > inMaxAge) {
      continue;
    }

    if (inMinAge != -1 && computeAge(otherPlayer) < inMinAge) {
      continue;
    }

    if (inTargetID != -1 && otherPlayer->id != inTargetID) {
      continue;
    }
    ...
    if (otherPlayer->xd == otherPlayer->xs &&
      ...
      if (otherPlayer->xd == inX && otherPlayer->yd == inY) {
...
static int isPlayerCountable(LiveObject *p, int inLineageEveID = -1) {
  ...
  if (inLineageEveID != -1 && p->lineageEveID != inLineageEveID) {
    return false;
  }
  return true;
}
...
static int getNextBabyFamilyLineageEveIDFewestFemales() {
  ...
  waitSecondsPerMom = SettingsManager::getDoubleSetting(
      "weakFamilyPickWaitSecondsPerMom", 1.5 * 3600);

  for (int i = 0; i < uniqueFams.size(); i++) {
    int lineageEveID = uniqueFams.getElementDirect(i);
...
static void setupToolSlots(LiveObject *inPlayer) {
  ...
  int slots = lrint(A / (1 + pow(C, -(inPlayer->fitnessScore - D))) + B);
  ...
  char *message = autoSprintf("YOUR GENETIC FITNESS SCORE IS %.1lf**"
                              ...
                              "FOR A TOTAL OF %d %s.",
                              inPlayer->fitnessScore, slots - min, slotWord,
                              slots, slotTotalWord);
  ...
  for (int i = 0; i < numParts; i++) {
    ...
    if (strstr(lines[i], inEmail) == lines[i]) {
      ...
      if (on == 1) {
        ...
        int numRead =
            sscanf(lines[i], "%99s %d %d,%d %lf %19s %19s %d %d %d %d %d %d",
                   emailBuff, &on, &outRecordToFill->pos.x,
                   &outRecordToFill->pos.y, &outRecordToFill->age,
                   outRecordToFill->firstName, outRecordToFill->lastName,
                   &outRecordToFill->displayID, &outRecordToFill->hatID,
                   &outRecordToFill->tunicID, &outRecordToFill->bottomID,
                   &outRecordToFill->frontShoeID, &outRecordToFill->backShoeID);
...
static char shouldBeEveInjection(float inFitnessScore) {
  ...
  for (int i = numPlayers - 1; i >= numPlayers - eveFitnessWindow && i >= 0;
       i--) {
...
int processLoggedInPlayer(char inAllowReconnect, Socket *inSock,
                          SimpleVector<char> *inSockBuffer, char *inEmail,
                          int inTutorialNumber, CurseStatus inCurseStatus,
                          PastLifeStats inLifeStats, float inFitnessScore,
                          // set to -2 to force Eve
                          int inForceParentID = -1, int inForceDisplayID = -1,
                          GridPos *inForcePlayerPos = NULL) {

  int usePersonalCurses =
  ...
  if (usePersonalCurses) {
    ...
    for (int p = 0; p < players.size(); p++) {
      ...
      if (!o->error && !o->isTutorial && o->curseStatus.curseLevel == 0 &&
        ...
        addPersonToPersonalCurseTest(o->email, inEmail, getPlayerPos(o));
      }
    }
  }
  ...
  for (int p = 0; p < players.size(); p++) {
    ...
    if (!o->error && o->connected && strcmp(o->email, inEmail) == 0) {
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (!o->error && !o->connected && strcmp(o->email, inEmail) == 0) {
      ...
      if (o->heldByOther) {
        ...
        LiveObject *holdingPlayer = getLiveObject(o->heldByOtherID);
      ...
      AppLog::infoF("Player %d (%s) has reconnected.", o->id, o->email);
  ...
  int familyLimitAfterEveWindow =
      SettingsManager::getIntSetting("familyLimitAfterEveWindow", 15);

  int minFamiliesAfterEveWindow =
      SettingsManager::getIntSetting("minFamiliesAfterEveWindow", 5);
  ...
  if (!eveWindow && !eveInjectionOn) {
    ...
    float babyMotherRatio =
        SettingsManager::getFloatSetting("babyMotherApocalypseRatio", 6.0);

    float babyPlayerRatio =
        SettingsManager::getFloatSetting("babyToPlayerApocalypseRatio", 0.33);
    ...
    if (cM == 0 || (float)cB / (float)cM >= babyMotherRatio) {
      ...
      char *logMessage =
          autoSprintf("Too many babies per mother inside barrier: "
      ...
      char *logMessage =
          autoSprintf("Too many babies per player inside barrier: "
    ...
    if (!apocalypseTriggered && familyLimitAfterEveWindow > 0) {
      ...
      if (cFam > familyLimitAfterEveWindow) {
        ...
        triggerApocalypseNow("Too many families after Eve window closed");
      }
    }

    if (!apocalypseTriggered) {
      int maxSeconds = SettingsManager::getIntSetting("arcRunMaxSeconds", 0);

      if (maxSeconds > 0 && getArcRunningSeconds() > maxSeconds) {
  ...
  foodScaleFactor = SettingsManager::getFloatSetting("foodScaleFactor", 1.0);

  babyBirthFoodDecrement =
      SettingsManager::getIntSetting("babyBirthFoodDecrement", 10);

  indoorFoodDecrementSecondsBonus =
      SettingsManager::getFloatSetting("indoorFoodDecrementSecondsBonus", 20);
  ...
  if (familyDataLogFile != NULL) {
    ...
    fprintf(familyDataLogFile,
            ...
            "avAge:%.2f\n",
            Time::getCurrentTime(), newObject.id, cFam, cM, cB, players.size(),
            eveCount, inCount, averageAge);
  }
  ...
  for (int i = 0; i < numPlayers; i++) {
    ...
    if (isFertileAge(player)) {
      ...
      for (int s = 0; s < tempTwinEmails.size(); s++) {
        if (!isLinePermitted(tempTwinEmails.getElementDirect(s), motherPos)) {
          twinBanned = true;
          break;
        }
        if (usePersonalCurses &&
      ...
      if (numPastBabies >= badMotherLimit) {
        ...
        for (int b = 0; b < numPastBabies; b++) {
          ...
          int bID = player->babyIDs->getElementDirect(b);
  ...
  if (!eveWindow && eveInjectionOn && !forceParentChoices &&
  ...
  if (inCurseStatus.curseLevel <= 0 && !forceParentChoices &&
      parentChoices.size() == 0 &&
      !(eveWindow || familyLimitAfterEveWindow > 0) && !apocalypseTriggered) {
    ...
    for (int i = 0; i < numPlayers; i++) {
      ...
      if (usePersonalCurses &&
          isBirthLocationCurseBlocked(newObject.email, playerPos)) {
        // this spot forbidden because someone nearby cursed new player
      ...
      if (usePersonalCurses) {
        for (int s = 0; s < tempTwinEmails.size(); s++) {
          if (isBirthLocationCurseBlockedNoCache(
                  tempTwinEmails.getElementDirect(s), playerPos)) {
            twinBanned = true;
            break;
          }
        }
  ...
  if (inCurseStatus.curseLevel <= 0 && !forceParentChoices &&
      parentChoices.size() == 0 && eveWindow && !apocalypseTriggered &&
      usePersonalCurses) {
    ...
    for (int i = 0; i < numPlayers; i++) {
      ...
      if (isFertileAge(player)) {
        ...
        if (isBirthLocationCurseBlocked(newObject.email, playerPos)) {
  ...
  if (parentChoices.size() == 0 || numOfAge == 0) {
    ...
    for (int r = 0; r < numRaces; r++) {
      ...
      if (counts[r] == 0 && getRaceSize(races[r]) >= 2) {
        foundMin = races[r];
        break;
  ...
  if (!forceParentChoices && parentChoices.size() == 0 && numOfAge == 0 &&
      inCurseStatus.curseLevel == 0) {
  ...
  newObject.foodDecrementETASeconds =
      Time::getCurrentTime() + computeFoodDecrementTimeSeconds(&newObject);
  ...
  if (parentChoices.size() > 0) {
    ...
    if (newObject.isEve) {
      ...
      int parentIndex =
          randSource.getRandomBoundedInt(0, parentChoices.size() - 1);
      ...
      SimpleVector<LiveObject *> filteredParentChoices(parentChoices.size());
      ...
      double choice = randSource.getRandomBoundedDouble(0, totalWeight);
      ...
      for (int i = 0; i < filteredParentChoices.size(); i++) {
        ...
        totalWeight += filteredParentChoiceWeights.getElementDirect(i);
    ...
    if (!newObject.isEve) {
      ...
      if (parentRaceIndex != -1) {
        ...
        if (forceDifferentRace ||
            randSource.getRandomDouble() > childSameRaceLikelihood) {
        ...
        if (childRace == parentObject->race) {
          newObject.displayID = getRandomFamilyMember(
              parentObject->race, parent->displayID, familySpan, forceGirl);
        } else {
    ...
    if (parent->xs == parent->xd && parent->ys == parent->yd) {
    ...
    placed = loadTutorialStart(&(newObject.tutorialLoad), mapFileName, startX,
                               startY);
  ...
  if (!placed) {
    ...
    for (int i = 0; i < numPlayers; i++) {
      ...
      if (player->error || !player->connected || player->isTutorial ||
          player->vogMode) {
        continue;
      }
      ...
      if (seekingCursed && player->curseStatus.curseLevel <= 0) {
        continue;
      } else if (!seekingCursed && player->curseStatus.curseLevel > 0) {
        continue;
      }
    ...
    int startX, startY;
    getEvePosition(newObject.email, newObject.id, &startX, &startY,
                   &otherPeoplePos, allowEveRespawn, incrementEvePlacement);
  ...
  if (newObject.curseStatus.curseLevel == 0 && !newObject.isTwin &&
  ...
  if (parent != NULL && isFertileAge(parent)) {
    ...
    for (int i = 0; i < parent->lineage->size() && i < maxLineageTracked - 1;
         i++) {

      newObject.lineage->push_back(parent->lineage->getElementDirect(i));
    }
    ...
    if (strstr(newObject.email, "paxkiosk")) {
      ...
      sendGlobalMessage(
          ...
          parent);
    } else if (isUsingStatsServer() && !newObject.lifeStats.error &&
      ...
      // let mother know
      char *motherMessage =
          SettingsManager::getSettingContents("newPlayerMessageForMother", "");
  ...
  if (forceSpawn) {
    ...
    newObject.lifeStartTimeSeconds =
        Time::getCurrentTime() - forceSpawnInfo.age * (1.0 / getAgeRate());

    newObject.name =
        autoSprintf("%s %s", forceSpawnInfo.firstName, forceSpawnInfo.lastName);
    ...
    newObject.clothing.frontShoe = getObject(forceSpawnInfo.frontShoeID, true);
    newObject.clothing.backShoe = getObject(forceSpawnInfo.backShoeID, true);
  ...
  for (int j = 0; j < players.size(); j++) {
    ...
    if (!getFemale(otherPlayer)) {
      ...
      if (otherPlayer->parentID > 0) {
        ...
        for (int i = 1; i < newObject.lineage->size(); i++) {
          ...
          if (newObject.lineage->getElementDirect(i) == otherPlayer->parentID) {
      ...
      for (int i = 0; i < newObject.lineage->size(); i++) {
        ...
        if (newObject.lineage->getElementDirect(i) == otherPlayer->id) {
    ...
    if (newObject.parentID > 0 && newObject.parentID == otherPlayer->parentID) {
      // sibs

      newObject.ancestorIDs->push_back(otherPlayer->id);

      newObject.ancestorEmails->push_back(stringDuplicate(otherPlayer->email));
  ...
  if (newObject.isTutorial) {
    AppLog::infoF("New player %s pending tutorial load (tutorial=%d)",
                  newObject.email, inTutorialNumber);
  ...
  if (!newObject.isTutorial)
    logBirth(newObject.id, newObject.email, newObject.parentID, parentEmail,
             !getFemale(&newObject), newObject.xd, newObject.yd, players.size(),
             newObject.parentChainLength);

  AppLog::infoF("New player %s connected as player %d (tutorial=%d) (%d,%d)"
                " (maxPlacementX=%d)",
                newObject.email, newObject.id, inTutorialNumber, newObject.xs,
                newObject.ys, maxPlacementX);
...
static void processWaitingTwinConnection(FreshConnection inConnection) {
  AppLog::infoF("Player %s waiting for twin party of %d", inConnection.email,
  ...
  for (int i = 0; i < waitingForTwinConnections.size(); i++) {
    FreshConnection *nextConnection = waitingForTwinConnections.getElement(i);
  ...
  if (twinConnections.size() + 1 >= inConnection.twinCount) {
    ...
    for (int i = 0; i < twinConnections.size(); i++) {
      FreshConnection *nextConnection = twinConnections.getElementDirect(i);
    ...
    int newID = processLoggedInPlayer(
        false, inConnection.sock, inConnection.sockBuffer, inConnection.email,
        inConnection.tutorialNumber, anyTwinCurseLevel, inConnection.lifeStats,
    ...
    if (inConnection.tutorialNumber == 0) {
      ...
      newPlayer =
          tutorialLoadingPlayers.getElement(tutorialLoadingPlayers.size() - 1);
    }
    ...
    for (int i = 0; i < twinConnections.size(); i++) {
      ...
      processLoggedInPlayer(false, nextConnection->sock,
                            nextConnection->sockBuffer, nextConnection->email,
                            // ignore tutorial number of all but
                            // first player
                            0, anyTwinCurseLevel, nextConnection->lifeStats,
                            nextConnection->fitnessScore, parent, displayID,
                            forcedEvePos);
      ...
      if (usePersonalCurses) {
        ...
        // pass it on.
        LiveObject *newTwinPlayer = players.getElement(players.size() - 1);
        newTwinPlayer->curseStatus = newPlayer->curseStatus;
      }

      LiveObject newTwinPlayer = players.getElementDirect(players.size() - 1);
...
char removeFromContainerToHold(LiveObject *inPlayer, int inContX, int inContY,
                               int inSlotNumber);

// find index of spot on container held item can swap with, or -1 if none found
static int getContainerSwapIndex(LiveObject *inPlayer, int idToAdd,
                                 int inStillHeld, int inSearchLimit,
  ...
  for (int botInd = 0; botInd < inSearchLimit; botInd++) {
    ...
    int bottomItem = getContained(inContX, inContY, botInd, 0);

    if (bottomItem > 0 && !canPickup(bottomItem, playerAge)) {
      // too young to hold!
      same = true;
      ...
      if (bottomItem > 0) {
        ...
        int bottomNum = getNumContained(inContX, inContY, botInd + 1);
        int topNum;
        ...
        if (inStillHeld) {
          ...
          topNum = getNumContained(inContX, inContY, inSearchLimit + 1);
        }
        ...
        if (bottomNum != topNum) {
          ...
          for (int b = 0; b < bottomNum; b++) {
            int subB = getContained(inContX, inContY, b, botInd + 1);
            int subT;

            if (inStillHeld) {
              subT = inPlayer->containedIDs[b];
            } else {
              subT = getContained(inContX, inContY, b, inSearchLimit + 1);
            }
...
static char addHeldToContainer(LiveObject *inPlayer, int inTargetID,
                               int inContX, int inContY, char inSwap = false) {

  int target = inTargetID;

  int targetSlots = getNumContainerSlots(target);
  ...
  float slotSize = targetObj->slotSize;

  float containSize = getObject(inPlayer->holdingID)->containSize;

  int numIn = getNumContained(inContX, inContY);
  ...
  if (numIn < targetSlots) {
    ...
    if (inSwap) {
      ...
      int swapInd = getContainerSwapIndex(inPlayer, idToAdd, true, numIn,
                                          inContX, inContY);
      if (swapInd != -1) {
        isRoom = true;
      }
  ...
  if (isRoom && isContainable(inPlayer->holdingID) && containSize <= slotSize) {
    ...
    if (r != NULL && r->newActor == 0 && r->newTarget > 0) {
      ...
      handleHoldingChange(inPlayer, r->newTarget);
    }
    ...
    addContained(inContX, inContY, idToAdd, inPlayer->holdingEtaDecay);
    ...
    if (inPlayer->numContained > 0) {
      ...
      for (int c = 0; c < inPlayer->numContained; c++) {
        ...
        addContained(inContX, inContY, inPlayer->containedIDs[c],
                     inPlayer->containedEtaDecays[c], numIn + 1);
      }
    ...
    if (inSwap && numInNow > 1) {
      ...
      int swapInd = getContainerSwapIndex(inPlayer, idToAdd, false,
                                          // don't consider top slot
                                          // where we just put this
                                          // new item
                                          numInNow - 1, inContX, inContY);
      if (swapInd != -1) {
        // found one to swap
        removeFromContainerToHold(inPlayer, inContX, inContY, swapInd);
      }
...
char removeFromContainerToHold(LiveObject *inPlayer, int inContX, int inContY,
  ...
  if (isGridAdjacent(inContX, inContY, inPlayer->xd, inPlayer->yd) ||
      (inContX == inPlayer->xd && inContY == inPlayer->yd)) {
    ...
    if (target != 0) {
      ...
      if (inSlotNumber < 0) {
        ...
        int toRemoveID = getContained(inContX, inContY, inSlotNumber);

        if (toRemoveID < 0) {
          toRemoveID *= -1;
        }

        while (inSlotNumber > 0 && !canPickup(toRemoveID, playerAge)) {
      ...
      if (inPlayer->holdingID == 0 && numIn > 0 &&
        ...
        if (subContain) {
          ...
          inPlayer->containedIDs = getContained(
              inContX, inContY, &(inPlayer->numContained), subSlotNumber + 1);
          inPlayer->containedEtaDecays = getContainedEtaDecay(
              inContX, inContY, &(inPlayer->numContained), subSlotNumber + 1);
        ...
        inPlayer->holdingID = removeContained(inContX, inContY, inSlotNumber,
...
// to contain it)
static char addHeldToClothingContainer(LiveObject *inPlayer, int inC,
  ...
  ObjectRecord *cObj = clothingByIndex(inPlayer->clothing, inC);

  if (cObj != NULL && isContainable(inPlayer->holdingID)) {

    int oldNum = inPlayer->clothingContained[inC].size();

    float slotSize = cObj->slotSize;

    float containSize = getObject(inPlayer->holdingID)->containSize;

    if (containSize <= slotSize && cObj->numSlots > 0 &&
        outCouldHaveGoneIn != NULL) {
      *outCouldHaveGoneIn = true;
    }

    if ((oldNum < cObj->numSlots || (oldNum == cObj->numSlots && inWillSwap)) &&
        containSize <= slotSize) {
      // room (or will swap, so we can over-pack it)
      inPlayer->clothingContained[inC].push_back(inPlayer->holdingID);

      if (inPlayer->holdingEtaDecay != 0) {

        timeSec_t curTime = Time::timeSec();

        timeSec_t offset = inPlayer->holdingEtaDecay - curTime;

        offset = offset / cObj->slotTimeStretch;

        inPlayer->holdingEtaDecay = curTime + offset;
      }

      inPlayer->clothingContainedEtaDecays[inC].push_back(
          inPlayer->holdingEtaDecay);
...
static void pickupToHold(LiveObject *inPlayer, int inX, int inY,
                         int inTargetID) {
  inPlayer->holdingEtaDecay = getEtaDecay(inX, inY);

  FullMapContained f = getFullMapContained(inX, inY);
...
static char removeFromClothingContainerToHold(LiveObject *inPlayer, int inC,
                                              int inI = -1) {
  ...
  int oldNumContained = inPlayer->clothingContained[inC].size();
  ...
  if (oldNumContained > 0 && oldNumContained > slotToRemove &&
      slotToRemove >= 0) {
  ...
  if (oldNumContained > 0 && oldNumContained > slotToRemove &&
    ...
    inPlayer->holdingEtaDecay =
        inPlayer->clothingContainedEtaDecays[inC].getElementDirect(
            slotToRemove);
...
static ObjectRecord **getClothingSlot(LiveObject *targetPlayer, int inIndex) {
  ...
  if (inIndex == 2 && targetPlayer->clothing.frontShoe != NULL) {
    clothingSlot = &(targetPlayer->clothing.frontShoe);
  } else if (inIndex == 3 && targetPlayer->clothing.backShoe != NULL) {
    clothingSlot = &(targetPlayer->clothing.backShoe);
  } else if (inIndex == 0 && targetPlayer->clothing.hat != NULL) {
    clothingSlot = &(targetPlayer->clothing.hat);
  } else if (inIndex == 1 && targetPlayer->clothing.tunic != NULL) {
    clothingSlot = &(targetPlayer->clothing.tunic);
  } else if (inIndex == 4 && targetPlayer->clothing.bottom != NULL) {
    clothingSlot = &(targetPlayer->clothing.bottom);
  } else if (inIndex == 5 && targetPlayer->clothing.backpack != NULL) {
    clothingSlot = &(targetPlayer->clothing.backpack);
  }
...
static void removeClothingToHold(LiveObject *nextPlayer,
  ...
  nextPlayer->holdingID = (*clothingSlot)->id;
  holdingSomethingNew(nextPlayer);

  *clothingSlot = NULL;
  nextPlayer->holdingEtaDecay = targetPlayer->clothingEtaDecay[ind];
  targetPlayer->clothingEtaDecay[ind] = 0;

  nextPlayer->numContained = targetPlayer->clothingContained[ind].size();
...
static TransRecord *getBareHandClothingTrans(LiveObject *nextPlayer,
  ...
  if (clothingSlot != NULL) {
    bareHandClothingTrans = getPTrans(0, (*clothingSlot)->id);

    if (bareHandClothingTrans != NULL) {
      int na = bareHandClothingTrans->newActor;

      if (na > 0 && !canPickup(na, computeAge(nextPlayer))) {
        // too young for trans
        bareHandClothingTrans = NULL;
      }

      if (bareHandClothingTrans != NULL) {
        int nt = bareHandClothingTrans->newTarget;

        if (nt > 0 && getObject(nt)->clothing == 'n') {
...
static void handleHoldingChange(LiveObject *inPlayer, int inNewHeldID) {
  ...
  int oldContained = nextPlayer->numContained;
  ...
  if (newHeldSlots < oldContained) {
    ...
    GridPos dropPos = getPlayerPos(inPlayer);
    ...
    if (getMapObject(dropPos.x, dropPos.y) == 0) {
      ...
      found = findDropSpot(dropPos.x, dropPos.y, dropPos.x, dropPos.y, &spot);
    }

    if (found) {

      // throw it on map temporarily
      handleDrop(spot.x, spot.y, inPlayer,
      ...
      shrinkContainer(spot.x, spot.y, newHeldSlots);

      setResponsiblePlayer(-1);

      // pick it back up
      nextPlayer->holdingEtaDecay = getEtaDecay(spot.x, spot.y);

      FullMapContained f = getFullMapContained(spot.x, spot.y);
  ...
  if (newHeldSlots > 0 && oldHolding != 0) {

    restretchDecays(newHeldSlots, nextPlayer->containedEtaDecays, oldHolding,
...
static void sendMessageToPlayer(LiveObject *inPlayer, char *inMessage,
                                int inLength) {
  ...
  int numSent = inPlayer->sock->send(message, len, false, false);
...
LiveObject *getClosestOtherPlayer(LiveObject *inThisPlayer, double inMinAge = 0,
  ...
  for (int j = 0; j < players.size(); j++) {
    ...
    if (otherPlayer != inThisPlayer && !otherPlayer->error &&
        computeAge(otherPlayer) >= inMinAge &&
        (!inNameMustBeNULL || otherPlayer->name == NULL)) {

      GridPos otherPos = getPlayerPos(otherPlayer);

      double dist = distance(thisPos, otherPos);
...
void apocalypseStep() {
  ...
  if (!apocalypseTriggered) {
    ...
    if (apocalypseRequest == NULL && curTime - lastRemoteApocalypseCheckTime >
      ...
      if (SettingsManager::getIntSetting("remoteReport", 0) &&
        ...
        char *url = autoSprintf("%s?action=check_apocalypse", reflectorURL);

        apocalypseRequest = new WebRequest("GET", url, NULL);
      ...
      if (result == -1) {
        AppLog::info("Apocalypse check:  Request to reflector failed.");
      } else if (result == 1) {
        // done, have result

        char *webResult = apocalypseRequest->getResult();

        if (strstr(webResult, "OK") == NULL) {
          AppLog::infoF("Apocalypse check:  Bad response from reflector:  %s.",
                        webResult);
        } else {
          ...
          if (newApocalypseNumber > lastApocalypseNumber) {
            ...
            AppLog::infoF("Apocalypse check:  New remote apocalypse:  %d.",
  ...
  if (apocalypseTriggered) {
    ...
    if (!apocalypseStarted) {
      ...
      if (!apocalypseRemote &&
        ...
        char *reflectorSharedSecret =
            SettingsManager::getStringSetting("reflectorSharedSecret");

        if (reflectorSharedSecret != NULL) {
          lastApocalypseNumber++;

          AppLog::infoF("Apocalypse trigger:  New local apocalypse:  %d.",
                        lastApocalypseNumber);
          ...
          for (int i = 0; i < players.size(); i++) {
            ...
            if (!nextPlayer->error) {
              ...
              double dist = abs(nextPlayer->xd - apocalypseLocation.x) +
          ...
          if (closestPlayerIndex != -1) {
            name = players.getElement(closestPlayerIndex)->name;
          }
          ...
          char *url =
              autoSprintf("%s?action=trigger_apocalypse"
      ...
      for (int i = 0; i < players.size(); i++) {
        ...
        if (!nextPlayer->error && nextPlayer->connected) {
          ...
          int numSent = nextPlayer->sock->send((unsigned char *)message,
                                               messageLength, false, false);
    ...
    if (apocalypseRequest != NULL) {
      ...
      if (result == -1) {
        AppLog::info("Apocalypse trigger:  Request to reflector failed.");
      } else if (result == 1) {
        // done, have result
        ...
        if (strstr(webResult, "OK") == NULL) {
          AppLog::infoF("Apocalypse trigger:  "
    ...
    for (int i = 0; i < players.size(); i++) {
      ...
      if (!nextPlayer->error && nextPlayer->connected) {
        ...
        char *message = autoSprintf("MN\n%d %d %d\n#",
                                    monumentCallX - nextPlayer->birthPos.x,
                                    monumentCallY - nextPlayer->birthPos.y,
                                    hideIDForClient(monumentCallID));
        int messageLength = strlen(message);
    ...
    *outSuffixAdded = false;
    ...
    if (numNames == 1) {
      ...
      while (dup) {
        ...
        dup = isNameDuplicateForCurses(getFirstName(i, &nextI, inFemale));
      ...
      if (inIsEve) {
        ...
        while (dup) {
          ...
          int nextI;
          tempName =
              autoSprintf("%s %s", getFirstName(i, &nextI, inFemale), lastName);
...
ForcedEffects checkForForcedEffects(int inHeldObjectID) {
  ...
  if (o != NULL) {
    ...
    if (emotPos != NULL) {
      sscanf(emotPos, "emot_%d_%d", &(e.emotIndex), &(e.ttlSec));
    }

    char *foodPos = strstr(o->description, "food_");

    if (foodPos != NULL) {
      int numRead = sscanf(foodPos, "food_%lf", &(e.foodCapModifier));
      if (numRead == 1) {
    ...
    if (feverPos != NULL) {
      int numRead = sscanf(feverPos, "fever_%f", &(e.fever));
      if (numRead == 1) {
        e.feverSet = true;
      }
    }
...
void setNoLongerDying(LiveObject *inPlayer,
  ...
  if (inPlayer->murderPerpEmail != NULL) {
    delete[] inPlayer->murderPerpEmail;
    inPlayer->murderPerpEmail = NULL;
  }
  ...
  inPlayerIndicesToSendHealingAbout->push_back(
      getLiveObjectIndex(inPlayer->id));
}
...
static void checkSickStaggerTime(LiveObject *inPlayer) {
  ...
  if (strstr(heldObj->description, "sick") != NULL) {
    isSick = true;
    ...
    int staggerTime = SettingsManager::getIntSetting("deathStaggerTime", 20);
    ...
    // side-effects
    inPlayer->dyingETA = currentTime + 10 * staggerTime;
  }
  ...
  if (isSick) {
    ...
    // source
    inPlayer->deathSourceID = inPlayer->holdingID;

    setDeathReason(inPlayer, "succumbed", inPlayer->holdingID);
  }
}
...
static void updatePosseSize(LiveObject *inTarget,
  ...
  if (inRemovedKiller != NULL) {
    ...
    if (oldSize != 0) {
      killStatePosseChangedPlayerIDs.push_back(inRemovedKiller->id);
    }
  }
}
...
static char isNoWaitWeapon(int inObjectID) {
  return strstr(getObject(inObjectID)->description, "+noWait") != NULL;
}
...
char addKillState(LiveObject *inKiller, LiveObject *inTarget,
  ...
  if (!inInfiniteRange &&
      distance(getPlayerPos(inKiller), getPlayerPos(inTarget)) > 8) {
    // out of range
    return false;
  }
  ...
  if (!found) {
    ...
    double curTime = Time::getCurrentTime();
    KillState s = {
        inKiller->id, inKiller->holdingID, inTarget->id, curTime, curTime, 30};
...
static void removeKillState(LiveObject *inKiller, LiveObject *inTarget) {
  for (int i = 0; i < activeKillStates.size(); i++) {
    ...
    if (s->killerID == inKiller->id && s->targetID == inTarget->id) {
  ...
  if (newPosseSize == 0 && inTarget != NULL && inTarget->emotFrozen &&
...
static void interruptAnyKillEmots(int inPlayerID, int inInterruptingTTL) {
  for (int i = 0; i < activeKillStates.size(); i++) {
    KillState *s = activeKillStates.getElement(i);
...
static void setPerpetratorHoldingAfterKill(LiveObject *nextPlayer,
                                           ...
                                           TransRecord *rHit, TransRecord *r) {
  ...
  if (rHit != NULL) {
    ...
    holdingSomethingNew(nextPlayer, oldHolding);
  } else if (woundHit != NULL) {
    // result of hit on held weapon
    // could also be
    // specified in wound trans
    nextPlayer->holdingID = woundHit->newActor;
    holdingSomethingNew(nextPlayer, oldHolding);
  } else if (r != NULL) {
...
void executeKillAction(int inKillerIndex, int inTargetIndex,
  ...
  if (nextPlayer->holdingID > 0) {
    ...
    // holding something
    ObjectRecord *heldObj = getObject(nextPlayer->holdingID);
    ...
    if (heldObj->deadlyDistance > 0) {
      ...
      double d = distance(targetPos, playerPos);
      ...
      if (heldObj->deadlyDistance >= d &&
        ...
        if (hitPlayer != NULL &&
            strstr(heldObj->description, "otherFamilyOnly")) {
          // make sure victim is in
          // different family
          // and no treaty

          if (hitPlayer->lineageEveID == nextPlayer->lineageEveID ||
              isPeaceTreaty(hitPlayer->lineageEveID,
                            nextPlayer->lineageEveID) ||
              !isWarState(hitPlayer->lineageEveID, nextPlayer->lineageEveID)) {
            hitPlayer = NULL;
          }
        }
        ...
        if (hitPlayer != NULL) {
          ...
          TransRecord *woundHit =
              getPTrans(nextPlayer->holdingID, 0, true, false);
          ...
          if (woundHit != NULL && woundHit->newTarget > 0 &&
            ...
            TransRecord *r = getPTrans(nextPlayer->holdingID, 0);

            setPerpetratorHoldingAfterKill(nextPlayer, woundHit, rHit, r);

            ForcedEffects e = checkForForcedEffects(woundHit->newTarget);

            // emote-effect only for no_replace
            // no fever or food effect
            if (e.emotIndex != -1) {
              hitPlayer->emotFrozen = true;
              hitPlayer->emotFrozenIndex = e.emotIndex;

              hitPlayer->emotUnfreezeETA = Time::getCurrentTime() + e.ttlSec;

              newEmotPlayerIDs->push_back(hitPlayer->id);
              newEmotIndices->push_back(e.emotIndex);
              newEmotTTLs->push_back(e.ttlSec);

              interruptAnyKillEmots(hitPlayer->id, e.ttlSec);
            }
            return;
          }
        }
        ...
        if (hitPlayer != NULL) {
          ...
          hitPlayer->murderSourceID = nextPlayer->holdingID;

          hitPlayer->murderPerpID = nextPlayer->id;
          ...
          hitPlayer->murderPerpEmail = stringDuplicate(nextPlayer->email);

          setDeathReason(hitPlayer, "killed", nextPlayer->holdingID);

          // if not already dying
          if (!hitPlayer->dying) {
            int staggerTime =
                SettingsManager::getIntSetting("deathStaggerTime", 20);

            double currentTime = Time::getCurrentTime();

            hitPlayer->dying = true;
            hitPlayer->dyingETA = currentTime + staggerTime;

            playerIndicesToSendDyingAbout->push_back(
                getLiveObjectIndex(hitPlayer->id));

            hitPlayer->errorCauseString = "Player killed by other player";
          } else {
            // already dying,
            ...
            double staggerTimeLeft = hitPlayer->dyingETA - currentTime;

            if (staggerTimeLeft > 0) {
              staggerTimeLeft /= 2;
              hitPlayer->dyingETA = currentTime + staggerTimeLeft;
            }
          }
        }
        ...
        if (someoneHit) {
          ...
          // or on ground
          rHit = getPTrans(nextPlayer->holdingID, 0, false, true);

          if (rHit != NULL && rHit->newTarget > 0) {
            hitPlayer->customGraveID = rHit->newTarget;
          }

          char wasSick = false;

          if (hitPlayer->holdingID > 0 &&
              strstr(getObject(hitPlayer->holdingID)->description, "sick") !=
                  NULL) {
            wasSick = true;
          }
          ...
          woundHit = getPTrans(nextPlayer->holdingID, 0, true, false);

          if (woundHit != NULL && woundHit->newTarget > 0) {

            // don't drop their wound
            if (hitPlayer->holdingID != 0 && !hitPlayer->holdingWound &&
                !hitPlayer->holdingBiomeSickness) {
              handleDrop(targetPos.x, targetPos.y, hitPlayer,
            ...
            if (!hitPlayer->holdingWound || wasSick) {
              woundChange = true;

              hitPlayer->holdingID = woundHit->newTarget;
              holdingSomethingNew(hitPlayer);
              setFreshEtaDecayForHeld(hitPlayer);
            }
            ...
            if (woundChange) {
              ...
              ForcedEffects e = checkForForcedEffects(hitPlayer->holdingID);
              ...
              if (e.foodModifierSet && e.foodCapModifier != 1) {
                hitPlayer->yummyBonusStore = 0;
                hitPlayer->foodCapModifier = e.foodCapModifier;
              ...
              checkSickStaggerTime(hitPlayer);
        ...
        timeSec_t oldEtaDecay = nextPlayer->holdingEtaDecay;

        if (r != NULL) {

          if (hitPlayer != NULL && r->newTarget != 0) {

            hitPlayer->embeddedWeaponID = r->newTarget;

            if (oldHolding == r->newTarget) {
              // what we are holding
              // is now embedded in them
              // keep old decay
              hitPlayer->embeddedWeaponEtaDecay = oldEtaDecay;
            } else {

              TransRecord *newDecayT = getMetaTrans(-1, r->newTarget);
              ...
              if (newDecayT != NULL) {
                ...
                hitPlayer->embeddedWeaponEtaDecay = 0;
                     ...
                     isMapSpotEmpty(targetPos.x, targetPos.y)) {
            // this is old code, and probably never gets executed

            // no player hit, and target ground
            // spot is empty
            setMapObject(targetPos.x, targetPos.y, r->newTarget);
            ...
            if (oldHolding == r->newTarget) {
              ...
              setEtaDecay(targetPos.x, targetPos.y, oldEtaDecay);
            }
          }
...
static void nameEve(LiveObject *nextPlayer, char *name) {
  ...
  nextPlayer->name =
      getUniqueCursableName(nextPlayer->name, &(nextPlayer->nameHasSuffix),
                            true, getFemale(nextPlayer));
  ...
  if (nextPlayer->nameHasSuffix) {
    ...
    sscanf(nextPlayer->name, "%s %s %s", firstName, lastName, suffix);
  } else {
    sscanf(nextPlayer->name, "%s %s", firstName, lastName);
  }

  nextPlayer->familyName = stringDuplicate(lastName);

  if (!nextPlayer->isTutorial) {
    logName(nextPlayer->id, nextPlayer->email, nextPlayer->name,
            nextPlayer->lineageEveID);
  }
}
...
void nameBaby(LiveObject *inNamer, LiveObject *inBaby, char *inName,
  ...
  if (false && nextPlayer->name != NULL) {
    ...
    if (lastName == NULL) {
      ...
      char *suffixPos = strstr((char *)lastName, " ");
  ...
  const char *close = findCloseFirstName(name, getFemale(inBaby));

  if (strcmp(lastName, "") != 0) {
    babyO->name = autoSprintf("%s %s", close, lastName);
  } else {
    babyO->name = stringDuplicate(close);
  }

  if (babyO->familyName == NULL && nextPlayer->familyName != NULL) {
    // mother didn't have a family
    // name set when baby was born
    // now she does
    // or whatever player named
    // this orphaned baby does
    babyO->familyName = stringDuplicate(nextPlayer->familyName);
  }
  ...
  babyO->name = getUniqueCursableName(babyO->name, &(babyO->nameHasSuffix),
                                      false, getFemale(babyO));

  logName(babyO->id, babyO->email, babyO->name, babyO->lineageEveID);

  playerIndicesToSendNamesAbout->push_back(getLiveObjectIndex(babyO->id));
}
...
void replaceNameInSaidPhrase(char *inSaidName, char **inSaidPhrase,
  ...
  char found = false;
  char *newPhrase = replaceOnce(*inSaidPhrase, inSaidName, trueName, &found);
  delete[] trueName;
...
void getLineageLineForPlayer(LiveObject *inPlayer,
  ...
  for (int j = 0; j < inPlayer->lineage->size(); j++) {
    char *mID = autoSprintf(" %d", inPlayer->lineage->getElementDirect(j));
    inVector->appendElementString(mID);
    delete[] mID;
  }
  // include eve tag at end
  char *eveTag = autoSprintf(" eve=%d", inPlayer->lineageEveID);
...
static void
endBiomeSickness(LiveObject *nextPlayer, int i,
                 SimpleVector<int> *playerIndicesToSendUpdatesAbout) {
  ...
  int newEmot = getBiomeReliefEmot(oldSickness);
...
void logFitnessDeath(LiveObject *nextPlayer) {
  ...
  if (deadPersonAge < forceDeathAge) {
    for (int i = 0; i < players.size(); i++) {
      ...
      if (o->error || o->isTutorial || o->id == nextPlayer->id) {
        continue;
      }
  ...
  if (nextPlayer->suicide) {
    ...
    for (int i = 0; i < ancestorEmails->size(); i++) {
      double startTime = ancestorLifeStartTimeSeconds->getElementDirect(i);
  ...
  logFitnessDeath(players.size(), nextPlayer->email, nextPlayer->name,
                  nextPlayer->displayID, computeAge(nextPlayer), ancestorEmails,
                  ancestorRelNames);
}
...
static void logClientTag(FreshConnection *inConnection) {
  ...
  if (log != NULL) {
    fprintf(log, "%.0f %s %s\n", Time::getCurrentTime(), inConnection->email,
            tagToLog);
...
static void sendLearnedToolMessage(LiveObject *inPlayer,
  ...
  for (int i = 0; i < inNewToolSets->size(); i++) {
    getToolSetMembership(inNewToolSets->getElementDirect(i), &(setList));
  }
  ...
  for (int i = 0; i < setList.size(); i++) {
    ...
    char *idString = autoSprintf("%d", setList.getElementDirect(i));
...
static char learnTool(LiveObject *inPlayer, int inToolID) {
  ...
  if (toolSet != -1 && inPlayer->learnedTools.getElementIndex(toolSet) == -1 &&
    ...
    if ((desLen > 0 && des[desLen - 1] == 'S') ||
        (desLen > 2 && des[desLen - 1] == 'G' && des[desLen - 2] == 'N' &&
         des[desLen - 3] == 'I')) {
    ...
    if (numLeft > 0) {
      message = autoSprintf("YOU LEARNED %s%s.**"
                            ...
                            article, des, numLeft, inPlayer->numToolSlots);
    } else {
      message = autoSprintf("YOU LEARNED %s%s.**"
...
static char isBiomeAllowedForPlayer(LiveObject *inPlayer, int inX, int inY) {
  if (inPlayer->vogMode || inPlayer->forceSpawn || inPlayer->isTutorial) {
    return true;
  }

  if (inPlayer->holdingID > 0) {
    ObjectRecord *heldO = getObject(inPlayer->holdingID);
    if (heldO->permanent && heldO->speedMult == 0) {
...
static char isAccessBlocked(LiveObject *inPlayer, int inTargetX, int inTargetY,
                            int inTargetID) {
  int target = inTargetID;
  ...
  if (target > 0) {
    ...
    if (isGridAdjacent(x, y, inPlayer->xd, inPlayer->yd)) {

      if (targetObj->sideAccess) {

        if (y > inPlayer->yd || y < inPlayer->yd) {
          // access from N or S
          wrongSide = true;
        }
    ...
    if (targetObj->isOwned) {
      ...
      ownershipBlocked = !isOwned(inPlayer, x, y);
    }
  }
...
char isHungryWorkBlocked(LiveObject *inPlayer, int inNewTarget, int *outCost) {
  *outCost = 0;
  ...
  char *des = getObject(inNewTarget)->description;

  char *desPos = strstr(des, "+hungryWork");

  if (desPos != NULL) {

    int cost = 0;

    sscanf(desPos, "+hungryWork%d", &cost);

    if (inPlayer->foodStore + inPlayer->yummyBonusStore < cost + 4) {
      // block hungry work,
...
static LiveObject *getPlayerByName(char *inName, LiveObject *inSkip) {
  for (int j = 0; j < players.size(); j++) {
    LiveObject *otherPlayer = players.getElement(j);
    if (!otherPlayer->error && otherPlayer != inSkip &&
        otherPlayer->name != NULL && strcmp(otherPlayer->name, inName) == 0) {
...
static unsigned char *getFollowingMessage(char inAll, int *outLength) {
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (nextPlayer->followingUpdate || inAll) {
      ...
      char *line = autoSprintf("%d %d %d\n", nextPlayer->id,
                               nextPlayer->followingID, colorIndex);
  ...
  if (numAdded > 0) {
    ...
    if (numAdded > 0) {
      ...
      char *followingMessageText = followingWorking.getElementString();
      ...
      if (followingMessageLength < maxUncompressedSize) {
        ...
        followingMessage =
            makeCompressedMessage(followingMessageText, followingMessageLength,
...
static unsigned char *getExileMessage(char inAll, int *outLength) {
  ...
  if (numAdded > 0) {
    ...
    if (numAdded > 0) {
      ...
      char *exileMessageText = exileWorking.getElementString();
      ...
      if (exileMessageLength < maxUncompressedSize) {
        ...
        exileMessage = makeCompressedMessage(
            exileMessageText, exileMessageLength, &exileMessageLength);
...
static void leaderDied(LiveObject *inLeader) {
  ...
  for (int i = 0; i < players.size(); i++) {
    ...
    if (otherPlayer != inLeader && !otherPlayer->error) {

      int exileIndex = otherPlayer->exiledByIDs.getElementIndex(inLeader->id);
    ...
    if (otherPlayer != inLeader && !otherPlayer->error) {
      ...
      if (otherPlayer->followingID == inLeader->id) {
        ...
        for (int e = 0; e < exiledByThisLeader.size(); e++) {
          LiveObject *eO = exiledByThisLeader.getElementDirect(e);

          if (eO != NULL &&
              // never have them exile themselves
              eO->id != oID && eO->exiledByIDs.getElementIndex(oID) == -1) {
...
static void tryToStartKill(LiveObject *nextPlayer, int inTargetID) {
  if (inTargetID > 0 && nextPlayer->holdingID > 0 &&
      canPlayerUseOrLearnTool(nextPlayer, nextPlayer->holdingID)) {
    ...
    if (heldObj->deadlyDistance > 0) {
      ...
      LiveObject *targetPlayer = getLiveObject(inTargetID);
      ...
      if (targetPlayer != NULL) {
        ...
        if (strstr(heldObj->description, "otherFamilyOnly")) {
          // make sure victim is in
          // different family
          // AND that there's no peace treaty
          if (targetPlayer->lineageEveID == nextPlayer->lineageEveID ||
              isPeaceTreaty(targetPlayer->lineageEveID,
                            nextPlayer->lineageEveID) ||
              !isWarState(targetPlayer->lineageEveID,
                          nextPlayer->lineageEveID)) {
        ...
        if (!weaponBlocked && !isAlreadyInKillState(nextPlayer)) {
          // they aren't already in one

          removeAnyKillState(nextPlayer);

          char enteredState = addKillState(nextPlayer, targetPlayer);

          if (enteredState && !isNoWaitWeapon(nextPlayer->holdingID)) {
            ...
            nextPlayer->emotFrozenIndex = killEmotionIndex;
            ...
            if (!targetPlayer->emotFrozen) {
              ...
              targetPlayer->emotFrozenIndex = victimEmotionIndex;

              newEmotPlayerIDs.push_back(targetPlayer->id);
...
static const char *leadershipNames[NUM_LEADERSHIP_NAMES][2] = {
    {"LORD", "LADY"},
    {"BARON", "BARONESS"},
    {"COUNT", "COUNTESS"},
    {"DUKE", "DUCHESS"},
    {"KING", "QUEEN"},
    {"EMPEROR", "EMPRESS"},
    {"HIGH EMPEROR", "HIGH EMPRESS"},
    {"SUPREME EMPEROR", "SUPREME EMPRESS"}};
...
static char *translatePhraseFromSpeaker(char *inPhrase, LiveObject *speakerObj,
  ...
  if (speakerObj == NULL || nextPlayer->vogMode || nextPlayer->forceSpawn ||
      (speakerObj != NULL && speakerObj->vogMode) ||
      (speakerObj != NULL && speakerObj->forceSpawn) ||
      players.size() < minActivePlayersForLanguages ||
      strlen(trimmedPhrase) == 0 || trimmedPhrase[0] == '[' ||
      isPolylingual(nextPlayer->displayID) ||
      (speakerObj != NULL && isPolylingual(speakerObj->displayID))) {

    return stringDuplicate(trimmedPhrase);
  } else {
    int speakerDrunkenness = speakerObj->drunkenness;

    return mapLanguagePhrase(trimmedPhrase, speakerObj->lineageEveID,
                             nextPlayer->lineageEveID, speakerObj->id,
                             nextPlayer->id, computeAge(speakerObj),
                             computeAge(nextPlayer), speakerObj->parentID,
                             nextPlayer->parentID, speakerDrunkenness / 10.0);
  }
}
      ...
      if (l != NULL && l->currentOrder != NULL &&
        ...
        if (d <= orderDistance) {
          ...
          while (nextToCheck != NULL) {
            if (o->exiledByIDs.getElementIndex(nextToCheck->id) != -1) {
              exiled = true;
              break;
            }
            if (nextToCheck->followingID != -1) {
              nextToCheck = getLiveObject(nextToCheck->followingID);
            } else {
              nextToCheck = NULL;
            }
          }
          ...
          if (!exiled) {
            ...
            if (messageStart != NULL) {
              ...
              char *transOrder = translatePhraseFromSpeaker(
                  messageStart, getLiveObject(l->currentOrderOriginatorID), o);

              char *fullTransOrder =
                  autoSprintf("%s**%s", fullOrder, transOrder);
              delete[] transOrder;
              delete[] fullOrder;
...
int main() {
  ...
  ticketServerURL = SettingsManager::getStringSetting("ticketServerURL");
  ...
  childSameRaceLikelihood =
      (double)SettingsManager::getFloatSetting("childSameRaceLikelihood", 0.90);

  familySpan = SettingsManager::getIntSetting("familySpan", 2);

  eveName = SettingsManager::getStringSetting("eveName", "EVE");
  ...
  orderPhrase = SettingsManager::getSettingContents("orderPhrase", "ORDER,");

  orderDistance = SettingsManager::getIntSetting("orderDistance", 10);

  curseYouPhrase =
      SettingsManager::getSettingContents("curseYouPhrase", "CURSE YOU");

  curseBabyPhrase =
      SettingsManager::getSettingContents("curseBabyPhrase", "CURSE MY BABY");
  ...
  int epochSeconds = SettingsManager::getIntSetting("epochSeconds", 3600);
  ...
  if (false) {
    ...
    printf("Average count %d (%s) = %f,  %d (%s) = %f  over %d runs\n", idA,
           getObject(idA)->description, totalCountA / (double)numRuns, idB,
           getObject(idB)->description, totalCountB / (double)numRuns, numRuns);
  ...
  while (!quit) {
    ...
    if (curStepTime - lastPastPlayerFlushTime > 3600) {
      ...
      for (int i = 0; i < pastPlayers.size(); i++) {
        ...
        if (curStepTime - o->lifeStartTimeSeconds > pastPlayerFlushTime) {
          // stale
          delete[] o->name;
          delete o->lineage;
    ...
    if (forceShutdownMode) {
      ...
      for (int i = 0; i < players.size(); i++) {
        ...
        if (nextPlayer->connected) {
          nextPlayer->sock->send((unsigned char *)shutdownMessage,
                                 messageLength, false, false);
        ...
        setDeathReason(nextPlayer, "forced_shutdown");
        nextPlayer->error = true;
        nextPlayer->errorCauseString = "Forced server shutdown";
      }
      ...
      for (int i = 0; i < players.size(); i++) {
        ...
        if (!nextPlayer->error && !nextPlayer->connected) {
          ...
          setDeathReason(nextPlayer, "disconnect_shutdown");

          nextPlayer->error = true;
          nextPlayer->errorCauseString = "Disconnected during shutdown";
        }
      }
    }
    ...
    if (periodicStepThisStep) {
      ...
      // arcs never end
      int eveInjectionOn = SettingsManager::getIntSetting("eveInjectionOn", 0);
      ...
      if (arcMilestone != -1 && !eveInjectionOn) {
        ...
        if (familyLimitAfterEveWindow > 0 && !eveWindow) {
          familyLine = autoSprintf("of %d", familyLimitAfterEveWindow);
        } else {
        ...
        if (!eveWindow && minFamiliesAfterEveWindow > 0) {
          arcEndMessage =
              autoSprintf(" (ARC ENDS BELOW %d)", minFamiliesAfterEveWindow);
        } else {
        ...
        char *message = autoSprintf("ARC HAS LASTED %d YEARS.  "
                                    "ARC NAME IS :%s:**"
                                    "%d %s %s ALIVE%s",
                                    arcMilestone, getArcName(), numFams,
                                    familyLine, familyWord, arcEndMessage);
        delete[] familyLine;
    ...
    for (int i = 0; i < numLive; i++) {
      ...
      if (nextPlayer->xd != nextPlayer->xs ||
        ...
        double moveTimeLeft = nextPlayer->moveTotalSeconds -
      ...
      if (!nextPlayer->vogMode) {
        ...
        timeLeft = nextPlayer->foodDecrementETASeconds - curTime;
      ...
      for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
        ...
        for (int cc = 0; cc < nextPlayer->clothingContained[c].size(); cc++) {
          timeSec_t decay =
    ...
    if (readySock != NULL && !readySock->isSocket) {
      ...
      if (sock != NULL) {
        ...
        if (a == NULL) {
          ...
          AppLog::infoF("Got connection from %s:%d", a->mAddressString,
                        a->mPort);
          delete a;
        }
        ...
        char *secretString = SettingsManager::getStringSetting(
        ...
        newConnection.sequenceNumberString =
            autoSprintf("%s%lu", nonce, newConnection.sequenceNumber);
        ...
        if (apocalypseTriggered || shutdownMode) {
          ...
          message =
              autoSprintf("SN\n"
                          ...
                          "%lu\n#",
                          currentPlayers, maxPlayers,
                          newConnection.sequenceNumberString, versionNumber);
        ...
        int numSent =
            sock->send((unsigned char *)message, messageLength, false, false);
        ...
        AppLog::infoF("Listening for another connection on port %d", port);
      }
    }
    ...
    for (int i = 0; i < newConnections.size(); i++) {
      ...
      if (nextConnection->email != NULL &&
        ...
        if (nextConnection->curseStatus.curseLevel != -1) {
          AppLog::infoF("Got curse level for %s from curse server: "
                        "%d (excess %d)",
        ...
        int statsResult = getPlayerLifeStats(
            nextConnection->email, &(nextConnection->lifeStats.lifeCount),
        ...
        if (statsResult == -1) {
          ...
          AppLog::infoF("Got life stats for %s from stats server: "
        ...
        int fitResult = getFitnessScore(nextConnection->email,
        ...
        if (result == -1) {
          ...
          nextConnection->errorCauseString = "Ticket server failed";
        } else if (result == 1) {
          // done, have result

          char *webResult = nextConnection->ticketServerRequest->getResult();

          if (strstr(webResult, "INVALID") != NULL) {
            AppLog::info("Client key hmac rejected by ticket server, "
                         "client rejected.");
            nextConnection->error = true;
            nextConnection->errorCauseString = "Client key check failed";
          } else if (strstr(webResult, "VALID") != NULL) {
            // correct!
            nextConnection->ticketServerAccepted = true;
          } else {
            AppLog::errorF("Unexpected result from ticket server, "
                           "client rejected:  %s",
                           webResult);
            nextConnection->error = true;
            nextConnection->errorCauseString = "Client key check failed "
        ...
        for (int p = 0; p < players.size(); p++) {
          LiveObject *o = players.getElement(p);
          if (!o->error && strcmp(o->email, nextConnection->email) == 0) {
        ...
        if (liveButDisconnected) {
          ...
          int spendResult = spendLifeToken(nextConnection->email);
          if (spendResult == -1) {
            AppLog::info("Failed to spend life token for client, "
                         "client rejected.");
            ...
            nextConnection->error = true;
            nextConnection->errorCauseString = "Client life token spend failed";
        ...
        int numSent = nextConnection->sock->send((unsigned char *)message,
                                                 messageLength, false, false);
        ...
        if (numSent != messageLength) {
          ...
          if (nextConnection->twinCode != NULL &&
            ...
            processLoggedInPlayer(
                true, nextConnection->sock, nextConnection->sockBuffer,
                nextConnection->email, nextConnection->tutorialNumber,
                nextConnection->curseStatus, nextConnection->lifeStats,
                nextConnection->fitnessScore);
          }
        ...
        double timeDelta =
            Time::getCurrentTime() - nextConnection->connectionStartTimeSeconds;

        char result =
            readSocketFull(nextConnection->sock, nextConnection->sockBuffer);
        ...
        if (!result) {
          ...
          nextConnection->errorCauseString = "Socket read failed";
        }
        ...
        if (!nextConnection->shutdownMode) {
          message = getNextClientMessage(nextConnection->sockBuffer);
        } else {
          timeLimit = 5;
        }
        ...
        if (message != NULL) {
          ...
          if (strstr(message, "LOGIN") != NULL) {
            ...
            if (strstr(message, "client_") != NULL) {
              ...
              // save and remove it
              nextConnection->clientTag = tokens->getElementDirect(1);
            ...
            if (tokens->size() == 4 || tokens->size() == 5 ||
              ...
              nextConnection->email =
                  stringDuplicate(tokens->getElementDirect(1));
              char *pwHash = tokens->getElementDirect(2);
              char *keyHash = tokens->getElementDirect(3);

              if (tokens->size() >= 5) {
                sscanf(tokens->getElementDirect(4), "%d",
                       &(nextConnection->tutorialNumber));
              }

              if (tokens->size() == 7) {
                nextConnection->twinCode =
                    stringDuplicate(tokens->getElementDirect(5));

                sscanf(tokens->getElementDirect(6), "%d",
                       &(nextConnection->twinCount));

                int maxCount =
                    SettingsManager::getIntSetting("maxTwinPartySize", 4);
              ...
              if (requireClientPassword && !nextConnection->error) {
                ...
                char *trueHash = hmac_sha1(
                    clientPassword, nextConnection->sequenceNumberString);

                if (strcmp(trueHash, pwHash) != 0) {
                  AppLog::info("Client password hmac bad, "
                               "client rejected.");
                  nextConnection->error = true;
                  nextConnection->errorCauseString = "Password check failed";
                }
              ...
              if (requireTicketServerCheck && !nextConnection->error) {
                ...
                char *encodedEmail = URLUtils::urlEncode(nextConnection->email);

                char *url = autoSprintf("%s?action=check_ticket_hash"
                                        "&email=%s"
                                        "&hash_value=%s"
                                        "&string_to_hash=%s",
                                        ticketServerURL, encodedEmail, keyHash,
                ...
                int numSent = nextConnection->sock->send(
                    (unsigned char *)message, messageLength, false, false);

                if (numSent != messageLength) {
                  AppLog::info("Failed to send on client socket, "
                               "client rejected.");
                  nextConnection->error = true;
                  nextConnection->errorCauseString = "Socket write failed";
                } else {
              ...
              AppLog::info("LOGIN message has wrong format, "
                           "client rejected.");
              nextConnection->error = true;
              nextConnection->errorCauseString = "Bad login message";
            }
            ...
            AppLog::info("Client's first message not LOGIN, "
                         "client rejected.");
            nextConnection->error = true;
            nextConnection->errorCauseString = "Unexpected first message";
          }
          ...
          nextConnection->error = true;
          nextConnection->errorCauseString = "Login timeout";
        }
      }
    }
    ...
    for (int c = 0; c < 2; c++) {
      ...
      for (int i = 0; i < list->size(); i++) {
        ...
        if (nextConnection->error) {
          ...
          } else if (currentTime - nextConnection->rejectedSendTime > 5) {
    ...
    for (int i = 0; i < tutorialLoadingPlayers.size(); i++) {
      ...
      char moreLeft = loadTutorialStep(&(nextPlayer->tutorialLoad), timeLimit);
      ...
      AppLog::infoF("New player %s tutorial loaded after %u steps, "
                    ...
                    nextPlayer->email, nextPlayer->tutorialLoad.stepCount,
                    Time::getCurrentTime() - nextPlayer->tutorialLoad.startTime,
    ...
    for (int i = 0; i < numLive; i++) {
      ...
      if (curCrossTime <
          nextPlayer->playerCrossingCheckTime + playerCrossingCheckStepTime) {
      ...
      if (checkCrossing) {
        ...
        if (!nextPlayer->heldByOther && !nextPlayer->vogMode &&
            curOverID != 0 && !isMapObjectInTransit(curPos.x, curPos.y) &&
          ...
          if (!riding && curOverObj->permanent &&
              curOverObj->deadlyDistance > 0) {

            char wasSick = false;

            if (nextPlayer->holdingID > 0 &&
                strstr(getObject(nextPlayer->holdingID)->description, "sick") !=
                    NULL) {
              wasSick = true;
            }
            ...
            setDeathReason(nextPlayer, "killed", curOverID);
            ...
            nextPlayer->errorCauseString = "Player killed by permanent object";
            ...
            if (!nextPlayer->dying || wasSick) {
              ...
              nextPlayer->dyingETA = currentTime + staggerTime;
              ...
              double staggerTimeLeft = nextPlayer->dyingETA - currentTime;

              if (staggerTimeLeft > 0) {
                staggerTimeLeft /= 2;
                nextPlayer->dyingETA = currentTime + staggerTimeLeft;
              }
            }

            // generic on-person
            TransRecord *r = getPTrans(curOverID, 0);
            ...
            if (r != NULL) {
              ...
              if (r->newTarget > 0 && (!nextPlayer->holdingWound || wasSick)) {
                // don't drop their wound
                if (nextPlayer->holdingID != 0 && !nextPlayer->holdingWound &&
                    !nextPlayer->holdingBiomeSickness) {
                  handleDrop(curPos.x, curPos.y, nextPlayer,
                             &playerIndicesToSendUpdatesAbout);
                }
                nextPlayer->holdingID = r->newTarget;
                holdingSomethingNew(nextPlayer);
                ...
                ForcedEffects e = checkForForcedEffects(nextPlayer->holdingID);
                ...
                if (e.emotIndex != -1) {
                  ...
                  newEmotIndices.push_back(e.emotIndex);
                  newEmotTTLs.push_back(e.ttlSec);
                  interruptAnyKillEmots(nextPlayer->id, e.ttlSec);
                }
                if (e.foodModifierSet && e.foodCapModifier != 1) {
                  nextPlayer->yummyBonusStore = 0;
                  nextPlayer->foodCapModifier = e.foodCapModifier;
          ...
          } else if (riding && curOverObj->permanent &&
                     curOverObj->deadlyDistance > 0) {
            // rode over something deadly
            // see if it affects what we're riding

            TransRecord *r = getPTrans(nextPlayer->holdingID, curOverID);

            if (r != NULL) {
              handleHoldingChange(nextPlayer, r->newActor);
      ...
      if (curLookTime - nextPlayer->lastRegionLookTime > 5) {
        lookAtRegion(nextPlayer->xd - 8, nextPlayer->yd - 7, nextPlayer->xd + 8,
                     nextPlayer->yd + 7);
      ...
      if (message != NULL) {
        ...
        AppLog::infoF("Got client message from %d: %s", nextPlayer->id,
                      message);
        ...
        if (m.type == UNKNOWN) {
          ...
          setPlayerDisconnected(nextPlayer, "Unknown message type");
        }
        ...
        if (m.type == BUG) {
          ...
          if (allow) {
            char *bugName = autoSprintf("bug_%d_%d_%f", m.bug, nextPlayer->id,
                                        Time::getCurrentTime());
            char *bugInfoName = autoSprintf("%s_info.txt", bugName);
            char *bugOutName = autoSprintf("%s_out.txt", bugName);
          ...
          int allow = SettingsManager::getIntSetting("allowMapRequests", 0);

          if (allow) {

            SimpleVector<char *> *list =
                SettingsManager::getSetting("mapRequestAllowAccounts");

            allow = false;

            for (int i = 0; i < list->size(); i++) {
              if (strcmp(nextPlayer->email, list->getElementDirect(i)) == 0) {
          ...
          if (allow && nextPlayer->connected) {
            ...
            nextPlayer->foodStore = computeFoodCapacity(nextPlayer);
            ...
            unsigned char *mapChunkMessage = getChunkMessage(
                m.x - chunkDimensionX / 2, m.y - chunkDimensionY / 2,
                chunkDimensionX, chunkDimensionY, centerPos, &length);

            int numSent =
                nextPlayer->sock->send(mapChunkMessage, length, false, false);
          ...
          int allow = SettingsManager::getIntSetting("allowVOGMode", 0);

          if (allow) {

            SimpleVector<char *> *list =
                SettingsManager::getSetting("vogAllowAccounts");
          ...
          if (nextPlayer->vogMode && players.size() > 1) {
            ...
            LiveObject *otherPlayer =
                players.getElement(nextPlayer->vogJumpIndex);
            ...
            char *message = autoSprintf(
                "VU\n%d %d\n#", nextPlayer->xs - nextPlayer->birthPos.x,
                nextPlayer->ys - nextPlayer->birthPos.y);
          ...
          if (nextPlayer->vogMode) {
            ...
            newLocationSpeech.push_back(stringDuplicate(m.saidText));
          ...
          if (m.x == nextPlayer->xd && m.y == nextPlayer->yd) {
            ...
            AppLog::infoF("FORCE message has unexpected "
                          "absolute pos (%d,%d), expecting (%d,%d)",
                          m.x, m.y, nextPlayer->xd, nextPlayer->yd);
          }
        } else if (m.type == PING) {
          ...
          if (computeAge(nextPlayer) < 2) {
            ...
            LiveObject *parentO = getLiveObject(parentID);
            ...
            int babyBonesID = SettingsManager::getIntSetting("babyBones", -1);

            if (adult != NULL) {

              if (babyBonesID != -1) {
                ObjectRecord *babyBonesO = getObject(babyBonesID);
                ...
                if (babyBonesO != NULL) {
                  ...
                  GridPos adultPos = getPlayerPos(adult);
                  ...
                  int stepCount = 0;
                  while (getGravePlayerID(gravePos.x, gravePos.y) > 0 &&
                         stepCount < 100) {
                    gravePos.x++;
                    stepCount++;
                  }

                  GraveInfo graveInfo = {gravePos, nextPlayer->id,
                                         nextPlayer->lineageEveID};
                  newGraves.push_back(graveInfo);

                  setGravePlayerID(gravePos.x, gravePos.y, nextPlayer->id);

                  setHeldGraveOrigin(adult, gravePos.x, gravePos.y, 0);
                    ...
                    if (cObj != NULL) {
                      ...
                      if (nextPlayer->clothingContained[c].size() > 0) {
                        ...
                        adult->subContainedIDs =
                            new SimpleVector<int>[adult->numContained];
                        adult->subContainedEtaDecays =
                            new SimpleVector<timeSec_t>[adult->numContained];
                      }

                      handleDrop(adultPos.x, adultPos.y, adult, NULL);
                    }
                  }
                  ...
                  adult->heldTransitionSourceID = nextPlayer->displayID;
              ...
              int babyBonesGroundID =
                  SettingsManager::getIntSetting("babyBonesGround", -1);
              ...
              if (babyBonesGroundID != -1) {
                ...
                TransRecord *groundTrans = getPTrans(babyBonesID, -1);

                if (groundTrans != NULL && groundTrans->newTarget > 0) {

                  nextPlayer->customGraveID = groundTrans->newTarget;
                }
              }
          ...
          SimpleVector<int> *defaultLineage = new SimpleVector<int>();

          defaultLineage->push_back(0);
          DeadObject defaultO = {0, 0, stringDuplicate("~"), defaultLineage,
                                 0, 0};
            ...
            for (int i = 0; i < players.size(); i++) {
              ...
              if (oThis->id == id) {
                ...
                if (oThis->name != NULL) {
                  ...
                  defaultO.name = stringDuplicate(oThis->name);
                }

                defaultO.lineage->push_back_other(oThis->lineage);

                defaultO.lineageEveID = oThis->lineageEveID;
                defaultO.lifeStartTimeSeconds = oThis->lifeStartTimeSeconds;
                defaultO.deathTimeSeconds = oThis->deathTimeSeconds;
              }
            }
          }
          ...
          if (o != NULL) {
            ...
            if (o->name != NULL) {
              char found;
              formattedName = replaceAll(o->name, " ", "_", &found);
            } else {
            ...
            for (int j = 0; j < o->lineage->size(); j++) {
              char *mID = autoSprintf(" %d", o->lineage->getElementDirect(j));
            ...
            char *message =
                autoSprintf("GO\n%d %d %d %d %lf %s%s eve=%d\n#",
                            ...
                            m.y - nextPlayer->birthPos.y, o->id, o->displayID,
                            age, formattedName, linString, o->lineageEveID);
            printf("Processing %d,%d from birth pos %d,%d\n", m.x, m.y,
                   nextPlayer->birthPos.x, nextPlayer->birthPos.y);
          ...
          char *leadString = autoSprintf("%d %d", m.x - nextPlayer->birthPos.x,
          ...
          char *photoServerSharedSecret = SettingsManager::getStringSetting(
              "photoServerSharedSecret", "secret_phrase");
          ...
          if (oID > 0) {
            if (strstr(getObject(oID)->description, "+photo") != NULL) {
              photo = true;
            }
          }
        ...
        // except for move interrupts
                  ...
                  !nextPlayer->heldByOther) ||
                 m.type == MOVE || m.type == JUMP || m.type == SAY ||
                 m.type == EMOT) {

          if (m.type == MOVE && m.sequenceNumber != -1) {
            nextPlayer->lastMoveSequenceNumber = m.sequenceNumber;
          }

          if ((m.type == MOVE || m.type == JUMP) && nextPlayer->heldByOther) {
            ...
            if (m.type == JUMP) {
              ...
              if (computeAge(nextPlayer) >= 1 ||
                  nextPlayer->heldByOtherID != nextPlayer->parentID) {
            ...
            */
            ...
            if (nextPlayer->pathLength > 0 && nextPlayer->pathToDest != NULL &&
                (nextPlayer->xs != m.x || nextPlayer->ys != m.y)) {
              ...
              if (!cOnTheirNewPath && nextPlayer->pathLength > 0) {
                ...
                */
                ...
                if (pathStep != 0) {
                  ...
                  for (int p = c + pathStep; p != theirPathIndex + pathStep;
                       p += pathStep) {
                    GridPos pos = nextPlayer->pathToDest[p];
            ...
            */
            ...
            if (nextPlayer->xd == nextPlayer->xs &&
              ...
              */
              ...
              if (validPath.size() == 0) {
                ...
                double dist = measurePathLength(nextPlayer->xs, nextPlayer->ys,
                ...
                if (startIndex > 0) {
                  naiveStart = nextPlayer->pathToDest[startIndex - 1];
                } else {
                ...
                double naiveStartDist =
                    distance(naiveStart, nextPlayer->pathToDest[startIndex]);
                ...
                doublePair newFirstSpot = {
                    (double)nextPlayer->pathToDest[startIndex].x,
                    (double)nextPlayer->pathToDest[startIndex].y};

                // now add in true distance to first spot
                dist += distance(startPosPrecise, newFirstSpot);

                double distAlreadyDone =
                    measurePathLength(nextPlayer->xs, nextPlayer->ys,
                ...
                */
                nextPlayer->moveStartTime =
                    Time::getCurrentTime() - secondsAlreadyDone;
                  ...
                  if (oID > 0 && getObject(oID)->autoDefaultTrans) {
                    ...
                    if (t != NULL && t->newTarget > 0) {
                      ...
                      if (timeT != NULL && timeT->autoDecaySeconds < 20) {
                        ...
                        // pass-through time
                        double timeLeft =
                            nextPlayer->moveTotalSeconds - secondsAlreadyDone;
                        ...
                        if (plannedETADecay > actual) {
                          setEtaDecay(x, y, plannedETADecay);
                        }
                      }
                    }
                  }
                ...
                for (int p = 0; p < nextPlayer->pathLength; p++) {
                  ...
                  sicknessObjectID = getBiomeSickness(
                      nextPlayer->displayID, nextPlayer->pathToDest[p].x,
                      nextPlayer->pathToDest[p].y);
                ...
                if (nextPlayer->vogMode || nextPlayer->forceSpawn ||
                ...
                if (sicknessObjectID > 0 && nextPlayer->holdingID > 0 &&
                    getObject(nextPlayer->holdingID)->rideable) {

                  sicknessObjectID = -1;
                }

                if (sicknessObjectID > 0 && !nextPlayer->holdingWound &&
                    nextPlayer->holdingID != sicknessObjectID) {
                  ...
                  endBiomeSickness(nextPlayer, i,
                                   &playerIndicesToSendUpdatesAbout);
                }
              }
            }
          } else if (m.type == SAY && m.saidText != NULL &&
                     Time::getCurrentTime() - nextPlayer->lastSayTimeSeconds >
                         minSayGapInSeconds) {

            nextPlayer->lastSayTimeSeconds = Time::getCurrentTime();
            ...
            if (nextPlayer->ownedPositions.size() > 0) {
              ...
              if (namedOwner != NULL) {
                LiveObject *o = getPlayerByName(namedOwner, nextPlayer);
              ...
              if (newOwners.size() == 0) {
                ...
                if (isYouGivingSay(m.saidText)) {
                  ...
                  for (int n = 0; n < players.size(); n++) {
                    ...
                    if (o->error || o->id == nextPlayer->id) {
                      continue;
                    }
                    if (o->lineageEveID == nextPlayer->lineageEveID) {
                      newOwners.push_back(o);
                    }
                  }
              ...
              if (newOwners.size() > 0) {
                ...
                for (int j = 0; j < nextPlayer->ownedPositions.size(); j++) {
                  GridPos nextPos =
                      nextPlayer->ownedPositions.getElementDirect(j);
                ...
                if (minDist < DBL_MAX) {
                  ...
                  for (int n = 0; n < newOwners.size(); n++) {
                    LiveObject *newOwnerPlayer = newOwners.getElementDirect(n);

                    if (!isOwned(newOwnerPlayer, closePos)) {
                      newOwnerPlayer->ownedPositions.push_back(closePos);
            ...
            if (nextPlayer->holdingID > 0 && isPosseJoiningSay(m.saidText)) {
              ...
              if (closestState != NULL && !isAlreadyInKillState(nextPlayer)) {
                ...
                char enteredState = addKillState(
                    nextPlayer, getLiveObject(closestState->targetID), true);
                if (enteredState) {
                  nextPlayer->emotFrozen = true;
                  nextPlayer->emotFrozenIndex = killEmotionIndex;
            ...
            if (nextPlayer->isEve && nextPlayer->name == NULL) {
              ...
              if (name != NULL && strcmp(name, "") != 0) {
                ...
                replaceNameInSaidPhrase(name, &(m.saidText), nextPlayer, true);
                ...
                if (strstr(m.saidText, "EVE EVE") != NULL) {
                  ...
                  char found;
                  char *fixed =
                      replaceOnce(m.saidText, "EVE EVE", "EVE", &found);
                  delete[] m.saidText;
                  m.saidText = fixed;
                }
              }
            ...
            if (isYouFollowSay(m.saidText)) {
              ...
              if (namedPlayer != NULL) {
                ...
                otherToFollow = getPlayerByName(namedPlayer, nextPlayer);
            ...
            if (otherToFollow != NULL) {
              ...
              } else if (nextPlayer->followingID != otherToFollow->id) {
                nextPlayer->followingID = otherToFollow->id;
                nextPlayer->followingUpdate = true;

                if (otherToFollow->leadingColorIndex == -1) {
                  otherToFollow->leadingColorIndex = getUnusedLeadershipColor();
                }
              ...
              if (isYouExileSay(m.saidText)) {
                otherToExile = getClosestOtherPlayer(nextPlayer);
              } else {
                char *namedPlayer = isNamedExileSay(m.saidText);

                if (namedPlayer != NULL) {
                  otherToExile = getPlayerByName(namedPlayer, nextPlayer);
                }
              }

              if (otherToExile != NULL) {
                if (otherToExile->exiledByIDs.getElementIndex(nextPlayer->id) ==
                    -1) {
                  otherToExile->exiledByIDs.push_back(nextPlayer->id);

                  otherToExile->exileUpdate = true;
                }
              } else {
                if (isYouRedeemSay(m.saidText)) {
                  otherToRedeem = getClosestOtherPlayer(nextPlayer);
                } else {
                  char *namedPlayer = isNamedRedeemSay(m.saidText);

                  if (namedPlayer != NULL) {
                    otherToRedeem = getPlayerByName(namedPlayer, nextPlayer);
                  }
                }
                ...
                if (otherToRedeem != NULL) {
                  ...
                  for (int e = 0; e < otherToRedeem->exiledByIDs.size(); e++) {

                    // for
                    LiveObject *exiler = getLiveObject(
                        otherToRedeem->exiledByIDs.getElementDirect(e));

                    if (exiler == nextPlayer) {

                      otherToRedeem->exiledByIDs.deleteElement(e);
                      e--;
                      otherToRedeem->exileUpdate = true;
                    } else if (exiler != NULL &&
                               isFollower(nextPlayer, exiler)) {
                      otherToRedeem->exiledByIDs.deleteElement(e);
                      e--;
                      otherToRedeem->exileUpdate = true;
                    }
            ...
            if (strstr(m.saidText, orderPhrase) == m.saidText) {
              ...
              char *order = trimWhitespace(&(m.saidText[strlen(orderPhrase)]));

              char *leadershipName = getLeadershipName(nextPlayer);

              if (leadershipName != NULL) {
                // they are a leader

                char *formattedOrder = autoSprintf("ORDER FROM YOUR %s:**%s",
                                                   leadershipName, order);

                delete[] leadershipName;

                // originated with this player
                replaceOrder(nextPlayer, formattedOrder, nextOrderNumber,
                             nextPlayer->id);
            ...
            if (nextPlayer->holdingID > 0 &&
                getObject(nextPlayer->holdingID)->deadlyDistance > 0) {
              // are they speaking intent to kill?

              LiveObject *otherToKill = NULL;

              if (isYouKillSay(m.saidText)) {
                otherToKill = getClosestOtherPlayer(nextPlayer);
              } else {
                char *namedPlayer = isNamedKillSay(m.saidText);

                if (namedPlayer != NULL) {
                  otherToKill = getPlayerByName(namedPlayer, nextPlayer);
                  delete[] namedPlayer;
                }
              }
            ...
            if (nextPlayer->holdingID < 0) {
              ...
              LiveObject *babyO = getLiveObject(-nextPlayer->holdingID);
              ...
              if (babyO != NULL && babyO->name == NULL) {
                ...
                if (name != NULL && strcmp(name, "") != 0) {
                  ...
                  replaceNameInSaidPhrase(name, &(m.saidText), babyO);
                }
              }
            } else {
              // not holding anyone
              ...
              if (name != NULL && strcmp(name, "") != 0) {
                ...
                LiveObject *closestOther =
                    getClosestOtherPlayer(nextPlayer, babyAge, true);
                  ...
                  if (closestOther->isEve) {
                    ...
                    if (name != NULL && strcmp(name, "") != 0) {
                      ...
                      playerIndicesToSendNamesAbout.push_back(
                          getLiveObjectIndex(closestOther->id));

                      replaceNameInSaidPhrase(name, &(m.saidText), closestOther,
                                              true);
                    ...
                    // non-Eve
                    nameBaby(nextPlayer, closestOther, name,
                             &playerIndicesToSendNamesAbout);
              ...
              if (nextPlayer->holdingID > 0 && len < MAP_METADATA_LENGTH &&
                  getObject(nextPlayer->holdingID)->writable &&
                  // and no metadata already on it
                  !getMetadata(nextPlayer->holdingID, metaData)) {

                char *textToAdd = NULL;

                if (strstr(getObject(nextPlayer->holdingID)->description,
                           "+map") != NULL) {
                  ...
                  textToAdd =
                      autoSprintf("%s *map %d %d", m.saidText, p.x, p.y);

                  if (strlen(textToAdd) >= MAP_METADATA_LENGTH) {
                ...
                memcpy(metaData, textToAdd, strlen(textToAdd) + 1);

                delete[] textToAdd;

                nextPlayer->holdingID =
                    addMetadata(nextPlayer->holdingID, metaData);
                ...
                if (writingHappenTrans != NULL &&
                    ...
                    getObject(writingHappenTrans->newTarget)->written) {
            ...
            GridPos newGroundObjectOrigin = {nextPlayer->heldGraveOriginX,
            ...
            if (isBiomeAllowedForPlayer(nextPlayer, m.x, m.y))
              if (distanceUseAllowed ||
                  isGridAdjacent(m.x, m.y, nextPlayer->xd, nextPlayer->yd) ||
                ...
                if (accessBlocked) {
                  ...
                  if (targetObj->permanent && targetObj->written) {
                  ...
                  if ( // if what we're holding contains
                    ...
                    r = getPTrans(nextPlayer->holdingID, target);

                    ObjectRecord *heldObj = getObject(nextPlayer->holdingID);

                    if (r != NULL && r->newActor == 0 && r->newTarget > 0) {
                      ObjectRecord *newTargetObj = getObject(r->newTarget);

                      if (targetObj->numSlots == 0 &&
                          newTargetObj->numSlots >= heldObj->numSlots &&
                          newTargetObj->slotSize >= heldObj->slotSize) {
                  ...
                  if (nextPlayer->holdingID >= 0 && heldCanBeUsed) {
                  ...
                  if (nextPlayer->holdingID > 0 && r != NULL &&
                      r->newActor != 0) {
                    ...
                    if (targetObj->numSlots > 0 && heldO->containable &&
                        targetObj->slotSize >= heldO->containSize &&
                    ...
                    if (!insertion && r->newTarget > 0) {

                      int hCost = 0;
                      hungBlocked =
                          isHungryWorkBlocked(nextPlayer, r->newTarget, &hCost);
                    }

                    if (!insertion && !hungBlocked &&
                  ...
                  if (!blockedTool && target > 0 && r != NULL) {
                    ...
                    if (getObject(target)->permanent) {
                      ...
                      if (r->newTarget > 0 &&
                          getObject(r->newTarget)->permanent) {
                        couldBeTool = true;
                      }
                    }
                    ...
                    if (couldBeTool &&
                        !canPlayerUseOrLearnTool(nextPlayer, target)) {
                      r = NULL;
                      blockedTool = true;
                    }
                  }
                  ...
                  if (r != NULL && targetObj->numSlots > 0) {
                    ...
                    if (r->newTarget > 0) {
                      numSlotsInNew = getObject(r->newTarget)->numSlots;
                    }

                    if (numContained > numSlotsInNew && numSlotsInNew == 0) {
                      // not enough room in new target

                      // check if new actor will contain
                      // them (reverse containment transfer)

                      if (r->newActor > 0 && nextPlayer->numContained == 0) {
                        // old actor empty

                        int numSlotsNewActor = getObject(r->newActor)->numSlots;
                  ...
                  if (r == NULL &&
                      (nextPlayer->holdingID != 0 || targetObj->permanent) &&
                      (isGridAdjacent(m.x, m.y, nextPlayer->xd,
                                      nextPlayer->yd) ||
                       (m.x == nextPlayer->xd && m.y == nextPlayer->yd))) {
                  ...
                  if (r != NULL && r->newTarget > 0 && r->newTarget != target) {
                    ...
                    if (getMapFloor(m.x, m.y) != 0) {
                      ...
                      // to exist on floor
                      if (strstr(newTargetObj->description, "groundOnly") !=
                          NULL) {
                        r = NULL;
                      }
                    }
                    if (newTargetObj->isBiomeLimited &&
                        !canBuildInBiome(newTargetObj, getMapBiome(m.x, m.y))) {
                  ...
                  if (r == NULL && nextPlayer->holdingID > 0 && !blockedTool) {

                    logTransitionFailure(nextPlayer->holdingID, target);
                  }
                  ...
                  if (r != NULL && r->newTarget > 0) {
                    ...
                    if (isHungryWorkBlocked(nextPlayer, r->newTarget,
                                            &hungryWorkCost)) {
                      r = NULL;
                    }
                  }
                  ...
                  if (r != NULL && containmentTransfer) {
                    ...
                    transferHeldContainedToMap(nextPlayer, m.x, m.y);
                    handleHoldingChange(nextPlayer, r->newActor);

                    setHeldGraveOrigin(nextPlayer, m.x, m.y, r->newTarget);
                  } else if (r != NULL &&
                             ...
                             (r->newTarget == 0 ||
                              ...
                              isMapSpotEmptyOfPlayers(m.x, m.y))) {
                    ...
                    if (!defaultTrans) {
                      ...
                      if (r->target > 0) {
                        nextPlayer->heldTransitionSourceID = r->target;
                      } else {
                    ...
                    int oldSlots = getNumContainerSlots(target);
                    int newSlots = getNumContainerSlots(r->newTarget);

                    if (oldSlots > 0 && newSlots == 0 && r->actor == 0 &&
                        r->newActor > 0 &&
                      ...
                      FullMapContained f = getFullMapContained(m.x, m.y);
                      ...
                      restretchDecays(nextPlayer->numContained,
                                      nextPlayer->containedEtaDecays, target,
                                      r->newActor);
                    } else {
                      ...
                      if (newSlots > 0) {
                        restretchMapContainedDecays(m.x, m.y, target,
                                                    r->newTarget);
                      }
                    }

                    timeSec_t oldEtaDecay = getEtaDecay(m.x, m.y);
                    ...
                    if (r->newTarget > 0 && getObject(r->newTarget)->floor) {
                      ...
                      if (r->newTarget == target) {
                        ...
                        setFloorEtaDecay(m.x, m.y, oldEtaDecay);
                      }
                    } else {
                      setMapObject(m.x, m.y, r->newTarget);
                      newGroundObject = r->newTarget;
                    ...
                    if (hungryWorkCost > 0) {
                      if (nextPlayer->yummyBonusStore > 0) {
                        if (nextPlayer->yummyBonusStore >= hungryWorkCost) {
                          nextPlayer->yummyBonusStore -= hungryWorkCost;
                          hungryWorkCost = 0;
                        } else {
                          hungryWorkCost -= nextPlayer->yummyBonusStore;
                          nextPlayer->yummyBonusStore = 0;
                        }
                      }
                    ...
                    if (target > 0 && r->newTarget > 0 &&
                        target != r->newTarget && !getObject(target)->isOwned &&
                      ...
                      nextPlayer->ownedPositions.push_back(newPos);
                      newOwnerPos.push_back(newPos);
                    }

                    if (r->actor == 0 && target > 0 && r->newTarget > 0 &&
                        target != r->newTarget) {

                      TransRecord *oldDecayTrans = getTrans(-1, target);

                      TransRecord *newDecayTrans = getTrans(-1, r->newTarget);
                      ...
                      if (oldDecayTrans != NULL && newDecayTrans != NULL &&
                          ...
                          oldDecayTrans->autoDecaySeconds > 0) {
                    ...
                    if (r->newTarget != 0) {
                      ...
                      handleMapChangeToPaths(m.x, m.y, getObject(r->newTarget),
                                             &playerIndicesToSendUpdatesAbout);
                    }
                  } else if (nextPlayer->holdingID == 0 &&
                             !targetObj->permanent &&
                             canPickup(targetObj->id, computeAge(nextPlayer))) {
                    ...
                    // container
                    removeFromContainerToHold(nextPlayer, m.x, m.y, m.i);
                  } else if (nextPlayer->holdingID > 0) {
                    // try adding what we're holding to
                    // target container

                    addHeldToContainer(nextPlayer, target, m.x, m.y);
                  }

                  if (targetObj->permanent && targetObj->foodValue > 0) {

                    // just touching this object
                    // causes us to eat from it

                    nextPlayer->justAte = true;
                    nextPlayer->justAteID = targetObj->id;

                    nextPlayer->lastAteID = targetObj->id;
                    nextPlayer->lastAteFillMax = nextPlayer->foodStore;

                    nextPlayer->foodStore +=
                        lrint(foodScaleFactor * targetObj->foodValue);

                    updateYum(nextPlayer, targetObj->id);

                    logEating(targetObj->id, targetObj->foodValue + eatBonus,
                              computeAge(nextPlayer), m.x, m.y);
                    ...
                    if (nextPlayer->foodStore > cap) {
                      ...
                      int overflowCap = computeOverflowFoodCapacity(cap);
                    ...
                    if (oldHolding == 0 && nextPlayer->holdingID != 0) {
                      ...
                      ObjectRecord *newHeld = getObject(nextPlayer->holdingID);

                      if (newHeld->foodValue > 0) {
                        nextPlayer->foodStore -= newHeld->foodValue;

                        if (nextPlayer->lastAteFillMax >
                            nextPlayer->foodStore) {

                          nextPlayer->foodStore = nextPlayer->lastAteFillMax;
                        }
                      }
                    }

                    if (targetObj->alcohol != 0) {
                      drinkAlcohol(nextPlayer, targetObj->alcohol);
                    }
                  ...
                  if (floorID > 0) {
                    ...
                    TransRecord *r = getPTrans(nextPlayer->holdingID, floorID);
                    ...
                    if (nextPlayer->holdingID > 0 && r != NULL) {
                      ...
                      if (!canPlayerUseOrLearnTool(nextPlayer,
                                                   nextPlayer->holdingID)) {
                        r = NULL;
                        blockedTool = true;
                      }
                    ...
                    if (r != NULL) {
                      if (!canPlayerUseOrLearnTool(nextPlayer, floorID)) {
                        r = NULL;
                        blockedTool = true;
                      }
                    }
                    ...
                    if (r == NULL && !blockedTool) {
                      logTransitionFailure(nextPlayer->holdingID, floorID);
                    }
                    ...
                    if (r != NULL &&
                        ...
                        // transition
                        (r->newActor == 0 ||
                         canPickup(r->newActor, computeAge(nextPlayer)))) {
                      ...
                      if (getObject(resultID)->floor) {
                        ...
                        if (resultID != floorID) {
                          setMapFloor(m.x, m.y, resultID);
                        }
                        handleHoldingChange(nextPlayer, r->newActor);

                        setHeldGraveOrigin(nextPlayer, m.x, m.y, resultID);
                      } else {
                  ...
                  ObjectRecord *obj = getObject(nextPlayer->holdingID);
                  ...
                  if (!usedOnFloor && obj->foodValue == 0 &&
                    ...
                    //   value is 0)
                    TransRecord *r = getPTrans(nextPlayer->holdingID, -1);
                    ...
                    if (r != NULL && r->newTarget != 0 &&
                      ...
                      if (newTargetObj->blocksWalking &&
                        ...
                        // object
                        canPlace = false;
                      } else if (strstr(newTargetObj->description,
                                        "groundOnly") != NULL &&
                    ...
                    if (canPlace) {
                      ...
                      if (nextPlayer->numContained > 0 && r->newActor == 0 &&
                          r->newTarget > 0 &&
                ...
                if (target == 0 && newGroundObject > 0) {
                  ...
                  if (strstr(o->description, "origGrave") != NULL) {
                    ...
                    setGravePlayerID(m.x, m.y, heldGravePlayerID);

                    int swapDest = isGraveSwapDest(m.x, m.y, nextPlayer->id);

                    GraveMoveInfo g = {
                        {newGroundObjectOrigin.x, newGroundObjectOrigin.y},
                        {m.x, m.y},
                        swapDest};
                    newGraveMoves.push_back(g);
                  }
            ...
            if (computeAge(nextPlayer) >= minPickupBabyAge &&
              ...
              if (nextPlayer->holdingID == 0) {
                ...
                LiveObject *hitPlayer =
                    getHitPlayer(m.x, m.y, m.id, false, babyAge);

                if (hitPlayer != NULL && !hitPlayer->heldByOther &&
                  ...
                  if (hitPlayer->heldByOtherID == hitPlayer->parentID) {
                  ...
                  if (isFertileAge(nextPlayer)) {
                    ...
                    hitPlayer->foodStore = computeFoodCapacity(hitPlayer);

                    hitPlayer->foodUpdate = true;
                    hitPlayer->responsiblePlayerID = nextPlayer->id;

                    // reset their food decrement time
                    hitPlayer->foodDecrementETASeconds =
                        Time::getCurrentTime() +
                        computeFoodDecrementTimeSeconds(hitPlayer);
                    ...
                    if (nextPlayer->yummyBonusStore > 0) {
                      nextPlayer->yummyBonusStore -= nurseCost;
                      nurseCost = 0;
                      if (nextPlayer->yummyBonusStore < 0) {
            ...
            if (nextPlayer->holdingID < 0) {
              ...
              if (isGridAdjacent(m.x, m.y, nextPlayer->xd, nextPlayer->yd) ||
                  (m.x == nextPlayer->xd && m.y == nextPlayer->yd)) {
                ...
                int hitIndex;
                LiveObject *hitPlayer =
                    getHitPlayer(m.x, m.y, m.id, false, babyAge, -1, &hitIndex);
                ...
                if (hitPlayer == NULL || hitPlayer == nextPlayer) {
                  // try click on elderly
                  hitPlayer =
                      getHitPlayer(m.x, m.y, m.id, false, -1, 55, &hitIndex);
                }
                ...
                if ((hitPlayer == NULL || hitPlayer == nextPlayer) &&
                  ...
                  hitPlayer =
                      getHitPlayer(m.x, m.y, m.id, false, -1, -1, &hitIndex);
                }
                ...
                if ((hitPlayer == NULL || hitPlayer == nextPlayer) &&
                  ...
                  hitPlayer =
                      getHitPlayer(m.x, m.y, m.id, false, -1, -1, &hitIndex);
                  if (hitPlayer != NULL && !hitPlayer->dying) {
                    hitPlayer = NULL;
                  }
                }
                ...
                if (hitPlayer != NULL && hitPlayer != nextPlayer) {
                  ...
                  playerIndicesToSendUpdatesAbout.push_back(hitIndex);

                  targetPlayer->responsiblePlayerID = nextPlayer->id;
                }
              }
            }
            ...
            if (targetPlayer != NULL) {
              ...
              if (targetPlayer != nextPlayer && targetPlayer->dying &&
                  !holdingFood) {

                // try healing wound

                TransRecord *healTrans = getMetaTrans(nextPlayer->holdingID,
                ...
                if (healTrans != NULL) {
                  ...
                  if (healerWillHold > 0) {
                    if (!canPickup(healerWillHold, computeAge(nextPlayer))) {
                      oldEnough = false;
                    }
                  }
                }
                ...
                if (oldEnough && healTrans != NULL) {
                  targetPlayer->holdingID = healTrans->newTarget;
                  ...
                  nextPlayer->holdingID = healTrans->newActor;
                  holdingSomethingNew(nextPlayer);

                  setFreshEtaDecayForHeld(nextPlayer);
                  setFreshEtaDecayForHeld(targetPlayer);

                  nextPlayer->heldOriginValid = 0;
                  nextPlayer->heldOriginX = 0;
                  nextPlayer->heldOriginY = 0;
                  nextPlayer->heldTransitionSourceID = healTrans->target;

                  targetPlayer->heldOriginValid = 0;
                  targetPlayer->heldOriginX = 0;
                  targetPlayer->heldOriginY = 0;
                  targetPlayer->heldTransitionSourceID = -1;

                  if (targetPlayer->holdingID == 0) {
                    // not dying anymore
                    setNoLongerDying(targetPlayer,
                                     &playerIndicesToSendHealingAbout);
                  } else {
                    // wound changed?

                    ForcedEffects e =
                        checkForForcedEffects(targetPlayer->holdingID);

                    if (e.emotIndex != -1) {
                      targetPlayer->emotFrozen = true;
                      targetPlayer->emotFrozenIndex = e.emotIndex;
                      newEmotPlayerIDs.push_back(targetPlayer->id);
                      newEmotIndices.push_back(e.emotIndex);
                      newEmotTTLs.push_back(e.ttlSec);
                      interruptAnyKillEmots(targetPlayer->id, e.ttlSec);
                    }
                    if (e.foodCapModifier != 1) {
                      targetPlayer->yummyBonusStore = 0;
                      targetPlayer->foodCapModifier = e.foodCapModifier;
                ...
                if (m.i >= 0 && m.i < NUM_CLOTHING_PIECES) {
                  clickedClothing = clothingByIndex(nextPlayer->clothing, m.i);

                  if (clickedClothing != NULL) {

                    clickedClothingTrans =
                        getPTrans(nextPlayer->holdingID, clickedClothing->id);

                    if (clickedClothingTrans != NULL) {
                      int na = clickedClothingTrans->newActor;
                      ...
                      int nt = clickedClothingTrans->newTarget;
                ...
                if (targetPlayer == nextPlayer && m.i >= 0 &&
                    m.i < NUM_CLOTHING_PIECES &&
                    addHeldToClothingContainer(nextPlayer, m.i, false,
                                               &couldHaveGoneIn)) {
                  // worked!
                }
                // next case:  can what they're holding
                // be used to transform clothing?
                else if (m.i >= 0 && m.i < NUM_CLOTHING_PIECES &&
                  ...
                  setClothingByIndex(
                      &(nextPlayer->clothing), m.i,
                      getObject(clickedClothingTrans->newTarget));
                }
                // next case, holding food
                // that couldn't be put into clicked clothing
                else if (obj->foodValue > 0 && targetPlayer->foodStore < cap &&
                         !couldHaveGoneIn) {

                  targetPlayer->justAte = true;
                  targetPlayer->justAteID = nextPlayer->holdingID;

                  targetPlayer->lastAteID = nextPlayer->holdingID;
                  targetPlayer->lastAteFillMax = targetPlayer->foodStore;

                  targetPlayer->foodStore +=
                      lrint(foodScaleFactor * obj->foodValue);

                  updateYum(targetPlayer, obj->id, targetPlayer == nextPlayer);

                  logEating(obj->id, obj->foodValue + eatBonus,
                            computeAge(targetPlayer), m.x, m.y);

                  targetPlayer->foodStore += eatBonus;

                  if (targetPlayer->foodStore > cap) {
                    int over = targetPlayer->foodStore - cap;
                  ...
                  targetPlayer->foodDecrementETASeconds =
                      Time::getCurrentTime() +
                      computeFoodDecrementTimeSeconds(targetPlayer);
                  ...
                  if (obj->alcohol != 0) {
                    drinkAlcohol(targetPlayer, obj->alcohol);
                  }
                ...
                // we could put on
                         ...
                         (targetPlayer == nextPlayer ||
                          computeAge(targetPlayer) < babyAge)) {
                  ...
                  case 's':
                    if (targetPlayer->clothing.backShoe == NULL) {
                    ...
                    } else if (targetPlayer->clothing.frontShoe == NULL) {
                      ...
                      if (targetPlayer->clothing.backShoe == obj) {

                        clothingSlot = &(targetPlayer->clothing.frontShoe);
                        clothingSlotIndex = 2;
                      } else if (targetPlayer->clothing.frontShoe == obj) {
                        clothingSlot = &(targetPlayer->clothing.backShoe);
                        clothingSlotIndex = 3;
                      } else {
                        // both shoes are
                  ...
                  if (clothingSlot != NULL) {
                    ...
                    oldCEtaDecay = targetPlayer->clothingEtaDecay[ind];

                    oldNumContained =
                        targetPlayer->clothingContained[ind].size();

                    if (oldNumContained > 0) {
                      oldContainedIDs = targetPlayer->clothingContained[ind]
                                            .getElementArray();
                    ...
                    *clothingSlot = obj;
                    targetPlayer->clothingEtaDecay[ind] = oldEtaDecay;
                    ...
                    if (nextPlayer->numContained > 0) {
                      ...
                      targetPlayer->clothingContained[ind].appendArray(
                          nextPlayer->containedIDs, nextPlayer->numContained);

                      targetPlayer->clothingContainedEtaDecays[ind].appendArray(
                          nextPlayer->containedEtaDecays,
                          nextPlayer->numContained);
                      ...
                      clearPlayerHeldContained(nextPlayer);
                    }

                    if (oldC != NULL) {
                      nextPlayer->holdingID = oldC->id;
                      holdingSomethingNew(nextPlayer);

                      nextPlayer->holdingEtaDecay = oldCEtaDecay;

                      nextPlayer->numContained = oldNumContained;

                      freePlayerContainedArrays(nextPlayer);

                      nextPlayer->containedIDs = oldContainedIDs;
                      nextPlayer->containedEtaDecays = oldContainedETADecays;
                ...
                TransRecord *bareHandClothingTrans =
                    getBareHandClothingTrans(nextPlayer, clothingSlot);
                ...
                if (targetPlayer == nextPlayer &&
                  ...
                  nextPlayer->holdingID = bareHandClothingTrans->newActor;
                  ...
                  if (bareHandClothingTrans->newTarget > 0) {
                    *clothingSlot = getObject(bareHandClothingTrans->newTarget);
                  } else {
                    *clothingSlot = NULL;
                  ...
                  removeClothingToHold(nextPlayer, targetPlayer, clothingSlot,
                                       clothingSlotIndex);
                }
              }
            }
            ...
            char accessBlocked = isAccessBlocked(nextPlayer, m.x, m.y, target);
              ...
              if (isBiomeAllowedForPlayer(nextPlayer, m.x, m.y))
                if ((isGridAdjacent(m.x, m.y, nextPlayer->xd, nextPlayer->yd) ||
                  ...
                  if (nextPlayer->holdingID != 0) {
                    ...
                    if (nextPlayer->holdingID < 0) {
                    ...
                    } else if (canDrop && isMapSpotEmpty(m.x, m.y)) {

                      // empty spot to drop non-baby into

                      handleDrop(m.x, m.y, nextPlayer,
                                 &playerIndicesToSendUpdatesAbout);
                    } else if (canDrop && m.c >= 0 &&
                      ...
                      // if possible
                      addHeldToClothingContainer(nextPlayer, m.c, true);
                      if (nextPlayer->holdingID == 0) {
                        // add to top worked
                        ...
                        for (int s = 0;
                          ...
                          int otherID = nextPlayer->clothingContained[m.c]
                                            .getElementDirect(s);

                          if (otherID != oldHeld &&
                              canPickup(otherID, playerAge)) {

                            removeFromClothingContainerToHold(nextPlayer, m.c,
                                                              s);
                            break;
                          }
                        }
                        ...
                        ObjectRecord *cObj =
                            clothingByIndex(nextPlayer->clothing, m.c);
                        if (nextPlayer->clothingContained[m.c].size() >
                            cObj->numSlots) {
                          ...
                          removeFromClothingContainerToHold(
                              nextPlayer, m.c,
                              nextPlayer->clothingContained[m.c].size() - 1);
                        }
                      }
                      ...
                      if (target != 0) {
                        ...
                        if (!canDrop) {
                          ...
                          if (!targetObj->permanent &&
                            ...
                            // trans exists?
                            TransRecord *r =
                                getPTrans(nextPlayer->holdingID, -1);
                        ...
                        int targetSlots = targetObj->numSlots;

                        float targetSlotSize = 0;

                        if (targetSlots > 0) {
                          targetSlotSize = targetObj->slotSize;
                        }

                        char canGoIn = false;

                        if (canDrop && droppedObj->containable &&
                            targetSlotSize >= droppedObj->containSize) {
                          canGoIn = true;
                        }
                        ...
                        if (canDrop && canGoIn &&
                          ...
                          TransRecord *useTrans =
                              getPTrans(nextPlayer->holdingID, target);
                          // handle simple case
                          // stacking containers
                          // client sends DROP for this
                          if (useTrans != NULL && useTrans->newActor == 0) {
                            ...
                            if (useTrans->newTarget > 0) {
                              newTargetObj = getObject(useTrans->newTarget);
                            }
                            ...
                            if (canUse) {
                              ...
                              setMapObject(m.x, m.y, useTrans->newTarget);
                            }
                          }
                        } else if (canDrop && !canGoIn &&
                                   !targetObj->permanent &&
                          ...
                          int oldHeld = nextPlayer->holdingID;
                          int oldNumContained = nextPlayer->numContained;

                          // now swap
                          swapHeldWithGround(nextPlayer, target, m.x, m.y,
                                             &playerIndicesToSendUpdatesAbout);

                          if (oldHeld == nextPlayer->holdingID &&
                              oldNumContained == nextPlayer->numContained) {
                            // no change
                            // are they the same object?
                            if (oldNumContained == 0 && oldHeld == target) {
                              // try using empty held
                              // on target
                              TransRecord *sameTrans =
                                  getPTrans(oldHeld, target);
                              if (sameTrans != NULL &&
                                  sameTrans->newActor == 0) {
                                // keep it simple
                                // for now
                                // this is usually
                                // just about
                                ...
                                setMapObject(m.x, m.y, sameTrans->newTarget);
                              }
                            }
                          }
                        }
            ...
            if (isBiomeAllowedForPlayer(nextPlayer, m.x, m.y))
              if (isGridAdjacent(m.x, m.y, nextPlayer->xd, nextPlayer->yd) ||
                ...
                if (!accessBlocked)
                  if (handEmpty && nextPlayer->holdingID == 0) {
                    // hand still empty?
                    ...
                    if (target > 0) {
                      ...
                      if (!targetObj->permanent &&
                        ...
                        TransRecord *handTrans = getPTrans(0, target);
                        ...
                        //  use parent)
                        if (handTrans != NULL && handTrans->newTarget > 0 &&
                            getObject(handTrans->newTarget)->numSlots ==
                                targetObj->numSlots &&
                            handTrans->newActor > 0 &&
                            canPickup(handTrans->newActor,
                                      computeAge(nextPlayer))) {

                          handleHoldingChange(nextPlayer, handTrans->newActor);
                          setMapObject(m.x, m.y, handTrans->newTarget);
                        }
                      }
                    }
                  }
            ...
            if (m.x == nextPlayer->xd && m.y == nextPlayer->yd &&
              ...
              if (m.c >= 0 && m.c < NUM_CLOTHING_PIECES) {
                worked =
                    removeFromClothingContainerToHold(nextPlayer, m.c, m.i);
              }
            }

            if (nextPlayer->holdingID == 0 && m.c >= 0 &&
                m.c < NUM_CLOTHING_PIECES && !worked) {
              ...
              ObjectRecord **clothingSlot = getClothingSlot(nextPlayer, m.c);
              ...
              if (bareHandClothingTrans != NULL) {
                ...
                removeClothingToHold(nextPlayer, nextPlayer, clothingSlot, m.c);
              }
            }
          } else if (m.type == EMOT && !nextPlayer->emotFrozen) {
            // ignore new EMOT requres from player if emot
            // frozen

            if (m.i <= SettingsManager::getIntSetting("allowedEmotRange", 6)) {

              SimpleVector<int> *forbidden =
                  SettingsManager::getIntSettingMulti("forbiddenEmots");
              ...
              if (forbidden->getElementIndex(m.i) == -1) {
                ...
                if (nextPlayer->emoteCooldown) {
                  if (curTime - nextPlayer->emoteCooldownStartTimeSeconds >
                      emoteCooldownSeconds) {
                    // cooldown over
                    nextPlayer->emoteCooldown = false;
                    nextPlayer->firstEmoteTimeSeconds = curTime;
                ...
                if (!cooldown) {
                  ...
                  if (curTime - nextPlayer->firstEmoteTimeSeconds >
                      emoteWindowSeconds) {
                    // window expired
                    // start a new one
                    nextPlayer->firstEmoteTimeSeconds = curTime;
                    ...
                    if (nextPlayer->emoteCountInWindow > maxEmotesInWindow) {
                      // put 'em on cooldown
                      nextPlayer->emoteCooldown = true;
                      nextPlayer->emoteCooldownStartTimeSeconds = curTime;
                    }
                  }
                }
              }
    ...
    for (int i = 0; i < activeKillStates.size(); i++) {
      ...
      if (killer == NULL || target == NULL || killer->error || target->error ||
          killer->holdingID != s->killerWeaponID || target->heldByOther) {
      ...
      if (curTime - s->killStartTime > killDelayTime &&
        ...
        executeKillAction(
            getLiveObjectIndex(s->killerID), getLiveObjectIndex(s->targetID),
            &playerIndicesToSendUpdatesAbout, &playerIndicesToSendDyingAbout,
            &newEmotPlayerIDs, &newEmotIndices, &newEmotTTLs);
      } else {
    ...
    for (int i = 0; i < numLive; i++) {
      ...
      if (nextPlayer->emotFrozen && nextPlayer->emotUnfreezeETA != 0 &&
      ...
      if (nextPlayer->dying && !nextPlayer->error &&
        ...
        if (!nextPlayer->isTutorial) {
          GridPos deathPos = getPlayerPos(nextPlayer);
          logDeath(nextPlayer->id, nextPlayer->email, nextPlayer->isEve,
                   computeAge(nextPlayer), getSecondsPlayed(nextPlayer),
                   !getFemale(nextPlayer), deathPos.x, deathPos.y,
                   players.size() - 1, false, nextPlayer->murderPerpID,
                   nextPlayer->murderPerpEmail);

          if (shutdownMode) {
            handleShutdownDeath(nextPlayer, nextPlayer->xd, nextPlayer->yd);
          }
        }
      ...
      if (nextPlayer->isNew) {
        ...
        // demise
        if (!nextPlayer->isTutorial &&
            nextPlayer->curseStatus.curseLevel == 0 && !isEveWindow()) {
          int minFamiliesAfterEveWindow =
              SettingsManager::getIntSetting("minFamiliesAfterEveWindow", 5);
          if (minFamiliesAfterEveWindow > 0) {
            ...
            if (nextPlayer->familyName != NULL) {
              ...
              for (int n = 0; n < players.size(); n++) {
                LiveObject *otherPlayer = players.getElement(n);
                ...
                if (otherPlayer->lineageEveID == nextPlayer->lineageEveID) {
              ...
              if (otherCount == 0) {
                ...
                char *message = autoSprintf("%s FAMILY JUST DIED OUT**"
                                            "%d %s LEFT "
                                            "(ARC ENDS BELOW %d)",
                                            nextPlayer->familyName, cFam,
                                            famWord, minFamiliesAfterEveWindow);
        ...
        if (nextPlayer->heldByOther) {
          ...
          handleForcedBabyDrop(nextPlayer, &playerIndicesToSendUpdatesAbout);
        } else if (nextPlayer->holdingID < 0) {
          LiveObject *babyO = getLiveObject(-nextPlayer->holdingID);

          handleForcedBabyDrop(babyO, &playerIndicesToSendUpdatesAbout);
        }

        newDeleteUpdates.push_back(getUpdateRecord(nextPlayer, true));
        ...
        if (nextPlayer->xd == nextPlayer->xs &&
          ...
          dropPos = computePartialMoveSpot(nextPlayer);
        }
        ...
        if (!nextPlayer->isTutorial)
          recordPlayerLineage(nextPlayer->email, age, nextPlayer->id,
                              nextPlayer->parentID, nextPlayer->displayID,
                              killerID, nextPlayer->name, nextPlayer->lastSay,
                              male);

        // both tutorial and non-tutorial players
        logFitnessDeath(nextPlayer);

        if (SettingsManager::getIntSetting("babyApocalypsePossible", 1) &&
            players.size() > SettingsManager::getIntSetting(
          ...
          if (!nextPlayer->isEve) {
            ...
            if (barrierOn && (abs(dropPos.x) > barrierRadius ||
              ...
              if (lastBabyPassedThresholdTime > 0 &&
                  curTime - lastBabyPassedThresholdTime >
                      SettingsManager::getIntSetting(
                          "babySurvivalWindowSecondsBeforeApocalypse", 3600)) {
                // we're outside the window
                // people have been dying young for a long time

                triggerApocalypseNow("Everyone dying young for too long");
        ...
        double yearsLived = getSecondsPlayed(nextPlayer) * getAgeRate();

        if (!nextPlayer->isTutorial) {

          recordLineage(nextPlayer->email, nextPlayer->originalBirthPos,
                        yearsLived,
                        // count true murder victims here, not suicide
                        (killerID > 0 && killerID != nextPlayer->id),
                        // killed other or committed SID suicide
                        nextPlayer->everKilledAnyone || nextPlayer->suicide);

          if (nextPlayer->suicide) {
            // add to player's skip list
            skipFamily(nextPlayer->email, nextPlayer->lineageEveID);
          }
        }
        ...
        if (!nextPlayer->deathLogged) {
          ...
          if (!nextPlayer->isTutorial) {
            logDeath(nextPlayer->id, nextPlayer->email, nextPlayer->isEve, age,
                     getSecondsPlayed(nextPlayer), male, dropPos.x, dropPos.y,
                     players.size() - 1, disconnect);
        ...
        if (nextPlayer->email != NULL) {
          ...
          nextPlayer->origEmail = stringDuplicate(nextPlayer->email);
          delete[] nextPlayer->email;
        }
        ...
        if (deathID > 0) {
          deathMarkerHasSlots = (getObject(deathID)->numSlots > 0);
        }
        ...
        if (deathID != 0) {
          ...
          while (oldObject != 0 && n < 4) {
            ...
            if (!isGrave(oldObject)) {
              ...
              if (deathMarkerHasSlots && r->numSlots == 0 && !r->permanent &&
                  !r->rideable) {
        ...
        if (!isMapSpotEmpty(dropPos.x, dropPos.y, false)) {
          ...
          GridPos newDropPos =
              findClosestEmptyMapSpot(dropPos.x, dropPos.y, 100, &foundEmpty);
        ...
        if (isMapSpotEmpty(dropPos.x, dropPos.y, false)) {
          ...
          if (deathID > 0) {
            ...
            setResponsiblePlayer(-nextPlayer->id);
            setMapObject(dropPos.x, dropPos.y, deathID);
            setResponsiblePlayer(-1);
            ...
            setGravePlayerID(dropPos.x, dropPos.y, nextPlayer->id);
            ...
            if (roomLeft >= 1) {
              ...
              if (nextPlayer->embeddedWeaponID != 0) {
                addContained(dropPos.x, dropPos.y, nextPlayer->embeddedWeaponID,
            ...
            if (roomLeft >= 5) {
              ...
              if (nextPlayer->clothing.hat != NULL) {
                ...
                addContained(dropPos.x, dropPos.y, nextPlayer->clothing.hat->id,
            ...
            for (int c = 0; c < NUM_CLOTHING_PIECES && roomLeft > 0; c++) {

              float oldStretch = 1.0;

              ObjectRecord *cObj = clothingByIndex(nextPlayer->clothing, c);
              ...
              for (int cc = 0;
                   cc < nextPlayer->clothingContained[c].size() && roomLeft > 0;
                   cc++) {

                if (nextPlayer->clothingContainedEtaDecays[c].getElementDirect(
                        cc) != 0 &&
                    oldStretch != newStretch) {

                  timeSec_t offset = nextPlayer->clothingContainedEtaDecays[c]
                                         .getElementDirect(cc) -
                                     curTime;
                ...
                addContained(
                    ...
                    nextPlayer->clothingContainedEtaDecays[c].getElementDirect(
                        cc));
                roomLeft--;
              }
            }
        ...
        if (nextPlayer->holdingID != 0) {
          ...
          if (nextPlayer->murderSourceID > 0) {
            ...
            TransRecord *woundHit =
                getPTrans(nextPlayer->murderSourceID, 0, true, false);
          ...
          if (nextPlayer->holdingWound || nextPlayer->holdingBiomeSickness) {
          ...
          if (!doNotDrop) {
            ...
            handleDrop(dropPos.x, dropPos.y, nextPlayer,
                       &playerIndicesToSendUpdatesAbout);
          } else {
        ...
        if (nextPlayer->holdingEtaDecay != 0 &&
          ...
          if (t != NULL) {
            ...
            if (newID == 0 && nextPlayer->holdingWound && nextPlayer->dying) {

              // wound decayed naturally, count as healed
              setNoLongerDying(nextPlayer, &playerIndicesToSendHealingAbout);
            }
            ...
            if (newObj != NULL && newObj->permanent && oldObj != NULL &&
                !oldObj->permanent) {
              // object decayed into a permanent
              // force drop
              GridPos dropPos = getPlayerPos(nextPlayer);
        ...
        // has decayed
        if (nextPlayer->holdingID > 0 && nextPlayer->numContained > 0) {
          ...
          for (int c = 0; c < nextPlayer->numContained; c++) {
            ...
            timeSec_t newDecay = nextPlayer->containedEtaDecays[c];

            SimpleVector<int> subCont = nextPlayer->subContainedIDs[c];
            ...
            if (newDecay != 0 && newDecay < curTime) {
              ...
              if (t != NULL) {
                ...
                if (newID != 0) {
                  ...
                  if (newDecayT != NULL) {
                    newDecay =
                        Time::timeSec() + newDecayT->autoDecaySeconds / stretch;
                  } else {
                    // no further decay
                    newDecay = 0;
                  }
            ...
            if (newID != 0) {
              ...
              if (newID != oldID && newSlots < oldSlots) {
            ...
            for (int s = 0; s < subCont.size(); s++) {
              ...
              timeSec_t newSubDecay = subContDecay.getElementDirect(s);
              ...
              if (newSubDecay != 0 && newSubDecay < curTime) {
                ...
                if (t != NULL) {
                  ...
                  if (newSubID != 0) {
                    float subStretch = getObject(newID)->slotTimeStretch;

                    TransRecord *newSubDecayT = getMetaTrans(-1, newSubID);

                    if (newSubDecayT != NULL) {
                      newSubDecay = Time::timeSec() +
                                    newSubDecayT->autoDecaySeconds / subStretch;
                    } else {
                      // no further decay
                      newSubDecay = 0;
                    }
          ...
          if (change) {
            ...
            if (nextPlayer->numContained == 0) {
              ...
              nextPlayer->containedIDs = newContained.getElementArray();
              nextPlayer->containedEtaDecays =
                  newContainedETA.getElementArray();

              nextPlayer->subContainedIDs = newSubContained.getElementArray();
              nextPlayer->subContainedEtaDecays =
        ...
        for (int c = 0; c < NUM_CLOTHING_PIECES; c++) {
          ...
          if (cObj != NULL && nextPlayer->clothingEtaDecay[c] != 0 &&
              nextPlayer->clothingEtaDecay[c] < curTime) {
            ...
            if (t != NULL) {
              ...
              setClothingByIndex(&(nextPlayer->clothing), c, newCObj);

              int oldSlots = nextPlayer->clothingContained[c].size();
              ...
              if (newSlots < oldSlots) {
                ...
                for (int s = newSlots; s < oldSlots; s++) {
                  ...
                  if (getMapObject(dropPos.x, dropPos.y) == 0) {
                    ...
                    found = findDropSpot(dropPos.x, dropPos.y, dropPos.x,
                                         dropPos.y, &spot);
                  }
                  ...
                  if (found) {
                    ...
                    timeSec_t eta = nextPlayer->clothingContainedEtaDecays[c]
                                        .getElementDirect(s);

                    if (stretch != 1.0) {
                      timeSec_t offset = eta - curTime;
              ...
              float oldStretch = cObj->slotTimeStretch;
              float newStretch;
              ...
              if (oldStretch != newStretch) {
                ...
                for (int cc = 0;
                  ...
                  if (nextPlayer->clothingContainedEtaDecays[c]
                    ...
                    *(nextPlayer->clothingContainedEtaDecays[c].getElement(
                        cc)) = curTime + offset;
                  }
                }
              }
          ...
          if (cObj != NULL &&
            ...
            for (int cc = 0;
                 cc < nextPlayer->clothingContainedEtaDecays[c].size(); cc++) {
              ...
              if (decay != 0 && decay < curTime) {
                ...
                if (t != NULL) {
                  ...
                  if (newID != 0) {
                    ...
                    if (newDecayT != NULL) {
                      newDecay = Time::timeSec() + newDecayT->autoDecaySeconds /
            ...
            if (change) {
              ...
              nextPlayer->clothingContained[c] = newContained;
              nextPlayer->clothingContainedEtaDecays[c] = newContainedETA;
            }
          }
        }
        ...
        if (nextPlayer->xd != nextPlayer->xs ||
          ...
          if (!nextPlayer->newMove &&
            ...
            if (nextPlayer->holdingBiomeSickness) {
              int sicknessObjectID = getBiomeSickness(
                  nextPlayer->displayID, nextPlayer->xs, nextPlayer->ys);
              if (sicknessObjectID == -1) {
                endBiomeSickness(nextPlayer, i,
                                 &playerIndicesToSendUpdatesAbout);
              }
            ...
            if (nextPlayer->holdingFlightObject &&
                moveSpeed >= minFlightSpeed && !nextPlayer->pathTruncated &&
              ...
              double xDir =
                  nextPlayer->pathToDest[nextPlayer->pathLength - 1].x -
                  nextPlayer->pathToDest[nextPlayer->pathLength - 2].x;
              double yDir =
                  nextPlayer->pathToDest[nextPlayer->pathLength - 1].y -
                  nextPlayer->pathToDest[nextPlayer->pathLength - 2].y;

              int beyondEndX = nextPlayer->xs + xDir;
              int beyondEndY = nextPlayer->ys + yDir;

              int endFloorID = getMapFloor(nextPlayer->xs, nextPlayer->ys);

              int beyondEndFloorID = getMapFloor(beyondEndX, beyondEndY);
              ...
              if (beyondEndFloorID != endFloorID) {
                ...
                int barrierBlocksPlanes =
                    SettingsManager::getIntSetting("barrierBlocksPlanes", 1);
                ...
                if (Time::getCurrentTime() -
                  ...
                  destPos = getClosestLandingPos(nextPlayer->forceFlightDest,
                                                 &foundMap);

                  // find strip closest to last
                  // read map position
                  AppLog::infoF("Player %d flight taking off from (%d,%d), "
                                "map dest (%d,%d), found=%d, found (%d,%d)",
                                nextPlayer->id, nextPlayer->xs, nextPlayer->ys,
                                nextPlayer->forceFlightDest.x,
                                nextPlayer->forceFlightDest.y, foundMap,
                                destPos.x, destPos.y);
                }
                if (!foundMap) {
                  ...
                  destPos = getNextFlightLandingPos(
                      nextPlayer->xs, nextPlayer->ys, takeOffDir, radiusLimit);

                  AppLog::infoF("Player %d non-map flight taking off "
                                "from (%d,%d), "
                                "flightDir (%f,%f), dest (%d,%d)",
                                nextPlayer->id, nextPlayer->xs, nextPlayer->ys,
                                xDir, yDir, destPos.x, destPos.y);
                }
                ...
                int destID = getMapObject(destPos.x, destPos.y);

                char heldTransHappened = false;

                if (destID > 0 && getObject(destID)->isFlightLanding) {
                  // found a landing place
                  TransRecord *tr = getPTrans(nextPlayer->holdingID, destID);

                  if (tr != NULL) {
                    heldTransHappened = true;

                    setMapObject(destPos.x, destPos.y, tr->newTarget);

                    transferHeldContainedToMap(nextPlayer, destPos.x,
                                               destPos.y);

                    handleHoldingChange(nextPlayer, tr->newActor);
                ...
                if (!heldTransHappened) {
                  ...
                  TransRecord *decayTrans =
                      getPTrans(-1, nextPlayer->holdingID);

                  if (decayTrans != NULL) {
                    handleHoldingChange(nextPlayer, decayTrans->newTarget);
                  }
                }

                FlightDest fd = {nextPlayer->id, destPos};
        ...
        if (!nextPlayer->vogMode &&
            curTime > nextPlayer->foodDecrementETASeconds) {
          ...
          if (nextPlayer->heldByOther) {
            ...
            if (adultO != NULL && isFertileAge(adultO)) {
          ...
          nextPlayer->foodDecrementETASeconds =
              curTime + computeFoodDecrementTimeSeconds(nextPlayer);
          ...
          if (decrementedPlayer != NULL && decrementedPlayer->foodStore < 0) {
            // player has died

            // break the connection with them

            if (heldByFemale) {
              setDeathReason(decrementedPlayer, "nursing_hunger");
            } else {
              setDeathReason(decrementedPlayer, "hunger");
            }
            ...
            if (decrementedPlayer->xd == decrementedPlayer->xs &&
                decrementedPlayer->yd == decrementedPlayer->ys) {
              // deleted player standing still
              ...
              deathPos = computePartialMoveSpot(decrementedPlayer);
            }

            if (!decrementedPlayer->deathLogged &&
                !decrementedPlayer->isTutorial) {
              logDeath(decrementedPlayer->id, decrementedPlayer->email,
                       decrementedPlayer->isEve, computeAge(decrementedPlayer),
                       getSecondsPlayed(decrementedPlayer),
                       !getFemale(decrementedPlayer), deathPos.x, deathPos.y,
                       players.size() - 1, false);
            }

            if (shutdownMode && !decrementedPlayer->isTutorial) {
              handleShutdownDeath(decrementedPlayer, deathPos.x, deathPos.y);
            }
    ...
    if (playerIndicesToSendUpdatesAbout.size() > 0) {
      ...
      AppLog::infoF("Need to send updates about these %d players: %s",
                    playerIndicesToSendUpdatesAbout.size(), updateListString);
    ...
    for (int i = 0; i < players.size(); i++) {
      ...
      float oldDiffFromTarget = targetHeat - nextPlayer->bodyHeat;

      if (nextPlayer->lastBiomeHeat != nextPlayer->biomeHeat) {

        float lastBiomeDiffFromTarget = targetHeat - nextPlayer->lastBiomeHeat;
        ...
        if (lastBiomeDiffFromTarget != 0 && biomeDiffFromTarget != 0 &&
            sign(oldDiffFromTarget) != sign(biomeDiffFromTarget)) {
      ...
      float heatDelta = clothingLeak * (envHeatTarget - nextPlayer->bodyHeat);
    ...
    for (int i = 0; i < playerIndicesToSendUpdatesAbout.size(); i++) {
      ...
      if (nextPlayer->posForced && nextPlayer->connected &&
    ...
    if (newUpdates.size() > 0) {
      ...
      for (int i = 0; i < newUpdates.size(); i++) {
        char *s = autoSprintf("%d, ", newUpdatePlayerIDs.getElementDirect(i));
      ...
      AppLog::infoF("Sending updates about these %d players: %s",
                    newUpdatePlayerIDs.size(), updateListString);
      delete[] updateListString;
    }
    ...
    if (periodicStepThisStep) {
      ...
      for (int i = 0; i < newCurseTokenEmails.size(); i++) {
        ...
        for (int j = 0; j < numLive; j++) {
          ...
          if (!nextPlayer->isTwin && nextPlayer->curseStatus.curseLevel == 0 &&
      ...
      if (numAdded > 0) {
        ...
        if (lineageMessageLength < maxUncompressedSize) {
          ...
          lineageMessage = makeCompressedMessage(
              lineageMessageText, lineageMessageLength, &lineageMessageLength);
      ...
      if (numAdded > 0) {
        ...
        if (cursesMessageLength < maxUncompressedSize) {
          ...
          cursesMessage = makeCompressedMessage(
              cursesMessageText, cursesMessageLength, &cursesMessageLength);
    ...
    int exileMessageLength = 0;
    unsigned char *exileMessage = getExileMessage(false, &exileMessageLength);
      ...
      if (numAdded > 0) {
        ...
        if (namesMessageLength < maxUncompressedSize) {
          ...
          namesMessage = makeCompressedMessage(
              namesMessageText, namesMessageLength, &namesMessageLength);
      ...
      if (numAdded > 0) {
        ...
        if (dyingMessageLength < maxUncompressedSize) {
          ...
          dyingMessage = makeCompressedMessage(
              dyingMessageText, dyingMessageLength, &dyingMessageLength);
      ...
      if (numAdded > 0) {
        ...
        if (healingMessageLength < maxUncompressedSize) {
          ...
          healingMessage = makeCompressedMessage(
              healingMessageText, healingMessageLength, &healingMessageLength);
    ...
    if (newEmotPlayerIDs.size() > 0) {
      ...
      for (int i = 0; i < newEmotPlayerIDs.size(); i++) {
        ...
        if (ttl == 0) {
          line = autoSprintf("%d %d\n", pID, eInd);
        } else {
          line = autoSprintf("%d %d %d\n", pID, eInd, ttl);
      ...
      if (numAdded > 0) {
        ...
        if (emotMessageLength < maxUncompressedSize) {
          ...
          emotMessage = makeCompressedMessage(
              emotMessageText, emotMessageLength, &emotMessageLength);
    ...
    for (int i = 0; i < numLive; i++) {
      ...
      if (nextPlayer->firstMapSent || nextPlayer->inFlight) {
        ...
        if (newFlightDest.size() > 0) {
          ...
          for (int u = 0; u < newFlightDest.size(); u++) {
            ...
            char *flightMessage =
                autoSprintf("FD\n%d %d %d\n#", f->playerID,
                            f->destPos.x - nextPlayer->birthPos.x,
                            f->destPos.y - nextPlayer->birthPos.y);
      ...
      if (!nextPlayer->firstMessageSent) {
        ...
        sendLearnedToolMessage(nextPlayer, &(nextPlayer->learnedTools));
        ...
        char *valleyMessage =
            autoSprintf("VS\n"
                        "%d %d\n#",
                        valleySpacing, nextPlayer->birthPos.y % valleySpacing);

        sendMessageToPlayer(nextPlayer, valleyMessage, strlen(valleyMessage));
        ...
        for (int i = 0; i < numPlayers; i++) {
          ...
          if ((o != nextPlayer && o->error) || o->vogMode) {
            continue;
          }

          char oWasForced = o->posForced;

          if (nextPlayer->inFlight || nextPlayer->vogMode ||
              nextPlayer->postVogMode) {
          ...
          char *messageLine = getUpdateLine(
              o, nextPlayer->birthPos, getPlayerPos(nextPlayer), false, true);
          ...
          if (o->id != nextPlayer->id) {
            ...
            double d = intDist(o->xd, o->yd, nextPlayer->xd, nextPlayer->yd);
        ...
        char *movesMessage = getMovesMessage(false, nextPlayer->birthPos,
                                             getPlayerPos(nextPlayer));

        if (movesMessage != NULL) {

          sendMessageToPlayer(nextPlayer, movesMessage, strlen(movesMessage));
        ...
        if (numAdded > 0) {
          ...
          sendMessageToPlayer(nextPlayer, linMessage, strlen(linMessage));
        ...
        if (numAdded > 0) {
          ...
          sendMessageToPlayer(nextPlayer, namesMessage, strlen(namesMessage));
        ...
        if (numAdded > 0) {
          ...
          sendMessageToPlayer(nextPlayer, cursesMessage, strlen(cursesMessage));
        ...
        if (nextPlayer->curseStatus.curseLevel > 0) {
          ...
          char *message =
              autoSprintf("CS\n%d#", nextPlayer->curseStatus.excessPoints);
        ...
        if (followM != NULL && nextPlayer->connected) {
          nextPlayer->sock->send(followM, followL, false, false);
          delete[] followM;
        }
        ...
        if (exileM != NULL && nextPlayer->connected) {
          nextPlayer->sock->send(exileM, exileL, false, false);
          delete[] exileM;
        }
        ...
        if (numAdded > 0) {
          ...
          sendMessageToPlayer(nextPlayer, dyingMessage, strlen(dyingMessage));
        ...
        if (!nextPlayer->isTutorial && !nextPlayer->forceSpawn) {
          ...
          sendMessageToPlayer(nextPlayer, bbMessage, strlen(bbMessage));
        ...
        sendMessageToPlayer(nextPlayer, emotMessage, strlen(emotMessage));
        ...
        if (nextPlayer->monumentPosSet && !nextPlayer->monumentPosSent &&
          ...
          char *monMessage = autoSprintf(
              "MN\n%d %d %d\n#",
              nextPlayer->lastMonumentPos.x - nextPlayer->birthPos.x,
              nextPlayer->lastMonumentPos.y - nextPlayer->birthPos.y,
              nextPlayer->lastMonumentID);

          sendMessageToPlayer(nextPlayer, monMessage, strlen(monMessage));
        ...
        if (newGraves.size() > 0) {
          ...
          for (int u = 0; u < newGraves.size(); u++) {
            ...
            if (distance(g->pos, getPlayerPos(nextPlayer)) < maxDist2 ||
                g->lineageEveID == nextPlayer->lineageEveID) {

              char *graveMessage = autoSprintf(
                  "GV\n%d %d %d\n#", g->pos.x - nextPlayer->birthPos.x,
                  g->pos.y - nextPlayer->birthPos.y, g->playerID);
        ...
        if (newGraveMoves.size() > 0) {
          ...
          for (int u = 0; u < newGraveMoves.size(); u++) {
            ...
            if (distance(g->posStart, playerPos) < maxDist2 ||
                distance(g->posEnd, playerPos) < maxDist2) {

              char *graveMessage = autoSprintf(
                  "GM\n%d %d %d %d %d\n#",
                  g->posStart.x - nextPlayer->birthPos.x,
                  g->posStart.y - nextPlayer->birthPos.y,
                  g->posEnd.x - nextPlayer->birthPos.x,
                  g->posEnd.y - nextPlayer->birthPos.y, g->swapDest);
        ...
        if (newOwnerPos.size() > 0) {
          ...
          for (int u = 0; u < newOwnerPos.size(); u++) {
            ...
            if (known || distance(p, getPlayerPos(nextPlayer)) < maxDist2 ||
        ...
        if (nextPlayer->heldByOther) {
          LiveObject *holdingPlayer = getLiveObject(nextPlayer->heldByOtherID);
        ...
        if (abs(playerXD - nextPlayer->lastSentMapX) > 7 ||
          ...
          sendMapChunkMessage(nextPlayer,
                              // override if held
                              nextPlayer->heldByOther, playerXD, playerYD);
          ...
          for (int j = 0; j < numLive; j++) {
            ...
            if (otherPlayer->heldByOther) {
              LiveObject *adultO = getAdultHolding(otherPlayer);
              ...
              if (adultO != NULL) {
                ...
                if (adultO->id != nextPlayer->id &&
                  ...
                  double d =
                      intDist(playerXD, playerYD, adultO->xd, adultO->yd);
                  ...
                  if (d <= getMaxChunkDimension() / 2) {
                    ...
                    char *updateLine =
                        getUpdateLine(otherPlayer, nextPlayer->birthPos,
                                      getPlayerPos(nextPlayer), false);
          ...
          for (int j = 0; j < numLive; j++) {
            ...
            if (otherPlayer->error || otherPlayer->vogMode) {
              continue;
            }
            ...
            if (!otherPlayer->heldByOther &&
              ...
              double d =
                  intDist(playerXD, playerYD, otherPlayer->xd, otherPlayer->yd);
          ...
          if ((playerToCheck->xd != playerToCheck->xs ||
              ...
              (nextPlayer->mapChunkPathCheckedDest.x != playerToCheck->xd ||
               nextPlayer->mapChunkPathCheckedDest.y != playerToCheck->yd)) {
            ...
            for (int i = 0; i < playerToCheck->pathLength; i++) {
              ...
              int xdist = abs(p.x - nextPlayer->lastSentMapX);
              int ydist = abs(p.y - nextPlayer->lastSentMapY);
            ...
            if (xFarPosDist > 0 &&
                abs(xFarPos.x - nextPlayer->lastSentMapX) > 7) {

              sendMapChunkMessage(nextPlayer,
                                  // override chunk pos
                                  true, xFarPos.x, xFarPos.y);
            }
            if (yFarPosDist > 0 &&
                abs(yFarPos.y - nextPlayer->lastSentMapY) > 7) {

              sendMapChunkMessage(nextPlayer,
                                  // override chunk pos
                                  true, yFarPos.x, yFarPos.y);
            }
          }
        }
        ...
        if (dyingMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(dyingMessage, dyingMessageLength,
                                               false, false);
        ...
        if (healingMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(
              healingMessage, healingMessageLength, false, false);
        ...
        if (emotMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(emotMessage, emotMessageLength,
                                               false, false);
        ...
        if (wiggleMessage != NULL && nextPlayer->connected) {
          int numSent =
              nextPlayer->sock->send((unsigned char *)wiggleMessage,
                                     wiggleMessageLength, false, false);
        ...
        if (newUpdates.size() > 0 && nextPlayer->connected) {
          ...
          for (int u = 0; u < newUpdatesPos.size(); u++) {
            ...
            if (p->global) {
              ...
              double d = intDist(p->x, p->y, playerXD, playerYD);
          ...
          if (minUpdateDist <= maxDist) {
            ...
            for (int u = 0; u < newUpdates.size(); u++) {
              ...
              char *line = getUpdateLineFromRecord(newUpdates.getElement(u),
                                                   nextPlayer->birthPos,
                                                   getPlayerPos(nextPlayer));
            ...
            if (updateChars.size() > 0) {
              ...
              if (updateMessageLength < maxUncompressedSize) {
                ...
                updateMessage = makeCompressedMessage(updateMessageText,
                                                      updateMessageLength,
                                                      &updateMessageLength);
            ...
            if (updateMessage != NULL) {
              playersReceivingPlayerUpdate.push_back(nextPlayer->id);

              int numSent = nextPlayer->sock->send(
                  updateMessage, updateMessageLength, false, false);
        ...
        if (moveList.size() > 0 && nextPlayer->connected) {
          ...
          if (minUpdateDist <= maxDist) {
            ...
            for (int u = 0; u < movesPos.size(); u++) {
              ...
              closeMoves.push_back(moveList.getElementDirect(u));
            }

            if (closeMoves.size() > 0) {

              char *moveMessageText =
                  getMovesMessageFromList(&closeMoves, nextPlayer->birthPos);
              ...
              if (moveMessageText != NULL) {
                ...
                if (moveMessageLength > maxUncompressedSize) {
                  moveMessage = makeCompressedMessage(
                      moveMessageText, moveMessageLength, &moveMessageLength);
                  delete[] moveMessageText;
                }
              }

              int numSent = nextPlayer->sock->send(
                  moveMessage, moveMessageLength, false, false);
        ...
        if (middleDistancePlayerIDs.size() > 0 && nextPlayer->connected) {
          ...
          if (middleDistancePlayerIDs.size() > 0) {
            ...
            for (int i = 0; i < middleDistancePlayerIDs.size(); i++) {
              char buffer[20];
              sprintf(buffer, "%d\n",
            ...
            if (outOfRangeMessageLength < maxUncompressedSize) {
              ...
              outOfRangeMessage = makeCompressedMessage(
                  outOfRangeMessageText, outOfRangeMessageLength,
          ...
          int numSent = nextPlayer->sock->send(
              outOfRangeMessage, outOfRangeMessageLength, false, false);
        ...
        if (mapChanges.size() > 0 && nextPlayer->connected) {
          ...
          if (minUpdateDist <= maxDist) {
            ...
            for (int u = 0; u < mapChanges.size(); u++) {
              ...
              char *lineString = getMapChangeLineString(
                  r, nextPlayer->birthPos.x, nextPlayer->birthPos.y);
            ...
            if (mapChangeChars.size() > 0) {
              ...
              if (mapChangeMessageLength < maxUncompressedSize) {
                ...
                mapChangeMessage = makeCompressedMessage(
                    mapChangeMessageText, mapChangeMessageLength,
            ...
            if (mapChangeMessage != NULL) {
              ...
              int numSent = nextPlayer->sock->send(
                  mapChangeMessage, mapChangeMessageLength, false, false);
          ...
          if (minUpdateDist <= maxSpeechRadius) {
            ...
            for (int u = 0; u < newSpeechPos.size(); u++) {
              ...
              if (d <= maxSpeechRadius) {
                ...
                int speakerID = newSpeechPlayerIDs.getElementDirect(u);
                LiveObject *speakerObj = getLiveObject(speakerID);
                ...
                char *trimmedPhrase =
                    stringDuplicate(newSpeechPhrases.getElementDirect(u));

                char *starLoc = strstr(trimmedPhrase, " *map");
                ...
                if (starLoc != NULL) {
                  if (speakerID != listenerID) {
                    ...
                    int numRead = sscanf(starLoc, " *map %d %d", &mapX, &mapY);
                    if (numRead == 2) {
                      starLoc[0] = '\0';
                      char *newTrimmed =
                          autoSprintf("%s *map %d %d", trimmedPhrase,
                      ...
                      if (speakerObj != NULL) {
                        speakerObj->forceFlightDest.x = mapX;
                        speakerObj->forceFlightDest.y = mapY;
                        speakerObj->forceFlightDestSetTime =
                            Time::getCurrentTime();
                      }
                    }
                  }
                ...
                char *translatedPhrase = translatePhraseFromSpeaker(
                    trimmedPhrase, speakerObj, nextPlayer);

                if (speakerEveID != listenerEveID && speakerAge > 55 &&
                    listenerAge > 55) {

                  if (strcmp(translatedPhrase, "PEACE") == 0) {
                    // an elder speaker
                    // said PEACE
                    // in elder listener's language
                    addPeaceTreaty(speakerEveID, listenerEveID);
                  } else if (strcmp(translatedPhrase, "WAR") == 0) {
                    // an elder speaker
                    // said WAR
                    // in elder listener's language
                    removePeaceTreaty(speakerEveID, listenerEveID);
                  }
                }

                if (speakerObj != NULL && speakerObj->drunkenness > 0) {
                  // slur their speech

                  char *slurredPhrase =
                      slurSpeech(speakerObj->id, translatedPhrase,
                ...
                int curseFlag = newSpeechCurseFlags.getElementDirect(u);
            ...
            char *messageText = messageWorking.getElementString();

            int messageLen = strlen(messageText);

            unsigned char *message = (unsigned char *)messageText;

            if (messageLen >= maxUncompressedSize) {
              char *old = messageText;
              int oldLen = messageLen;

              message = makeCompressedMessage(old, oldLen, &messageLen);

              delete[] old;
            }

            int numSent =
                nextPlayer->sock->send(message, messageLen, false, false);
        ...
        if (newLocationSpeech.size() > 0 && nextPlayer->connected) {
          ...
          for (int u = 0; u < newLocationSpeechPos.size(); u++) {
            ChangePosition *p = newLocationSpeechPos.getElement(u);
          ...
          if (minUpdateDist <= maxSpeechRadius) {
            ...
            char *message = working.getElementString();
            int len = working.size();

            if (len > maxUncompressedSize) {
              int compLen = 0;

              unsigned char *compMessage =
                  makeCompressedMessage(message, len, &compLen);
            ...
            int numSent = nextPlayer->sock->send((unsigned char *)message, len,
                                                 false, false);
        ...
        if (nextPlayer->connected) {
          ...
          for (int u = 0; u < newDeleteUpdates.size(); u++) {
            ...
            char *line = getUpdateLineFromRecord(newDeleteUpdates.getElement(u),
                                                 nextPlayer->birthPos,
                                                 getPlayerPos(nextPlayer));
          ...
          if (deleteUpdateChars.size() > 0) {
            ...
            if (deleteUpdateMessageLength < maxUncompressedSize) {
              ...
              deleteUpdateMessage = makeCompressedMessage(
                  deleteUpdateMessageText, deleteUpdateMessageLength,
          ...
          if (deleteUpdateMessage != NULL) {
            int numSent = nextPlayer->sock->send(
                deleteUpdateMessage, deleteUpdateMessageLength, false, false);
        ...
        if (lineageMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(
              lineageMessage, lineageMessageLength, false, false);
        ...
        if (cursesMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(
              cursesMessage, cursesMessageLength, false, false);
        ...
        if (namesMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(namesMessage, namesMessageLength,
                                               false, false);
        ...
        if (followingMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(
              followingMessage, followingMessageLength, false, false);
        ...
        if (exileMessage != NULL && nextPlayer->connected) {
          int numSent = nextPlayer->sock->send(exileMessage, exileMessageLength,
                                               false, false);
        ...
        if (nextPlayer->foodUpdate) {
          ...
          if (cap > nextPlayer->lastReportedFoodCapacity) {
            ...
            int extraCap = cap - nextPlayer->lastReportedFoodCapacity;

            while (nextPlayer->yummyBonusStore > 0 && extraCap > 0 &&
                   nextPlayer->foodStore < cap) {
              nextPlayer->foodStore++;
              extraCap--;
              nextPlayer->yummyBonusStore--;
          ...
          if (nextPlayer->connected) {
            ...
            char *foodMessage = autoSprintf(
                ...
                nextPlayer->lastAteFillMax, computeMoveSpeed(nextPlayer),
                nextPlayer->responsiblePlayerID, nextPlayer->yummyBonusStore,
                yumMult);

            int messageLength = strlen(foodMessage);

            int numSent = nextPlayer->sock->send((unsigned char *)foodMessage,
                                                 messageLength, false, false);
        ...
        if (nextPlayer->heatUpdate && nextPlayer->connected) {
          ...
          char *heatMessage =
              autoSprintf("HX\n"
                          "%.2f %.2f %.2f#",
                          nextPlayer->heat, nextPlayer->foodDrainTime,
                          nextPlayer->indoorBonusTime);

          int messageLength = strlen(heatMessage);

          int numSent = nextPlayer->sock->send((unsigned char *)heatMessage,
                                               messageLength, false, false);
        ...
        if (nextPlayer->curseTokenUpdate && nextPlayer->connected) {
          // send this player a curse token status change

          char *tokenMessage = autoSprintf("CX\n"
                                           "%d#",
                                           nextPlayer->curseTokenCount);

          int messageLength = strlen(tokenMessage);

          int numSent = nextPlayer->sock->send((unsigned char *)tokenMessage,
                                               messageLength, false, false);
    ...
    if (newUpdates.size() > 0) {
      ...
      for (int i = 0; i < playersReceivingPlayerUpdate.size(); i++) {
        char *playerString = autoSprintf(
            "%d, ", playersReceivingPlayerUpdate.getElementDirect(i));
      ...
      AppLog::infoF("%d/%d players were sent part of a %d-line PU: %s",
                    playersReceivingPlayerUpdate.size(), numLive,
                    newUpdates.size(), playerListString);
    ...
    for (int i = 0; i < players.size(); i++) {
      ...
      if (nextPlayer->gotPartOfThisFrame && nextPlayer->connected) {
        int numSent = nextPlayer->sock->send((unsigned char *)frameMessage,
                                             frameMessageLength, false, false);
